...
return new class extends Migration
{
     ...
     * ✅ VARCHAR  → prefix allowed
     * ❌ ENUM     → prefix NOT allowed (MySQL Error 1089)
     * ❌ INT / TINYINT / DATE / DECIMAL → no prefix allowed
     */
    public function up(): void
    {
        Schema::create('interacted_caches', function (Blueprint $table) {
            $table->increments('id');
            $table->unsignedInteger('interact_log_id');
            $table->unsignedInteger('interact_record_id')->index('interacted_caches_interact_record_id_foreign');
            $table->timestamps();
            ...
            // Small 2-column index — safe inside Blueprint, no string byte issue
            $table->index(['interact_log_id', 'status_id'], 'idx_interacted_caches_report');
        ...
        /*
         * ================================================================
         * idx_interacted_caches — raw SQL with prefix lengths
         *
         * Column               Type           Prefix
         * ───────────────────  ─────────────  ──────
         * interact_log_id      int unsigned   none
         * interact_record_id   int unsigned   none
         ...
         * purchase_currency    varchar(10)    none (already short)
         ...
         */
        DB::statement('
            ALTER TABLE `interacted_caches` ADD INDEX `idx_interacted_caches` (
                `interact_log_id`,
                `interact_record_id`,
                `asset_tag`(50),
                `model_id`,
    ...
    {
        Schema::dropIfExists('interacted_caches');
    }
};
