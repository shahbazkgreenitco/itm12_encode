...
class UserSyncFromAzureMultiple extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'ms_user_sync_multiple:azure';
    ...
    private $endpoints, $tenantID, $client_secret, $client_id;
    ...
    {
        parent::__construct();
        $this->tenantID = explode(",", config("services.azure_multi.tenant"));
        $this->client_secret = explode(",", config("services.azure_multi.client_secret"));
        $this->client_id = explode(",", config("services.azure_multi.client_id"));
    ...
    {
        if (config("services.azure_multi.client_id") == "") {
            return;
        }

        $current_datetime = Carbon::now(config('app.timezone'));
        $twenty_four_hours_back = Carbon::now(config('app.timezone'))->subHours(24);
        Log::info("UserSyncFromAzureMultiple Start");
        foreach($this->tenantID as $key => $tenantID) {
            try {
                $headers = [
                ...
                $postInput = [
                    ...
                    "client_secret" => $this->client_secret[$key],
                    "client_id" => $this->client_id[$key],
                ...
                $r = $client->request('POST', 'https://login.microsoftonline.com/'.$tenantID.'/oauth2/v2.0/token', [
                    'form_params' => $postInput
                ]);
                $response = json_decode($r->getBody(), true);

                $this->access_token = $response['access_token'];
                // $this->info("access_token: " . $response['access_token']);
                ...
                $this->info("success");
                Log::info("UserSyncFromAzureMultiple: success");
            } catch (\Exception $e) {
                Log::error("UserSyncFromAzureMultiple: " . $e->getMessage());
            }
        }
        Log::info("UserSyncFromAzureMultiple Success");
    }
    ...
    public function syncUsers($users) {
        ...
        foreach ($users as $user) {
            ...
            $usrObj = User::where("email", "like", $user["userPrincipalName"])->limit(1)->withTrashed()->first();
            ...
            if($user["userType"] != "Guest") {
                ...
                if (isset($user["manager"])) {
                    $managerObj = User::where("email", "like", $user["manager"]["userPrincipalName"])->limit(1)->first();
                    if(!empty($managerObj)) {
                ...
                if(config("app.client") != "ltts") {
                    if($user["officeLocation"] != "" && !preg_match('/^[a-z]+$/i', $user["officeLocation"])) {
                        $usrObj->employee_num = ($user["officeLocation"] != "") ? $user["officeLocation"] : $user["employeeId"];
                    } else {
                        $usrObj->employee_num = ($user["employeeId"] != "") ? $user["employeeId"] : $usrObj->employee_num;
                    }
                    $usrObj->activated = ($user["accountEnabled"]) ? 1 : 0;
