...
[1.0.1] - Updated search bar width and pagination dropdown class
[1.0.2] - Added scrollX and scrollCollapse options to DataTable initialization
------------------------------------------------------------
*/
--}}
@extends('layouts.layout1')
@section('title', trans("config.model_fields.models"))
@section('content')
<section class="content">
    <div id="main-user-list-wrapper">
        <div class="header-actions-wrapper d-flex align-items-center justify-content-between pe-4">
            <h3 class="h3-text mb-0">{{ trans("config.model_fields.models") }}</h3>
            <div class="d-flex gap-8">

                @can("ModelRead")
                <button class="header-icon-btn-only header-icon-btn-only-sm btn-show-models"  type="button" data-bs-toggle="tooltip"
                    title="{{ trans('config.model_fields.show_deleted_models') }}">
                    <svg xmlns="http://www.w3.org/2000/svg" width="22" height="17" viewBox="0 0 22 17" fill="none">
                        ...
                        <path
                            ...
                            fill="currentColor" mask="url(#path-1-inside-1_2605_28551)"></path>
                    </svg>
                </button>
                @endcan
                @can("ModelDownload")
                <button class="header-icon-btn-only header-icon-btn-only-sm btn-models-export"  type="button" data-bs-toggle="tooltip"
                    title="{{ trans('config.model_fields.download') }}">
                ...
                <button class="amg-btn amg-btn-primary amg-btn-sm open-add-modal" type="button" data-bs-toggle="tooltip"
                    title="{{ trans('config.model_fields.add_model') }}">
                    ...
                    <span>{{ trans('config.model_fields.add_model') }}</span>
                </button>
            </div>
        </div>
                            ...
                            <div class="table-responsive">
                                <table id="mytable" class="amg-datatable  table display app-data-table">
                                    <thead>
                                        <tr>
                                            <th>
                                                <h4>{{ trans("config.model_fields.id") }}</h4>
                                            </th>
                                            <th>
                                                <h4>{{ trans("config.model_fields.manufacturer") }}</h4>
                                            </th>
                                            <th>
                                                <h4>{{ trans("config.model_fields.model") }}</h4>
                                            </th>
                                            <th>
                                                <h4>{{ trans("config.model_fields.model_no") }}</h4>
                                            </th>
                                            <th>
                                                <h4>{{ trans("config.model_fields.device_count") }}</h4>
                                            </th>
                                            <th>
                                                <h4>{{ trans("config.model_fields.depreciation") }}</h4>
                                            </th>
                                            <th>
                                                <h4>{{ trans("config.model_fields.category") }}</h4>
                                            </th>
                                            <th>
                                                <h4>{{ trans("config.model_fields.eol") }}</h4>
                                            </th>
                                            <th>
                                                <h4>{{ trans("config.model_fields.image") }}</h4>
                                            </th>
                                            <th>
                                                <h4>{{ trans("config.model_fields.actions") }}</h4>
                                            </th>
                                        </tr>
                                    </thead>
                                    <tbody>
...
<script>
    ...
    config.url = {
        ...
        edit: "{{ url('model/edit') }}",
        list: "{{ url('model-list') }}",
        delete: "{{ url('model-delete') }}",
        restore: "{{ url('model-restore') }}",
        info: "{{ url('model/history') }}",
        download_url: "{{ url('export-model-details') }}",
        formData: "{{ url('form-data') }}",
        get: "{{ url('model/ajax-get') }}",
        update: "{{ url('model/ajax-edit') }}",
        getCustomFieldsetByModule: "{{ url('getCustomFieldsetByModule') }}",
        deleteImage: "{{ url('models/delete-image') }}"

        };

    config.token = "{{ csrf_token() }}";
    config.imgviewpath = "{{ url('uploads/models') }}";
    config.permissions = {!! json_encode($permissionArray) !!};
    config.datatable_translations=  @json(trans('datatable.datatable'));
    config.translations = {
        history: '{{ trans("config.model_fields.models_history") }}',
        something_went_wrong: '{{ trans("config.model_fields.something_went_wrong") }}',
        are_you_delete: '{{ trans("config.model_fields.are_you_delete") }}',
        press_enter_with_Search: '{{ trans("config.model_fields.search") }}',
        edit: '{{ trans("config.model_fields.edit_model") }}',
        delete: '{{ trans("config.model_fields.delete") }}',
        show_deleted_models: '{{ trans("config.model_fields.show_deleted_models") }}',
        show_present_model: '{{ trans("config.model_fields.show_present_model") }}',
        Refresh_List: '{{ trans("config.model_fields.refresh") }}',
        Search: '{{ trans("config.model_fields.search") }}',
        restore: '{{ trans("config.model_fields.restore") }}',
        add_model: '{{ trans("config.model_fields.add_model") }}',
        select_mannufacturer: '{{ trans("config.model_fields.select_mannufacturer") }}',
        select_category: '{{ trans("config.model_fields.select_category") }}',
        select_depreciation: '{{ trans("config.model_fields.select_depreciation") }}',
        upload_type: '{{ trans("config.model_fields.upload_type") }}',
        select_fieldset: '{{ trans("config.model_fields.select_fieldset") }}',
        ok: '{{ trans("config.model_fields.ok") }}',
        cancel: '{{ trans("config.model_fields.cancel") }}',
        restore_desc: '{{ trans("config.model_fields.restore_desc") }}',
        yes_restore: '{{ trans("config.model_fields.yes_restore") }}',

        history: '{{ trans("config.model_fields.models_history") }}',
        something_went_wrong: '{{ trans("config.model_fields.something_went_wrong") }}',

        model_name_required: '{{ trans("config.model_fields.model_name_required") }}',
        minimum_2_characters_required: '{{ trans("config.model_fields.minimum_2_characters_required") }}',
        maximum_50_characters_allowed: '{{ trans("config.model_fields.maximum_50_characters_allowed") }}',

        please_select_manufacturer: '{{ trans("config.model_fields.please_select_manufacturer") }}',
        please_select_category: '{{ trans("config.model_fields.please_select_category") }}',
        please_select_depreciation: '{{ trans("config.model_fields.please_select_depreciation") }}',

        model_number_required: '{{ trans("config.model_fields.model_number_required") }}',

        only_numbers_allowed: '{{ trans("config.model_fields.only_numbers_allowed") }}',

        only_jpg_jpeg_png_allowed: '{{ trans("config.model_fields.only_jpg_jpeg_png_allowed") }}',
        file_must_be_less_than_2mb: '{{ trans("config.model_fields.file_must_be_less_than_2mb") }}',
    };
</script>
<script src="{!! CommonHelper::asset('js/models/list.js') !!}"></script>
