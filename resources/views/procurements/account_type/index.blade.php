...
* Module: Procurment Account Type
* PAT/26/01
* ------------------------------------------------------------
* Version: 1.0.0
...
@section('title', trans("config.procurAccountType_fields.account_types"))
@section('content')
<div id="main-user-list-wrapper">
    <section class="content">        
        <div class="header-actions-wrapper d-flex align-items-center justify-content-between pe-4">
            <h3 class="h3-text mb-0">{{ trans("config.procurAccountType_fields.account_types") }}</h3>
            <div class="d-flex gap-8">         
                              
                <button class="amg-btn amg-btn-primary amg-btn-sm open-add-modal" type="button" data-bs-toggle="tooltip" title="{{ trans('config.procurAccountType_fields.add_account_type') }}">
                        ...
                        <span>{{ trans('config.procurAccountType_fields.add_account_type') }}</span>
                </button>
                ...
                <div class="card rounded-0">
                    <div class="card-body">
                         <div class="d-flex align-items-center gap-2 mb-1">
                            <div class="col-auto">
                                <select class="amg-table-pagination-dropdown userModulePageLenth list-page-length">
                                    <option value="10" selected>{{ trans("header.header.show") }} (10)</option>
                                    <option value="25">{{ trans("header.header.show") }} (25)</option>
                                    <option value="50">{{ trans("header.header.how") }} (50)</option>
                                    <option value="100">{{ trans("header.header.show") }} (100)</option>
                                </select>
                            </div>
                            ...
                            <div>
                                <div class="amg-list-searchbar">
                                    ...
                                    <input type="text" class="amg-list-searchbar__input searchbox" placeholder="{{ trans("config.procurAccountType_fields.search") }}...">
                                </div>
                            </div>
                            ...
                            <button class="amg-refresh-btn btn-reload-list">
                                ...
                                <span>{{ trans("config.procurAccountType_fields.refresh_list") }}</span>
                            </button>
                        </div>
                        <div class="table-responsive" style="overflow-y:hidden;">
                            <table id="mytable" class="table display" style="width:100%">
                                <thead>
                                <tr>
                                    <th><h4>{{ trans("config.procurAccountType_fields.company") }}</h4></th>
                                    <th><h4>{{ trans("config.procurAccountType_fields.name") }}</h4></th>                                   
                                    <th><h4>{{ trans("config.procurAccountType_fields.updated_at") }}</h4></th>
                                    <th><h4>{{ trans("config.procurAccountType_fields.actions") }}</h4></th>
                                </tr>
                                </thead>
                                <tbody></tbody>
                            </table>
        ...
        @include("procurements.account_type.modal")
    </section>
</div>
@endsection
@push('scripts')

<script type="text/javascript" src="{{ CommonHelper::asset('js/procurements/account_type/index.js') }}"></script>
...
<script type="text/javascript">
	...
	config.translations = {
		...
		are_you_delete: '{{ trans('config.procurAccountType_fields.are_you_delete') }}',
		something_went_wrong: '{{ trans('config.procurAccountType_fields.something_went_wrong') }}',
		add_account_type: '{{ trans('config.procurAccountType_fields.add_account_type') }}',
		edit_account_type: '{{ trans('config.procurAccountType_fields.edit_account_type') }}',
		...
		cancel: '{{ trans('button.cancel') }}',
		Refresh_List: '{{ trans('config.procurAccountType_fields.refresh_list') }}',
		search: '{{ trans('config.procurAccountType_fields.search') }}',
        Previous: '{{ trans('header.header.Previous') }}',
        Next: '{{ trans('header.header.Next') }}',
        Showing: '{{ trans('header.header.Showing') }}',
        to: '{{ trans('header.header.to') }}',
        of: '{{ trans('header.header.of') }}',
        entries: '{{ trans('header.header.entries') }}',
        filtered_from: '{{ trans('header.header.filtered_from') }}',
        total_entries: '{{ trans('header.header.total_entries') }}',
        No_matching_records_found: '{{ trans('header.header.No_matching_records_found') }}',
	};
	config.url.accountTypes = "{{ url('jx-account-types') }}";
	config.url.add = "{{ url('jx-account-types-add') }}";
	config.url.edit = "{{ url('jx-account-types-edit') }}";
	config.url.get = "{{ url('jx-account-types-get') }}";
	config.url.delete = "{{ url('jx-account-types-delete') }}";
