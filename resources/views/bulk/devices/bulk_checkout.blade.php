...
@section('title', trans('devices.device_checkout.title'))
@section('content')
<style>
    :root {
        --primary: #1a2e5a;
        ...
        <div class="py-3">
            <div class="container-fluid">
                <button onclick="location.href='{{ url('devices') }}'"
                    ...
                    <h2 class="h2-text mb-0">{{ trans('devices.device_checkout.bulk_checkout') }}</h2>
                </button>
            </div>
        </div>
    </div>
            ...
            <ul class="nav nav-underline" id="myTab" role="tablist">
                <li class="nav-item">
                    <a class="nav-link active" id="info-tab-link" data-bs-toggle="tab" href="#info-tab" role="tab">
                        <span>{{ trans('devices.device_checkout.tab_bulk_checkout') }}</span>
                    </a>
                </li>
                <li class="nav-item">
                    <a class="nav-link" id="check-out-info-tab-link" data-bs-toggle="tab" href="#check-out-info-tab"
                        role="tab">
                        <span>{{ trans('devices.device_checkout.tab_checkout_info') }}</span>
                    </a>
                </li>
            </ul>
        </div>
            ...
            <div class="tab-pane fade show active container-fluid pt-3 pb-5" id="info-tab">
                ...
                <div class="alert customize-alert alert-dismissible alert-light-success bg-success-subtle text-success fade show remove-close-icon"
                    ...
                    <div class="d-flex align-items-center me-3 gap-3 me-md-0">
                        ...
                        {{ trans('devices.device_checkout.import_success', ['count' => session('success',0)]) }}
                    </div>
                </div>
                @endif
                ...
                <div class="alert customize-alert alert-dismissible alert-light-danger bg-danger-subtle text-danger fade show remove-close-icon"
                    ...
                    <div class="d-flex align-items-start me-3 gap-3 me-md-0">
                        ...
                        <div>
                            {{ trans('devices.device_checkout.import_failed', ['count' => session('fail',0)]) }}
                ...
                <div class="alert customize-alert alert-dismissible alert-light-warning bg-warning-subtle text-warning fade show remove-close-icon"
                    ...
                    <div class="d-flex align-items-center me-3 gap-3 me-md-0">
                        ...
                        {{ trans('devices.device_checkout.import_default_success') }}
                    </div>
                </div>
                @endif
                            ...
                            <div class="drop-zone" id="dropZone">
                                ...
                                <div id="defaultState">
                                    ...
                                    <p class="drag-text mt-3 mb-2 b1-text">{{
                                        trans("devices.device_checkout.drag_drop_file") }}
                                    </p>
                                    <button class="amg-btn amg-btn-secondary" style="min-width:203px;">
                                        ...
                                        <span>{{ trans("devices.device_checkout.upload_file_btn") }}</span>
                                    </button>
                                    <span class="file-label b1-text">{{
                                        trans("devices.device_checkout.no_file_selected")
                                        }}</span>
                                </div>
                                ...
                                <div id="selectedState" style="display:none;">
                                    ...
                                    <p class="file-selected-name mb-2 mt-3" id="selectedFileName">{{
                                        trans('devices.device_checkout.format_excel_sheet') }}</p>
                                    <button class="amg-btn amg-btn-primary" style="min-width:203px" type="button"
                                        ...
                                        <span>{{ trans('devices.device_checkout.save_checkout') }}</span>
                                    </button>
                                    <button class="amg-btn amg-btn-link amg-btn-link-plain mt-2" id="btnRemove">{{
                                        trans('devices.device_checkout.remove_file') }}</button>
                                </div>
                            </div>
                        ...
                        <div class="col-12 col-sm-7">
                            <div class="right-panel h-100">
                                <a class="amg-btn amg-btn-white amg-btn-lg"
                                    href="{!! CommonHelper::asset('samples/DeviceCheckoutFormat.xlsx') !!}"
                                    target="_blank">
                                    ...
                                    <span class="font-weight-normal">{{
                                        trans('devices.device_checkout.download_template') }}</span>
                                </a>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="col-12 px-5">
                        <div class="instructions-card mt-3">
                            <h4 class="h4-text" style="color:#7F7F7F">
                                {{ trans("devices.device_checkout.instructions_title") }}
                            </h4>
                            ...
                            <div class="ps-3 ps-sm-4">
                                <div class="card-scroll-body" id="cardScrollBody">
                                    <ul class="ps-3">
                                        <li>
                                            <span class="s1-text me-3">{{ trans("devices.device_checkout.field_serial") }} <span class="mandatory"></span></span>
                                            <span class="b1-text text-muted">{{ trans("devices.device_checkout.serial_field_desc") }}</span>
                                        </li>
                                        <li>
                                            <span class="s1-text">{{ trans("devices.device_checkout.field_device_name") }}</span>
                                            <span class="b1-text text-muted">{{ trans("devices.device_checkout.device_name_field_desc") }}</span>
                                        </li>
                                        <li>
                                            <span class="s1-text me-3">{{ trans("devices.device_checkout.field_checkout_to") }} <span class="mandatory"></span></span>
                                            <span class="b1-text text-muted">{{ trans("devices.device_checkout.checkout_to_field_desc") }}</span>
                                        </li>
                                        <li>
                                            <span class="s1-text me-3">{{ trans("devices.device_checkout.field_checkout_user_place") }} <span class="mandatory"></span></span>
                                            <span class="b1-text text-muted">{{ trans("devices.device_checkout.checkout_user_place_field_desc") }}</span>
                                        </li>
                                        <li>
                                            <span class="s1-text me-3">{{ trans("devices.device_checkout.field_checkout_reason") }}
                                                @if(in_array(strtolower(trim(config('app.client'))), ['knightfrank', 'rolepermission']))
                                                <span class="mandatory"></span>
                                                @endif
                                            </span>
                                            <span class="b1-text text-muted">{{ trans("devices.device_checkout.checkout_reason_field_desc") }}</span>
                                        </li>
                                        <li>
                                            <span class="s1-text">{{ trans("devices.device_checkout.field_project_name") }}</span>
                                            <span class="b1-text text-muted">{{ trans("devices.device_checkout.project_name_field_desc") }}</span>
                                        </li>
                                        <li>
                                            <span class="s1-text">{{ trans("devices.device_checkout.field_last_checkout") }}</span>
                                            <span class="b1-text text-muted">{{ trans("devices.device_checkout.last_checkout_field_desc") }}</span>
                                        </li>
                                        <li>
                                            <span class="s1-text">{{ trans("devices.device_checkout.field_expected_checkin") }}</span>
                                            <span class="b1-text text-muted">{{ trans("devices.device_checkout.expected_checkin_field_desc") }}</span>
                                        </li>
                                        @if(in_array(config('app.client'), ["knightfrank"]))
                                        <li>
                                            <span class="s1-text me-3">{{ trans("devices.device_checkout.field_notes") }} <span class="mandatory"></span></span>
                                            <span class="b1-text text-muted">{{ trans("devices.device_checkout.notes_field_desc_kf") }}</span>
                                        </li>
                                        @else
                                        <li>
                                            <span class="s1-text me-3">{{ trans("devices.device_checkout.field_notes") }}</span>
                                            <span class="b1-text text-muted">{{ trans("devices.device_checkout.notes_field_desc") }}</span>
                                        </li>
                                        @endif
                                        <li>
                                            <span class="s1-text me-3">{{ trans("devices.device_checkout.field_allocation_type") }} <span class="mandatory"></span></span>
                                            <span class="b1-text text-muted">{{ trans("devices.device_checkout.allocation_type_field_desc") }}</span>
                                        </li>
                                    </ul>
                                    <p><i>{{ trans("devices.device_checkout.nvr_ch") }}</i></p>
                                </div>

                                <a class="amg-btn amg-btn-link amg-btn-link-plain s1-text mt-2" id="readMoreBtn">
                                    {{ trans("devices.device_checkout.read_more") }}
                                </a>
                            </div>

                            <h4 class="h4-text" style="color:#7F7F7F">
                                {{ trans('devices.device_checkout.never_change_header') }}. <strong>{{ trans("devices.device_checkout.date_values") }}</strong>
                            </h4>
                        </div>
                    </div>
                </div>
            </div>
             <div class="mt-3 tab-pane fade table-responsive border-top-0" id="check-out-info-tab">
                <div class="card">
                    <div class="card-body">
                        <div class="d-flex align-items-center gap-2 mb-4" data-select2-id="select2-data-5-1om7">
                            <div id="customLengthContainer" class="col-auto" data-select2-id="select2-data-4-sbib">
                                <select id="showSelect" class="showSelect">
                                    <option value="10" selected>{{ trans('devices.device_checkout.show') }}(10)</option>
                                    <option value="25">{{ trans('devices.device_checkout.show') }}(25)</option>
                                    <option value="50">{{ trans('devices.device_checkout.show') }}(50)</option>
                                    <option value="100">{{ trans('devices.device_checkout.show') }}(100)</option>
                                </select>
                            </div>
                            ...
                            <div>
                                <div class="amg-list-searchbar">
                                    ...
                                    <input type="text" class="amg-list-searchbar__input" id="tableSearch"
                                        placeholder="{{ trans('devices.device_checkout.search_placeholder') }}">
                                </div>
                            </div>
                            ...
                            <button class="amg-refresh-btn btn-reload-list">
                                ...
                                <span>{{ trans('devices.device_checkout.refresh') }}</span>
                            </button>
                        </div>
                        ...
                        <table id="tblCheckout" class="table display">
                            <thead>
                                <tr>
                                    <th><h4>{{ trans('devices.device_checkout.id') }}</h4></th>
                                    <th><h4>{{ trans('devices.device_checkout.document_name') }}</h4></th>
                                    <th><h4>{{ trans('devices.device_checkout.total_upload') }}</h4></th>
                                    <th><h4>{{ trans('devices.device_checkout.total_success') }}</h4></th>
                                    <th><h4>{{ trans('devices.device_checkout.total_failure') }}</h4></th>
                                    <th><h4>{{ trans('devices.device_checkout.updated_at') }}</h4></th>
                                    <th><h4>{{ trans('devices.device_checkout.username') }}</h4></th>
                                    <th><h4>{{ trans('devices.device_checkout.actions') }}</h4></th>
                                </tr>
                            </thead>
                            <tbody></tbody>
                        </table>
...
<script type="text/javascript" src="{!! CommonHelper::asset('js/bulk/devices/bulk_checkout.js') !!}"></script>

<script>
    var config = {};
    config.bulk_checkout_info = "{{ url('devices/bulk/checkout-info') }}";
    config.document_download = "{{ url('uploads/bulk_documents') }}";
    config.token = "{{ csrf_token() }}";
    config.translations = {
        Search: '{{ trans('devices.device_checkout.search_js') }}',
        Refresh_List: '{{ trans('devices.device_checkout.refresh_list_js') }}',
        Download_Document: '{{ trans('devices.device_checkout.download_document_js') }}',
        Reload: '{{ trans('devices.device_checkout.reload_js') }}',
        press_enter_with_Search: '{{ trans('devices.device_checkout.press_enter_with_search_js') }}',
        please_enter_valid_search: '{{ trans('devices.device_checkout.please_enter_valid_search_js') }}',
        please_choose_file: '{{ trans('devices.device_checkout.please_choose_file_js') }}',
        Choose_file_for_checkout: '{{ trans('devices.device_checkout.choose_file_for_checkout_js') }}',
        please_upload_valid_xlsx: '{{ trans('devices.device_checkout.please_upload_valid_xlsx_js') }}',
        download: '{{ trans('devices.device_checkout.download') }}',
    };
    ...
    $(function () {
        ...
        $('#btnSave').on('click', function (e) {
            ...
            if (!file) {
                swal.fire('{{ trans('devices.device_checkout.please_choose_file_js') }}');
                return;
            }

            var ext = file.name.split('.').pop().toLowerCase();
            if (!['xlsx'].includes(ext)) {
                swal.fire('{{ trans('devices.device_checkout.please_upload_valid_xlsx_js') }}');
                return;
            }
        ...
        $('#readMoreBtn').on('click', function (e) {
            ...
            if (expanded) {
                ...
                $(this).text('{{ trans('devices.device_checkout.read_less') }}');
            } else {
                $('#cardScrollBody').removeClass('expanded');
                $(this).text('{{ trans('devices.device_checkout.read_more') }}');
                $('html, body').animate({
