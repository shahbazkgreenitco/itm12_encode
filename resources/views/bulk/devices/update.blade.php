...
@section('title', trans('devices.device_update.title'))
@section('content')
<style>
    :root {
        --primary: #1a2e5a;
    ...
    <div class="header-actions-wrapper header-actions-white-wrapper d-flex align-items-center justify-content-between pe-4">
        <div class="py-3">
            <div class="container-fluid">
                <button onclick="location.href='{{ url('devices') }}'"
                    ...
                    <h2 class="h2-text mb-0">{{ trans('devices.device_update.bulk_update') }}</h2>
                </button>
            </div>
        </div>
    </div>
    ...
    <main class="main-content" id="mainContent">
        <div class="tab-bar">
            <ul class="nav nav-underline" id="myTab" role="tablist">
                <li class="nav-item">
                    <a class="nav-link active" id="info-tab-link" data-bs-toggle="tab" href="#info-tab" role="tab">
                        <span>{{ trans('devices.device_update.tab_bulk_update') }}</span>
                    </a>
                </li>
                <li class="nav-item">
                    <a class="nav-link" id="update-info-tab-link" data-bs-toggle="tab" href="#update-info-tab"
                        role="tab">
                        <span>{{ trans('devices.device_update.tab_update_information') }}</span>
                    </a>
                </li>
            </ul>
        </div>
        ...
        <div class="tab-content">
            <div class="tab-pane fade show active container-fluid pt-3 pb-5" id="info-tab">
                @if(session('msg'))
                <div class="alert customize-alert alert-dismissible 
                 {{ session('msg')['status'] == 'success'
                 ? 'alert-light-success bg-success-subtle text-success'
                 : 'alert-light-danger bg-danger-subtle text-danger' }}
                  fade show remove-close-icon" role="alert">
                ...
                <div class="alert customize-alert alert-dismissible alert-light-success bg-success-subtle text-success fade show remove-close-icon"
                    ...
                    <div class="d-flex align-items-center me-3 gap-3 me-md-0">
                        ...
                        {{ trans('devices.device_update.import_success', ['count' => session('success',0)]) }}
                    </div>
                </div>
                @endif
                ...
                <div class="alert customize-alert alert-dismissible alert-light-danger bg-danger-subtle text-danger fade show remove-close-icon"
                    ...
                    <div class="d-flex align-items-start me-3 gap-3 me-md-0">
                        ...
                        <div>
                            {{ trans('devices.device_update.import_failed', ['count' => session('fail',0)]) }}
                ...
                @if(session('success', 0) == 0 && session('fail', 0) == 0 && !session('msg'))
                <div class="alert customize-alert alert-dismissible alert-light-warning bg-warning-subtle text-warning fade show remove-close-icon"
                    ...
                    <div class="d-flex align-items-center me-3 gap-3 me-md-0">
                        ...
                        {{ trans('devices.device_update.import_default_success') }}
                    </div>
                </div>
                @endif
                @endif
                ...
                <div class="import-wrapper">
                    <div class="row g-3 align-items-stretch">
                            ...
                            <div class="drop-zone" id="dropZone">
                                ...
                                <div id="defaultState">
                                    ...
                                    <p class="drag-text mt-3 mb-2 b1-text">{{
                                        trans("devices.device_update.drag_drop_file") }}
                                    </p>
                                    <button class="amg-btn amg-btn-secondary" style="min-width:203px;">
                                        ...
                                        <span>{{ trans("devices.device_update.upload_file_btn") }}</span>
                                    </button>
                                    <span class="file-label b1-text">{{
                                        trans("devices.device_update.no_file_selected")
                                        }}</span>
                                </div>
                                ...
                                <div id="selectedState" style="display:none;">
                                    ...
                                    <p class="file-selected-name mb-2 mt-3" id="selectedFileName">{{
                                        trans('devices.device_update.format_excel_sheet') }}</p>
                                    <button class="amg-btn amg-btn-primary" style="min-width:203px" type="button"
                                        ...
                                        <span>{{ trans('devices.device_update.save_update') }}</span>
                                    </button>
                                    <button class="amg-btn amg-btn-link amg-btn-link-plain mt-2" id="btnRemove">{{
                                        trans('devices.device_update.remove_file') }}</button>
                                </div>
                            </div>

                            {{-- Hidden form for file submission --}}
                            <form id="importForm" method="POST" action="{{ url('devices/bulk/update') }}" enctype="multipart/form-data"
                                style="display:none;">
                                @csrf
                        ...
                        <div class="col-12 col-sm-7">
                            <div class="right-panel h-100 d-flex flex-wrap gap-2">
                                <a class="amg-btn amg-btn-white amg-btn-lg"
                                    href="{!! CommonHelper::asset('samples/DeviceUpdateFormat.xlsx') !!}"
                                    target="_blank">
                                    ...
                                    <span class="font-weight-normal">{{
                                        trans('devices.device_update.download_template') }}</span>
                                </a>
                                {{-- <a class="amg-btn amg-btn-white amg-btn-lg"
                                    href="{!! url('download-currencies') !!}"
                                    target="_blank">
                                    <svg width="14" height="17" viewBox="0 0 14 17" fill="currentColor"
                                        xmlns="http://www.w3.org/2000/svg">
                                        <path
                                    ...
                                    <span class="font-weight-normal">{{
                                        trans('devices.device_update.download_currency_codes') }}</span>
                                </a> --}}
                                {{-- <a class="amg-btn amg-btn-white amg-btn-lg"
                                    href="{{ url('download-custom-fields-code') }}"
                                    target="_blank">
                                    <svg width="14" height="17" viewBox="0 0 14 17" fill="currentColor"
                                        xmlns="http://www.w3.org/2000/svg">
                                        <path
                                    ...
                                    <span class="font-weight-normal">{{
                                        trans('devices.device_update.download_custom_field_codes') }}</span>
                                </a> --}}
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="col-12 px-5">
                        <div class="instructions-card mt-3">
                            <h4 class="h4-text" style="color:#7F7F7F">
                                {{ trans("devices.device_update.instructions_title") }}
                            </h4>

                            <div class="ps-3 ps-sm-4">
                                <div class="card-scroll-body" id="cardScrollBody">
                                    <p class="mb-2">{{ trans("devices.device_update.instructions_note") }}</p>
                                    <ul class="ps-3">
                                        <li>
                                            <span class="s1-text me-3">{{ trans("devices.device_update.field_company") }}</span>
                                            <span class="b1-text text-muted">{{ trans("devices.device_update.field_company_desc") }}</span>
                                        </li>
                                        <li>
                                            <span class="s1-text me-3">{{ trans("devices.device_update.field_name") }}</span>
                                            <span class="b1-text text-muted">{{ trans("devices.device_update.field_name_desc") }}</span>
                                        </li>
                                        <li>
                                            <span class="s1-text me-3">{{ trans("devices.device_update.field_asset_tag") }}</span>
                                            <span class="b1-text text-muted">{{ trans("devices.device_update.field_asset_tag_desc") }}</span>
                                        </li>
                                        <li>
                                            <span class="s1-text me-3">{{ trans("devices.device_update.field_serial") }} <span class="mandatory"></span></span>
                                            <span class="b1-text text-muted">{{ trans("devices.device_update.field_serial_desc") }}</span>
                                        </li>
                                        <li>
                                            <span class="s1-text me-3">{{ trans("devices.device_update.field_product_number") }}</span>
                                            <span class="b1-text text-muted">{{ trans("devices.device_update.field_product_number_desc") }}</span>
                                        </li>
                                        <li>
                                            <span class="s1-text me-3">{{ trans("devices.device_update.field_manufacture") }}</span>
                                            <span class="b1-text text-muted">{{ trans("devices.device_update.field_manufacture_desc") }}</span>
                                        </li>
                                        <li>
                                            <span class="s1-text me-3">{{ trans("devices.device_update.field_model") }}</span>
                                            <span class="b1-text text-muted">{{ trans("devices.device_update.field_model_desc") }}</span>
                                        </li>
                                        <li>
                                            <span class="s1-text me-3">{{ trans("devices.device_update.field_category") }}</span>
                                            <span class="b1-text text-muted">{{ trans("devices.device_update.field_category_desc") }}</span>
                                        </li>
                                        <li>
                                            <span class="s1-text me-3">{{ trans("devices.device_update.field_status") }}</span>
                                            <span class="b1-text text-muted">{{ trans("devices.device_update.field_status_desc") }}</span>
                                        </li>
                                        <li>
                                            <span class="s1-text me-3">{{ trans("devices.device_update.field_department") }}</span>
                                            <span class="b1-text text-muted">{{ trans("devices.device_update.field_department_desc") }}</span>
                                        </li>
                                        <li>
                                            <span class="s1-text me-3">{{ trans("devices.device_update.field_location") }}</span>
                                            <span class="b1-text text-muted">{{ trans("devices.device_update.field_location_desc") }}</span>
                                        </li>
                                        <li>
                                            <span class="s1-text me-3">{{ trans("devices.device_update.field_internal_places") }}</span>
                                            <span class="b1-text text-muted">{{ trans("devices.device_update.field_internal_places_desc") }}</span>
                                        </li>
                                        <li>
                                            <span class="s1-text me-3">{{ trans("devices.device_update.field_uuid") }}</span>
                                            <span class="b1-text text-muted">{{ trans("devices.device_update.field_uuid_desc") }}</span>
                                        </li>
                                        <li>
                                            <span class="s1-text me-3">{{ trans("devices.device_update.field_purchase_date") }}</span>
                                            <span class="b1-text text-muted">{{ trans("devices.device_update.field_purchase_date_desc") }}</span>
                                        </li>
                                        <li>
                                            <span class="s1-text me-3">{{ trans("devices.device_update.field_purchase_currency") }}</span>
                                            <span class="b1-text text-muted">{{ trans("devices.device_update.field_purchase_currency_desc") }}</span>
                                        </li>
                                        <li>
                                            <span class="s1-text me-3">{{ trans("devices.device_update.field_purchase_cost") }}</span>
                                            <span class="b1-text text-muted">{{ trans("devices.device_update.field_purchase_cost_desc") }}</span>
                                        </li>
                                        <li>
                                            <span class="s1-text me-3">{{ trans("devices.device_update.field_purchase_reference") }}</span>
                                            <span class="b1-text text-muted">{{ trans("devices.device_update.field_purchase_reference_desc") }}</span>
                                        </li>
                                        <li>
                                            <span class="s1-text me-3">{{ trans("devices.device_update.field_warranty_start_date") }}</span>
                                            <span class="b1-text text-muted">{{ trans("devices.device_update.field_warranty_start_date_desc") }}</span>
                                        </li>
                                        <li>
                                            <span class="s1-text me-3">{{ trans("devices.device_update.field_warranty_end_date") }}</span>
                                            <span class="b1-text text-muted">{{ trans("devices.device_update.field_warranty_end_date_desc") }}</span>
                                        </li>
                                        <li>
                                            <span class="s1-text me-3">{{ trans("devices.device_update.field_warranty_months") }}</span>
                                            <span class="b1-text text-muted">{{ trans("devices.device_update.field_warranty_months_desc") }}</span>
                                        </li>
                                        <li>
                                            <span class="s1-text me-3">{{ trans("devices.device_update.field_amc_expire_date") }}</span>
                                            <span class="b1-text text-muted">{{ trans("devices.device_update.field_amc_expire_date_desc") }}</span>
                                        </li>
                                        <li>
                                            <span class="s1-text me-3">{{ trans("devices.device_update.field_amc_supplier") }}</span>
                                            <span class="b1-text text-muted">{{ trans("devices.device_update.field_amc_supplier_desc") }}</span>
                                        </li>
                                        <li>
                                            <span class="s1-text me-3">{{ trans("devices.device_update.field_notes") }}</span>
                                            <span class="b1-text text-muted">{{ trans("devices.device_update.field_notes_desc") }}</span>
                                        </li>
                                        <li>
                                            <span class="s1-text me-3">{{ trans("devices.device_update.field_order_number") }}</span>
                                            <span class="b1-text text-muted">{{ trans("devices.device_update.field_order_number_desc") }}</span>
                                        </li>
                                        <li>
                                            <span class="s1-text me-3">{{ trans("devices.device_update.field_asset_owner") }}</span>
                                            <span class="b1-text text-muted">{{ trans("devices.device_update.field_asset_owner_desc") }}</span>
                                        </li>
                                        <li>
                                            <span class="s1-text me-3">{{ trans("devices.device_update.field_supplier") }}</span>
                                            <span class="b1-text text-muted">{{ trans("devices.device_update.field_supplier_desc") }}</span>
                                        </li>
                                        <li>
                                            <span class="s1-text me-3">{{ trans("devices.device_update.field_ip") }}</span>
                                            <span class="b1-text text-muted">{{ trans("devices.device_update.field_ip_desc") }}</span>
                                        </li>
                                        <li>
                                            <span class="s1-text me-3">{{ trans("devices.device_update.field_mac") }}</span>
                                            <span class="b1-text text-muted">{{ trans("devices.device_update.field_mac_desc") }}</span>
                                        </li>
                                        <li>
                                            <span class="s1-text me-3">{{ trans("devices.device_update.field_device_type") }}</span>
                                            <span class="b1-text text-muted">{{ trans("devices.device_update.field_device_type_desc") }}</span>
                                        </li>
                                        <li>
                                            <span class="s1-text me-3">{{ trans("devices.device_update.field_device_from") }}</span>
                                            <span class="b1-text text-muted">{{ trans("devices.device_update.field_device_from_desc") }}</span>
                                        </li>
                                        <li>
                                            <span class="s1-text me-3">{{ trans("devices.device_update.field_stock_place") }}</span>
                                            <span class="b1-text text-muted">{{ trans("devices.device_update.field_stock_place_desc") }}</span>
                                        </li>
                                        <li>
                                            <span class="s1-text me-3">{{ trans("devices.device_update.field_requestable") }}</span>
                                            <span class="b1-text text-muted">{{ trans("devices.device_update.field_requestable_desc") }}</span>
                                        </li>
                                        <li>
                                            <span class="s1-text me-3">{{ trans("devices.device_update.field_high_priority") }}</span>
                                            <span class="b1-text text-muted">{{ trans("devices.device_update.field_high_priority_desc") }}</span>
                                        </li>
                                        <li>
                                            <span class="s1-text me-3">{{ trans("devices.device_update.field_sez_device") }}</span>
                                            <span class="b1-text text-muted">{{ trans("devices.device_update.field_sez_device_desc") }}</span>
                                        </li>
                                        <li>
                                            <span class="s1-text me-3">{{ trans("devices.device_update.field_custom_fields") }}</span>
                                            <span class="b1-text text-muted">{{ trans("devices.device_update.custom_fields_note") }}</span>
                                        </li>
                                    </ul>
                                    <p><i>{{ trans("devices.device_update.nvr_ch") }}.</i></p>
                                </div>

                                <a class="amg-btn amg-btn-link amg-btn-link-plain s1-text mt-2" id="readMoreBtn">
                                    {{ trans("devices.device_update.read_more") }}
                                </a>
                            </div>

                            <h4 class="h4-text" style="color:#7F7F7F">
                                {{ trans('devices.device_update.never_change_header') }}. <strong>{{ trans("devices.device_update.date_values") }}</strong>
                            </h4>
                        </div>
                    </div>
                </div>
            ...
            <div class="mt-3 tab-pane fade table-responsive border-top-0" id="update-info-tab">
                <div class="card">
                    <div class="card-body">
                        <div class="d-flex align-items-center gap-2 mb-4" data-select2-id="select2-data-5-1om7">
                            <div id="customLengthContainer" class="col-auto" data-select2-id="select2-data-4-sbib">
                                <select id="showSelect" class="showSelect">
                                    <option value="10" selected>{{ trans('devices.device_update.show') }}(10)</option>
                                    <option value="25">{{ trans('devices.device_update.show') }}(25)</option>
                                    <option value="50">{{ trans('devices.device_update.show') }}(50)</option>
                                    <option value="100">{{ trans('devices.device_update.show') }}(100)</option>
                                </select>
                            </div>
                            ...
                            <div>
                                <div class="amg-list-searchbar">
                                    ...
                                    <input type="text" class="amg-list-searchbar__input" id="tableSearch"
                                        placeholder="{{ trans('devices.device_update.search_placeholder') }}">
                                </div>
                            </div>
                            ...
                            <button class="amg-refresh-btn btn-reload-list">
                                ...
                                <span>{{ trans('devices.device_update.refresh') }}</span>
                            </button>
                        </div>

                        <table id="tblUpdate" class="table display">
                            <thead>
                                <tr>
                                    <th><h4>{{ trans('devices.device_update.id') }}</h4></th>
                                    <th><h4>{{ trans('devices.device_update.document_name') }}</h4></th>
                                    <th><h4>{{ trans('devices.device_update.total_upload') }}</h4></th>
                                    <th><h4>{{ trans('devices.device_update.total_success') }}</h4></th>
                                    <th><h4>{{ trans('devices.device_update.total_failure') }}</h4></th>
                                    <th><h4>{{ trans('devices.device_update.updated_at') }}</h4></th>
                                    <th><h4>{{ trans('devices.device_update.username') }}</h4></th>
                                    <th><h4>{{ trans('devices.device_update.actions') }}</h4></th>
                                </tr>
                            </thead>
                            <tbody></tbody>
                        </table>
...
<script type="text/javascript" src="{!! CommonHelper::asset('js/bulk/devices/update.js') !!}"></script>

<script>
    var config = {};
    config.bulk_update_info = "{{ url('devices/bulk/checkout-info') }}";
    config.document_download = "{{ url('uploads/bulk_documents') }}";
    config.token = "{{ csrf_token() }}";
    config.translations = {
        Search: '{{ trans('devices.device_update.search_js') }}',
        Refresh_List: '{{ trans('devices.device_update.refresh_list_js') }}',
        Download_Document: '{{ trans('devices.device_update.download_document_js') }}',
        Reload: '{{ trans('devices.device_update.reload_js') }}',
        press_enter_with_Search: '{{ trans('devices.device_update.press_enter_with_search_js') }}',
        please_enter_valid_search: '{{ trans('devices.device_update.please_enter_valid_search_js') }}',
        please_choose_file: '{{ trans('devices.device_update.please_choose_file_js') }}',
        Choose_file_for_update: '{{ trans('devices.device_update.choose_file_for_update_js') }}',
        please_upload_valid_xlsx: '{{ trans('devices.device_update.please_upload_valid_xlsx_js') }}',
        download: '{{ trans('devices.device_update.download') }}',
    };
    ...
    $(function () {
        ...
        $('#btnSave').on('click', function (e) {
            ...
            if (!file) {
                swal.fire('{{ trans('devices.device_update.please_choose_file_js') }}');
                return;
            }

            var ext = file.name.split('.').pop().toLowerCase();
            if (!['xlsx'].includes(ext)) {
                swal.fire('{{ trans('devices.device_update.please_upload_valid_xlsx_js') }}');
                return;
            }
        ...
        $('#readMoreBtn').on('click', function (e) {
            ...
            if (expanded) {
                ...
                $(this).text('{{ trans('devices.device_update.read_less') }}');
            } else {
                $('#cardScrollBody').removeClass('expanded');
                $(this).text('{{ trans('devices.device_update.read_more') }}');
                $('html, body').animate({
