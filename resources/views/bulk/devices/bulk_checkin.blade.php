...
@section('title', trans('devices.device_checkin.title'))
@section('content')
<style>
    :root {
        --primary: #1a2e5a;
        ...
        <div class="py-3">
            <div class="container-fluid">
                <button onclick="location.href='{{ url('devices') }}'"
                    ...
                    <h2 class="h2-text mb-0">{{ trans('devices.device_checkin.bulk_checkin') }}</h2>
                </button>
            </div>
        </div>
    </div>
            ...
            <ul class="nav nav-underline" id="myTab" role="tablist">
                <li class="nav-item">
                    <a class="nav-link active" id="info-tab-link" data-bs-toggle="tab" href="#info-tab" role="tab">
                        <span>{{ trans('devices.device_checkin.tab_bulk_checkin') }}</span>
                    </a>
                </li>
                <li class="nav-item">
                    <a class="nav-link" id="checkin-info-tab-link" data-bs-toggle="tab" href="#checkin-info-tab" role="tab">
                        <span>{{ trans('devices.device_checkin.tab_checkin_info') }}</span>
                    </a>
                </li>
            </ul>
        </div>
        ...
        <div class="tab-content">
            <div class="tab-pane fade show active container-fluid pt-3 pb-5" id="info-tab">
                ...
                <div class="alert customize-alert alert-dismissible 
                    {{ session('msg')['status'] == 'success' ? 'alert-light-success bg-success-subtle text-success' : 'alert-light-danger bg-danger-subtle text-danger' }}
                ...
                @if(session()->has('success') || session()->has('fail'))
                    ...
                    <div class="alert customize-alert alert-dismissible alert-light-success bg-success-subtle text-success fade show remove-close-icon" role="alert">
                        ...
                        <div class="d-flex align-items-center me-3 gap-3 me-md-0">
                            ...
                            {{ trans('devices.device_checkin.import_success', ['count' => session('success', 0)]) }}
                        </div>
                    </div>
                    @endif
                    ...
                    <div class="alert customize-alert alert-dismissible alert-light-danger bg-danger-subtle text-danger fade show remove-close-icon" role="alert">
                        ...
                        <div class="d-flex align-items-start me-3 gap-3 me-md-0">
                            ...
                            <div>
                                {{ trans('devices.device_checkin.import_failed', ['count' => session('fail', 0)]) }}
                    ...
                    <div class="alert customize-alert alert-dismissible alert-light-warning bg-warning-subtle text-warning fade show remove-close-icon" role="alert">
                        ...
                        <div class="d-flex align-items-center me-3 gap-3 me-md-0">
                            ...
                            {{ trans('devices.device_checkin.import_default_success') }}
                        </div>
                    </div>
                    @endif
                @endif
                        ...
                        <div class="col-12 col-sm-5">
                            <div class="drop-zone" id="dropZone">
                                ...
                                <div id="defaultState">
                                    ...
                                    <p class="drag-text mt-3 mb-2 b1-text">{{ trans("devices.device_checkin.drag_drop_file") }}</p>
                                    <button class="amg-btn amg-btn-secondary" style="min-width:203px;">
                                        ...
                                        <span>{{ trans("devices.device_checkin.upload_file_btn") }}</span>
                                    </button>
                                    <span class="file-label b1-text">{{ trans("devices.device_checkin.no_file_selected") }}</span>
                                </div>
                                ...
                                <div id="selectedState" style="display:none;">
                                    ...
                                    <p class="file-selected-name mb-2 mt-3" id="selectedFileName">{{ trans('devices.device_checkin.format_excel_sheet') }}</p>
                                    <button class="amg-btn amg-btn-primary" style="min-width:203px" type="button" id="btnSave">
                                        <svg width="14" height="17" viewBox="0 0 14 17" fill="none" xmlns="http://www.w3.org/2000/svg">
                                            <path d="M5 12.5924L9 12.5924C9.55 12.5924 10 12.1424 10 11.5924L10 6.59244L11.59 6.59244C12.48 6.59244 12.93 5.51244 12.3 4.88244L7.71 0.292444C7.61749 0.19974 7.5076 0.126193 7.38662 0.0760114C7.26565 0.02583 7.13597 4.15368e-10 7.005 4.07744e-10C6.87403 4.00121e-10 6.74435 0.02583 6.62338 0.0760114C6.5024 0.126193 6.39251 0.19974 6.3 0.292444L1.71 4.88244C1.08 5.51244 1.52 6.59244 2.41 6.59244L4 6.59244L4 11.5924C4 12.1424 4.45 12.5924 5 12.5924ZM1 14.5924L13 14.5924C13.55 14.5924 14 15.0424 14 15.5924C14 16.1424 13.55 16.5924 13 16.5924L1 16.5924C0.45 16.5924 -9.39615e-10 16.1424 -9.076e-10 15.5924C-8.75586e-10 15.0424 0.45 14.5924 1 14.5924Z" fill="currentColor" />
                                        </svg>
                                        <span>{{ trans('devices.device_checkin.save_checkin') }}</span>
                                    </button>
                                    <button class="amg-btn amg-btn-link amg-btn-link-plain mt-2" id="btnRemove">{{ trans('devices.device_checkin.remove_file') }}</button>
                                </div>
                            </div>
                        ...
                        <div class="col-12 col-sm-7">
                            <div class="right-panel h-100">
                                <a class="amg-btn amg-btn-white amg-btn-lg"
                                    href="{!! CommonHelper::asset('samples/DeviceCheckinFormat.xlsx') !!}"
                                    target="_blank">
                                    ...
                                    <span class="font-weight-normal">{{ trans('devices.device_checkin.download_template') }}</span>
                                </a>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="col-12 px-5">
                        <div class="instructions-card mt-3">
                            <h4 class="h4-text" style="color:#7F7F7F">
                                {{ trans("devices.device_checkin.instructions_title") }}
                            </h4>
                            ...
                            <div class="ps-3 ps-sm-4">
                                <div class="card-scroll-body" id="cardScrollBody">
                                    <ul class="ps-3">
                                        <li>
                                            <span class="s1-text me-3">{{ trans("devices.device_checkin.field_serial") }} <span class="mandatory"></span></span>
                                            <span class="b1-text text-muted">{{ trans("devices.device_checkin.serial_field_desc") }}</span>
                                        </li>
                                        <li>
                                            <span class="s1-text me-3">{{ trans("devices.device_checkin.field_status") }} <span class="mandatory"></span></span>
                                            <span class="b1-text text-muted">{{ trans("devices.device_checkin.status_field_desc") }}</span>
                                        </li>
                                        <li>
                                            <span class="s1-text me-3">{{ trans("devices.device_checkin.field_checkin_reason") }}
                                                @if(in_array(strtolower(trim(config('app.client'))), ['knightfrank', 'rolepermission']))
                                                <span class="mandatory"></span>
                                                @endif
                                            </span>
                                            <span class="b1-text text-muted">{{ trans("devices.device_checkin.checkin_reason_field_desc") }}</span>
                                        </li>
                                        @if(in_array(config('app.client'), ["knightfrank"]))
                                        <li>
                                            <span class="s1-text me-3">{{ trans("devices.device_checkin.field_notes") }} <span class="mandatory"></span></span>
                                            <span class="b1-text text-muted">{{ trans("devices.device_checkin.notes_field_desc_kf") }}</span>
                                        </li>
                                        @else
                                        <li>
                                            <span class="s1-text me-3">{{ trans("devices.device_checkin.field_notes") }}</span>
                                            <span class="b1-text text-muted">{{ trans("devices.device_checkin.notes_field_desc") }}</span>
                                        </li>
                                        @endif
                                    </ul>
                                    <p><i>{{ trans("devices.device_checkin.never_change_first_row") }}</i></p>
                                </div>

                                <a class="amg-btn amg-btn-link amg-btn-link-plain s1-text mt-2" id="readMoreBtn">
                                    {{ trans("devices.device_checkin.read_more") }}
                                </a>
                            </div>

                            <h4 class="h4-text" style="color:#7F7F7F">
                                {{ trans('devices.device_checkin.never_change_header') }}
                            </h4>
                        </div>
                    </div>
                </div>
            ...
            <div class="mt-3 tab-pane fade table-responsive border-top-0" id="checkin-info-tab">
                <div class="card">
                    <div class="card-body">
                        <div class="d-flex align-items-center gap-2 mb-4" data-select2-id="select2-data-5-1om7">
                            <div id="customLengthContainer" class="col-auto" data-select2-id="select2-data-4-sbib">
                                <select id="showSelect" class="showSelect">
                                    <option value="10" selected>{{ trans('devices.device_checkin.show') }}(10)</option>
                                    <option value="25">{{ trans('devices.device_checkin.show') }}(25)</option>
                                    <option value="50">{{ trans('devices.device_checkin.show') }}(50)</option>
                                    <option value="100">{{ trans('devices.device_checkin.show') }}(100)</option>
                                </select>
                            </div>
                            ...
                            <div>
                                <div class="amg-list-searchbar">
                                    ...
                                    <input type="text" class="amg-list-searchbar__input" id="tableSearch"
                                        placeholder="{{ trans('devices.device_checkin.search_placeholder') }}">
                                </div>
                            </div>
                            ...
                            <button class="amg-refresh-btn btn-reload-list">
                                ...
                                <span>{{ trans('devices.device_checkin.refresh') }}</span>
                            </button>
                        </div>
                        ...
                        <table id="tblHistory" class="table display">
                            <thead>
                                <tr>
                                    <th><h4>{{ trans('devices.device_checkin.id') }}</h4></th>
                                    <th><h4>{{ trans('devices.device_checkin.document_name') }}</h4></th>
                                    <th><h4>{{ trans('devices.device_checkin.total_upload') }}</h4></th>
                                    <th><h4>{{ trans('devices.device_checkin.total_success') }}</h4></th>
                                    <th><h4>{{ trans('devices.device_checkin.total_failure') }}</h4></th>
                                    <th><h4>{{ trans('devices.device_checkin.updated_at') }}</h4></th>
                                    <th><h4>{{ trans('devices.device_checkin.username') }}</h4></th>
                                    <th><h4>{{ trans('devices.device_checkin.actions') }}</h4></th>
                                </tr>
                            </thead>
                            <tbody></tbody>
                        </table>
...
<script type="text/javascript" src="{!! CommonHelper::asset('js/bulk/devices/bulk_checkin.js') !!}"></script>

<script>
    var config = {};
    config.bulk_checkin_info = "{{ url('devices/bulk/checkin-info') }}";
    config.document_download = "{{ url('uploads/bulk_documents') }}";
    config.token = "{{ csrf_token() }}";
    config.translations = {
        Search: '{{ trans('devices.device_checkin.search_js') }}',
        Refresh_List: '{{ trans('devices.device_checkin.refresh_list_js') }}',
        Download_Document: '{{ trans('devices.device_checkin.download_document_js') }}',
        Reload: '{{ trans('devices.device_checkin.reload_js') }}',
        press_enter_with_Search: '{{ trans('devices.device_checkin.press_enter_with_search_js') }}',
        please_enter_valid_search: '{{ trans('devices.device_checkin.please_enter_valid_search_js') }}',
        please_choose_file: '{{ trans('devices.device_checkin.please_choose_file_js') }}',
        Choose_file_for_checkin: '{{ trans('devices.device_checkin.choose_file_for_checkin_js') }}',
        please_upload_valid_xlsx: '{{ trans('devices.device_checkin.please_upload_valid_xlsx_js') }}',
        download: '{{ trans('devices.device_checkin.download') }}',
    };
    ...
    $(function () {
        ...
        $('#btnSave').on('click', function (e) {
            ...
            if (!file) {
                swal.fire('{{ trans('devices.device_checkin.please_choose_file_js') }}');
                return;
            }

            var ext = file.name.split('.').pop().toLowerCase();
            if (!['xlsx'].includes(ext)) {
                swal.fire('{{ trans('devices.device_checkin.please_upload_valid_xlsx_js') }}');
                return;
            }
        ...
        $('#readMoreBtn').on('click', function (e) {
            ...
            if (expanded) {
                ...
                $(this).text('{{ trans('devices.device_checkin.read_less') }}');
            } else {
                $('#cardScrollBody').removeClass('expanded');
                $(this).text('{{ trans('devices.device_checkin.read_more') }}');
                $('html, body').animate({
