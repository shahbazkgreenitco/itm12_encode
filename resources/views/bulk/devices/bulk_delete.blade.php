...
@section('title', trans('devices.device_delete.title'))
...
<section class="content">
        ...
        <div class="py-3">
            <div class="container-fluid">
                <button onclick="location.href='{{ url('devices') }}'"
                    ...
                    <h2 class="h2-text mb-0">{{ trans('devices.device_delete.bulk_delete') }}</h2>
                </button>
            </div>
        </div>
    </div>
    ...
    <main class="main-content" id="mainContent">
            ...
            <ul class="nav nav-underline" id="myTab" role="tablist">
                <li class="nav-item">
                    <a class="nav-link active" id="info-tab-link" data-bs-toggle="tab" href="#info-tab" role="tab">
                        <span>{{ trans('devices.device_delete.tab_bulk_delete') }}</span>
                    </a>
                </li>
                <li class="nav-item">
                    <a class="nav-link" id="delete-info-tab-link" data-bs-toggle="tab" href="#delete-info-tab" role="tab">
                        <span>{{ trans('devices.device_delete.tab_delete_information') }}</span>
                    </a>
                </li>
            </ul>
        </div>
        ...
        <div class="tab-content">
                ...
                @if(session()->has('success') || session()->has('fail'))
                    ...
                    <div class="alert customize-alert alert-dismissible alert-light-success bg-success-subtle text-success fade show remove-close-icon" role="alert">
                        ...
                        <div class="d-flex align-items-center me-3 gap-3 me-md-0">
                            ...
                            {{ trans('devices.device_delete.import_success', ['count' => session('success', 0)]) }}
                        </div>
                    </div>
                    @endif
                    ...
                    <div class="alert customize-alert alert-dismissible alert-light-danger bg-danger-subtle text-danger fade show remove-close-icon" role="alert">
                        ...
                        <div class="d-flex align-items-start me-3 gap-3 me-md-0">
                            ...
                            <div>
                                {{ trans('devices.device_delete.import_failed', ['count' => session('fail', 0)]) }}
                    ...
                    <div class="alert customize-alert alert-dismissible alert-light-warning bg-warning-subtle text-warning fade show remove-close-icon" role="alert">
                        ...
                        <div class="d-flex align-items-center me-3 gap-3 me-md-0">
                            ...
                            {{ trans('devices.device_delete.import_default_success') }}
                        </div>
                    </div>
                    @endif
                @endif
                        ...
                        <div class="col-12 col-sm-5">
                            <div class="drop-zone" id="dropZone">
                                ...
                                <div id="defaultState">
                                    ...
                                    <p class="drag-text mt-3 mb-2 b1-text">{{ trans("devices.device_delete.drag_drop_file") }}</p>
                                    <button class="amg-btn amg-btn-secondary" style="min-width:203px;">
                                        ...
                                        <span>{{ trans("devices.device_delete.upload_file_btn") }}</span>
                                    </button>
                                    <span class="file-label b1-text">{{ trans("devices.device_delete.no_file_selected") }}</span>
                                </div>
                                ...
                                <div id="selectedState" style="display:none;">
                                    ...
                                    <p class="file-selected-name mb-2 mt-3" id="selectedFileName">{{ trans('devices.device_delete.format_excel_sheet') }}</p>
                                    <button class="amg-btn amg-btn-primary" style="min-width:203px" type="button" id="btnSave">
                                        <svg width="14" height="17" viewBox="0 0 14 17" fill="none" xmlns="http://www.w3.org/2000/svg">
                                            <path d="M5 12.5924L9 12.5924C9.55 12.5924 10 12.1424 10 11.5924L10 6.59244L11.59 6.59244C12.48 6.59244 12.93 5.51244 12.3 4.88244L7.71 0.292444C7.61749 0.19974 7.5076 0.126193 7.38662 0.0760114C7.26565 0.02583 7.13597 4.15368e-10 7.005 4.07744e-10C6.87403 4.00121e-10 6.74435 0.02583 6.62338 0.0760114C6.5024 0.126193 6.39251 0.19974 6.3 0.292444L1.71 4.88244C1.08 5.51244 1.52 6.59244 2.41 6.59244L4 6.59244L4 11.5924C4 12.1424 4.45 12.5924 5 12.5924ZM1 14.5924L13 14.5924C13.55 14.5924 14 15.0424 14 15.5924C14 16.1424 13.55 16.5924 13 16.5924L1 16.5924C0.45 16.5924 -9.39615e-10 16.1424 -9.076e-10 15.5924C-8.75586e-10 15.0424 0.45 14.5924 1 14.5924Z" fill="currentColor" />
                                        </svg>
                                        <span>{{ trans('devices.device_delete.save_delete') }}</span>
                                    </button>
                                    <button class="amg-btn amg-btn-link amg-btn-link-plain mt-2" id="btnRemove">{{ trans('devices.device_delete.remove_file') }}</button>
                                </div>
                            </div>

                            <form id="importForm" method="POST" action="{{ url('devices/bulk/delete') }}" enctype="multipart/form-data" style="display:none;">
                                @csrf
                        ...
                        <div class="col-12 col-sm-7">
                            <div class="right-panel h-100">
                                <a class="amg-btn amg-btn-white amg-btn-lg"
                                    href="{!! CommonHelper::asset('samples/DeviceDeleteFormat.xlsx') !!}"
                                    target="_blank">
                                    ...
                                    <span class="font-weight-normal">{{ trans('devices.device_delete.download_template') }}</span>
                                </a>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="col-12 px-5">
                        <div class="instructions-card mt-3">
                            <h4 class="h4-text" style="color:#7F7F7F">
                                {{ trans("devices.device_delete.instructions_title") }}
                            </h4>
                            ...
                            <div class="ps-3 ps-sm-4">
                                <div class="card-scroll-body" id="cardScrollBody">
                                    <ul class="ps-3">
                                        <li>
                                            <span class="s1-text me-3">{{ trans("devices.device_delete.field_serial") }} <span class="mandatory"></span></span>
                                            <span class="b1-text text-muted">{{ trans("devices.device_delete.field_serial_desc") }}</span>
                                        </li>
                                    </ul>
                                    <p><i>{{ trans("devices.device_delete.nvr_ch") }}</i></p>
                                </div>

                                <a class="amg-btn amg-btn-link amg-btn-link-plain s1-text mt-2" id="readMoreBtn">
                                    {{ trans("devices.device_delete.read_more") }}
                                </a>
                            </div>

                            <h4 class="h4-text" style="color:#7F7F7F">
                                {{ trans('devices.device_delete.never_change_header') }}. <strong>{{ trans("devices.device_delete.date_values") }}</strong>
                            </h4>
                        </div>
                    </div>
                </div>
            ...
            <div class="mt-3 tab-pane fade table-responsive border-top-0" id="delete-info-tab">
                <div class="card">
                    <div class="card-body">
                        <div class="d-flex align-items-center gap-2 mb-4">
                            <div id="customLengthContainer" class="col-auto">
                                <select id="showSelect" class="showSelect">
                                    <option value="10" selected>{{ trans('devices.device_delete.show') }}(10)</option>
                                    <option value="25">{{ trans('devices.device_delete.show') }}(25)</option>
                                    <option value="50">{{ trans('devices.device_delete.show') }}(50)</option>
                                    <option value="100">{{ trans('devices.device_delete.show') }}(100)</option>
                                </select>
                            </div>
                            ...
                            <div>
                                <div class="amg-list-searchbar">
                                    ...
                                    <input type="text" class="amg-list-searchbar__input" id="tableSearch"
                                        placeholder="{{ trans('devices.device_delete.search_placeholder') }}">
                                </div>
                            </div>
                            ...
                            <button class="amg-refresh-btn btn-reload-list">
                                ...
                                <span>{{ trans('devices.device_delete.refresh') }}</span>
                            </button>
                        </div>
                        ...
                        <table id="tblHistory" class="table display">
                            <thead>
                                <tr>
                                    <th><h4>{{ trans('devices.device_delete.document_name') }}</h4></th>
                                    <th><h4>{{ trans('devices.device_delete.total_upload') }}</h4></th>
                                    <th><h4>{{ trans('devices.device_delete.total_success') }}</h4></th>
                                    <th><h4>{{ trans('devices.device_delete.total_failure') }}</h4></th>
                                    <th><h4>{{ trans('devices.device_delete.updated_at') }}</h4></th>
                                    <th><h4>{{ trans('devices.device_delete.username') }}</h4></th>
                                    <th><h4>{{ trans('devices.device_delete.actions') }}</h4></th>
                                </tr>
                            </thead>
                            <tbody></tbody>
                        </table>
...
<script type="text/javascript" src="{!! CommonHelper::asset('js/bulk/devices/bulk_delete.js') !!}"></script>

<script>
    var config = {};
    config.bulk_delete_info = "{{ url('devices/bulk/delete-info') }}";
    config.document_download = "{{ url('uploads/bulk_documents') }}";
    config.token = "{{ csrf_token() }}";
    config.translations = {
        Search: '{{ trans('devices.device_delete.search_js') }}',
        Refresh_List: '{{ trans('devices.device_delete.refresh_list_js') }}',
        Download_Document: '{{ trans('devices.device_delete.download_document_js') }}',
        Reload: '{{ trans('devices.device_delete.reload_js') }}',
        press_enter_with_Search: '{{ trans('devices.device_delete.press_enter_with_search_js') }}',
        please_enter_valid_search: '{{ trans('devices.device_delete.please_enter_valid_search_js') }}',
        please_choose_file: '{{ trans('devices.device_delete.please_choose_file_js') }}',
        Choose_file_for_delete: '{{ trans('devices.device_delete.choose_file_for_delete_js') }}',
        please_upload_valid_xlsx: '{{ trans('devices.device_delete.please_upload_valid_xlsx_js') }}',
        download: '{{ trans('devices.device_delete.download') }}',
    };
    ...
    $(function () {
        ...
        $('#btnSave').on('click', function (e) {
            ...
            if (!file) {
                swal.fire('{{ trans('devices.device_delete.please_choose_file_js') }}');
                return;
            }

            var ext = file.name.split('.').pop().toLowerCase();
            if (!['xlsx'].includes(ext)) {
                swal.fire('{{ trans('devices.device_delete.please_upload_valid_xlsx_js') }}');
                return;
            }
        ...
        $('#readMoreBtn').on('click', function (e) {
            ...
            if (expanded) {
                ...
                $(this).text('{{ trans('devices.device_delete.read_less') }}');
            } else {
                $('#cardScrollBody').removeClass('expanded');
                $(this).text('{{ trans('devices.device_delete.read_more') }}');
                $('html, body').animate({
