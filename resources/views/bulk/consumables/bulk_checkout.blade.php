...
* [1.0.1] - Changes for translation changes
* ------------------------------------------------------------
*/
--}}

@extends('layouts.layout1')
@section('title', trans("consumables.bulk_checkout.view.header"))
                ...
                <div class="py-3">
                    <div class="container-fluid ps-0">
                        <button onclick="location.href='{{ url('consumables') }}'" class="d-flex gap-3 align-items-center bg-transparent outline-none border-0">
                            ...
                            <h2 class="h2-text mb-0">{{ trans('consumables.bulk_checkout.view.heading') }}</h2>
                        </button>
                    </div>
                </div>
            </div>
                    ...
                    <ul class="nav nav-underline" id="myTab" role="tablist">
                        <li class="nav-item">
                            <a class="nav-link active" id="info-tab-link" data-bs-toggle="tab" href="#info-tab" role="tab">
                                <span>{{ trans('consumables.bulk_checkout.view.bulk_checkout') }}</span>
                            </a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link" id="import-info-tab-link" data-bs-toggle="tab" href="#import-info-tab" role="tab">
                                <span>{{ trans('consumables.bulk_checkout.view.checkout_info') }}</span>
                            </a>
                        </li>
                    </ul>
                </div>
                            ...
                            @if(session('success', 0) > 0)
                                <div class="alert customize-alert alert-dismissible alert-light-success bg-success-subtle text-success fade show remove-close-icon" role="alert">
                                    ...
                                    <div class="d-flex align-items-center me-3 gap-3 me-md-0">
                                        ...
                                        {{ trans("consumables.controller.Consumable_checked_out_successfully_to") }}:{{ session('success', 0) }}
                                    </div>
                                </div>
                            @endif
                            ...
                            @if(session('fail', 0) > 0)
                                <div class="alert customize-alert alert-dismissible alert-light-danger bg-danger-subtle text-danger fade show remove-close-icon" role="alert">
                                    ...
                                    <div class="d-flex align-items-start me-3 gap-3 me-md-0">
                                        ...
                                        <div>
                                            {{ trans('consumables.bulk_checkout.view.failure_records_count') }} {{ session('fail', 0) }}
                            ...
                            @if(session('success', 0) == 0 && session('fail', 0) == 0)
                                <div class="alert customize-alert alert-dismissible alert-light-warning bg-warning-subtle text-warning fade show remove-close-icon" role="alert">
                                    ...
                                    <div class="d-flex align-items-center me-3 gap-3 me-md-0">
                                        ...
                                        {{ trans('consumables.bulk_checkout.view.no_items_checkout') }}
                                    </div>
                                </div>
                            @endif
                        @endif
                            ...
                            <div class="row g-3 align-items-stretch">
                                <div class="col-12 col-sm-3">
                                    <div class="drop-zone" id="dropZone">
                                        ...
                                        <div id="defaultState">
                                            ...
                                            <p class="drag-text mt-2 mb-1">{{ trans('consumables.bulk_checkout.view.drag') }} &amp; {{ trans('consumables.bulk_checkout.view.drop_here') }}</p>
                                            <button class="amg-btn amg-btn-secondary upload-doc-btn">
                                                ...
                                                <span>{{ trans('consumables.bulk_checkout.view.upload_file') }}</span>
                                            </button>
                                            <span class="file-label">{{ trans('consumables.bulk_checkout.view.files_selected') }}</span>
                                        </div>
                                        <div id="selectedState" style="display:none;">
                                            ...
                                            <button class="amg-btn amg-btn-link amg-btn-link-plain p-0" id="btnRemove">{{ trans('consumables.bulk_checkout.view.remove_file') }}</button>
                                        </div>
                                    </div>
                                ...
                                <div class="col-12 col-sm-9">
                                    <div class="right-panel h-100">
                                        <a class="amg-btn amg-btn-white amg-btn-md" href="{!! CommonHelper::asset('samples/ConsumableCheckoutFormat.xlsx') !!}" target="_blank">
                                            ...
                                            <span class="font-weight-normal">{{ trans('consumables.bulk_checkout.view.download_template_button') }}</span>
                                        </a>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="col-12 px-3">
                                <div class="instructions-card mt-1">
                                    <h4 class="b2-text" style="color:#7F7F7F">
                                        {{ trans('consumables.bulk_checkout.view.read_notes') }}
                                    </h4>
                                    <div class="ps-3 ps-sm-4">
                                        <div class="card-scroll-body" id="cardScrollBody">
                                            <ul class="ps-3">
                                                <li>
                                                    <span class="b3-text me-3">{{ trans('consumables.bulk_checkout.view.batch_code') }}
                                                    <span class="mandatory"></span></span>
                                                    <span class="b5-text text-muted">{{ trans('consumables.bulk_checkout.view.batch_code_text') }}</span>
                                                </li>
                                                <li>
                                                    <span class="b3-text me-3">{{ trans('consumables.bulk_checkout.view.assigned_for') }}
                                                    <span class="mandatory"></span></span>
                                                    <span class="b5-text text-muted">{{ trans('consumables.bulk_checkout.view.assigned_for_text') }}</span>
                                                </li>
                                                <li>
                                                    <span class="b3-text me-3">{{ trans('consumables.bulk_checkout.view.assigned_to') }}
                                                    <span class="mandatory"></span></span>
                                                    <span class="b5-text text-muted">{{ trans('consumables.bulk_checkout.view.assigned_to_text') }}</span>
                                                </li>
                                                <li>
                                                    <span class="b3-text">{{ trans('consumables.bulk_checkout.view.notes') }}</span>
                                                    <span class="b5-text text-muted">{{ trans('consumables.bulk_checkout.view.notes_text') }}</span>
                                                </li>
                                            </ul>
                                        </div>
                                    </div>
                                    <h4 class="b2-text" style="color:#7F7F7F">
                                        {{ trans('consumables.bulk_checkout.view.change_first_row') }}
                                    </h4>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="card-body">
                                <div class="d-flex align-items-center gap-2 mb-2" data-select2-id="select2-data-5-1om7">
                                    <div id="customLengthContainer" class="col-auto" data-select2-id="select2-data-4-sbib">
                                        <select id="showSelect" class="showSelect amg-table-pagination-dropdown userModulePageLenth user-list-page-length">
                                            <option value="10" selected>{{ trans('consumables.bulk_checkout.view.show') }} (10)</option>
                                            <option value="25">{{ trans('consumables.bulk_checkout.view.show') }} (25)</option>
                                            <option value="50">{{ trans('consumables.bulk_checkout.view.show') }} (50)</option>
                                            <option value="100">{{ trans('consumables.bulk_checkout.view.show') }} (100)</option>
                                        </select>
                                    </div>
                                    ...
                                    <div>
                                        <div class="amg-list-searchbar">
                                            ...
                                            <input type="text" class="amg-list-searchbar__input" id="tableSearch" placeholder="{{ trans('consumables.bulk_checkout.view.search') }}...">
                                        </div>
                                    </div>
                                    ...
                                    <button class="amg-refresh-btn btn-reload-list">
                                        ...
                                        <span>{{ trans('consumables.bulk_checkout.view.refresh') }}</span>
                                    </button>
                                </div>
                                <table id="tblCheckout" class="table display">
                                    <thead>
                                        <tr>
                                            <th>
                                                <h4>{{ trans('consumables.bulk_checkout.table.id') }}</h4>
                                            </th>
                                            <th>
                                                <h4>{{ trans('consumables.bulk_checkout.table.document_name') }}</h4>
                                            </th>
                                            <th>
                                                <h4>{{ trans('consumables.bulk_checkout.table.total_upload') }}</h4>
                                            </th>
                                            <th>
                                                <h4>{{ trans('consumables.bulk_checkout.table.total_success') }}</h4>
                                            </th>
                                            <th>
                                                <h4>{{ trans('consumables.bulk_checkout.table.total_failure') }}</h4>
                                            </th>
                                            <th>
                                                <h4>{{ trans('consumables.bulk_checkout.table.updated_at') }}</h4>
                                            </th>
                                            <th>
                                                <h4>{{ trans('consumables.bulk_checkout.table.username') }}</h4>
                                            </th>
                                            <th>
                                                <h4>{{ trans('consumables.bulk_checkout.table.actions') }}</h4>
                                            </th>
                                        </tr>
                                    </thead>
                                    <tbody></tbody>
...
@push('scripts')
    ...
    <script src="{!! CommonHelper::asset('js/bulk/consumables/bulk_checkout.js') !!}"></script>
    <script>
        var config = {};
        config.bulk_checkout_info = "{{ url('consumables/bulk/checkout-info')}}";
        config.document_download = "{{ url('uploads/bulk_documents') }}";
        config.token = "{{ csrf_token() }}";

        config.translations = {
            Download_Document: "{{ trans('consumables.bulk_checkout.config.Download_Documents') }}",
            search: "{{ trans('consumables.bulk_checkout.config.search') }}",
            reload: "{{ trans('consumables.bulk_checkout.config.reload') }}",
            please_enter_valid_search: "{{ trans('consumables.bulk_checkout.config.please_enter_valid_search') }}",
            import_file: "{{ trans('consumables.bulk_checkout.config.import_file') }}",
            Please_upload_valid_xlsx: "{{ trans('consumables.bulk_checkout.config.upload_valid') }}",
        };
        ...
        $(function() {
            ...
            $('#btnSave').on('click', function(e) {
                ...
                if (!file) {
                    alert(@json(trans('consumables.bulk_checkout.view.no_files_selected')));
                    return;
                }
                var ext = file.name.split('.').pop().toLowerCase();
                if (!['xlsx'].includes(ext)) {
                    alert(@json(trans("consumables.bulk_checkout.view.invalid_upload_xlsx")));
                    return;
                }
            ...
            $('#dropZone').on('drop', function(e) {
                ...
                if (files.length > 0) {
                    ...
                    if (!['xlsx'].includes(ext)) {
                        alert(@json(trans("consumables.bulk_checkout.view.invalid_drop_xlsx")));
                        return;
                    }
                    droppedFile = files[0];
