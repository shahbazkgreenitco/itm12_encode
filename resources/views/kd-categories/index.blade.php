...
@section('title', 'KD Categories')

@section('content')
    <section class="content">
        <!-- section header -->
            <div class="header-actions-wrapper d-flex align-items-center justify-content-between pe-4">
                <h3 class="h3-text mb-0">{{ trans('config.kd_category_fields.kd_category_heading') }}</h3>
                <div class="d-flex gap-8">
                    <!-- Download -->
                    @if (Auth::user()->hasPermissionTo('KnowledgeDocumentCategoryDownload'))
                        <button class="header-icon-btn-only header-icon-btn-only-sm btn-kd-category-export" id="btn-kd-category-export" type="button" data-bs-toggle="tooltip" title="{{ trans('config.kd_category_fields.download_excel') }}">
                    ...
                    @endif
                    <button class="header-icon-btn header-icon-btn-sm btn-open-filter" type="button" data-bs-toggle="tooltip" title="{{ trans('config.kd_category_fields.filter') }}">
                        ...
                        <span class="b6-text opacity-50">{{ trans('config.kd_category_fields.filter') }}</span>
                        <span class="filter-count-badge d-none" aria-label="Active filters">0</span>
                    </button>
                    <!-- Add -->
                    @if(Auth::user()->hasPermissionTo('KnowledgeDocumentCategoryAdd'))
                        <button class="amg-btn amg-btn-primary amg-btn-sm open-add-modal" type="button" data-bs-toggle="tooltip" title="{{ trans('config.kd_category_fields.add_kd_category') }} ">
                                ...
                                <span>{{ trans('config.kd_category_fields.add_kd_category') }}</span>
                        </button>
                    @endif
                </div>
            </div>
                    ...
                    <div class="card rounded-0">
                        <div class="card-body">
                            <div class="d-flex align-items-center gap-2 mb-1">                       
                                    <div class="col-auto">
                                        <select id="kd-category-page-length" class="amg-table-pagination-dropdown userModulePageLenth user-list-page-length">
                                            <option value="10" selected>{{ trans('config.kd_category_fields.show') }} (10)</option>
                                            <option value="25">{{ trans('config.kd_category_fields.show') }} (25)</option>
                                            <option value="50">{{ trans('config.kd_category_fields.show') }} (50)</option>
                                            <option value="100">{{ trans('config.kd_category_fields.show') }} (100)</option>
                                        </select>
                                    </div>
                                    ...
                                    <div class="amg-list-searchbar">
                                        ...
                                        <input type="text" class="amg-list-searchbar__input kd-category-list-search searchbox plain-search" id="kd-category-list-search" placeholder="{{ trans('config.kd_category_fields.search') }}">
                                    </div>
                                    <button class="amg-refresh-btn btn-reload-list">
                                        ...
                                        <span>{{  trans('config.kd_category_fields.refresh') }}</span>
                                    </button>
                                </div>
                            <div class="table-responsive" style="overflow-y:hidden;">
                                ...
                                <thead>
                                    <tr>
                                        <th>
                                            <h4>{{ trans('config.kd_category_fields.category_name') }}</h4>
                                        </th>
                                        <th>
                                            <h4>{{ trans('config.kd_category_fields.parent_category') }}</h4>
                                        </th>
                                        <th>
                                            <h4>{{ trans('config.kd_category_fields.company') }}</h4>
                                        </th>
                                        <th>
                                            <h4>{{ trans('config.kd_category_fields.department') }}</h4>
                                        </th>
                                        <th>
                                            <h4>{{ trans('config.kd_category_fields.updated_at') }}</h4>
                                        </th>
                                        <th>
                                            <h4>{{ trans('config.kd_category_fields.actions') }}</h4>
                                        </th>
                                    </tr>
                                </thead>
                                <tbody></tbody>
            ...
            @include('kd-categories.add-category-modal')
            @include('kd-categories.filter-category-modal')
    </section>  
...
@push('scripts')
    ...
    <script src="{!! CommonHelper::asset('js/kd-category/index.js') !!}"></script>
    <script src="{!! CommonHelper::asset('js/common.js') !!}"></script>
<script>
$(function () {
    var config = {
        token: "{{ csrf_token() }}",
        dashboard_company_id: @json($company),
        selected_company: @json($user_detail),
        permissions: {!! json_encode($permissionArray) !!},
        datatable_translations:  @json(trans('datatable.datatable')),
        url: {
            getList: "{{ url('knowledge_document/jx-category-list') }}",
            get_company_by_user_access : "{{ url('getCompanyByUserAccess') }}",
            // department : "{{ url('getDepartmentsWithCompanyByQuery') }}",
            department : "{{ url('tickets/departments') }}",
            problem_categories_by_dept : "{{ url('knowledge_document/problem-categories/by-dept') }}",
            store: "{{ url('knowledge_document/categories/store') }}",
            editCategory: "{{ url('knowledge_document/edit_category') }}",
            updateCategory: "{{ url('knowledge_document/update_category') }}",
            delete_category: "{{ url('knowledge_document/delete_category') }}",
            download_url: "{{ url('knowledge_document/export-category') }}",
            fetch_category_by_ajax: "{{ url('knowledge_document/fetch_category_by_ajax') }}",
            fetch_subcategory_by_ajax: "{{ url('knowledge_document/fetch_subcategory_by_ajax') }}"
        },
        translations: {
            add_category_title:"{{ trans('config.kd_category_fields.add_kd_category') }}",
            edit_category_title:"{{ trans('config.kd_category_fields.edit_kd_category') }}",
            add_category:"{{ trans('config.kd_category_fields.add_category') }}",
            update_category:"{{ trans('config.kd_category_fields.update_category') }}",
            are_you_want_Delete_cat: "{{ trans('content.knowledge_document.are_you_want_Delete_cat') }}",
            edit: "{{ trans('config.kd_category_fields.edit') }}",
            delete: "{{ trans('config.kd_category_fields.delete') }}",
        }
    };
    new MyApp(config);
});
