...
@section('title', trans('failed_jobs/failed_job.failed_jobs'))
@section('content')
<div id="main-failed-jobs-wrapper">
    <div class="header-actions-wrapper d-flex align-items-center justify-content-between pe-4">
        <h3 class="h3-text mb-0">{{ trans('failed_jobs/failed_job.failed_jobs') }}</h3>
        <div class="d-flex gap-8">
            {{-- Filter button --}}
            <button class="header-icon-btn header-icon-btn-sm btn-open-filter" type="button" data-bs-toggle="tooltip" title="{{ trans('failed_jobs/failed_job.filter') }}">
                ...
                <span class="b6-text opacity-50">{{ trans('failed_jobs/failed_job.filter') }}</span>
                <span class="filter-count-badge d-none" aria-label="{{ trans('failed_jobs/failed_job.active_filters') }}">0</span>
            </button>

            {{-- Download button --}}
            <button class="header-icon-btn-only header-icon-btn-only-sm btn-download" type="button" data-bs-toggle="tooltip" title="{{ trans('failed_jobs/failed_job.download_report') }}">
            ...
            {{-- Refresh button --}}
            <button class="amg-refresh-btn btn-reload-list btn-reload-list" type="button" data-bs-toggle="tooltip" title="{{ trans('failed_jobs/failed_job.refresh_list') }}">
                ...
                <span>{{ trans('failed_jobs/failed_job.refresh_list') }}</span>
            </button>
        </div>
    </div>
            ...
            <div class="card rounded-0">
                <div class="card-body">
                    <div class="d-flex align-items-center gap-2 mb-1">
                        <div class="col-auto">
                            <select class="userModulePageLenth failed-jobs-page-length amg-table-pagination-dropdown">
                                <option value="10" selected>{{ trans('failed_jobs/failed_job.show_10') }}</option>
                                <option value="25">{{ trans('failed_jobs/failed_job.show_25') }}</option>
                                <option value="50">{{ trans('failed_jobs/failed_job.show_50') }}</option>
                                <option value="100">{{ trans('failed_jobs/failed_job.show_100') }}</option>
                            </select>
                        </div>
                        ...
                        <div>
                            <div class="amg-list-searchbar">
                                <button type="button" class="amg-list-searchbar__icon-btn btn-searchbox" aria-label="{{ trans('failed_jobs/failed_job.search') }}" title="{{ trans('failed_jobs/failed_job.search') }}">
                                ...
                                <input type="text" name="search" class="amg-list-searchbar__input searchbox plain-search" placeholder="{{ trans('failed_jobs/failed_job.search') }}" autocomplete="off">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="list-view-panel">
                        <div class="table-responsive">
                            <table id="failedJobTable" class="table display app-data-table">
                                <thead>
                                    <tr>
                                        <th><h4>{{ trans('failed_jobs/failed_job.payload') }}</h4></th>
                                        <th><h4>{{ trans('failed_jobs/failed_job.exception') }}</h4></th>
                                        <th><h4>{{ trans('failed_jobs/failed_job.failed_at') }}</h4></th>
                                    </tr>
                                </thead>
                                <tbody></tbody>
                            </table>
...
@push('scripts')
    ...
    <script src="{!! CommonHelper::asset('js/failed_jobs/index.js') !!}"></script>
    <script>
        $(document).ready(function() {
            var config = {};
            config.url = {};
            config.url.failed_jobs = "{{ url('jx-failed_jobs') }}";
            config.url.export_failed_jobs = "{{ url('jx-export-failed-jobs') }}";
            config.token = "{{ csrf_token() }}";
            config.translations = {
                please_enter_valid_search: '{{ trans('failed_jobs/failed_job.please_enter_valid_search') }}',
                press_enter_with_Search: '{{ trans('failed_jobs/failed_job.press_enter_with_search') }}',
                search: '{{ trans('failed_jobs/failed_job.search') }}',
                filter: '{{ trans('failed_jobs/failed_job.filter') }}',
                refresh: '{{ trans('failed_jobs/failed_job.refresh_list') }}',
                download: '{{ trans('failed_jobs/failed_job.download_report') }}',
                something_went_wrong: '{{ trans('failed_jobs/failed_job.something_went_wrong') }}',
                no_filter: '{{ trans('failed_jobs/failed_job.no_filter') }}',
                select_date_range: '{{ trans('failed_jobs/failed_job.select_date_range') }}'
            };
            new FailedJobs(config);
        });
    </script>
@endpush