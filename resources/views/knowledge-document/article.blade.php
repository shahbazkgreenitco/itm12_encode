...
@section('content')
           ...
           <svg width="0" height="0" style="position:absolute" aria-hidden="true" focusable="false">
            ...
            <div class="header-actions-wrapper d-flex align-items-center justify-content-between pe-4">
                ...
                <div class="d-flex align-items-center gap-1">
                    ...
                    <button onclick="window.location.href='{{ route('kdLists') }}'" class="d-flex gap-3 align-items-center bg-transparent outline-none border-0" data-bs-toggle="tooltip" title="Back">
                        ...
                        <div class="card-body pt-3">
                            <div class="kd-page" id="article-info-page">
                                <section class="kd-detail-hero">
                                    ...
                                    <div class="kd-hero-word">{{ trans('tkt_knowledge_document.article') }}</div>
                                ...
                                <div class="kd-detail-grid">
                                    <article class="kd-main-article">
                                        <div class="kd-main-image" style="background-image:url('{{ $articleImage }}')">
                                            ...
                                            <a href="#" class="kd-back-tab js-act-go-back" data-bs-toggle="tooltip" data-bs-placement="bottom" title="{{ trans('button.back') }}">
                                                <svg><use href="#kd-arrow-left"></use></svg>
                                            </a>
                                        </div>

                                        @if(isset($kd->ticket_id))
                                            <section class="kd-ticket-card">
                                                <div class="kd-ticket-header">
                                                    <div>
                                                        <span>{{ trans('content.service_ticket_fields.Company') }}</span>
                                                        <strong>@if($kd->tkt_dep_name && $kd->tkt_comp_name) {{ $kd->tkt_comp_name }} @endif</strong>
                                                    </div>
                                                    <div>
                                                        <span>{{ trans('content.service_ticket_fields.Ticket_Status') }}</span>
                                                        <strong class="kd-success-pill">{{ $kd->tkt_status }}</strong>
                                                    </div>
                                                    <div class="kd-ticket-user">
                                                        <img src="{{ $assign_by && $assign_by->getProfileImg() ? $assign_by->getProfileImg() : asset('imgs/profile-75.jpg') }}" alt="{{ trans('tkt_knowledge_document.user') }}">
                                                        <div>
                                                            <span>{{ trans('content.service_ticket_fields.Assigned_To') }}</span>
                                                            <strong>@if($kd->assigned_to_name) {{ $kd->assigned_to_name }} @else {{ trans('tkt_knowledge_document.not_yet_assigned') }} @endif</strong>
                                                        </div>
                                                    </div>
                                                </div>
                                                <div class="kd-ticket-details">
                                                    <div><span><i class="bi bi-building"></i> {{ trans('content.service_ticket_fields.department') }}</span>
                                                        <strong>@if($kd->tkt_dep_name) {{ $kd->tkt_dep_name }} @endif</strong>
                                                    </div>
                                                    <div><span><i class="bi bi-folder2-open"></i> {{ trans('content.service_ticket_fields.Prob_Category') }} </span>
                                                        <strong>@if($kd->tkt_prob_cat) {{ $kd->tkt_prob_cat }} @endif</strong>
                                                    </div>

                                                    <div><span><i class="bi bi-tags"></i> {{ trans('content.service_ticket_fields.sub_category') }}</span>
                                                        <strong>{{ $kd->tkt_sub_cat }}</strong>
                                                    </div>

                                                    <div><span><i class="bi bi-geo-alt"></i> {{ trans('content.service_ticket_fields.location') }}</span>
                                                        <strong>@if($kd->tkt_creator_location) {{ $kd->tkt_creator_location }} @endif</strong>
                                                    </div>

                                                    <div><span><i class="bi bi-calendar-event"></i> {{ trans('content.service_ticket_fields.Created_At') }}</span>
                                                        <strong>{{ CommonHelper::getDateAs($kd->tkt_created_at, 'd/m/Y h:i A', 'Y-m-d H:i:s') }}</strong>
                                                    </div>

                                                    <div><span><i class="bi bi-arrow-repeat"></i> {{ trans('content.common_doc_list.updated_at') }}</span>
                                                        <strong>{{ CommonHelper::getDateAs($kd->tkt_updated_at, 'd/m/Y h:i A', 'Y-m-d H:i:s') }}</strong>
                                                    </div>
                                                </div>
                                            </section>
                                        @endif
                                    ...
                                    <aside class="kd-detail-sidebar">
                                        <div class="kd-action-panel">
                                            <button class="amg amg-btn-primary js-act-staring button-round">
                                                <i class="bi bi-star"></i>
                                                {{ trans("content.service_ticket_fields.Add_Star") }}
                                            </button>
                                            @if(isset($creator_by) && Auth::user()->id == $creator_by->id)
                                                <button type="button" class="amg-btn amg-btn-secondary js-act-edit-ticket button-round" data-id="{{ $kd->id }}">
                                                    <i class="bi bi-pencil"></i>
                                                    {{ trans("content.service_ticket_fields.edit_ticket") }}
                                                </button>
                                                <button class="amg amg-btn js-act-delete button-round">
                                                    <i class="bi bi-trash"></i>
                                                    {{ trans("content.service_ticket_fields.Delete") }}
                                                </button>
                                            @endif
                                        </div>
                                        ...
                                        <div class="kd-sidebar-panel">
                                                    ...
                                                    <div class="article-info">
                                                        <div class="user-info">
                                                            ...
                                                            <div class="user-details">
                                                                <a href="{{ url('user/info', $creator_by->id) }}">
                                                                    <b>{{ $creator_by->fullName() }}</b>
                                                                    <span>{{ $creator_by->username }}</span>
                                                                </a>
                                                            </div>
                                                        </div>
                                            ...
                                            @if((Auth::user()->hasAnyRole(['SuperAdmin', 'Admin']) || Auth::user()->hasPermission('service_tickets'))&& isset($tags)&& count($tags) > 0)
                                        ...
                                        @if ($relatedArticles->count() > 0 && config('services.knowledge_document.enabled'))
                                            @can('KnowledgeDocumentRead')
                                                <div class="kd-related-panel">
                                                    <h3 class="h3-text">{{ trans('tkt_knowledge_document.related_most') }}</h3>
                                                        ...
                                                        <article class="kd-related-card">
                                                            <a href="{{ url('knowledge_document/article/view/' . $kda->id) }}" target="_blank">
                                                                <div class="kd-related-image" style="background-image:url('{{ !empty($kda->card_img) ? asset('/uploads/article/' . $kda->card_img) : asset('imgs/kd_image.png') }}')">
                                                                    <span class="kd-arrow" data-bs-toggle="tooltip" data-bs-placement="bottom" title="View"><svg><use href="#kd-arrow-right"></use></svg></span>
                                                                </div>
                                                                <div class="kd-related-copy">
                                                                    <h4 class="h4-text text-truncate">{{ $kda->title }}</h4>
    ...
    @include('knowledge-document.update_article_modal')
@endsection
...
@push('css')
    ...
    <style>
        ...
        .kd-tag.red { background-color: #e74c3c; }
        .kd-tag.blue { background-color: #3498db; }
        .kd-tag.green { background-color: #2ecc71; }
        .kd-tag.orange { background-color: #f39c12; }
        .kd-tag.purple { background-color: #9b59b6; }
        .kd-tag.teal { background-color: #1abc9c; }
       ...
       .kd-related-card + .kd-related-carda {
        ...
        .kd-related-image .kd-arrow::before,
        .kd-related-image .kd-arrow::after {
            position: absolute;
            content: "";
            width: 0.9375rem;
            height: 0.9375rem;
...
@push('scripts')
    ...
    <script type="text/javascript" src="{!! CommonHelper::asset('js/knowledge-document/article.js') !!}"></script>
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            config.url.attachment_download = "{{ url('knowledge_document/attachment/download') }}";
            config.url.attachment_view = "{{ url('knowledge_document/attachment/view') }}";
            ...
            config.star_exsist = {!! json_encode($kd->selfCheckStarred()) !!};
            ...
            config.url.searchurl = "{{ url('knowledge_document/document/search') }}";
            config.url.view = "{{ url('knowledge_document/article/view') }}";
            ...
            config.translations = {
                are_you_star: '{{ trans('content.knowledge_document.are_you_star') }}',
                unstar_article: '{{ trans('content.knowledge_document.unstar_article') }}',
            ...
            @if (!empty($kd->ticket_id))
                config.ticket = {!! json_encode($kd->ticket_id) !!};
            @endif
