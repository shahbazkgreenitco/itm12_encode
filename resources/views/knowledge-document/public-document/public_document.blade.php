{{-- * ------------------------------------------------------------
* File: public_document.blade.php
                ...
                <div class="container-fluid px-0">
                    <div class="card rounded-0 kd-shell-card">
                        <div class="card-body pt-3">
                            ...
                            @include('knowledge-document.public-document.filter')
                            <div class="kd-page kd-api-pending" id="kd-article-page">
                                <section class="kd-top-grid">
                                    <article class="kd-feature">
                                        <h2 class="h2-text">{{ trans('tkt_knowledge_document.popular_articles') }}</h2>
                                        @foreach ($popularItems->take(1) as $article)
                                            <a href="{{ $kdArticleUrl($article) }}" target="_blank">
                                                ...
                                                <div class="kd-meta">
                                                    <span>{{ $article->dep_name ?? ($article->category_name ?? '') }}</span>
                                                    <span>{{ $article->updated_at_format ?? '' }}</span>
                                                </div>
                                                <h3>{{ $kdPlainText($article->title ?? '', 52) }}</h3>
                                    ...
                                    <aside class="kd-most">
                                        <h3>{{ trans('tkt_knowledge_document.most_viewed') }}</h3>
                                ...
                                <button type="button" class="kd-load-more" id="kdLoadMoreBtn">
                                    <svg>
...
@push('css')
    ...
    <style>
        ...
        .amg-form-invalid .form-control,
        .amg-form-invalid .form-select,
        .amg-form-invalid .select2-selection {
        ...
        [data-bs-theme="dark"] .amg-form-icon-error,
        .dark .amg-form-icon-error,
        body.dark-mode .amg-form-icon-error {
            background: #24324d !important;
            color: #ff4d4f !important;
            border-color: #ff4d4f !important;
        }

        .amg-form-select-error {
            border-color: #ff3b3b !important;
        }
      ...
      .department .btn,
        ...
        [data-bs-theme="dark"] .kd-tags .active_filter_btn {
           border: 0.0625rem solid rgb(243, 240, 240);
        }
...
@push('scripts')
    ...
    <script type="text/javascript" src="{!! CommonHelper::asset('js/knowledge-document/public_document.js') !!}"></script>
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            config.url.article_list = "{{ url('public-articles/jx-public-knowledge-list') }}";
            ...
            config.url.get_company_by_user_access = "{{ url('getCompanyForPublic') }}";
            config.url.departments_with_company = "{{ url('getPublicDepartmentsWithCompanyByQuery') }}";
            config.url.view_article = "{{ url('public-article/view') }}";
            ...
            config.url.fetch_category_by_ajax = "{{ url('publicFetch_category_by_ajax') }}";
            config.url.fetch_subcategory_by_ajax = "{{ url('publicFetch_subcategory_by_ajax') }}";
            config.translations = {
