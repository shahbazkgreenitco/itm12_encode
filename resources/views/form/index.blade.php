  ...
  "versions": [
    {
      ...
      "from": "2026-05-15",
      "reviewer": null,
      "description": "Implemented Dynamic form CRUD"
    }
  ]
}
--}}
@extends('layouts.layout1')
@section('title', trans('content.dynamic_form.dynamic_form'))
@section('content')
    <div id="dynamic-form-wrapper">
        <div class="header-actions-wrapper d-flex align-items-center justify-content-between pe-4">
            <h2 class="h2-text mb-0">{{ trans('content.dynamic_form.dynamic_form') }}</h2>
            <div class="d-flex gap-8">

                @if (Auth::user()->hasPermissionTo('DynamicFormAdd'))
                <button class="amg-btn amg-btn-primary amg-btn-sm btn-add-form" type="button" data-bs-toggle="tooltip"
                    title="Add New Form">
                    ...
                    <span>{{ trans('content.dynamic_form.add_form') }}</span>
                </button>
                @endif
            </div>
                ...
                <div class="card rounded-0">
                    <div class="card-body">
                        <div class="d-flex align-items-center gap-2 mb-3">
                            <div class="col-auto">
                                <select class="amg-table-pagination-dropdown amgTablePageLenth form-list-page-length">
                            ...
                            <div>
                                <div class="amg-list-searchbar">
                                    ...
                                    <input type="text" class="amg-list-searchbar__input dynamic-list-search"
                        ...
                        <div class="">
                            <div class="table-responsive">
                                <table id="dynamic-form" class="amg-datatable table display app-data-table">
                                    <thead>
                                        <tr>
                                            ...
                                            <th>{{ trans('content.dynamic_form.updated_at') }}</th>
...
@endpush
@push('scripts')
    <script type="text/javascript" src="{!! CommonHelper::asset('js/form/form.js') !!}"></script>
    <script>
        $(document).ready(function() {
            var config = {};
            config.url = {};
            config.url.form_list = "{{ url('jx-form-list') }}";
            config.url.add = "{{ url('dynamic_form/add') }}";
            config.url.edit = "{{ url('dynamic_form/edit') }}";
            config.url.delete = "{{ url('dynamic_form/delete') }}";
            config.url.update = "{{ url('dynamic_form/update') }}";
            config.url.list_form = "{{ url('dynamic_form') }}";
            config.url.history = "{{ url('dynamic_form_history') }}",
            ...
            config.translations = {
                ...
                delete: '{{ trans('content.dynamic_form.delete') }}',
                are_you_delete: '{{ trans('content.dynamic_form.are_you_delete') }}',
