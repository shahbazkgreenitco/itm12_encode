...
@section('content')
    <div class="content" id="reportMainContent">
                ...
                <div class="card rounded-0">
                    <div class="card-body">
                        <div class="panel-body box-body">
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="form-group" id="reportDropdown">
                                        <label for="reportName" class="form-label">{{ trans("custom_report.select_report") }}</label>
                                        <div class="input-group">
                                            <span class="input-group-text">
                                                <i class="bi bi-file-earmark-bar-graph"></i>
                                            </span>
                                            <select id="reportName" class="form-select">
                                                <option value="">{{ trans("custom_report.select_report") }}</option>
                            ...
                            <div class="mt-3" id="maintable_waraper" style="display:none">
                                <div class="d-flex align-items-center gap-2 mb-1">
                                    <div class="col-auto">
                                        <select class="userModulePageLenth amg-table-pagination-dropdown" id="customReportList">
                                    ...
                                    <button class="amg-refresh-btn btn-reload-list act-reload" data-bs-toggle="tooltip"
                                        data-bs-placement="top" title="{{trans('custom_report.refresh')}}">
                                    ...
                                    <button class="amg-refresh-btn btn-download-excel" data-bs-toggle="tooltip" data-bs-placement="top" title="{{ trans('custom_report.download_excel') }}">
                                        ...
                                        <span>{{ trans('custom_report.download_excel') }}</span>
                                    </button>
                                </div>
                                <div class="list-view-panel" id="main-role-permission-wrapper">
                                    <div class="table-responsive">
                                        <table id="mytable" class="mytable table display app-data-table">
                                            <thead>
                                                <tr></tr>
                                            </thead>
                                            <tbody></tbody>
...
@endpush
@push('scripts')
    <script type="text/javascript" src="{!! CommonHelper::asset('js/reports/custom/report_view.js') !!}"></script>
    <script type="text/javascript">
        $(document).ready(function () {
            var config = {};
            config.url = {};
            config.url.get_report_data = "{{ url('reports/custom-reports/get-report-data') }}";
            config.url.export_report_data = "{{ url('reports/custom-reports/export-report-data') }}";
            config.url.export_custom_report = "{{ url('reports/custom-reports/jx-download-custom-report') }}";
            ...
            config.translations = {
                ...
                Download: '{{ trans('custom_report.download') }}',
                Download_Excel: '{{ trans('custom_report.download_excel') }}',
                are_you_delete: '{{ trans('custom_report.are_you_delete') }}',
                View_Report: '{{ trans('custom_report.view_report') }}',
