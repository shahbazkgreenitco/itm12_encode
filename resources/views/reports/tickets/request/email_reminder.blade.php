...
@section('title', trans('content.email_reminder.email_reminder'))

@section('content')
    <div class="content" id="content-container">
        <div class="header-actions-wrapper d-flex align-items-center justify-content-between pe-4">
            <h3 class="h3-text mb-0">{{ trans('content.email_reminder.email_reminder') }}</h3>
                        ...
                        <div class="list-view-panel">
                            <div class="table-responsive">
                                <table id="lgTbl" class="table display app-data-table">
                                    <thead>
                                        <tr>
                                            <th><h4 class="b2-text">{{ trans("content.email_reminder.ticketID") }}</h4></th>
                                            <th><h4>{{ trans("content.email_reminder.service_request") }}</h4></th>
                                            <th><h4>{{ trans("content.email_reminder.ps_no") }}</h4></th>
                                            <th><h4>{{ trans("content.email_reminder.receiver") }}</h4></th>
                                            <th><h4>{{ trans("content.email_reminder.email_address") }}</h4></th>
                                            <th><h4>{{ trans("content.email_reminder.no_of_day") }}</h4></th>
                                            <th><h4>{{ trans("content.email_reminder.send_date") }}</h4></th>
                                        </tr>
                                    </thead>
                                    <tbody id="lg">
                                    </tbody>
                        ...
                        <div class="row g-3 px-3">
                            ...
                            <div class="col-lg-6 col-md-6">
                                ...
                                <select name="filter_by_date" id="filter_by_date" class="form-select">
                                    ...
                                    <option value="1">{{ trans("content.email_reminder.sendDate") }}</option>
                                </select>
                            </div>
...
@push('scripts')
<script type="text/javascript" src="{!! CommonHelper::asset('js/reports/tickets/request/email_reminder_report.js') !!}"></script>
<script type="text/javascript">
    $(document).ready(function() {
        var config = {};
        config.url = {};
        config.url.list_tickets = "{{ url('reports/tickets/jx-email-reminder-report') }}";
        config.url.export_ticket_xls = "{{ url('reports/export-email-reminder-report-xls') }}";
        ...
        config.translations = {
            ...
            something_wrong: '{{ trans('header.escalation_group.something_wrong') }}',
            ...
            filter_by_date: '{{ trans('content.filter_heading.filter_by_date') }}',
            filter_by_daterange: '{{ trans('content.filter_heading.filter_by_daterange') }}',
            sendDate: '{{ trans('content.email_reminder.sendDate') }}',
        };
        new EmailReminderReport(config);
    });
</script>
@endpush