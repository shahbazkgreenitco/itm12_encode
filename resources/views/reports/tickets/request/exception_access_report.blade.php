...
@section('title', trans("report.exception_access_report"))

@section('content')
    <div class="content" id="content-container">
        <div class="header-actions-wrapper d-flex align-items-center justify-content-between pe-4">
            <h3 class="h3-text mb-0">{{ trans("report.exception_access_report") }}</h3>
                            ...
                            <div>
                                <div class="amg-list-searchbar">
                                    ...
                                    <input type="text"
                                        ...
                                        id="announcement-list-search" placeholder="{{ trans('report.search') }}">
                                </div>
                            </div>
                        ...
                        <div class="list-view-panel">
                            <div class="table-responsive">
                                <table id="lgTbl" class="table display app-data-table">
                                    <thead>
                                        <tr>
                                            <th><h4 class="b2-text">{{ trans("content.ticket_report.ticketID") }}</h4></th>
                                            <th><h4>{{ trans("content.ticket_report.hostname") }}</h4></th>
                                            <th><h4>{{ trans("content.ticket_report.creator_info") }}</h4></th>
                                            <th><h4>{{ trans("content.ticket_report.ticket_info") }}</h4></th>
                                            <th><h4>{{ trans("content.ticket_report.location") }}</h4></th>
                                            <th><h4>{{ trans("content.report_fields.max_no_of_days") }}</h4></th>
                                            <th><h4>{{ trans("content.ticket_report.approved_days") }}</h4></th>
                                            <th><h4>{{ trans("content.report_fields.start_date_end_date") }}</h4></th>
                                            <th><h4>{{ trans("content.ticket_report.request_Created_At") }}</h4></th>
                                            <th><h4>{{ trans("content.ticket_report.approval_date") }}</h4></th>
                                            <th><h4>{{ trans("content.ticket_report.ticket_status") }}</h4></th>
                                            <th><h4>{{ trans("content.ticket_report.sr_status") }}</h4></th>
                                        </tr>
                                    </thead>
                                    <tbody id="lg">
                                    </tbody>
                ...
                <div class="modal-content rounded-5">
                    ...
                    <div class="modal-body">
                        <div class="row g-3 px-3">
                            ...
                            {{-- Row 2: Status, Location, Ticket Creator --}}
                            <div class="col-lg-4 col-md-6">
                                <label class="form-label b5-text">
                                    {{ trans("content.ticket_report.filter_by_ticket_status") }}
                                </label>
                            ...
                            <div class="col-lg-4 col-md-6">
                                <label class="form-label b5-text">
                                    {{ trans("content.filter_heading.Filter_By_location") }}
                                </label>
                                <select name="filter_by_location[]" id="filter_by_location" class="form-select" multiple>
                                </select>
                            </div>

                            <div class="col-lg-4 col-md-6">
                                <label class="form-label b5-text">
                                    {{ trans("content.filter_heading.Filter_By_Ticket_Creator") }}
                                </label>
                                <select name="filter_by_ticket_handlers[]" id="filter_by_ticket_handlers" class="form-select" multiple>
                                </select>
                            </div>

                            {{-- Row 3: Date Type, Date Range, SR Status --}}
                            <div class="col-lg-4 col-md-6">
                                <label class="form-label b5-text">
                                    {{ trans("content.filter_heading.filter_by_date") }}
                                </label>
                                <select name="filter_by_date" id="filter_by_date" class="form-select">
                                    <option value="null">{{ trans("content.procurement_fields.No_Filter") }}</option>
                                    <option value="1">{{ trans("content.procurement_fields.Created_Date") }}</option>
                                    <option value="5">{{ trans("content.procurement_fields.Approval_Date") }}</option>
                                </select>
                            </div>
                            ...
                            <div class="col-lg-4 col-md-6">
                                <label class="form-label b5-text">
                                    {{ trans("content.ticket_report.filter_by_sr_status") }}
                                </label>
                                <select name="filter_by_sr_status[]" id="filter_by_sr_status" class="form-select" multiple>
                                    @foreach($vd->sr_statuses as $key => $status)
                            ...
                            <div class="col-lg-4 col-md-6">
                                <label class="form-label b5-text">
                                    {{ trans("content.service_ticket_fields.approval_mode") }}
                                </label>
                                <select name="filter_hierarchy_approval" id="filter_hierarchy_approval" class="form-select">
                                    <option value="null">{{ trans("content.procurement_fields.No_Filter") }}</option>
...
@push('scripts')
<script type="text/javascript" src="{!! CommonHelper::asset('js/reports/tickets/request/exception_access_report.js') !!}"></script>
<script type="text/javascript">
    $(document).ready(function() {
        var config = {};
        config.url = {};
        config.url.list_tickets = "{{ url('reports/tickets/jx-exception-access-report') }}";
        config.url.export_ticket_xls = "{{ url('reports/tickets/export-exception-access-report-xls') }}";
        config.url.departments_by_company = "{{ url('getDepartmentsWithCompanyByQuery') }}";
        ...
        config.url.ticketInfo = "{{ url('ticket') }}";
        ...
        config.categories = {!! json_encode($vd->categories) !!};
        config.statuses = {!! json_encode($vd->statuses) !!};
        config.translations = {
            serach_option: '{{ trans('report.search') }}',
            No_Filter: '{{ trans('report.no_filter') }}',
            Show_Categories: '{{ trans('report.show_categories') }}',
            Filter_By_Ticket_Handler: '{{ trans('report.filter_by_ticket_handler') }}',
            Select_the_Ticket_Creator: '{{ trans('report.select_creator') }}',
            Select_the_Ticket_Handler: '{{ trans('report.select_handler') }}',
            Select_Merged_User: '{{ trans('report.select_merged_user') }}',
            filter_by_department: '{{ trans('report.filter_by_department') }}',
            Filter_By_Created_via: '{{ trans('report.filter_by_created_via') }}',
            Filter_By_Problem_Category: '{{ trans('report.filter_by_problem_category') }}',
            Filter_By_Sub_Category: '{{ trans('report.filter_by_sub_category') }}',
            filter_by_status: '{{ trans('report.filter_by_status') }}',
            Filter_By_Creator_Logger: '{{ trans('report.filter_by_creator_logger') }}',
            no_details: '{{ trans('report.no_records') }}',
            Filter_By_Priority: '{{ trans('report.filter_by_priority') }}',
            please_select_option: '{{ trans('report.please_select_option') }}',
            filter_by_location_id: '{{ trans('report.filter_by_location') }}',
            Filter_By_status: '{{ trans('report.filter_by_status') }}',
            ...
            filter_by_sr_status: '{{ trans('report.filter_by_sr_status') }}',
            approval_mode: '{{ trans('report.approval_mode') }}',
        ...
        new ExceptionAccessReport(config);
    });
</script>
@endpush