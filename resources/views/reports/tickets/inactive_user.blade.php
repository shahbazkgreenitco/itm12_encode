...
@section('title', trans("content.inactive_user.inactive_user"))
@section('content')

<div class="content">
    <div class="header-actions-wrapper d-flex align-items-center justify-content-between pe-4">
        <h3 class="h3-text mb-0">{{ trans("content.inactive_user.inactive_user") }}</h3>
        <div class="d-flex gap-8">
            ...
            <button class="header-icon-btn-only header-icon-btn-only-sm bulk_resolved" type="button"
                data-bs-toggle="tooltip" title="{{ trans('content.inactive_user.bulk_resolve') }}">
                    ...
                    <div class="js-user-list-view-panel">
                        <div class="table-responsive">
                             <table id="mytable" class="mytable amg-datatable table display app-data-table">
                                <thead>
                                    <tr id="tHeadeRow">
                                        <th><div class="checkAll"><input type="checkbox" class="chkParent" id="checkAll"></div></th>
                                        <th>{{trans('content.inactive_user.name')}}</th>
                                        <th>{{trans('content.inactive_user.ticket_id')}}</th>
                                        <th>{{trans('content.inactive_user.ticket_status')}}</th>
                                        <th>{{trans('content.inactive_user.assigned_to')}}</th>
                                        <th>{{trans('content.inactive_user.created_at')}}</th>
                                     </tr>
                                    </thead>
                                <tbody id="lg"></tbody>
                            </table>
                ...
                <div class="modal-body">
                    <div class="row g-3 px-3">
                        ...
                        <div class="col-xl-3 col-lg-3 col-md-6">
                            ...
                            <select id="filter_by_based_on" name="filter_by_based_on" class="form-select select2-modal">
    ...
    @include("reports.tickets.inactive_user_bulk_resolve")
</div>
@endsection
@push('css')
...
@push('scripts')
    ...
    <script type="text/javascript" src="{!! CommonHelper::asset('js/reports/tickets/inactive_user.js') !!}"></script>
    <script type="text/javascript">
        var config = new Object;
        config.url = new Object;
        config.translations = {
            add_holiday: '{{ trans('header.holiday_fields.add_holiday') }}',
            save: '{{trans('button.save')}}',
            press_enter_with_Search: '{{trans('content.inactive_user.press_enter_with_Search')}}',
            search: '{{trans('content.inactive_user.search')}}',
            confirmation_message:'{{ trans('header.escalation_group.confirmation_message') }}',
            Filter:'{{ trans('content.user_fields.Filter') }}',
            Refresh_List:'{{ trans('content.inactive_user.Refresh_List') }}',
            Download:'{{ trans('content.inactive_user.download') }}',
            select_user:'{{ trans('content.inactive_user.select_user') }}',
            select_status:'{{ trans('content.inactive_user.select_status') }}',
            bulk_resolve:'{{ trans('content.inactive_user.bulk_resolve') }}',
            select_any_one_ticket:'{{ trans('content.inactive_user.select_any_one_ticket') }}',
        };
        config.url.get_tickets = "{{ url('reports/tickets/inactive-user-ajaxList') }}";
        config.url.getUserByQuery = "{{url('getUserByQuery')}}";
        config.url.export_ticket = "{{ url('reports/tickets/inactive-user-export') }}";
        config.url.ticket_info = "{{url('ticket')}}";
        config.url.bulk_resolved = "{{url('reports/tickets/bulk_resolve')}}";
        config.status = {!! json_encode($statuses) !!};
        config.token = "{{ csrf_token() }}";
        config.permissions = {!! json_encode($permissionArray) !!};
        config.company = {!! json_encode($dashboardCompanyId) !!};
        config.is_resolve = {!! json_encode(Auth::user()->hasAnyRole(['SuperAdmin']) && Auth::user()->hasPermission("service_tickets")) !!};
        new MyApp(config);
    </script>
@endpush
