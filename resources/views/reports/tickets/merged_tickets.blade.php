...
@section('title', trans('report.ticket_merged_tickets_report'))
@section('content')

    <div class="content">
        <div class="header-actions-wrapper d-flex align-items-center justify-content-between pe-4">
            <h3 class="h3-text mb-0">{{ trans('report.ticket_merged_tickets_report') }}</h3>
            <div class="d-flex gap-8">
              ...
              <div class="amg-sort-wrapper position-relative">
                ...
                {{-- Download --}}
                @can("TicketMergedReportDownload")
                        ...
                        <div class="js-user-list-view-panel">
                            <div class="gtable-cover">
                                 <table id="lgTbl" class="amg-datatable mytable table table-striped">
                                    <thead>
                                        <tr id="tHeadeRow">
                                            <th>{{ trans("content.report_fields.Ticket_ID") }}</th>
                                            <th>{{ trans("content.report_fields.Merged_With") }}</th>
                                            <th>{{ trans("content.report_fields.Ticket_Subject") }}</th>
                                            ...
                                            <th>{{ trans("content.report_fields.Created_By") }}</th>
                                            <th>{{ trans("content.report_fields.Created_At") }}</th>
                                            <th>{{ trans("content.report_fields.Created_via") }}</th>
                                            <th>{{ trans("content.report_fields.status") }}</th>
                                            <th>{{ trans("content.report_fields.ticket_handlers") }}</th>
                                            <th>{{ trans("content.report_fields.TAT") }}</th>
                                            <th>{{ trans("content.report_fields.Priority") }}</th>
                                            <th>{{ trans("content.report_fields.Merged_By") }}</th>
                                            <th>{{ trans("content.report_fields.Merged_At") }}</th>
                                            <th>{{ trans("content.report_fields.ticket_type") }}</th>
                    ...
                    <div class="modal-body">
                        <div class="row g-3 px-3">
                            ...
                            <div class="col-xl-3 col-lg-4 col-md-6">
                                <label class="form-label b5-text">{{ trans('report.department') }}</label>
                                <select id="filter_by_department" name="filter_by_department[]" multiple></select>
                            </div>

                            <!-- Problem Category -->
                            <div class="col-xl-3 col-lg-4 col-md-6">
                                <label class="form-label b5-text">{{ trans('report.problem_category') }}</label>
                                <select id="filter_by_problem_category" name="filter_by_problem_category[]" multiple></select>
                            </div>

                            <!-- Sub Category -->
                            <div class="col-xl-3 col-lg-4 col-md-6">
                                <label class="form-label b5-text">{{ trans('report.sub_category') }}</label>
                                <select id="filter_by_sub_category" name="filter_by_sub_category[]" multiple></select>
                            </div>

                            <!-- Created Via -->
                            <div class="col-xl-3 col-lg-4 col-md-6">
                                <label class="form-label b5-text">{{ trans('report.created_via') }}</label>
                                <select id="filter_by_create_via" name="filter_by_create_via[]" multiple>
                                    <option value="1">{{ trans('report.portal') }}</option>
                                    <option value="2">{{ trans('report.chat') }}</option>
                                    <option value="3">{{ trans('report.email') }}</option>
                                    <option value="4">{{ trans('report.mobile') }}</option>
                                    <option value="5">{{ trans('report.call') }}</option>
                                    <option value="6">{{ trans('report.bot') }}</option>
                                </select>
                            </div>

                            <!-- Merged By -->
                            <div class="col-xl-3 col-lg-4 col-md-6">
                                <label class="form-label b5-text">{{ trans('report.merged_by') }}</label>
                                <select id="filter_by_merged_by" name="filter_by_merged_by[]" multiple></select>
                            </div>

                            <!-- Status -->
                            <div class="col-xl-3 col-lg-4 col-md-6">
                                <label class="form-label b5-text">{{ trans('report.status') }}</label>
                                <select id="filter_by_status" name="filter_by_status[]" multiple></select>
                            </div>

                            <!-- Handler -->
                            <div class="col-xl-3 col-lg-4 col-md-6">
                                <label class="form-label b5-text">{{ trans('report.ticket_handler') }}</label>
                                <select id="filter_by_handler" name="filter_by_handler[]" multiple></select>
                            </div>

                            <!-- Priority -->
                            <div class="col-xl-3 col-lg-4 col-md-6">
                                <label class="form-label b5-text">{{ trans('report.priority') }}</label>
                                <select id="filter_by_priority" name="filter_by_priority[]" multiple></select>
                            </div>

                            <!-- Ticket Type -->
                            <div class="col-xl-3 col-lg-4 col-md-6">
                                <label class="form-label b5-text">{{ trans('report.ticket_type') }}</label>
                                <select id="filter_by_ticket_type" name="filter_by_ticket_type[]" multiple>
                            ...
                            <!-- Date Type -->
                            <div class="col-xl-3 col-lg-4 col-md-6">
                                <label class="form-label b5-text">{{ trans('report.filter_by_date') }}</label>
                                <select id="filter_by_date" name="filter_by_date" class="form-select">
                                    <option value="null">{{ trans('report.no_filter') }}</option>
                                    <option value="1">{{ trans('report.created_date') }}</option>
                                    <option value="2">{{ trans('report.merged_date') }}</option>
                                </select>
                            </div>

                            <!-- Date Range -->
                            <div class="col-lg-4 col-md-6">
                                <label class="form-label b5-text">{{ trans('report.filter_by_daterange') }}</label>
                                <div id="reportrange"
                                    ...
                                    <svg width="15px" height="15px" viewBox="0 0 48 48" fill="currentColor">
                                        <title>{{ trans('report.filter_by_daterange') }}</title>
                            ...
                            <!-- Creator / Logger Type -->
                            <div class="col-xl-3 col-lg-4 col-md-6">
                                <label class="form-label b5-text">{{ trans('report.creator_logger') }}</label>
                                <select id="filter_based_on_cre_log" class="form-select">
                                    <option value="null">{{ trans('report.no_filter') }}</option>
                                    <option value="1">{{ trans('report.logger') }}</option>
                                    <option value="2">{{ trans('report.creator') }}</option>
                                </select>
                            </div>

                            <!-- Creator -->
                            <div class="col-xl-3 col-lg-4 col-md-6 creatorcover hide">
                                <label class="form-label b5-text">{{ trans('report.select_creator') }}</label>
                                <select id="filter_by_creator"></select>
                            </div>
...
@push('scripts')
    <script type="text/javascript" src="{!! CommonHelper::asset('js/reports/tickets/merged_tickets.js') !!}"></script>
    <script src="{!! CommonHelper::asset('js/common.js') !!}"></script>
    <script type="text/javascript">
        $(document).ready(function() {
            var config = {};
            config.url = {};
            config.url.list_Merged_tickets = "{{ url('reports/tickets/jx-merged-tickets') }}";
            config.url.export_ticket = "{{ url('reports/export-merged-tickets') }}";
            config.url.export_ticket_pdf = "{{ url('reports/export-merged-tickets-pdf') }}";
            ...
            config.translations = {
                ...
                Select_the_Ticket_Creator: '{{ trans('content.report_fields.Select_the_Ticket_Creator') }}',
                Select_the_Ticket_Handler: '{{ trans('content.report_fields.Select_the_Ticket_Handler') }}',
                Select_Merged_User: '{{ trans('content.report_fields.Select_Merged_User') }}',
                filter_by_department: '{{ trans('content.filter_heading.filter_by_department') }}',
                Filter_By_Created_via: '{{ trans('content.filter_heading.Filter_By_Created_via') }}',
                ...
                select_the_Department: '{{ trans('content.report_fields.select_the_department') }}',
                select_the_Problem_Category: '{{ trans('content.report_fields.select_the_pc') }}',
                select_the_Sub_Category: '{{ trans('content.report_fields.select_the_sc') }}',
                select_the_Created_Via: '{{ trans('content.report_fields.select_the_created_via') }}',
                select_the_Merged_User: '{{ trans('content.report_fields.select_the_merge_user') }}',
                select_the_Status: '{{ trans('content.report_fields.select_the_status') }}',
                select_the_Ticket_Handler: '{{ trans('content.report_fields.select_the_ticket_handler') }}',
                select_the_Priority: '{{ trans('content.report_fields.select_the_priority') }}',
                select_the_Date: '{{ trans('content.report_fields.filter_by_date') }}',
                select_the_Ticket_Type: '{{ trans('content.report_fields.select_the_ticket_type') }}',
                select_the_Creator_Logger: '{{ trans('content.report_fields.select_the_creator/logger') }}',
            };
            new MyApp(config);
        });
    </script>
