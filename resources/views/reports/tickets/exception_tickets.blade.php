...
@section('title', trans("content.report_fields.Tickets_Exception_Notifications"))

@section('content')
    <div class="content" id="content-container">
        <div class="header-actions-wrapper d-flex align-items-center justify-content-between pe-4">
            <h3 class="h3-text mb-0">{{ trans("content.report_fields.Tickets_Exception_Notifications") }}</h3>
            <div class="d-flex gap-8">
                {{-- Download Excel --}}
                @can("TicketExceptionReportDownload")
                    <button class="header-icon-btn-only header-icon-btn-only-sm btn-download" type="button"
                        data-bs-toggle="tooltip" title="{{ trans('report.download_excel') }}">
                    ...
                    {{-- Download PDF --}}
                    <button class="header-icon-btn-only header-icon-btn-only-sm btn-download-pdf" type="button"
                        data-bs-toggle="tooltip" title="{{ trans('report.download_pdf') }}">
                        <svg viewBox="0 0 18 22" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path
                                d="M16 2H4C2.89543 2 2 2.89543 2 4V18C2 19.1046 2.89543 20 4 20H14C15.1046 20 16 19.1046 16 18V4C16 2.89543 15.1046 2 14 2Z"
                                stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                            <path d="M6 6H12M6 10H12M6 14H9" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"/>
                            <path d="M13 14L15 16L13 18" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                            <path d="M15 16H10" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"/>
                        </svg>
                    </button>
                @endcan
            </div>
                        ...
                        <div class="list-view-panel">
                            <div class="table-responsive">
                                <table id="lgTbl" class="table display app-data-table">
                                    <thead>
                                        <tr>
                                            <th><h4 class="b2-text">{{ trans("content.report_fields.Ticket_ID") }}</h4></th>
                                            <th><h4>{{ trans("content.report_fields.Subject") }}</h4></th>
                                            <th><h4>{{ trans("content.report_fields.Mail_From") }}</h4></th>
                                            <th><h4>{{ trans("content.report_fields.Date_Time") }}</h4></th>
                                            <th><h4>{{ trans("content.report_fields.Reason") }}</h4></th>
                                            <th><h4>{{ trans("content.report_fields.Created_At") }}</h4></th>
                                        </tr>
                                    </thead>
                                    <tbody id="lg">
                                    </tbody>
...
@endsection
@push('scripts')
<script type="text/javascript" src="{!! CommonHelper::asset('js/reports/tickets/exception_tickets.js') !!}"></script>
<script type="text/javascript">
    $(document).ready(function() {
        var config = {};
        config.url = {};
        config.url.list_tickets = "{{ url('reports/tickets/jx-exception-notification') }}";
        config.url.export_ticket_xls = "{{ url('reports/export-exception-notification') }}";
        config.url.export_ticket_pdf = "{{ url('reports/export-exception-notification-pdf') }}";
        ...
        config.translations = {
            ...
            download_excel: '{{ trans('report.download_excel') }}',
            download_pdf: '{{ trans('report.download_pdf') }}',
            no_records: '{{ trans('report.no_records') }}',
        };
        new ExceptionTickets(config);
    });
</script>
@endpush