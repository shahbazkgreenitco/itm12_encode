...
@section('title', "Ticket - Category wise Aging Report")

@section('content')
    <div class="content" id="content-container">
        <div class="header-actions-wrapper d-flex align-items-center justify-content-between pe-4">
            <h3 class="h3-text mb-0">{{ "Ticket - Category wise Aging Report" }}</h3>
            <div class="d-flex gap-8">
                {{-- Show Categories Button --}}
                <button class="header-icon-btn header-icon-btn-sm" id="parent_category_wise_report" type="button"
                    data-bs-toggle="tooltip" title="{{ trans('report.show_categories') }}">
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M12 2L2 7L12 12L22 7L12 2Z" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                        <path d="M2 17L12 22L22 17" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                        <path d="M2 12L12 17L22 12" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                    </svg>
                    <span class="b6-text opacity-50">{{ trans("report.show_categories") }}</span>
                </button>

                {{-- Show Sub Categories Button --}}
                <button class="header-icon-btn header-icon-btn-sm" id="category_wise_report" type="button"
                    data-bs-toggle="tooltip" title="{{ trans('report.show_sub_categories') }}">
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M12 2L2 7L12 12L22 7L12 2Z" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                        <path d="M2 17L12 22L22 17" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                        <path d="M2 12L12 17L22 12" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                        <path d="M8 9.5L8 14.5" stroke="currentColor" stroke-width="2" stroke-linecap="round"/>
                        <path d="M16 9.5L16 14.5" stroke="currentColor" stroke-width="2" stroke-linecap="round"/>
                    </svg>
                    <span class="b6-text opacity-50">{{ trans("report.show_sub_categories") }}</span>
                </button>
                        ...
                        <div class="list-view-panel">
                            <div class="table-responsive">
                                <table id="lgTbl" class="table display app-data-table">
                                    <thead>
                                        <tr>
                                            <th rowspan="2"><h4>{{ trans("report.department") }}</h4></th>
                                            <th rowspan="2" class="prob_cate hide" id="prob_cate_header"><h4>{{ trans("report.problem_category") }}</h4></th>
                                            <th rowspan="2" class="sub_cate hide" id="sub_cate_header"><h4>{{ trans("report.sub_category") }}</h4></th>
                                            <th rowspan="2"><h4>{{ trans("report.total") }}</h4></th>
                                            <th colspan="2"><h4>{{ trans("report.created_at_less_than_7days") }}</h4></th>
                                            <th colspan="2"><h4>{{ trans("report.created_at_more_than_7days") }}</h4></th>
                                        </tr>
                                        <tr>
                                            <th><h4>{{ trans("report.active") }}</h4></th>
                                            <th><h4>{{ trans("report.inactive") }}</h4></th>
                                            <th><h4>{{ trans("report.active") }}</h4></th>
                                            <th><h4>{{ trans("report.inactive") }}</h4></th>
                                        </tr>
                                    </thead>
                ...
                <div class="modal-content rounded-5">
                        ...
                        <div class="row g-3 px-3">
                            ...
                            <div class="col-lg-4 col-md-6">
                                <label class="form-label b5-text">
                                    {{ trans("report.filter_by_department") }}
                                </label>
                            ...
                            <div class="col-lg-4 col-md-6">
                                <label class="form-label b5-text">
                                    {{ trans("report.filter_by_problem_category") }}
                                </label>
                            ...
                            <div class="col-lg-4 col-md-6">
                                <label class="form-label b5-text">
                                    {{ trans("report.filter_by_sub_category") }}
                                </label>
                                <select name="filter_by_sub_category[]" id="filter_by_sub_category" class="form-select" multiple>
                                </select>
                            </div>

                            {{-- Row 2: From Date, To Date with Calendar Icons --}}
                            <div class="col-lg-6 col-md-6">
                                <label class="form-label b5-text">
                                    {{ trans("report.from_date") }}
                                </label>
                                <div class="input-group date" id="fromDatePicker">
                                    <input id="filter_by_from_date" type="text" class="form-control datepicker" 
                                        placeholder="{{ trans('report.from_date') }}" autocomplete="off" readonly />
                            ...
                            <div class="col-lg-6 col-md-6">
                                ...
                                <div class="input-group date" id="toDatePicker">
                                    <input id="filter_by_to_date" type="text" class="form-control datepicker" 
                                        placeholder="{{ trans('report.to_date') }}" autocomplete="off" readonly />
                                    <span class="input-group-addon" style="cursor:pointer;">
                                    <svg width="15px" height="15px" viewBox="0 0 48 48" fill="currentColor">
                    ...
                    <div class="modal-footer justify-content-end mb-4 pb-4 py-0">
                        <div class="amg-btn-group mt-3 gap-3 px-4">
                            <button type="button" id="btnSubmit" class="amg-btn amg-btn-primary s2-text btn-filter"
...
<link href="{!! CommonHelper::asset('css/bootstrap-datetimepicker.min.css') !!}" rel="stylesheet" />
<link href="{!! CommonHelper::asset('css/bootstrap-datepicker3.css') !!}" rel="stylesheet" />
<style>
    ...
    .category_wise_agening {
        color: blue;
        padding: 5px 5px;
        text-decoration: none;
    }
    .category_wise_agening:hover {
        color: blue;
        text-decoration: underline;
        padding: 5px 5px;
    }
    .category_wise_agening:visited {
        color: blue;
        text-decoration: underline;
        padding: 5px 5px;
    }
...
@endpush
@push('scripts')
<script type="text/javascript" src="{!! CommonHelper::asset('js/bootstrap-datetimepicker.min.js') !!}"></script>
<script type="text/javascript" src="{!! CommonHelper::asset('js/reports/tickets/catwise_agening_report.js') !!}"></script>
<script type="text/javascript">
    $(document).ready(function() {
        var config = {};
        config.url = {};
        config.url.list_tickets = "{{ url('reports/tickets/jx-catwise-agening-report/') }}";
        config.url.export_ticket_xls = "{{ url('reports/downloaCatWiseAgeningReport') }}";
        ...
        config.translations = {
            ...
            Select_the_Escalate_To: '{{ trans('report.select_the_escalate_to') }}',
            ...
            show_sub_categories: '{{ trans('report.show_sub_categories') }}',
        };
        new CategoryAgeningReport(config);
    });
</script>
@endpush