...
@section('title', trans("custom_report.auto_resolved_tickets_report"))
@section('content')
    <div class="content" id="content-container">
        <div class="header-actions-wrapper d-flex align-items-center justify-content-between pe-4">
            <h3 class="h3-text mb-0">{{ trans("custom_report.auto_resolved_tickets_report") }}</h3>
            <div class="d-flex gap-8">
                ...
                <button class="header-icon-btn header-icon-btn-sm btn-open-filter" type="button" data-bs-toggle="tooltip"
                    ...
                    <span class="filter-count-badge d-none" aria-label="{{ trans('custom_report.active_filters') }}">0</span>
                </button>
                            ...
                            <div>
                                <div class="amg-list-searchbar">
                                    ...
                                    <input type="text"
                                        ...
                                        id="announcement-list-search" placeholder="{{ trans('custom_report.search') }}">
                                </div>
                            </div>
                        ...
                        <div class="list-view-panel">
                            <div class="table-responsive">
                                <table id="lgTbl" class="table display app-data-table">
                                    <thead>
                                        <tr>
                                            <th><h4 class="b2-text">{{ trans('custom_report.ticket_id') }}</h4></th>
                                            <th class="amg-table-col-176"><h4>{{ trans('custom_report.ticket_subject') }}</h4></th>
                                            <th class="amg-table-col-176"><h4>{{ trans('custom_report.department') }}</h4></th>
                                            <th class="amg-table-col-176"><h4>{{ trans('custom_report.problem_category') }}</h4></th>
                                            <th class="amg-table-col-176"><h4>{{ trans('custom_report.sub_category') }}</h4></th>
                                            <th class="amg-table-col-176"><h4>{{ trans('custom_report.status') }}</h4></th>
                                            <th><h4>{{ trans('custom_report.ticket_creator') }}</h4></th>
                                            <th><h4>{{ trans('custom_report.ticket_handler') }}</h4></th>
                                            <th><h4>{{ trans('custom_report.tat') }}</h4></th>
                                            <th><h4>{{ trans('custom_report.priority') }}</h4></th>
                                            <th class="amg-table-col-176"><h4>{{ trans('custom_report.updated_at') }}</h4></th>
                                        </tr>
                                    </thead>
                                    <tbody id="lg">
                                    </tbody>
            ...
            <div class="modal-dialog modal-lg modal-dialog-centered">
                <div class="modal-content rounded-5">
                    <div class="modal-header d-flex align-items-center py-3 pt-4">
                        ...
                        <button data-bs-dismiss="modal" class="modal-close px-4" aria-label="{{ trans('custom_report.close') }}">
                        ...
                        <div class="row g-3 px-3">
                            ...
                            <div class="col-lg-4 col-md-6">
                                ...
                                <select name="filter_by_date" id="filter_by_date" class="form-select">
                                    ...
                                    <option value="1">{{ trans('custom_report.updated_at') }}</option>
                                </select>
                            </div>
...
@push('scripts')
    <script type="text/javascript" src="{!! CommonHelper::asset('js/reports/tickets/auto_resolved_ticket_report.js') !!}"></script>
    <script type="text/javascript">
        $(document).ready(function () {
            var config = {};
            config.url = {};
            config.url.list_tickets = "{{ url('reports/tickets/jx-auto-resolved-ticket-report') }}";
            config.url.export_ticket_xls = "{{ url('reports/export-auto-resolved-ticket-xls') }}";
            config.url.export_ticket_pdf = "{{ url('reports/export-auto-resolved-ticket-pdf') }}";
            ...
            config.translations = {
                filter_by_department: '{{ trans('custom_report.filter_by_department') }}',
                filter_by_status: '{{ trans('custom_report.filter_by_status') }}',
                Filter_By_Problem_Category: '{{ trans('custom_report.filter_by_problem_category') }}',
                Filter_By_Sub_Category: '{{ trans('custom_report.filter_by_sub_category') }}',
                Filter_By_Priority: '{{ trans('custom_report.filter_by_priority') }}',
                Filter_By_Location: '{{ trans('custom_report.filter_by_location_id') }}',
                filter_by_date: '{{ trans('custom_report.filter_by_date') }}',
                Select_the_Ticket_Creator: '{{ trans('custom_report.select_creator') }}',
                Select_the_Ticket_Handler: '{{ trans('custom_report.select_handler') }}',
                no_details: '{{ trans('custom_report.no_records') }}',
                filter_by_tag: '{{ trans('custom_report.filter_by_tag') }}',
                filter_by_ticket_type: '{{ trans('custom_report.filter_by_ticket_type') }}',
                filter_by_base_location: '{{ trans('custom_report.filter_by_base_location') }}',
                filter_by_custom_field: '{{ trans('custom_report.filter_by_custom_field') }}',
                filter_by_custom_field_value: '{{ trans('custom_report.filter_by_custom_field_value') }}',
            };
