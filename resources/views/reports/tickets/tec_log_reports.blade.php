...
{
  "page_no": "TLMD-01",
  "file": "tec_log_reports.blade.php",
  "versions": [
    {
      "version": "1.0",
      ...
      "from": "2026-06",
      "reviewer": null,
      "description": "Initial setup - new UI (DataTable,sort dropdown,filter modal, Localization )"
    }
  ]
}
--}}
@extends('layouts.layout1')
@section('title', 'Technician Log Reports')
@section('content')
    <div id="tech-log-report-wrapper">
        <div class="header-actions-wrapper d-flex align-items-center justify-content-between pe-4">
            <h3 class="h3-text mb-0">{{ trans('ticket_reports.tech_log_report.title') }}</h3>
            <div class="d-flex gap-8">
                ...
                <button class="header-icon-btn header-icon-btn-sm btn-open-filter" type="button"
                    data-bs-toggle="tooltip" title="{{ trans('ticket_reports.tech_log_report.filter') }}">
                    ...
                    <span class="b6-text opacity-50">{{ trans('ticket_reports.tech_log_report.filter') }}</span>
                ...
                <button class="header-icon-btn-only header-icon-btn-only-sm btn-download" type="button"
                    data-bs-toggle="tooltip" title="{{ trans('ticket_reports.tech_log_report.download') }}">
                ...
                <div class="card rounded-0">
                    <div class="card-body">
                        <div class="d-flex align-items-center gap-2 mb-1">
                            <div class="col-auto">
                                <select class="amg-table-pagination-dropdown amgTablePageLenth tech-log-list-page-length">
                                    <option value="10" selected>{{ trans('ticket_reports.tech_log_report.show') }} (10)</option>
                                    <option value="25">{{ trans('ticket_reports.tech_log_report.show') }} (25)</option>
                                    <option value="50">{{ trans('ticket_reports.tech_log_report.show') }} (50)</option>
                                    <option value="100">{{ trans('ticket_reports.tech_log_report.show') }} (100)</option>
                                </select>
                            </div>
                            ...
                            <div>
                                <div class="amg-list-searchbar">
                                    ...
                                    <input type="text" class="amg-list-searchbar__input tech-log-list-search"
                                        placeholder="{{ trans('ticket_reports.tech_log_report.search') }}">
                                </div>
                            </div>
                            <button class="amg-refresh-btn btn-reload-list" title="{{ trans('ticket_reports.tech_log_report.refresh_list') }}">
                                ...
                                <span>{{ trans('ticket_reports.tech_log_report.refresh_list') }}</span>
                            </button>
                        </div>

                        <div class="table-responsive">
                            <table id="tech-log-table" class="amg-datatable table display app-data-table w-100">
                                <thead>
                                    <tr>
                                        <th>{{ trans('content.report_fields.full_name') }}</th>
                                        <th>{{ trans('content.report_fields.activity') }}</th>
                                        <th>{{ trans('content.report_fields.start_date') }}</th>
                                        <th>{{ trans('content.report_fields.start_time') }}</th>
                                        <th>{{ trans('content.report_fields.end_date') }}</th>
                                        <th>{{ trans('content.report_fields.end_time') }}</th>
                                        <th>{{ trans('content.report_fields.duration') }}</th>
                                        <th>{{ trans('content.report_fields.comment') }}</th>
                                    </tr>
                                </thead>
                                <tbody></tbody>
                            </table>
                ...
                <form class="w-100 amg-form-theme" >
                    <div class="modal-content rounded-5">
                        <div class="modal-header d-flex align-items-center py-3 pt-4 px-4">
                            <h4 class="modal-title s1-text fw-semibold">{{ trans('ticket_reports.tech_log_report.filter') }}</h4>
                            <button data-bs-dismiss="modal" class="modal-close" aria-label="Close">
                        ...
                        <div class="modal-body">
                            <div class="row g-3 px-3">
                                <div class="col-12">
                                    <label class="form-label b5-text">{{ trans('content.filter_heading.By_Ticket_Handler') }}</label>
                                    <select name="filter_by_ticket_handlers[]" id="filter_by_ticket_handlers" class="form-select" multiple></select>
                                </div>
                                <div class="col-12">
                                    <label class="form-label b5-text">{{ trans('content.filter_heading.filter_by_daterange') }}</label>
                                    <div id="reportrange" class="form-control d-flex align-items-center justify-content-between cursor-pointer">
                                        <span></span>
                                        <svg width="15px" height="15px" viewBox="0 0 48 48" fill="currentColor">
                                            ...
                                            <g><g><rect width="48" height="48" fill="none" /></g><g><path d="M44,8H35V4.1A2.1,2.1,0,0,0,33.3,2,2,2,0,0,0,31,4V8H17V4.1A2.1,2.1,0,0,0,15.3,2,2,2,0,0,0,13,4V8H4a2,2,0,0,0-2,2V42a2,2,0,0,0,2,2H44a2,2,0,0,0,2-2V10A2,2,0,0,0,44,8ZM42,40H6V20H42Zm0-24H6V12H42Z" /><rect x="8" y="24" width="8" height="8" rx="2" ry="2" /><rect x="32" y="24" width="8" height="8" rx="2" ry="2" /><rect x="20" y="24" width="8" height="8" rx="2" ry="2" /></g></g>
                                        </svg>
                        ...
                        <div class="modal-footer amg-form-footer justify-content-end py-4">
                            <div class="amg-btn-group gap-3">
                                <button type="button" class="amg-btn amg-btn-primary s2-text btn-filter">
                                   {{ trans('ticket_reports.tech_log_report.filter') }}
                                </button>
                                <button type="button" id="btnClrFilter" class="amg-btn amg-btn-ghost bg-black text-white s2-text">
                                   {{ trans('ticket_reports.tech_log_report.clear') }}
                                </button>
                            </div>
                        </div>
                    </div>
...
@push('scripts')
    ...
    <script type="text/javascript" src="{!! CommonHelper::asset('nfy/plugins/moment/moment.min.js') !!}"></script>
    <script type="text/javascript" src="{!! CommonHelper::asset('nfy/plugins/daterangepicker/daterangepicker.min.js') !!}"></script>
    <script src="{!! CommonHelper::asset('js/reports/tickets/tech-log-reports.js') !!}"></script>

    <script>
        $(document).ready(function () {
            var config = {};
            config.url = {};
            config.url.list_tech_logs  = "{{ url('reports/tickets/jx-list-tech-logs') }}";
            config.url.export_ticket   = "{{ url('reports/tickets/downloadTechLogReport') }}";
            config.getUserByQuery      = "{{ url('getUserByQuery') }}";
            config.company  = {!! json_encode($dashboardCompanyId) !!};
            config.token    = "{{ csrf_token() }}";
            config.sort_fields = {!! json_encode($sort_fields['ticket']) !!};
            config.sort_dir = { id: 6, dir: 2 };
            config.translations = {
            by_ticket_handler: '{{ trans('content.filter_heading.By_Ticket_Handler') }}',
            }
            new MyApp(config);
        });
    </script>
