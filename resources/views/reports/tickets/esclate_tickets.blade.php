...
@section('title', trans('report.ticket_escalation_tickets_report'))
@section('content')
    <div class="content">
        <div class="header-actions-wrapper d-flex align-items-center justify-content-between pe-4">
            <h3 class="h3-text mb-0">{{ trans('report.ticket_escalation_tickets_report') }}</h3>
            <div class="d-flex gap-8">
                ...
                {{-- Download --}}
                @can("TicketEscalationReportDownload")
                    <button class="header-icon-btn-only header-icon-btn-only-sm btn-download" type="button"
                        data-bs-toggle="tooltip" title="{{ trans('report.service_ticket_download') }}">
                        ...
                        <div class="d-flex align-items-center gap-2 mb-1">
                            <div class="col-auto">
                                <select id="pageLimiter" class="amg-table-pagination-dropdown amgTablePageLenth">
                                    <option value="10" selected>{{ trans('report.show') }} (10)</option>
                                    <option value="25">{{ trans('report.show') }} (25)</option>
                                    <option value="50">{{ trans('report.show') }} (50)</option>
                                    <option value="100">{{ trans('report.show') }} (100)</option>
                                </select>
                            </div>
                                 ...
                                 <table id="lgTbl" class="amg-datatable mytable table table-striped">
                                    <thead>
                                        <tr id="tHeadeRow">
                                            ...
                                            <th>{{ trans('report.ticket_created_at') }}</th>
                                            <th>{{ trans('report.department') }}</th>
                                            <th>{{ trans('report.problem_category') }}</th>
                                            <th>{{ trans('report.sub_category') }}</th>
                                            <th>{{ trans('report.ticket_handlers') }}</th>
                                            <th>{{ trans('report.created_by') }}</th>
                                            <th>{{ trans('report.escalate_for') }}</th>
                                            <th>{{ trans('report.escalate_to') }}</th>
                                            <th>{{ trans('report.status') }}</th>
                                            <th>{{ trans('report.priority') }}</th>
                                            <th>{{ trans('report.tat') }}</th>
                                            <th>{{ trans('report.location') }}</th>
                                            <th>{{ trans('report.escalation_created_at') }}</th>
                                            <th>{{ trans('report.ticket_type') }}</th>
                    ...
                    <div class="modal-body">
                        <div class="row g-3 px-3">
                            ...
                            <div class="col-xl-3 col-lg-4 col-md-6">
                                ...
                                <select id="filter_by_department" name="filter_by_department"></select>
                            </div>
                            ...
                            <div class="col-xl-3 col-lg-4 col-md-6">
                                ...
                                <select id="filter_by_problem_category"
                                    name="filter_by_problem_category[]" multiple></select>
                            </div>
                            ...
                            <div class="col-xl-3 col-lg-4 col-md-6">
                                ...
                                <select id="filter_by_sub_category"
                                    name="filter_by_sub_category[]" multiple></select>
                            </div>
                            ...
                            <div class="col-xl-3 col-lg-4 col-md-6">
                                ...
                                <select id="filter_by_status"
                                    name="filter_by_status[]" multiple></select>
                            </div>

                            <!-- Ticket Handler -->
                            <div class="col-xl-3 col-lg-4 col-md-6">
                                <label class="form-label b5-text">
                                    {{ trans('report.filter_by_ticket_handler') }}
                                </label>
                                <select id="filter_by_handler"
                                    name="filter_by_handler[]" multiple></select>
                            </div>

                            <!-- Priority -->
                            <div class="col-xl-3 col-lg-4 col-md-6">
                                <label class="form-label b5-text">
                                    {{ trans('report.filter_by_priority') }}
                                </label>
                                <select id="filter_by_priority"
                                    name="filter_by_priority[]" multiple></select>
                            </div>

                            <!-- Escalate To -->
                            <div class="col-xl-3 col-lg-4 col-md-6">
                                <label class="form-label b5-text">
                                    {{ trans('report.filter_by_escalate_to') }}
                                </label>
                                <select id="filter_by_escalate_to" multiple></select>
                            </div>

                            <!-- Location -->
                            <div class="col-xl-3 col-lg-4 col-md-6">
                                <label class="form-label b5-text">
                                    {{ trans('report.filter_by_location_id') }}
                                </label>

                                <select id="filter_location_id"
                                    name="filter_location_id[]" multiple></select>
                            </div>
                            ...
                            <div class="col-xl-3 col-lg-4 col-md-6">
                                ...
                                <select name="filter_by_date" id="filter_by_date"
                                    class="form-select">
                                    <option value="null">
                                        {{ trans('report.no_filter') }}
                                    </option>
                                    <option value="1">{{ trans('report.created_date') }}</option>
                                    <option value="2">{{ trans('report.escalated_date') }}</option>
                                </select>
                            </div>
                            ...
                            <div class="col-xl-3 col-lg-4 col-md-6">
                                <label class="form-label b5-text">
                                    {{ trans('report.filter_by_ticket_type') }}
                                </label>
                                <select name="filter_by_ticket_type[]"
                                    id="filter_by_ticket_type"
                                    multiple
                                    class="form-control">
                            ...
                            <div class="col-xl-3 col-lg-4 col-md-6">
                                <label class="form-label b5-text">
                                    {{ trans('report.filter_by_creator_logger') }}
                                </label>
                            ...
                            <!-- Creator Logger User -->
                            <div class="col-xl-3 col-lg-4 col-md-6 hide creatorcover">
                                <label class="form-label b5-text">
                                    {{ trans('report.select_creator_logger') }}
                                </label>

                                <select id="filter_by_creator" multiple></select>
                            </div>  
                        </div>
                    </div>
...
@push('scripts')
    <script type="text/javascript" src="{!! CommonHelper::asset('js/reports/tickets/esclate_tickets.js') !!}"></script>
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            var config = {};
            config.url = {};
            config.url.list_esclate_tickets = "{{ url('reports/tickets/jx-esclate-tickets') }}";
            config.url.export_ticket = "{{ url('reports/export-esclate-tickets') }}";
            config.url.export_ticket_pdf = "{{ url('reports/export-esclate-tickets-pdf') }}";
            ...
            config.url.ticket_view_url = "{{ url('ticket') }}";
            ...
            config.techID = {!! json_encode($techId) !!};
