...
@section('title', trans('content.report_fields.Overdue_Tickets_Report'))
@section('content')

    <div class="content">
        <div class="header-actions-wrapper d-flex align-items-center justify-content-between pe-4">
            <h3 class="h3-text mb-0">{{ trans('content.report_fields.Overdue_Tickets_Report') }}</h3>
            <div class="d-flex gap-8">
                ...
                {{-- Download --}}
                @can("TicketOverdueReportDownload")
                                ...
                                <table id="lgTbl" class="amg-datatable table display app-data-table">
                                    <thead>
                                        <tr id="tHeadeRow">
                                            ...
                                            <th>{{ trans('content.report_fields.TAT_Expiry_Date') }}</th>
                                            <th>{{ trans('content.report_fields.response_sla_expiry_at') }}</th>
                                            <th>{{ trans('content.report_fields.response_at') }}</th>
                                            <th>{{ trans('content.report_fields.response_sla_breach') }}</th>
                                            <th>{{ trans('content.report_fields.workaround_sla_expiry_at') }}</th>
                                            <th>{{ trans('content.report_fields.workaround_at') }}</th>
                                            <th>{{ trans('content.report_fields.workaround_sla_breach') }}</th>
                                            <th>{{ trans('content.report_fields.Priority') }}</th>
                                            <th>{{ trans('content.report_fields.Created_At') }}</th>
                                            <th>{{ trans('content.report_fields.location') }}</th>
                    ...
                    <div class="modal-body">
                        <div class="row g-3 px-3">
                            <div class="col-lg-4 col-md-6">
                                ...
                                <select id="filter_by_department" name="filter_by_department" class="form-select"></select>
                            </div>
                            ...
                            <div class="col-lg-4 col-md-6">
                                ...
                                <select id="filter_by_status" name="filter_by_status[]" class="form-select"
                                    multiple></select>
                            </div>

                            <div class="col-lg-4 col-md-6">
                                <label class="form-label b5-text">
                                    {{ trans('content.filter_heading.Filter_By_Created_via') }}
                                </label>
                                <select id="filter_by_create_via" class="form-select" multiple>
                            ...
                            <div class="col-lg-4 col-md-6">
                                ...
                                <select id="filter_by_priority" name="filter_by_priority[]" class="form-select"
                                    multiple></select>
                            </div>

                            <div class="col-lg-4 col-md-6">
                                <label class="form-label b5-text">
                                    {{ trans('content.filter_heading.Filter_By_Ticket_Handler') }}
                                </label>
                                <select id="filter_by_handler" name="filter_by_handler[]" class="form-select"
                                    multiple></select>
                            </div>
                            ...
                            <div class="col-lg-4 col-md-6">
                                ...
                                <select name="filter_by_date" id="filter_by_date" class="form-select">
                                    <option value="null">{{ trans('content.report_fields.No_Filter') }}</option>
                                    <option value="1">TAT Expiry Date</option>
                            ...
                            <div class="col-lg-4 col-md-6">
                                <label class="form-label b5-text">
                                    {{ trans('content.filter_heading.Filter_By_Creator_Logger') }}
                                </label>
                                <select id="filter_based_on_cre_log" class="form-select">
                                    <option value="">{{ trans('content.report_fields.No_Filter') }}</option>
                                    <option value="1">Logger</option>
                                    <option value="2">Creator</option>
                                </select>
                            </div>

                            <div class="col-lg-4 col-md-6 hide creatorcover">
                                <label class="form-label b5-text">
                                    Select Creator/Logger
                                </label>
                                <select id="filter_by_creator" class="form-select" multiple></select>
                            </div>

                            <div class="col-lg-4 col-md-6">
                                <label class="form-label b5-text">
                                    {{ trans('content.filter_heading.filter_by_ticket_type') }}
                                </label>
                            ...
                            <div class="col-lg-4 col-md-6">
                                ...
                                <select id="filter_non_handler" class="form-select">
                                    <option>{{ trans('content.report_fields.No_Filter') }}</option>
                                    <option value="null">Non Handler</option>
                                </select>
                            </div>
...
@push('scripts')
    <script type="text/javascript" src="{!! CommonHelper::asset('js/reports/tickets/overdue_tickets.js') !!}"></script>
    <script src="{!! CommonHelper::asset('js/common.js') !!}"></script>
    <script>
        $(document).ready(function() {
            var config = {};
            config.url = {};
            config.url.list_overdue_tickets = "{{ url('reports/tickets/jx-overdue-tickets') }}";
            config.url.export_ticket = "{{ url('reports/export-overdue-tickets') }}";
            config.url.export_ticket_pdf = "{{ url('reports/export-overdue-tickets-pdf') }}";
            ...
            config.translations = {
                ...
                Filter_By_Locations: '{{ trans('content.filter_heading.filter_by_location_id') }}',
            };
            new MyApp(config);
        });
    </script>
