...
@section('content')
    <div class="content">
                    ...
                    <div class="card-body">
                            ...
                            <div>
                                <div class="amg-list-searchbar">
                                    ...
                                    <input type="text"
                                        ...
                                        id="announcement-list-search"
                                        placeholder="{{ trans('internal_place.view.search') }}...">
                                </div>
                            </div>
                        ...
                        <div class="col-lg-12">
                            <span class="text-info">
                                <b>* This report consist of last 30 days data from created at. However you can filter it by date range.</b>
                            </span>
                        </div>
                        <div class="js-user-list-view-panel">
                            <div class="gtable-cover">
                                <table id="lgTbl" class="amg-datatable table display app-data-table">
                                    <thead>
                                    <tr id="tHeadeRow">
                                        <th>{{ trans("content.ticket_report.Ticket_Id") }}</th>
                                        <th>{{ trans("content.ticket_report.subject") }}</th>
                                        <th>{{ trans("content.ticket_report.Status") }}</th>
                                        <th>{{ trans("content.ticket_report.Tat") }}</th>
                                        <th>{{ trans("content.ticket_report.tat_expire_at") }}</th>
                                        <th>{{ trans("content.ticket_report.assign_to") }}</th>
                                        <th>{{ trans("content.ticket_report.assign_at") }}</th>
                                        <th>{{ trans("content.ticket_report.resolved_by") }}</th>
                                        <th>{{ trans("content.ticket_report.resolved_at") }}</th>
                                        <th>{{ trans("content.ticket_report.ageing") }}</th>
                                        <th>{{ trans("content.ticket_report.hold_date_time") }}</th>
                                        <th>{{ trans("content.ticket_report.expected_release_date_time") }}</th>
                                        <th>{{ trans("content.ticket_report.release_date_time") }}</th>
                                    </tr>
                                </thead>
                                    <tbody id="lg"></tbody>
                                </table>
        ...
        {{-- Filter Modal --}}
        <div class="amg-modal modal fade" id="FilterModal" tabindex="-1" aria-hidden="true" data-bs-backdrop="static" data-bs-keyboard="false">
                   ...
                   <div class="modal-body">
                        <div class="row g-3 px-3">
                            ...
                            <div class="col-lg-4 col-md-6">
                                ...
                                <select id="filter_by_department" name="filter_by_department[]" class="form-select" multiple></select>
                            </div>
                            ...
                            <div class="col-lg-4 col-md-6">
                                ...
                                <select id="filter_by_problem_category" name="filter_by_problem_category[]" class="form-select" multiple></select>
                            </div>
                            ...
                            <div class="col-lg-4 col-md-6">
                                ...
                                <select id="filter_by_sub_category" name="filter_by_sub_category[]" class="form-select" multiple></select>
                            </div>
                            ...
                            <div class="col-lg-4 col-md-6">
                                ...
                                <select id="filter_location_id" name="filter_location_id[]" class="form-select" multiple></select>
                            </div>

                            <div class="col-lg-4 col-md-6">
                                <label class="form-label b5-text">
                                    {{ trans("content.filter_heading.filter_by_base_location_id") }}
                                </label>
                                <select id="filter_base_location_id" class="form-select">
                            ...
                            <div class="col-lg-4 col-md-6">
                                <label class="form-label b5-text">
                                    {{ trans("content.filter_heading.By_Ticket_Handler") }}
                                </label>
                                <select id="filter_by_ticket_handlers" name="filter_by_ticket_handlers[]" class="form-select" multiple></select>
                            </div>
                            ...
                            <div class="col-lg-4 col-md-6">
                                ...
                                <select id="filter_by_status" name="filter_by_status[]" class="form-select" multiple></select>
                            </div>
                            ...
                            <div class="col-lg-4 col-md-6">
                                ...
                                <select name="filter_by_date" id="filter_by_date" class="form-select">
                                    ...
                                    <option value="1">{{ trans("content.filter_heading.created_date") }}</option>
                                    <option value="3">{{ trans("content.filter_heading.resolved_date") }}</option>
                                </select>
                            </div>
                    ...
                    <div class="modal-footer justify-content-end mb-4 pb-4 py-0">
                        <div class="amg-btn-group mt-3 gap-3 px-4">
                            <button type="button" class="amg-btn amg-btn-primary s2-text col-md-6 btn-filter">
                                {{ trans("content.report_fields.Filter") }}
                            </button>

                            <button type="button" id="btnClrFilter"
                                class="amg-btn amg-btn-ghost bg-black text-white col-md-6 s2-text">
...
@push('scripts')
<script type="text/javascript" src="{!! CommonHelper::asset('js/reports/tickets/hvac_report.js') !!}"></script>
...
<script type="text/javascript">
    $(document).ready(function() {
        var config = {};
        config.url = {};
        config.url.list_tickets = "{{ url('reports/tickets/jx-hvac-report') }}";
        config.url.export_ticket_xls = "{{ url('reports/export-hvac-report-xls') }}";
        ...
        config.translations = {
            ...
            select_Location: '{{ trans('content.filter_heading.select_location') }}',
            Select_the_Escalate_To: '{{ trans('content.report_fields.Select_the_Escalate_To') }}',
            no_details: '{{ trans('content.report_fields.no_details') }}',
            select_base_location :'{{  trans('content.filter_heading.select_base_location') }}',
            Resolver1_name :'{{  trans('content.ticket_report.Resolver1_name') }}',
            resolver1_time :'{{  trans('content.ticket_report.resolver1_time') }}',
            Resolver2_name :'{{  trans('content.ticket_report.Resolver2_name') }}',
            resolver2_time :'{{  trans('content.ticket_report.resolver2_time') }}',
            Resolver3_name :'{{  trans('content.ticket_report.Resolver3_name') }}',
            resolver3_time :'{{  trans('content.ticket_report.resolver3_time') }}',
            Ticket_Sla_Breached :'{{  trans('content.ticket_report.Ticket_Sla_Breached') }}',
            Sla_breached_Time :'{{  trans('content.ticket_report.Sla_breached_Time') }}',
            customFieldSet: '{{ trans('content.filter_heading.select_custom_field') }}',
