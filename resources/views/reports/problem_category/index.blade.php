...
@section('title', trans("ticket.service_ticket_fields.category_privilege_report"))
@section('content')
    <div class="content" id="content-container">
        <div class="header-actions-wrapper d-flex align-items-center justify-content-between pe-4">
            <h3 class="h3-text mb-0">{{ trans("ticket.service_ticket_fields.category_privilege_report") }}</h3>
                    ...
                    <div class="card-body">
                        ...
                        <div class="list-view-panel">
                            <div class="table-responsive">
                                <table id="category_privillege" class="table display app-data-table">
                                    <thead>
                                        <tr>
                                            <th><h4 class="b2-text">{{ trans('ticket.service_ticket_fields.problem_category') }}</h4></th>
                                            <th><h4>{{ trans('ticket.service_ticket_fields.sub_category') }}</h4></th>
                                            <th><h4>{{ trans('ticket.service_ticket_fields.max_number_of_day') }}</h4></th>
                                            <th><h4>{{ trans('ticket.service_ticket_fields.bu_or_du_head') }}</h4></th>
                                        </tr>
                                    </thead>
                                    <tbody id="lg">
                                    </tbody>
                ...
                <div class="modal-content rounded-5">
                    ...
                    <div class="modal-body">
                        <div class="row g-3 px-3">
                            {{-- Row 1: Department, Problem Category --}}
                            ...
                            {{-- Row 2: Authority Board --}}
                            <div class="col-lg-4 col-md-6">
                                <label class="form-label b5-text">
                                    {{ trans("content.filter_heading.filter_by_authority_board") }}
                                </label>
                                <select name="filter_by_authority_board[]" id="filter_by_authority_board" class="form-select" multiple>
                                </select>
                            </div>
                        </div>
                    </div>
...
@push('scripts')
<script type="text/javascript" src="{!! CommonHelper::asset('js/reports/problem_category/index.js') !!}"></script>
<script type="text/javascript">
    $(document).ready(function() {
        var config = {};
        config.url = {};
        config.url.list_tickets = "{{ url('reports/jx-category-privilege-report') }}";
        config.url.export_ticket_xls = "{{ url('reports/export-category-privilege-report') }}";
        ...
        config.url.authority_board = "{{ url('tickets/authority-board') }}";
        config.user = {!! json_encode(Auth::user()->only("company_id")) !!};
        config.sort_fields = {!! json_encode($fields) !!};
        ...
        config.translations = {
            ...
            filter_by_authority_board: '{{ trans('content.filter_heading.filter_by_authority_board') }}',
