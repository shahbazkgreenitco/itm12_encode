...
@section('title', trans("sidebar.report_menu.reports_email_trigger"))

@section('page_title')
    <div class="page-header">
        <h3>{{ trans("sidebar.report_menu.reports_email_trigger") }}</h3>
    </div>
@endsection
...
<main class="main-content" id="mainContent">
  <div class="container-fluid px-0">
    <div class="header-actions-wrapper d-flex align-items-center justify-content-between px-4">
        <div class="mb-0">
            <h3 class="h3-text mb-0">{{ trans("content.report_fields.reports_email_trigger") }}</h3>
        </div>
        <div class="d-flex gap-8">
			<a role="button" class="amg-btn amg-btn-primary amg-btn-sm open-add-modal"  title="{{ trans("report.reports_email_trigger") }}">
				<i class="bi bi-plus-lg"></i><span>{{ trans("report.create_report") }}</span>
			</a>
        </div>
    </div>
            ...
            <div class="card-body table-responsive">
                ...
                <div class="table-responsive">
                    <table id="mytable" class="mytable amg-datatable table display app-data-table" style="width:100%">
                        <thead>
                            <tr>
                                <th><h4 class="b2-text">{{ trans("content.report_fields.trigger_name") }}</h4></th>
                                <th><h4 class="b2-text">{{ trans("content.report_fields.Company") }}</h4></th>
                                <th><h4 class="b2-text">{{ trans("content.report_fields.enabled") }}</h4></th>
                                <th><h4 class="b2-text">{{ trans("content.report_fields.recipient") }}</h4></th>
                                <th><h4 class="b2-text">{{ trans("content.report_fields.trigger_report_name") }}</h4></th>
                                <th><h4 class="b2-text">{{ trans("content.report_fields.reports_department") }}</h4></th>
  ...
  @include('reports.email-triggers.add_email_report_trigger_modal')
</main>

@endsection
@push('scripts')
<script type="text/javascript" src="{!! CommonHelper::asset('js/reports/email_trigger/index.js') !!}"></script>
<script type="text/javascript">
	...
	config.translations = {
        ...
        save: '{{trans('button.create_report')}}',
    };
	config.url.getEmailReportTriggers = "{{ route('getEmailReportTriggers') }}";
	config.url.add = "{{ route('addEmailReportTriggers') }}";
    config.url.getUserByAjax = "{{ url('getUserByQuery') }}";
	config.url.manage_users = "{{ url('user-allocation-groups/manage-users') }}";
	config.url.getEmailTrigger = "{{ route('getEmailTrigger') }}";
	config.url.editEmailTrigger = "{{ route('editEmailTrigger') }}";
	config.url.delete = "{{ route('deleteEmailReportTriggers') }}";
	config.url.getDepartmentsWithTicketEnabled = "{{ url('tickets/departments') }}";
	config.url.getCompany = "{{ url('getCompanyByUserAccess') }}";
	config.token = "{{ csrf_token() }}";
	config.company = {!! json_encode($defaultCompany) !!};
  new EmailTriggers(config);
</script>
