...
@section('title', trans("config.manufacturer_fields.manufacturer"))
@section('content')
<section class="content">
    <div id="main-user-list-wrapper">
        <div class="header-actions-wrapper d-flex align-items-center justify-content-between pe-4">
            <h3 class="h3-text mb-0">{{ trans("config.manufacturer_fields.manufacturer") }}</h3>
            <div class="d-flex gap-8">
                <button class="header-icon-btn-only header-icon-btn-only-sm btn-download"
                    title="{{ trans('config.location_fields.download') }}" type="button" data-bs-toggle="tooltip">
                ...
                <button class="amg-btn amg-btn-primary amg-btn-sm  open-add-modal" type="button" data-bs-toggle="tooltip"
                    title="{{ trans('config.manufacturer_fields.add_manufacturer') }}">
                    ...
                    <span>{{ trans('config.manufacturer_fields.add_manufacturer') }}</span>
                </button>
            </div>
        </div>
                    ...
                    <div class="card-body pt-3">
                        <div class="d-flex align-items-center gap-2 mb-1">
                            <div class="col-auto">
                                <select class="amg-table-pagination-dropdown userModulePageLenth user-list-page-length">
                                    @foreach([10, 25, 50, 100] as $count)
                                        <option value="{{ $count }}">
                                            {{ __('config.manufacturer_fields.show_count', ['count' => $count]) }}
                                        </option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div>
                                <div class="amg-list-searchbar">
                                    ...
                                    <input type="text" class="amg-list-searchbar__input user-list-search"
                                        placeholder="{{ trans('config.manufacturer_fields.search') }}">
                                </div>
                            </div>
                            ...
                            <button class="amg-refresh-btn btn-reload-list">
                                ...
                                <span>{{ trans('config.manufacturer_fields.refresh') }}</span>
                            </button>
                        </div>
                            ...
                            <div class="table-responsive">
                                <table id="mytable" class="amg-datatable table display app-data-table">
                                    <thead>
                                        <tr>
                                            <th>
                                                <h4>{{ trans("config.manufacturer_fields.id") }}</h4>
                                            </th>
                                            <th>
                                                <h4>{{ trans("config.manufacturer_fields.name") }}</h4>
                                            </th>
                                            <th>
                                                <h4>{{ trans("config.manufacturer_fields.image") }}</h4>
                                            </th>
                                            <th>
                                                <h4>{{ trans("config.manufacturer_fields.actions") }}</h4>
                                            </th>
                                        </tr>
                                    </thead>
                                    <tbody>
...
@include('manufactures.modal_html')
...
<script>
    var config = {};
    config.imgviewpath = "{{ url('/') }}/uploads/manufacturers/";
    config.url = {
        list: "{{ url('manufactures/ajax-get-list') }}",
        add: "{{ url('manufacturer/ajax-add') }}",
        edit: "{{ url('manufacturer/ajax-edit') }}",
        get: "{{ url('manufactures/ajax-get') }}",
        delete: "{{ url('delete-manufacture') }}",
        download_url: "{{ url('export-manufactures') }}",
        del_attach: "{{ url('manufacturere/del-attachment') }}",
    ...
    config.translations = {
        minlength_validation:'{{ trans('config.manufacturer_fields.minlength_validation') }}',
        maxlength_validation:'{{ trans('config.manufacturer_fields.maxlength_validation') }}',
        extension_validation:'{{ trans('config.manufacturer_fields.extension_validation') }}',
        filesize_validation:'{{ trans('config.manufacturer_fields.filesize_validation') }}',
        delete_image_title: '{{ trans('config.manufacturer_fields.delete_image_title') }}',
        delete_image_text: '{{ trans('config.manufacturer_fields.delete_image_text') }}',
        record_deleted: '{{ trans('config.manufacturer_fields.record_deleted') }}',
        manufacturer: '{{ trans('config.manufacturer_fields.manufacturer') }}',
        something_went_wrong_refresh: '{{ trans('config.manufacturer_fields.something_went_wrong_refresh') }}',
        success: '{{ trans('config.manufacturer_fields.success') }}',
        saved_successfully: '{{ trans('config.manufacturer_fields.saved_successfully') }}',
        something_went_wrong: '{{ trans('config.manufacturer_fields.something_went_wrong') }}',
        deleted_successfully: '{{ trans('config.manufacturer_fields.deleted_successfully') }}',
        error: '{{ trans('config.manufacturer_fields.error') }}',
        confirm_delete: '{{ trans('config.manufacturer_fields.confirm_delete') }}',
        cancel: '{{ trans('config.manufacturer_fields.cancel') }}',
        request_failed: '{{ trans('config.manufacturer_fields.request_failed') }}',
        ...
        create: '{{ trans('config.manufacturer_fields.add_manufacturer') }}',
        edit: '{{ trans('config.manufacturer_fields.edit') }}',
        delete: '{{ trans('config.manufacturer_fields.delete') }}',
        save: '{{ trans('button.save') }}',
    };
</script>
<script src="{{ asset('js/manufacturer/list.js') }}"></script>
@endpush