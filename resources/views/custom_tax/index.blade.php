...
[1.0.1] - Updated UI and responsive design
------------------------------------------------------------
*/
--}}
...
{{--
    ...
    ------------------------------------------------------------
    Change Log:
    [1.0.2] - fixed layout issue when page looks cropped
    [1.0.2] - fixed issue where add button is not working
    [1.0.2] - Fixed Add Custom Tax not working.
    ...
    [1.0.2] - Fixed SweetAlert dark mode.
    [1.0.2] - Fixed dark mode for Custom Tax Add/Edit forms.
    [1.0.2] - Updated delete confirmation message.
    [1.0.2] - Added validation for already deleted records while editing.
    [1.0.2] - Added validation for already deleted records while deleting.
    [1.0.2] - Fixed DataTable page length dropdown styling.
    [1.0.2] - Fixed DataTable action button tooltips.
    [1.0.2] - Fixed DataTable font size.
    [1.0.2] - Updated Edit/Delete button styling and icons.
    [1.0.2] - Fixed pagination and record count alignment.
    [1.0.2] - Added localization for DataTable messages.
    ------------------------------------------------------------
    */
--}}

@extends('layouts.layout1')
@section('title', trans("custom_tax.title"))

@section('content')
<section id="custom-tax-list-wrapper" class="content">
        <div class="header-actions-wrapper d-flex align-items-center justify-content-between pe-4">
            <h3 class="h3-text mb-0">{{ trans("custom_tax.title") }}</h3>
            <div class="d-flex gap-8">
                <button class="amg-btn amg-btn-primary amg-btn-sm open-add-modal" type="button" data-bs-toggle="tooltip"
                    title="{{ trans('custom_tax.buttons.add_tax') }}">
                    ...
                    {{ trans('custom_tax.buttons.add_tax') }}
                </button>
            </div>
        </div>
                        ...
                        <div class="d-flex align-items-center gap-2 mb-1">
                            <div class="col-auto">
                                <select class="amg-table-pagination-dropdown userModulePageLenth user-list-page-length">
                                    <option value="10" selected>{{ trans('custom_tax.data_table.show') }} (10)</option>
                                    <option value="25">{{ trans('custom_tax.data_table.show') }} (25)</option>
                                    <option value="50">{{ trans('custom_tax.data_table.show') }} (50)</option>
                                    <option value="100">{{ trans('custom_tax.data_table.show') }} (100)</option>
                                </select>
                            </div>
                            ...
                            <div>
                                <div class="amg-list-searchbar">
                                    ...
                                    <input type="text" class="amg-list-searchbar__input user-list-search"
                                        id="user-list-search" placeholder="{{ trans('custom_tax.data_table.search') }}">
                                </div>
                            </div>
                            ...
                            <button class="amg-refresh-btn btn-reload-list">
                                ...
                                <span>{{ trans('custom_tax.data_table.refresh')}}</span>
                            </button>
                        </div>
                        ...
                        <div class="js-user-list-view-panel">
                            <div class="table-responsive">
                                <table id="mytable" class="amg-datatable table display app-data-table">
                                    <thead>
                                        <tr>
                                            <th><h4>{{ trans("custom_tax.table_fields.id") }}</h4></th>
                                            <th>
                                                <h4>{{ trans("custom_tax.table_fields.tax_name") }}</h4>
                                            </th>
                                            <th>
                                                <h4>{{ trans("custom_tax.table_fields.created_at") }}</h4>
                                            </th>
                                            <th>
                                                <h4>{{ trans("custom_tax.table_fields.updated_at") }}</h4>
                                            </th>
                                            <th>
                                                <h4>{{ trans("custom_tax.table_fields.actions") }}</h4>
                                            </th>
                                        </tr>
                                    </thead>
                                    <tbody>
    ...
    @include("custom_tax.modal_custom_tax")
</section>
@endsection
@push('css')
<style>
...
<script type="text/javascript" src="{!! CommonHelper::asset('js/custom_tax/custom_tax.js') !!}"></script>
<script type="text/javascript">
    $(document).ready(function() {
		var config = {};
		config.url = {};
        config.url.getCustomTax = "{{ url('custom_tax/getCustomTax') }}";
        config.url.saveCustomTax = "{{ url('custom_tax/save') }}";
        config.url.editCustomTax = "{{ url('custom_tax/edit') }}";
        config.url.updateCustomTax = "{{ url('custom_tax/update') }}";
        config.url.removeCustomTax = "{{ url('custom_tax/remove') }}";
		config.url.removeElement = "{{ url('custom_tax/remove-element') }}";
        config.token = "{{ csrf_token() }}";
        config.permissions = {!! json_encode($permissionArray) !!};
        config.translations = {
			add_tax: '{{ trans('custom_tax.buttons.add_tax') }}',
			edit_trigger: '{{ trans('custom_tax.buttons.edit_tax') }}',
            save: '{{ trans('custom_tax.buttons.save') }}',
            remove: '{{ trans('custom_tax.buttons.remove') }}',
			Edit: '{{ trans('custom_tax.buttons.edit') }}',
            Delete: '{{ trans('custom_tax.buttons.delete') }}',
			Add_Tax: '{{ trans('custom_tax.buttons.add_tax') }}',
			cancel: '{{ trans('custom_tax.buttons.cancel') }}',
			ok: '{{ trans('custom_tax.buttons.ok') }}',
			success: '{{ trans('custom_tax.alerts_and_messages.success') }}',
			confirm_delete: '{{ trans('custom_tax.buttons.confirm_delete') }}',
			Add_success: '{{ trans('custom_tax.alerts_and_messages.custom_tax_added') }}',
			delete_custom_tax: '{{ trans('custom_tax.alerts_and_messages.delete_custom_tax') }}',
			warning: '{{ trans('custom_tax.alerts_and_messages.warning') }}',
            oops:'{{ trans('custom_tax.alerts_and_messages.oops') }}',
			Search: '{{ trans('custom_tax.data_table.search') }}',
			delete_element: '{{ trans('custom_tax.alerts_and_messages.delete_element') }}',
			delete_tax: '{{ trans('custom_tax.alerts_and_messages.delete_tax') }}',
			name_required: '{{ trans('custom_tax.alerts_and_messages.name_required') }}',
			minimum_2_characters_required: '{{ trans('custom_tax.alerts_and_messages.minimum_2_characters_required') }}',
            maximum_100_characters_required: '{{ trans('custom_tax.alerts_and_messages.maximum_100_characters_required') }}',
            clean_text_only: '{{ trans('custom_tax.alerts_and_messages.clean_text_only') }}',
            required_field: '{{ trans('custom_tax.alerts_and_messages.field_is_required') }}',
			deleted: '{{ trans('custom_tax.alerts_and_messages.deleted') }}',
			tax_elements: '{{ trans('custom_tax.form.tax_elements') }}',
            previous: '{{ trans('custom_tax.data_table.previous') }}',
            next: '{{ trans('custom_tax.data_table.next') }}',
            showing_entries: '{{ trans('custom_tax.data_table.showing_entries') }}',
            no_entries: '{{ trans('custom_tax.data_table.no_entries') }}',
            filtered_from: '{{ trans('custom_tax.data_table.filtered_from') }}',
            no_matching_records: '{{ trans('custom_tax.data_table.no_matching_records') }}',
            no_data: '{{ trans('custom_tax.data_table.no_data') }}',
            search: '{{ trans('custom_tax.data_table.search') }}',
            length_menu: '{{ trans('custom_tax.data_table.length_menu') }}'
        };
        new TicketTrigger(config);
    });
</script>
@endpush