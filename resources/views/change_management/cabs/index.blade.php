  ...
  "versions": [
    {
      ...
      "from": "2026-03",
      "reviewer": null,
      "description": "CAB listing redesigned to new pattern"
    }
  ]
}
--}}
@extends('layouts.layout1')
@section('title', trans("content.change_management_fields.Change_Advisory_Board"))

@section('content')
<div id="main-change-cab-list-wrapper" id="change-management-cab-list">
    {{-- Header with actions --}}
    <div class="header-actions-wrapper d-flex align-items-center justify-content-between pe-4">
        <h3 class="h3-text mb-0">{{ trans("content.change_management_fields.Change_Advisory_Board") }}</h3>
        <div class="d-flex gap-8">
            ...
            {{-- Export Button --}}
            <button class="header-icon-btn-only header-icon-btn-only-sm btn-export-cabs" type="button" data-bs-toggle="tooltip" title="{{ trans("content.change_management_fields.Download") }}">
            ...
            {{-- Add CAB Button --}}
            @if(isset($permissionArray) && in_array('CABAdd', $permissionArray))
                <button class="amg-btn amg-btn-primary amg-btn-sm btn-add-cab" type="button"  data-bs-target="#cabfrmModal">
                    ...
                    <span>{{ trans("content.change_management_fields.add_cab") }}</span>
                </button>
            @endif
        </div>
    </div>
            ...
            <div class="card rounded-0">
                <div class="card-body pt-3">
                    ...
                    <div class="d-flex align-items-center gap-2 mb-1">
                        <div class="col-auto">
                            <select class="amg-table-pagination-dropdown cab-page-length userModulePageLenth">
                        ...
                        <div class="amg-list-searchbar">
                            ...
                            <input type="text" class="amg-list-searchbar__input cab-search searchbox" placeholder="{{ trans("content.change_management_fields.search") }}">
                        </div>
                    ...
                    {{-- Table --}}
                    <div class="js-change-cab-list-view-panel">
                        <div class="table-responsive">
                            <table id="mytable" class="mytable amg-datatable table display app-data-table">
                               <thead>
                                    <tr>
                                        <th><h4>{{ trans("content.change_management_fields.CAB_Name") }}</h4></th>
                                        <th><h4>{{ trans("ticket.create_ticket.company") }}</h4></th>
                                        <th><h4>{{ trans("content.ticket_procurement.Approval_Mode") }}</h4></th>
                                        <th><h4>{{ trans("content.ticket_procurement.Total_Members") }}</h4></th>
                                        <th><h4>{{ trans("content.change_management_fields.Description") }}</h4></th>
                                        <th><h4>{{ trans("content.change_management_fields.created_At") }}</h4></th>
                                        <th><h4>{{ trans("content.change_management_fields.updated_At") }}</h4></th>
    ...
    {{-- CAB Modal (Add/Edit) --}}
    @include("change_management.cabs.cab_modal")

    {{-- CAB History Modal --}}
    @include("change_management.cabs.cab_history")

    {{-- Description Modal (if needed) --}}
    {{-- @include('tickets.modal_description') --}}

    {{-- Filter Modal (new design) --}}
    @include('change_management.cabs.filter_modal')
...
@push('css')
    ...
    <style>
        /* Keep existing label styles if any */
        #cabfrmModal .input-group-addon {
...
@push('scripts')
    ...
    <script type="text/javascript" src="{!! CommonHelper::asset('js/cab/cab.js') !!}"></script>

    <script type="text/javascript">
        $(document).ready(function () {
            var config = new Object;
            config.url = new Object;
            config.url.getlist = "{{ url('change-management/cab/ajax-list') }}";
            config.url.edit = "{{ url('change-management/cab/edit') }}";
            config.url.updateCAB = "{{ url('change-management/cab/updateCAB') }}";
            config.url.delete = "{{ url('change-management/cab/delete') }}";
            config.url.history = "{{ url('tickets/pab/pab_history') }}";
            config.url.manage_users = "{{ url('change-management/cab/members') }}";
            config.url.getCabForEdit = "{{ url('change-management/cab/getCabForEdit') }}";
            config.url.addCab = "{{ url('change-management/cab/add') }}";
            config.url.viewHistory = "{{ url('change-management/cab/viewHistory') }}";
            config.url.export = "{{ url('change-management/cab/export') }}";
            config.url.get_company_by_user_access = "{{ url('getCompanyByUserAccess') }}";

            config.translations = {
                something_went_wrong: '{{ trans('change_management/cab.something_went_wrong') }}',
                serach_option: '{{ trans('change_management/cab.serach_option') }}',
                Select_Department: '{{ trans('change_management/cab.Select_Department') }}',
                press_enter_with_Search: '{{ trans('change_management/cab.press_enter_with_Search') }}',
                Search: '{{ trans('change_management/cab.search') }}',
                Refresh_List: '{{ trans('change_management/cab.Refresh_List') }}',
                Edit_CAB: '{{ trans('change_management/cab.Edit_CAB') }}',
                History: '{{ trans('change_management/cab.View_History') }}',
                Member_List: '{{ trans('change_management/cab.Member_List') }}',
                Delete_CAB: '{{ trans('change_management/cab.Delete_CAB') }}',
                Add_Financial_Year: '{{ trans('change_management/cab.Add_Financial_Year') }}',
                are_you_delete: '{{ trans('change_management/cab.are_you_delete') }}',
                save_financial: '{{ trans('change_management/cab.save_financial') }}',
                save_changes: '{{ trans('change_management/cab.save_changes') }}',
                confirmation_message: '{{ trans('change_management/cab.confirmation_message') }}',
                are_you_delete_problemcategory: '{{ trans('change_management/cab.are_you_delete_cab') }}',
                addCab: '{{ trans('change_management/cab.add_cab') }}',
                Filter: '{{ trans('change_management/cab.Filter') }}',
                export: '{{ trans('change_management/cab.Download') }}',
                select_company: '{{ __('change_management/cab.select_company') }}'
            };
