...
@section('title', trans("change-management.change_management_fields.categories"))

@section('content')
<div id="main-change-category-list-wrapper" id="cmCategory">
    <div class="header-actions-wrapper d-flex align-items-center justify-content-between pe-4">
        <h3 class="h3-text mb-0">{{ trans("change-management.change_management_fields.categories") }}</h3>
        ...
        <button class="header-icon-btn header-icon-btn-sm btn-open-filter" type="button" data-bs-toggle="tooltip" title="{{ trans("content.user_fields.Filter") }}">
            ...
            {{-- Export Button --}}
            @if(isset($permissionArray) && in_array('ChangeCategoryDownload', $permissionArray))
                <button class="header-icon-btn-only header-icon-btn-only-sm btn-export-categories" type="button" data-bs-toggle="tooltip" title="{{ trans("button.Download") }}">
            ...
            {{-- Add Button --}}
            @if(isset($permissionArray) && in_array('ChangeCategoryAdd', $permissionArray))
                <button class="amg-btn amg-btn-primary amg-btn-sm btn-add-type" type="button" data-bs-toggle="modal" data-bs-target="#cmCategoryModal">
                    ...
                    <span>{{ trans("change-management.change_management_fields.add_catgory") }}</span>
                </button>
            @endif
        </div>
    </div>
        ...
        <div class="container-fluid px-0">
            <div class="card rounded-0">
                <div class="card-body pt-3">
                    {{-- Page Length & Search Bar --}}
                    <div class="d-flex align-items-center gap-2 mb-1">
                        <div class="col-auto">
                            <select class="amg-table-pagination-dropdown userModulePageLenth change-category-page-length">
                        ...
                        <div class="amg-list-searchbar">
                            ...
                            <input type="text" class="amg-list-searchbar__input change-category-search searchbox" placeholder="{{ trans("change-management.change_management_fields.search") }}">
                        </div>
                        ...
                        <button class="amg-refresh-btn btn-reload-list">
                            ...
                            <span>{{ trans("change-management.change_management_fields.refresh_list") }}</span>
                        </button>
                    </div>

                    {{-- Table --}}
                    <div class="js-change-category-list-view-panel">
                        <div class="table-responsive">
                            <table id="mytable" class="mytable amg-datatable table display app-data-table">
                                <thead>
                                    <tr>
                                        <th><h4>{{ trans("change-management.change_management_fields.category_id") }}</h4></th>
                                        <th><h4>{{ trans("change-management.change_management_fields.Name") }}</h4></th>
                                        <th><h4>{{ trans("ticket.create_ticket.company") }}</h4></th>
                                        <th><h4>{{ trans("change-management.change_management_fields.CAB_Name") }}</h4></th>
                                        <th><h4>{{ trans("change-management.change_management_fields.department") }}</h4></th>
                                        <th><h4>{{ trans("change-management.change_management_fields.created_at") }}</h4></th>
                                        <th><h4>{{ trans("change-management.change_management_fields.updated_at") }}</h4></th>
                                        <th><h4>{{ trans("change-management.change_management_fields.Actions") }}</h4></th>
                                    </tr>
                                </thead>
                                <tbody id="lg"></tbody>
                            </table>
    ...
    @include('change_management.categories.category_modal')
    @include('change_management.categories.filter')
...
@push('scripts')
    ...
    <script type="text/javascript" src="{!! CommonHelper::asset('js/change_management/categories/index.js') !!}"></script>
    <script type="text/javascript">
    $(document).ready(function() {
        var config = new Object();
        config.url = new Object();
        config.url.ajaxCategoryList = "{{ url('change-management/ajaxCategoryList') }}";
        config.url.add = "{{ url('change-management/store_category') }}";
        config.url.edit = "{{ url('change-management/getCategory') }}";
        config.url.update_category = "{{ url('change-management/update-category') }}";
        config.url.delete = "{{ url('change-management/delete-category') }}";
        config.url.export_categories = "{{ url('change-management/export-categories') }}";
        config.url.departments_with_company = "{{ url('tickets/departments') }}";
        config.url.fetchCAB = "{{ url('change-management/cab/fetchCab') }}";
        config.url.get_company_by_user_access = "{{ url('getCompanyByUserAccess') }}";
        config.url.cr = "{{ url('change-management/myRequestList/all') }}";
        config.getUserByAjax = "{{ url('getUserByQuery') }}";
        config.sort_dir = {id:9,dir:1};
        ...
        config.company_user_detail = {!! json_encode($userDatail ?? null) !!};
        config.token = "{{ csrf_token() }}";
        config.translations = {
			Edit:'{{ trans('change_management/category.Edit') }}',
			Delete:'{{ trans('change_management/category.Delete') }}',
			Available:'{{ trans('change_management/category.Available') }}',
			Available_Records:'{{ trans('change_management/category.Available_Records') }}',
			something_went_wrong: '{{ trans('change_management/category.something_went_wrong') }}',
			Category:'{{ trans('change_management/category.Category') }}',
			No_Filter:'{{ trans('change_management/category.No_Filter') }}',
			edit_catgory:'{{ trans('change_management/category.edit_catgory') }}',
			add_catgory:'{{ trans('change_management/category.add_catgory') }}',
			are_you_delete_problemcategory:'{{ trans('change_management/category.are_you_delete_problemcategory') }}',
			add_catgory:'{{ trans('change_management/category.add_catgory') }}',
			create:'{{ trans('button.create') }}',
			update:'{{ trans('button.update') }}',
			press_enter_with_Search:'{{ trans('change_management/category.press_enter_with_Search') }}',
			refresh_list:'{{ trans('change_management/category.refresh_list') }}',
			search:'{{ trans('change_management/category.search') }}',
			Filter:'{{ trans('change_management/category.Filter') }}',
			Download:'{{ trans('button.Download') }}',
			select_company: '{{ __('change_management/category.select_company') }}',
			filter_by_department: '{{ trans('change_management/category.filter_by_department') }}',
			select_the_department: '{{ trans('change_management/category.select_the_department') }}',
			Select_CAB:'{{ trans('change_management/category.Select_CAB') }}',
        };
        var listing = new Listing(config);
    });
</script>
@endpush