...
@section('title', trans("content.email_details.email_details"))
@section('content')
	<div id="main-user-list-wrapper">
		<section class="content">
			<div class="header-actions-wrapper d-flex align-items-center justify-content-between pe-4">
				<h3 class="h3-text mb-0">{{ trans("content.email_details.email_details") }}</h3>
				<div class="d-flex gap-8">
					<!-- Download -->
					@if (Auth::user()->hasPermissionTo('EmailLogDownload'))
						<button class="header-icon-btn-only header-icon-btn-only-sm btn-export-emails" type="button" data-bs-toggle="tooltip" title="{{ trans('user.user_toolbar.download') }}">
					...
					<div class="dropdown">
						<button class="header-icon-btn-only header-icon-btn-only-sm"
							...
							<svg width="18" height="18" viewBox="0 0 16 16" fill="none">
								<path d="M1 2h4v12H1V2zm5 0h4v12H6V2zm5 0h4v12h-4V2z" fill="currentColor"/>
							</svg>

						</button>

						<div class="dropdown-menu dropdown-menu-end p-2"
							id="columnVisibilityControls">
						</div>
					</div>
				</div>
			</div>
							...
							<div class="d-flex align-items-center gap-2 mb-1">
								...
								<div>
									<div class="amg-list-searchbar">
										...
										<input type="text" class="amg-list-searchbar__input email-log-list-search searchbox plain-search" id="email-log-list-search" placeholder="Search...">
									</div>
								</div>
							...
							<div class="js-user-list-view-panel">
								<div class="table-responsive">
									<table id="mytable" class="table display app-data-table">
										<thead>
											<tr>
												 <th><h4>{{ trans("content.email_details.to") }}</h4></th>
												 <th><h4>{{ trans("content.email_details.subject") }}</h4></th>
												 <th><h4>{{ trans("content.email_details.sent_at") }}</h4></th>
												 <th><h4>{{ trans("content.email_details.created_at") }}</h4></th>
												 <th><h4>{{ trans("content.email_details.updated_at") }}</h4></th>
											</tr>
										</thead>
										<tbody>
										</tbody>
							...
							<div class="row">
								<div class="col-lg-4">
									...
									<select name="filter_by_date" id="filter_by_date" autocomplete="off" class="form-control">
										...
										<option value="1">Sent At</option>
										<option value="2">Created At</option>
										<option value="3">Updated At</option>
									</select>
								</div>
								<div class="col-lg-4">
									<label>{{ trans("content.filter_heading.filter_by_daterange") }}</label>
...
@push('css')
	<style>
		...
		#columnVisibilityControls .form-check-input {
		...
		#columnVisibilityControls .form-check-label {
			margin: 0;
			cursor: pointer;
		}

		#columnVisibilityControls.dropdown-menu {
			padding: 12px;
		}
	</style>
...
@push('scripts')
<script type="text/javascript" src="{!! CommonHelper::asset('js/email_logs/index.js') !!}"></script>
<script type="text/javascript">
	var config = new Object;
	config.url = new Object;
	config.translations = {
		Refresh_List: '{{trans('content.device_fields.refresh_list')}}',
		press_enter_with_Search: '{{ trans('content.user_fields.press_enter_with_Search') }}',
		Search:'{{ trans('content.user_fields.Search') }}',
		Filter: '{{ trans('content.accessory_fields.Filter') }}',
		download:  '{{ trans('content.device_type.download') }}',
	};
	config.url.email = "{{ url('jx-emails-details') }}";
	config.url.download_url = "{{ url('export-email-details') }}";
