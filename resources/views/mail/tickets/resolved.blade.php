{{-- @page-meta
{
    ...
    "versions": [
        ...
        "from": "2026-06",
        "reviewer": null,
        "description": "Ticket Resolved Mail"
        },
        {
        "version": "1.1",
        ...
        "from": "2026-07",
        "reviewer": null,
        "description": "New modern layout with greeting icon, resolved header, details table, feedback CTA, and back trails"
        }
    ]
}
--}}
...
@endphp
    ...
    <h1 style="font-size:22px; font-weight:700; color:#1a1a1a; line-height:1.35; margin:0 0 6px 0; font-family:'Inter',Arial,sans-serif;">
        Your ticket has been<br>
        <span style="color:{{ $themeColor }};">Resolved</span>
    </h1>
    ...
    @if(!empty($comment))
        <div style="text-align:left; font-family:'Inter',Arial,sans-serif; font-size:14px; color:#333333; background:#f8f9fa; padding:16px 20px; border-radius:6px; margin:0 0 24px 0; border-left:4px solid {{ $themeColor }};">
            <p style="margin:0 0 8px 0; font-weight:600; color:#1a1a1a; font-size:13px;">Resolution Comment:</p>
            <em style="font-style:italic;">{!! CommonHelper::renderTktContent($comment, $embedded_attachments, true) !!}</em>
        </div>
    @endif
    ...
    <table width="100%" cellpadding="0" cellspacing="0" border="0" style="border-collapse:collapse; margin:0 auto 28px; border:1px solid #e6ebf1; border-radius:6px; font-family:'Inter',Arial,sans-serif;">
        ...
        <tr style="background:{{ $bg }};">
            ...
            <td style="{{ $tdBase }} background-color:{{ $bg }}; border-bottom:1px solid #e6ebf1;">
                <span style="color:#008000; font-weight:550; background:#e6f7e6; border-radius:23px; padding:5px 10px; display:inline-block;">{{ optional($ticket->status)->name }}</span>
            </td>
        </tr>
        @endif
    ...
    <!-- Feedback Section -->
    @if($user)
    <p style="{{ $pBase }}">We would like to get your feedback about this ticket.</p>
                ...
                <table cellpadding="0" cellspacing="0" border="0">
                    <tr>
                        <td align="center" bgcolor="{{ $themeColor }}" style="border-radius:6px; padding:12px 28px; background:{{ $themeColor }};">
                            <a href="{{ url('ticket/feedback/' . base64_encode('###'.$ticket->id)) }}" style="color:#ffffff; font-size:14px; font-weight:600; font-family:'Inter',Arial,sans-serif; text-decoration:none; display:inline-block; background:{{ $themeColor }}; border-radius:6px;">
                                Share Your Feedback <span style="font-size:16px;">→</span>
                            </a>
                        </td>
                    </tr>
                </table>
    ...
    <p style="text-align:center; margin:0 0 20px 0;">
        <a href="{{ url('ticket/feedback/' . base64_encode('###'.$ticket->id)) }}">
            <img src="{{ asset('images/feedback.png') }}" alt="Share your feedback" width="200" style="display:inline-block; width:200px; max-width:200px; height:auto; border:0;" />
        </a>
    </p>
    @endif
