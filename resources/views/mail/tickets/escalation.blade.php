...
{
    "page_no": "STEN-27",
    "file": "escalation_notification.blade.php",
    "versions": [
        {
        "version": "1.0",
        "writer": "Priya Maru",
        "from": "2026-06",
        "reviewer": null,
        "description": "Ticket Escalation Notification Mail"
        },
        {
        "version": "1.1",
        "writer": "Priya Maru",
        ...
        "from": "2026-07",
        "reviewer": null,
        "description": "Theme alignment - updated to match modern layout with greeting icon, escalation warning, and proper styling"
        }
    ]
}
--}}
...
<h1 style="font-size:22px; font-weight:700; color:#1a1a1a; line-height:1.35; margin:0 0 6px 0; font-family:'Inter',Arial,sans-serif;">
    Ticket has reached<br>
    <span style="color:{{ $themeColor }};">Escalation Stage</span>
</h1>
...
<p style="font-size:13px; color:#666; margin:0 0 24px 0; font-family:'Inter',Arial,sans-serif;">
    This ticket has reached escalation stage. Please take immediate action.
</p>
...
<table width="100%" cellpadding="0" cellspacing="0" border="0" style="border-collapse:collapse; margin:0 auto 28px; border:1px solid #e6ebf1; border-radius:6px; font-family:'Inter',Arial,sans-serif;">
    ...
    <tr style="background:{{ $bg }};">
        ...
        <td width="62%" style="{{ $tdBase }} background-color:{{ $bg }}; border-bottom:1px solid #e6ebf1;">{{ "###".$ticket->id."###" }}</td>
    </tr>
    @endif
    ...
    <tr style="background:{{ $bg }};">
        ...
        <td style="{{ $tdBase }} background-color:{{ $bg }}; border-bottom:1px solid #e6ebf1;">
            <span style="color:#dc3545; font-weight:550; background:#ffe6e6; border-radius:23px; padding:5px 10px; display:inline-block;">{{ optional($ticket->status)->name }}</span>
        </td>
    </tr>
    @endif
    ...
    <tr style="background:{{ $bg }};">
        ...
        <td style="{{ $tdBase }} background-color:{{ $bg }}; border-bottom:1px solid #e6ebf1;">
            <span style="color:#e30613; font-weight:550; background:#ffe6e6; border-radius:23px; padding:5px 10px; display:inline-block;">{{ optional($ticket->priority)->name }}</span>
        </td>
    </tr>
    @endif
    ...
    <tr style="background:{{ $bg }};">
        ...
        <td style="{{ $tdBase }} background-color:{{ $bg }}; border-bottom:1px solid #e6ebf1;">
            <span style="color:#dc3545; font-weight:600;">{{ $formattedTatExpire }}</span>
        </td>
    </tr>
    @endif

    @if(!empty($ticket->assigned_to) && !empty($ticket->assignedTo))
    <!-- Assigned To -->
    @php $bg = ($rowIndex++ % 2 == 0) ? '#f4f7fb' : '#ffffff'; @endphp
    <tr style="background:{{ $bg }};">
        ...
        <td style="{{ $tdBase }} background-color:{{ $bg }}; border-bottom:1px solid #e6ebf1;">
            <table cellpadding="0" cellspacing="0" border="0" style="display:inline-table;">
                <tr>
                    ...
                    <td style="vertical-align:middle;">{{ $ticket->assignedTo->getGuranteedNameText(true) }}</td>
                </tr>
            </table>
        </td>
    </tr>
            ...
            <table cellpadding="0" cellspacing="0" border="0">
                <tr>
                    <td align="center" bgcolor="{{ $themeColor }}" style="border-radius:6px; padding:12px 28px; background:{{ $themeColor }};">
                        <a href="{{ Config::get('app.url') }}/ticket/{{ $ticket->id }}" 
