...
{
"page_no": "STCM-27",
"file": "intimate_new_creator.blade.php",
"versions": [
{
"version": "1.0",
"writer": "Priya Maru",
"from": "2026-06",
"reviewer": null,
"description": "Ticket Change Creator Mail"
},
{
"version": "1.1",
...
"from": "2026-07",
"reviewer": null,
"description": "Theme alignment - updated to match modern layout with greeting icon, creator assignment info, and proper
styling"
}
]
}
...
@section('content')
                ...
                <table cellpadding="0" cellspacing="0" border="0" style="display:inline-block;">
                    <tr>
                        <td style="padding-right:10px; vertical-align:middle;">
                            <span
                                style="display:inline-block; width:28px; height:28px; border-radius:50%; background:#e8eef5; text-align:center; line-height:28px; vertical-align:middle; overflow:hidden;">
    ...
    <h1
        ...
        You have been assigned as<br>
        <span style="color:{{ $themeColor }};">"Creator"</span>
    </h1>
    ...
    <p style="font-size:13px; color:#666; margin:0 0 24px 0; font-family:'Inter',Arial,sans-serif;">
        We will resolve your query on desired time.
    </p>
    ...
    <table width="100%" cellpadding="0" cellspacing="0" border="0"
        ...
        @if(!empty($ticket->id))
            ...
            <tr style="background:{{ $bg }};">
                <td width="35%" style="{{ $thBase }} background-color:{{ $bg }}; border-bottom:1px solid #e6ebf1;">Ticket ID
                </td>
        ...
        @if(!empty(optional($ticket->department)->name))
            ...
            <tr style="background:{{ $bg }};">
                ...
                <td style="{{ $tdBase }} background-color:{{ $bg }}; border-bottom:1px solid #e6ebf1;">
                    {{ optional($ticket->department)->name }}</td>
            </tr>
        @endif
        ...
        @if(!empty(optional($ticket->problemCategory)->name))
            ...
            <tr style="background:{{ $bg }};">
                ...
                <td style="{{ $tdBase }} background-color:{{ $bg }}; border-bottom:1px solid #e6ebf1;">
                    {{ optional($ticket->problemCategory)->name }}</td>
            </tr>
        @endif
        ...
        @php
            $embedded_attachments = App\Models\Ticket\Attachment::getEmbeddedAttachments($ticket->id ?? 0);
            $content = CommonHelper::renderTktContent($ticket->content ?? '', $embedded_attachments);
            $cleanContent = trim(strip_tags(str_replace('&nbsp;', ' ', $content)));
        ...
        @if(!empty($cleanContent))
            ...
            <tr style="background:{{ $bg }};">
                ...
                <td style="{{ $tdBase }} background-color:{{ $bg }}; border-bottom:1px solid #e6ebf1;">
                    <div
                        style="word-break: break-word; white-space: normal; padding:8px 12px; background:#f8f9fa; border-radius:6px; border-left:3px solid {{ $themeColor }};">
                        {!! $limitedContent !!}
                    </div>
                </td>
            </tr>
        ...
        @if(!empty(optional($ticket->status)->name))
            ...
            <tr style="background:{{ $bg }};">
                ...
                <td style="{{ $tdBase }} background-color:{{ $bg }}; border-bottom:1px solid #e6ebf1;">
                    <span
                        style="color:#7b2d8e; font-weight:550; background:#f8e6ff; border-radius:23px; padding:5px 10px; display:inline-block;">{{ optional($ticket->status)->name }}</span>
                </td>
            </tr>
        @endif
        ...
        @if(!empty(optional($ticket->priority)->name))
            ...
            <tr style="background:{{ $bg }};">
                ...
                <td style="{{ $tdBase }} background-color:{{ $bg }}; border-bottom:1px solid #e6ebf1;">
                    <span
                        style="color:#e30613; font-weight:550; background:#ffe6e6; border-radius:23px; padding:5px 10px; display:inline-block;">{{ optional($ticket->priority)->name }}</span>
                </td>
            </tr>
        @endif
        ...
        @if(!empty($ticket->tat))
            ...
            <tr style="background:{{ $bg }};">
                ...
                <td style="{{ $tdBase }} background-color:{{ $bg }}; border-bottom:1px solid #e6ebf1;">{{ $ticket->tat }} Hrs
                </td>
            </tr>
        @endif
        ...
        @if(!empty($formattedTatExpire))
            ...
            <tr style="background:{{ $bg }};">
                ...
                <td style="{{ $tdBase }} background-color:{{ $bg }}; border-bottom:1px solid #e6ebf1;">{{ $formattedTatExpire }}
                </td>
            </tr>
        @endif
                    ...
                    <table cellpadding="0" cellspacing="0" border="0" style="display:inline-table;">
                        <tr>
                            <td style="padding-right:6px; vertical-align:middle;">
                                <span
                                    style="display:inline-block; width:22px; height:22px; border-radius:50%; background:#e8eef5; text-align:center; line-height:22px; vertical-align:middle; overflow:hidden;">
                                    <svg viewBox="0 0 24 24" width="12" height="12"
                                        style="fill:#6b7c93; vertical-align:middle; display:inline-block;">
                                        <path
                                            d="M12 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm0 2c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5.33-4-8-4z" />
                                    </svg>
                                </span>
    ...
    @if($user && !$ticket->isCreatedViaEmail())
        ...
        <p style="font-size:14px; color:#333; margin:0 0 12px 0; font-family:'Inter',Arial,sans-serif;">
            You can access further updates on this ticket using the following link.
        </p>
                    ...
                    <table cellpadding="0" cellspacing="0" border="0">
                        <tr>
                            <td align="center" bgcolor="{{ $themeColor }}"
                                style="border-radius:6px; padding:10px 24px; background:{{ $themeColor }};">
