{{-- @page-meta
{
    ...
    "versions": [
        ...
        "from": "2026-06",
        "reviewer": null,
        "description": "Ticket comment Mail"
        },
        {
        "version": "1.1",
        "writer": "Priya Maru",
        ...
        "from": "2026-07",
        "reviewer": null,
        "description": "New modern layout with greeting icon, comment header, details table, and CTA"
        }
    ]
} 
--}}
...
@endphp
    ...
    <p style="font-size:13px; color:#666; margin:0 0 4px 0; font-family:'Inter',Arial,sans-serif;">
        Comment from: <b>@if($creator) {{ $creator->getGuranteedNameText(true) }} @endif</b>
    </p>
    <p style="font-size:13px; color:#666; margin:0 0 24px 0; font-family:'Inter',Arial,sans-serif;">
        Please make sure ticket is resolved within the given time frame.
    </p>
    ...
    @if(!empty($comment))
        <div style="text-align:left; font-family:'Inter',Arial,sans-serif; font-size:14px; color:#333333; background:#f8f9fa; padding:16px 20px; border-radius:6px; margin:0 0 24px 0; border-left:4px solid {{ $themeColor }};">
            {!! CommonHelper::renderTktContent($comment, $embedded_attachments, true) !!}
        </div>
    @endif
                ...
                <table cellpadding="0" cellspacing="0" border="0">
                    <tr>
                        <td align="center" bgcolor="{{ $themeColor }}" style="border-radius:6px; padding:12px 28px; background:{{ $themeColor }};">
                            <a href="{{ Config::get('app.url') }}/ticket/{{ $ticket->id }}" style="color:#ffffff; font-size:14px; font-weight:600; font-family:'Inter',Arial,sans-serif; text-decoration:none; display:inline-block; background:{{ $themeColor }}; border-radius:6px;">
    ...
    @if($add_back_trail && count($back_trails))
        @foreach($back_trails as $bt) 
            ...
            <p style="font-family:'Inter',Arial,sans-serif; font-size:12px; color:#666666; text-align:left; margin:0 0 5px 0;">{!! \Illuminate\Support\Str::limit(strip_tags(CommonHelper::renderTktContent($bt->msg)), 500, '...') !!}</p>
            <p style="font-family:'Inter',Arial,sans-serif; font-size:13px; color:#333333; text-align:left; margin:0 0 10px 0;">{!! \Illuminate\Support\Str::limit(strip_tags(CommonHelper::renderTktContent($bt->remarks, $embedded_attachments, true)), 500, '...') !!}</p>
        @endforeach
    @endif
