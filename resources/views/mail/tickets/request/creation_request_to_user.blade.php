...
{
    "page_no": "CR05-27",
    "file": "create_request_to_user.blade.php",
    "versions": [
        {
        "version": "1.0",
        "writer": "Priya Maru",
        "from": "2026-05",
        "reviewer": null,
        "description": "Create Request Mail"
        },
        {
        "version": "1.1",
        "writer": "Priya Maru",
        ...
        "from": "2026-07",
        "reviewer": null,
        "description": "Theme alignment - updated to match modern layout with greeting icon, request submitted info, and proper styling"
        }
    ]
}
--}}
            ...
            <table cellpadding="0" cellspacing="0" border="0" style="display:inline-block;">
                <tr>
                    ...
                    <td style="font-size:15px; color:#333; font-weight:400; vertical-align:middle; font-family:'Inter',Arial,sans-serif;">
                        Hello @if($user) {{ $user->getGuranteedNameText(true) }}@endif,
                    </td>
                </tr>
            </table>
        </td>
...
<h1 style="font-size:22px; font-weight:700; color:#1a1a1a; line-height:1.35; margin:0 0 6px 0; font-family:'Inter',Arial,sans-serif;">
    Service Request<br>
    <span style="color:{{ $themeColor }};">Submitted Successfully</span>
</h1>
<p style="font-size:13px; color:#666; margin:0 0 4px 0; font-family:'Inter',Arial,sans-serif;">
    Service Request ID: <b>{{ $ticketProcureRequestObj->procure_tag }}</b>
</p>
<p style="font-size:13px; color:#666; margin:0 0 24px 0; font-family:'Inter',Arial,sans-serif;">
    Your request has been submitted for approval. Once approved, it will be assigned to an engineer for execution.
</p>
...
<table width="100%" cellpadding="0" cellspacing="0" border="0" style="border-collapse:collapse; margin:0 auto 28px; border:1px solid #e6ebf1; border-radius:6px; font-family:'Inter',Arial,sans-serif;">
    ...
    <tr style="background:{{ $bg }};">
        ...
        <td width="65%" style="{{ $tdBase }} background-color:{{ $bg }}; border-bottom:1px solid #e6ebf1;">
            <strong style="color:{{ $themeColor }};">{{ $ticketProcureRequestObj->procure_tag }}</strong>
        </td>
    </tr>
    @endif

    @if(!empty(optional($ticketProcureRequestObj->company)->name))
    <!-- Company -->
    @php $bg = ($rowIndex++ % 2 == 0) ? '#f4f7fb' : '#ffffff'; @endphp
    <tr style="background:{{ $bg }};">
        <td style="{{ $thBase }} background-color:{{ $bg }}; border-bottom:1px solid #e6ebf1;">Company</td>
        <td style="{{ $tdBase }} background-color:{{ $bg }}; border-bottom:1px solid #e6ebf1;">{{ $ticketProcureRequestObj->company->name }}</td>
    </tr>
    @endif

    @if(!empty(optional($ticketProcureRequestObj->department)->name))
    <!-- Department -->
    @php $bg = ($rowIndex++ % 2 == 0) ? '#f4f7fb' : '#ffffff'; @endphp
    <tr style="background:{{ $bg }};">
        <td style="{{ $thBase }} background-color:{{ $bg }}; border-bottom:1px solid #e6ebf1;">Department</td>
        <td style="{{ $tdBase }} background-color:{{ $bg }}; border-bottom:1px solid #e6ebf1;">{{ $ticketProcureRequestObj->department->name }}</td>
    </tr>
    @endif
    ...
    @php
        ...
        $processedContent = CommonHelper::renderTktContent($ticketProcureRequestObj->content, $embedded_attachments);
    ...
    @if(!empty($ticketProcureRequestObj->subject))
    <!-- Subject -->
    @php $bg = ($rowIndex++ % 2 == 0) ? '#f4f7fb' : '#ffffff'; @endphp
    <tr style="background:{{ $bg }};">
        <td style="{{ $thBase }} background-color:{{ $bg }}; border-bottom:1px solid #e6ebf1;">Subject</td>
        <td style="{{ $tdBase }} background-color:{{ $bg }}; border-bottom:1px solid #e6ebf1;">{{ $ticketProcureRequestObj->subject }}</td>
    </tr>
    @endif

    @if(!empty($ticketProcureRequestObj->tat))
    <!-- TAT -->
    @php $bg = ($rowIndex++ % 2 == 0) ? '#f4f7fb' : '#ffffff'; @endphp
    <tr style="background:{{ $bg }};">
        <td style="{{ $thBase }} background-color:{{ $bg }}; border-bottom:1px solid #e6ebf1;">TAT</td>
        <td style="{{ $tdBase }} background-color:{{ $bg }}; border-bottom:1px solid #e6ebf1;">{{ $ticketProcureRequestObj->tat }} Hrs</td>
    </tr>
    @endif
...
<p style="font-size:14px; color:#333; margin:0 0 12px 0; font-family:'Inter',Arial,sans-serif;">
    You can access the updates on the link below.
</p>
