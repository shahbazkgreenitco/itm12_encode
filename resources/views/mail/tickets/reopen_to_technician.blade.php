...
{
    "page_no": null,
    "file": "ticket_reopened.blade.php",
    "versions": [
        {
        "version": "1.1",
        ...
        "from": "2026-07",
        "reviewer": null,
        "description": "New modern layout with greeting icon, reopened header, details table, and CTA"
        }
    ]
}
--}}
...
@endphp
    ...
    <h1 style="font-size:22px; font-weight:700; color:#1a1a1a; line-height:1.35; margin:0 0 6px 0; font-family:'Inter',Arial,sans-serif;">
        Ticket <span style="color:{{ $themeColor }};">Reopened</span><br>
        <span style="font-size:18px; color:#666;">#{{ $ticket->ticket_tag ?? $ticket->id }}</span>
    </h1>
    <p style="font-size:13px; color:#666; margin:0 0 4px 0; font-family:'Inter',Arial,sans-serif;">
        Kindly review the details, resolve and confirm back to the User.
    </p>
    ...
    <table width="100%" cellpadding="0" cellspacing="0" border="0" style="border-collapse:collapse; margin:0 auto 28px; border:1px solid #e6ebf1; border-radius:6px; font-family:'Inter',Arial,sans-serif;">
        ...
        <tr style="background:{{ $bg }};">
            ...
            <td style="{{ $tdBase }} background-color:{{ $bg }}; border-bottom:1px solid #e6ebf1;">
                <span style="color:#e30613; font-weight:550; background:#ffe6e6; border-radius:23px; padding:5px 10px; display:inline-block;">{{ optional($ticket->status)->name }}</span>
            </td>
        </tr>
        @endif
