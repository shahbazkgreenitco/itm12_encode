...
{
    "page_no": "STAM-26",
    "file": "initimate_assigned.blade.php",
    "versions": [
        {
        "version": "1.0",
        "writer": "shivam kumar",
        "from": "2026-07",
        "reviewer": null,
        "description": "New modern layout with greeting icon, assigned header, details table, and CTA"
        }
    ]
}
--}}
...
@endphp
    ...
    <!-- Header - You have been assigned a Ticket -->
    <h1 style="font-size:22px; font-weight:700; color:#1a1a1a; line-height:1.35; margin:0 0 6px 0; font-family:'Inter',Arial,sans-serif;">
        ...
        a <span style="color:{{ $themeColor }};">Ticket</span>.
    </h1>
    <p style="font-size:13px; color:#666; margin:0 0 4px 0; font-family:'Inter',Arial,sans-serif;">
        Carefully view the details, get it resolved within the timeline.
    </p>
    ...
    <table width="100%" cellpadding="0" cellspacing="0" border="0" style="border-collapse:collapse; margin:0 auto 28px; border:1px solid #e6ebf1; border-radius:6px; font-family:'Inter',Arial,sans-serif;">
        ...
        @if(!empty($ticket->expireInfo("d/m/Y h:i a")))
        <!-- TAT Expire -->
        @php $bg = ($rowIndex++ % 2 == 0) ? '#f4f7fb' : '#ffffff'; @endphp
        <tr style="background:{{ $bg }};">
            <td style="{{ $thBase }} background-color:{{ $bg }}; border-bottom:1px solid #e6ebf1;">TAT Expire</td>
            <td style="{{ $tdBase }} background-color:{{ $bg }}; border-bottom:1px solid #e6ebf1;">{{ $ticket->expireInfo("d/m/Y h:i a") }}</td>
        </tr>
        @endif
                        ...
                        <tr>
                            <td style="padding-right:6px; vertical-align:middle;">
                                <span style="display:inline-block; width:22px; height:22px; border-radius:50%; background:#e8eef5; text-align:center; line-height:22px; vertical-align:middle; overflow:hidden;">
                                    @if($engineer_name && $engineer_name->getProfileImg())
                                        <img src="{{ $engineer_name->getProfileImg() }}" 
                                            alt="{{ $engineer_name->getGuranteedNameText(true) }}" 
                                            style="width:22px; height:22px; border-radius:50%; display:block; object-fit:cover; border:none;"/>
                                    @else
                            ...
                            <td style="vertical-align:middle;">{{ $engineer_name->getGuranteedNameText(true) }}</td>
                        </tr>
                    </table>
                </td>
            </tr>
