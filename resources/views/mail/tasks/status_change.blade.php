{{-- @page-meta
{
    ...
    "versions": [
        ...
        "from": "2026-06",
        "reviewer": null,
        "description": "Task status change Mail"
        }
    ]
} 
--}}
...
<p>Hello ,</p>
<p>Please be informed that task <b>#{{ $task->id }}</b> status has been moved from <b>{{ $oldstatus }}</b> to <b>{{ $task->status->name }}</b>.</p>
<table class="table">
    ...
    @if(!empty($task->type_id) && $task->type_id == 4 && !empty($task->ticket))
        <tr style="background-color: {{ ($rowIndex++ % 2 == 0) ? '#f4f4f4' : '#ffffff' }};">
            ...
            <td>
                #{{ $task->ticket->id }} - {{ $task->ticket->subject }}
            </td>
        </tr>
    @endif
