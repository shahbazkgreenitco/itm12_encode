...
@section('title', trans("config.announcement.announcements"))
@section('content')
<div id="main-user-list-wrapper">
<section class="content">
    <div class="header-actions-wrapper d-flex align-items-center justify-content-between pe-4">
        <h3 class="h3-text mb-0">{{ trans("config.announcement.announcements") }}</h3>
        <div class="d-flex gap-8">
            <!-- Download -->
            @if (Auth::user()->hasPermissionTo('AnnouncementDownload'))
                <button class="header-icon-btn-only header-icon-btn-only-sm btn-download" type="button" data-bs-toggle="tooltip" title="{{ trans('user.user_toolbar.download') }}">
            ...
            @endif
            <button class="header-icon-btn header-icon-btn-sm btn-open-filter" type="button" data-bs-toggle="tooltip" title="{{ trans('config.announcement.Filter') }}">
                ...
                <span class="b6-text opacity-50">{{ trans('config.announcement.Filter') }}</span>
                <span class="filter-count-badge d-none" aria-label="Active filters">0</span>
            </button>
             <!-- Add -->
            @if(Auth::user()->hasPermissionTo('AnnouncementAdd'))
                <button class="amg-btn amg-btn-primary amg-btn-sm open-add-modal" type="button" data-bs-toggle="tooltip" title="{{ trans('config.announcement.add_announcement') }}">
                        ...
                        <span>{{ trans('config.announcement.add_announcement') }}</span>
                </button>
            @endif
        </div>
    </div>
                    ...
                    <div class="d-flex align-items-center gap-2 mb-1">
                        ...
                        <div>
                            <div class="amg-list-searchbar">
                                ...
                                <input type="text" class="amg-list-searchbar__input announcement-list-search searchbox plain-search" id="announcement-list-search" placeholder="Search...">
                            </div>
                        </div>
                    ...
                    <div class="js-user-list-view-panel">
                        <div class="table-responsive">
                            <table id="mytable" class="table display app-data-table">
                                <thead>
                                    <tr>
                                        <th><h4>{{ trans("config.announcement.company_name") }}</h4></th>
                                        <th><h4>{{ trans("config.announcement.title") }}</h4></th>
                                        <th><h4>{{ trans("config.announcement.start_date") }}</h4></th>
                                        <th><h4>{{ trans("config.announcement.end_date") }}</h4></th>
                                        <th><h4>{{ trans("config.announcement.department") }}</h4></th>
                                        <th><h4>{{ trans("config.announcement.location") }}</h4></th>
                                        <th><h4>{{ trans("config.announcement.member_count") }}</h4></th>
                                        <th><h4>{{ trans("config.announcement.status") }}</h4></th>
                                        <th><h4>{{ trans('user.table_headers.updated_on') }}</h4></th>
    ...
    @include('announcements.modal_html')
    @include('announcements.modal_department_location')


    <div class="amg-modal modal fade" id="announcementFilterModal" tabindex="-1" aria-hidden="true" data-bs-backdrop="static" data-bs-keyboard="false">
        <div class="modal-dialog modal-xl modal-dialog-centered" style="max-width: 1200px;">
            <div class="modal-content rounded-5">
                <div class="modal-header d-flex align-items-center py-3 pt-4">
                    <h3 class="modal-title px-4">Annoucment Filter</h3>
                ...
                <div class="modal-body">
                    <div class="row g-3 px-4">
                        <div class="col-12 col-lg-4 my-2 mb-xl-0">
                            <label>{{ trans('config.announcement.company_name') }}</label>
                            <select class="form-control" id="filter_by_company" name="filter_by_company" multiple></select>
                        </div>
                        <!-- User -->
                        <div class="col-12 col-lg-4 my-2 mb-xl-0">
                            <label>{{ trans('config.announcement.filter_by_user') }}</label>
                            <select class="form-control" id="filter_by_user" name="filter_by_user" multiple></select>
                        </div>

                        <!-- Department -->
                        <div class="col-12 col-lg-4 my-2 mb-xl-0">
                            <label>{{ trans('config.filter_heading.filter_by_department') }}</label>
                            <select class="form-control" id="filter_by_department" name="filter_by_department" multiple></select>
                        </div>

                        <!-- Location -->
                        <div class="col-12 col-lg-4 my-2 mb-xl-0">
                            <label>{{ trans('config.filter_heading.filter_by_location') }}</label>
                            <select class="form-control" id="filter_by_location" name="filter_by_location" multiple></select>
                        </div>

                        <!-- Date Filter -->
                        <div class="col-12 col-lg-4 my-2 mb-xl-0">
                            <label>{{ trans("config.filter_heading.filter_by_date") }}</label>
                        ...
                        <!-- Date Range -->
                        <div class="col-12 col-lg-4 my-2 mb-xl-0">
                            <label>{{ trans("config.filter_heading.filter_by_daterange") }}</label>
                    ...
                    <div class="modal-footer justify-content-start py-4">
...
<script type="text/javascript" src="{!! CommonHelper::asset('js/announcements/index.js') !!}"></script>
<script type="text/javascript">
	var config = new Object;
	config.url = new Object;
	config.query_filters = new Object();
	config.query_filters.status = "{{ $request['status'] ?? ''}}";
    config.query_filters.department = "{{ $request['department'] ?? '' }}";
	config.query_filters.location = "{{ $request['location'] ?? '' }}";
	config.query_filters.createdFrom = "{{ $request['createdFrom'] ?? '' }}";
	config.query_filters.createdTo = "{{ $request['createdTo'] ?? '' }}";
	...
	config.translations = {
        add_announcement: '{{ trans('config.announcement.add_announcement') }}',
        save: '{{trans('button.save')}}',
		press_enter_with_Search: '{{trans('config.announcement.press_enter_with_Search')}}',
		Search: '{{trans('config.announcement.Search')}}',
		Refresh_List: '{{trans('config.announcement.Refresh_List')}}',
		Filter: '{{trans('config.announcement.Filter')}}',
		Add: '{{trans('config.announcement.Add')}}',
		Download: '{{trans('config.announcement.Download')}}',
		select_departments: '{{trans('config.announcement.select_departments')}}',
		select_location: '{{trans('config.announcement.select_location')}}',
		view_member: '{{trans('config.announcement.view_member')}}',
		edit_announcement: '{{trans('config.announcement.edit_announcement')}}',
		select_users: '{{trans('config.announcement.select_users')}}',
		edit: '{{trans('button.edit')}}',
		delete: '{{trans('button.delete')}}',
		save_changes: '{{trans('button.save_changes')}}',
		all_department: '{{ trans('config.announcement.all_department') }}',
		all_location: '{{ trans('config.announcement.all_location') }}',
  	};
	config.url.announcements = "{{ url('jx-announcements') }}";
	config.url.add = "{{ url('jx-announcement-add') }}";
	config.url.get = "{{ url('jx-announcement-get') }}";
	config.url.edit = "{{ url('jx-announcement-edit') }}";
	config.url.delete = "{{ url('jx-announcement-delete') }}";
	config.url.department = "{{ url('getDepartmentsWithCompanyByQuery') }}";
	config.url.location = "{{ url('getLocationByQuery') }}";
	config.url.update = "{{ url('jx-announcement-update') }}";
	config.url.users = "{{ url('getUserForDropDown') }}";
	config.url.changeStatus = "{{ url('announcements/changeStatus') }}";
	config.url.export = "{{ url('announcements/export') }}";
	config.url.getAllUsers = "{{ url('announcements/getAllUsers') }}";
	config.url.base_url = "{{ url('announcements/member-list') }}";
	config.permissions = {!! json_encode($permissionArray) !!};
	config.token = "{{ csrf_token() }}";
    new Announcement(config);
