...
<section class="content">
    <div class="header-actions-wrapper d-flex align-items-center justify-content-between pe-4">
        <div class="d-flex align-items-center gap-1">
            <button onclick="location.href='{{ url('announcements') }}'" class="d-flex gap-3 align-items-center bg-transparent outline-none border-0" data-bs-toggle="tooltip" title="Back" >
            ...
            <h3 class="h3-text mb-0"  > {{ trans("config.announcement.announcement_member_list") }} - {{ $announcement->title }}</h3>
        </div>
        

        <div class="d-flex gap-8">
             <!-- Add -->
            @if(Auth::user()->hasPermissionTo('AnnouncementMemberAdd'))
                <button class="amg-btn amg-btn-primary open-add-modal" type="button" data-bs-toggle="tooltip" title="{{ trans('config.announcement.add_announcement') }}">
                        ...
                        <span>{{ trans('config.announcement.add_announcement_member') }}</span>
                </button>
            @endif
                    ...
                    <div class="app-list-toolbar d-flex flex-wrap align-items-center justify-content-between gap-3">
                        ...
                        <div>
                            <div class="amg-list-searchbar">
                                ...
                                <input type="text" id="announcement-list-search" class="amg-list-searchbar__input announcement-list-search" placeholder="Search...">
                            </div>
                        </div>
                        <!-- Refresh -->
                    ...
                    <div class="js-user-list-view-panel">
                        <div class="table-responsive">
                            <table id="mytable" class="table display app-data-table">
                                <thead>
                                    <tr>
                                        <th><h4>{{ trans("config.announcement.member_name") }}</h4></th>
										<th><h4>{{ trans("config.announcement.member_email") }}</h4></th>
										<th><h4>{{ trans("config.announcement.updated_at") }}</h4></th>
    ...
    @include('announcements.modal_html_member')
</section>
</div>
@endsection
@push('scripts')
<script type="text/javascript" src="{!! CommonHelper::asset('js/announcements/member.js') !!}"></script>
...
<script type="text/javascript">
	...
	config.translations = {
		...
		edit_announcement_member: '{{trans('config.announcement.edit_announcement_member')}}',
		add_announcement_member: '{{trans('config.announcement.add_announcement_member')}}',
		edit: '{{trans('button.edit')}}',
		delete: '{{trans('button.delete')}}',
		save_changes: '{{trans('button.save_changes')}}',
  	};
	config.url.users = "{{ url('getUserByQuery') }}";
	config.url.ajaxMember = "{{ url('announcements/ajax-member-list') }}";
	config.url.editMember = "{{ url('announcements/ajax-member-edit') }}";
	config.url.addMember = "{{ url('announcements/ajax-member-add') }}";
	config.url.updateMember = "{{ url('announcements/ajax-member-update') }}";
	config.url.deleteMember = "{{ url('announcements/ajax-member-delete') }}";
	config.permissions = {!! json_encode($permissionArray) !!};
	config.announcementID = {!! json_encode($announcement) !!};
	config.company_defulte = {!! json_encode($company) !!};
	config.company_user_detail = {!! json_encode($userDatail) !!};
	config.token = "{{ csrf_token() }}";
  new AnnouncementMember(config);
</script>
@endpush
