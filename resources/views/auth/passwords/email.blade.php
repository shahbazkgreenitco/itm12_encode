        ...
        <div class="position-relative w-100" style="min-height:100vh;">
            <div class="row g-0" style="min-height:100vh;">
                ...
                <div class="col-md-6 col-12 col-lg-6 col-xl-7 col-xxl-6 left-section">
                    <div class="mt-4">
                        <svg width="180" height="43" viewBox="0 0 226 63" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path
                ...
                <div class="col-12 col-lg-6 col-xl-5 col-xxl-6 auth-right">
                    ...
                    @if(session()->has('msg'))
                        @php
                            $msgData = session('msg');
                            $type = is_array($msgData) ? ($msgData['status'] ?? 'danger') : 'success';
                    ...
                    <div class="login-card">
                        ...
                        <p class="sub">{{ trans('login.b_login.reg_mail') }}</p>
                        <form name="user" id="mail" action="{{ url('forgot-password') }}" method="post">
                            {{ csrf_field() }}
                            <div class="mb-2">
                                <input type="text" name="email" id="email" class="form-control auth-input " placeholder="Enter Your Usename/Email Address" value="{{ old('email') }}" />

                            </div>
                            @if (count($errors))
                                <label class="error">{{ $errors->first('email') }}</label>
                            @endif
                             <button type="button" name="action_type" value="sign_in" class="btn-continue mb-2" id="btnSubmit">
                                <span class="btn-spinner" id="continueSpinner"></span>
                                <span id="continueTxt">{{ trans('login.b_login.Send') }}</span>
                            </button>
                            <div class="know_credencials text-right">
                                <a href="{{ url('login') }}">{{ trans('login.b_login.know_cre') }}</a>
                            </div>
                        </form>
