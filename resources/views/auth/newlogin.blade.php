...
@section('style')
    ...
    <style>
        ...
        *::after {
        ...
        .social-login.icons-only .btn-text {
            display: none;
        }
...
@section('content')
{{-- @dd($msg) --}}
    <div id="main-wrapper" class="cls-content">
            ...
            <div class="row g-0" style="min-height:100vh;">
                ...
                <div class="col-12 col-lg-6 col-xl-5 col-xxl-6 auth-right">
                    ...
                    @if(session()->has('msg'))
                        @php
                            $msgData = session('msg');
                            $type = is_array($msgData) ? ($msgData['type'] ?? 'danger') : 'success';
                            $message = is_array($msgData) ? ($msgData['msg'] ?? '') : $msgData;
                        @endphp
                        <div id="autoCloseAlert" class="alert alert-{{ $type }} alert-dismissible fade show mb-3" role="alert">
                            {{ $message }}
                    ...
                    <div class="login-card">
                        ...
                        <h3>{{ trans('login.login.welcome') }}</h3>
                        <p class="sub">{{ trans("content.b_login.s_text") }}</p>
                        ...
                        <form id="loginFrm" action="{{ url('login') }}" method="POST">
                            ...
                            {{-- ── USERNAME ── --}}
                            <div class="mb-2">
                                <input type="text" id="username" name="username" class="form-control auth-input @error('email') is-invalid @enderror" placeholder="{{ trans('login.login.username-placeholder') }}" value="{{ old('email') }}" autocomplete="username" required  autofocus />
                                @error('email')
                            ...
                            {{-- ── PASSWORD (hidden until username validated) ── --}}
                            <div class="mb-2 pwd-wrap passwordDiv">
                                <input type="password" name="password" id="password" class="form-control auth-input @error('password') is-invalid @enderror" placeholder="Enter Password" autocomplete="current-password" />
                            ...
                            {{-- ── IMPERSONATION EMAIL (conditional) ── --}}
                            @if (config('app.impersonate') === 'yes')
                                <div class="mb-2 emailDiv">
                                    <input type="email" id="email" name="email" class="form-control auth-input" placeholder="{{ trans('login.login.email-placeholder') }}" autocomplete="off" />
                                </div>
                            @endif

                            {{-- ── REMEMBER / FORGOT (shown with password) ── --}}
                            <div class="d-flex justify-content-between align-items-center mb-3 mt-1 remember-row">

                                <div class="d-flex align-items-center">
                                    <span class="p-1 d-flex align-items-center justify-content-center" title="Back">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16" class="btnback d-none">
                                            <path d="M8 0a8 8 0 1 0 0 16A8 8 0 0 0 8 0m3.5 7.5a.5.5 0 0 1 0 1H5.707l2.147 2.146a.5.5 0 0 1-.708.708l-3-3a.5.5 0 0 1 0-.708l3-3a.5.5 0 1 1 .708.708L5.707 7.5z"/>
                                        </svg>
                                    </span>
                                    <a href="{{ url('/forgot-password') }}" class="forgot-link ms-2">
                                        {{ trans('login.b_login.for_pass') }}
                                    </a>
                                </div>
                                ...
                                <div class="d-flex align-items-center gap-2">
                                    ...
                                    <label class="remember-lbl cursor-pointer" for="remember_me">
                                        {{ trans('login.b_login.Remember_me') }}
                                    </label>
                                </div>
                            ...
                            <button type="button" class="btn-continue btnContinue mb-2">
                                <span class="btn-spinner" id="continueSpinner"></span>
                                <span id="continueTxt">{{ trans('login.login.continue') }}</span>
                            </button>

                            {{-- ── SIGN IN button (step 2, shown after validation) ── --}}
                            <button type="submit" id="btnSubmit" class="btn-signin mb-2" name="action_type" value="sign_in">
                                {{ trans('login.login.sign-in') }}
                            </button>

                            {{-- ── OTP button (impersonate only) ── --}}
                            @if (config('app.impersonate') === 'yes')
                                <button type="submit" id="btnSubmitOtp" class="btn-continue mb-2" name="action_type" value="otp_login" style="display:none;">
                        ...
                        @php
                            $providers = 0;
                            if(config('services.google.client_id') != '') $providers++;
                            if(config('services.azure.client_id') != '') $providers++;
                        @endphp

                        @if($providers > 0)

                            <div class="auth-divider">{{ trans('login.login.or-sign-width') }}</div>

                            <div class="d-flex gap-2 social-login {{ $providers > 1 ? 'icons-only' : '' }}">

                                @if (config('services.google.client_id') != '')
                                <a href="{{route('loginWithGoogle')}}" class="btn-social flex-fill d-flex align-items-center justify-content-center">
                                    ...
                                    <span class="btn-text ms-2">{{ trans('login.login.sign-with-google') }}</span>
                                </a>
                                @endif

                                @if (config('services.azure.client_id') != '')
                                <a href="{{ route('loginWithOffice') }}" class="btn-social flex-fill d-flex align-items-center justify-content-center">
                                    <div class="ms-grid">
                                        <span class="ms-r"></span><span class="ms-g"></span>
                                        <span class="ms-b"></span><span class="ms-y"></span>
                                    </div>
                                    <span class="btn-text ms-2">{{ trans('login.login.sign-with-microsoft') }}</span>
                                </a>
                                @endif
                        ...
                        <div class="card-links text-center mt-2">
                            ...
                            <span class="sep">|</span>
                            <a href="{{ route('public_articles') }}" target="_blank">{{ trans('tkt_knowledge_document.public_articles') }}</a>
                        </div>

                    </div>

                    <div class="auth-copyright mt-4 card-links">
                        &copy; {{ date('Y') }} <a href="https://greenitco.com/" target="_blank">{{ trans('login.login.copy_right_company') }} </a>| {{ trans('login.login.all_right') }}
                    </div>
...
@push('script')
    ...
    <script type="text/javascript">
                        ...
                        fetch("{{ url('company/logo') }}/" + companyId)
                            ...
                            .then(data => {
                                if (data.status === 'success') {
                                    document.getElementById('loginCompanyLogo').src = data.logo;
                                }
                            });
                    }
                } catch (e) {
        ...
        $(document).ready(function() {
            ...
            config.type_status = @json($type ?? null);
            new LoginAdd(config);
            setTimeout(function() {
                $('#autoCloseAlert').alert('close');
            }, 3000);
            
            let msgData = localStorage.getItem('pass_reset_msg');
            if (msgData) {
                msgData = JSON.parse(msgData);
                $('.auth-right').prepend(`
                    <div 
                        class="alert alert-${msgData.type} alert-dismissible fade show mb-3">
                        ${msgData.msg}
                        <button type="button"
                                class="btn-close"
                ...
                localStorage.removeItem('pass_reset_msg');
