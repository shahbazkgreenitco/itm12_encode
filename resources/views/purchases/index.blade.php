 ...
 <div class="header-actions-wrapper d-flex align-items-center justify-content-between pe-4">
            ...
            <div class="d-flex gap-8">
                <button class="header-icon-btn header-icon-btn-sm btn-open-filter" type="button" data-bs-toggle="tooltip"
                    ...
                    <span class="b6-text opacity-50">{{ trans('user.user_toolbar.filter') }}</span>
                    <span class="filter-count-badge d-none" aria-label="Active filters">0</span>
                </button>
                {{-- Add Purchase --}}
                <button class="header-icon-btn-only header-icon-btn-only-sm btn-download btn-purchase-export" type="button" data-bs-toggle="tooltip" data-bs-original-title="Download">
                    <svg viewBox="0 0 20 20">
                        <path d="M10 3V13M10 13L6 9M10 13L14 9M3 17H17" stroke="currentColor" />
                    </svg>
                </button>

                <button class="header-icon-btn-only header-icon-btn-only-sm btn-import btn-import-purchase" type="button" data-bs-toggle="tooltip" data-bs-original-title="Import">
                    <svg viewBox="0 0 20 20">
                        <path d="M10 17V7M10 7L6 11M10 7L14 11M3 3H17" stroke="currentColor" />
                    </svg>
                </button>
                @if(Auth::user()->hasPermissionTo('PurchaseAdd'))
                    <button class="amg-btn amg-btn-primary amg-btn-sm open-add-modal" type="button" data-bs-toggle="tooltip"
                        data-bs-original-title="{{ trans('config.purchase_fields.add_purchase') }}">
                        ...
                        <span>{{ trans('config.purchase_fields.add_purchase') }}</span>
            ...
            <div class="card-body table-responsive">
                ...
                <table id="mytable" class="table display">
                    ...
                    <thead>
                        <tr>
                            <th>#</th>
                            <th><h4 class="b2-text">{{ trans("config.purchase_fields.purchase_date") }}</h4></th>
                            <th><h4 class="b2-text">{{ trans("config.purchase_fields.invoice_number/po_number") }}</h4></th>
                            <th><h4 class="b2-text">{{ trans("config.purchase_fields.received_date") }}</h4></th>
                            <th><h4 class="b2-text">{{ trans("config.purchase_fields.location") }}</h4></th>
                            <th><h4 class="b2-text">{{ trans("config.purchase_fields.company") }}</h4></th>
                            <th><h4 class="b2-text">{{ trans("config.purchase_fields.supplier") }}</h4></th>
                            <th><h4 class="b2-text">{{ trans("config.purchase_fields.bill_amount") }}</h4></th>
                            <th><h4 class="b2-text">{{ trans("config.purchase_fields.status") }}</h4></th>
                            <th><h4 class="b2-text">{{ trans("config.purchase_fields.documents_count") }}</h4></th>
                            <th><h4 class="b2-text">{{ trans("config.purchase_fields.created_on") }}</h4></th>
                            <th><h4 class="b2-text">{{ trans("config.purchase_fields.updated_on") }}</h4></th>
                            <th><h4 class="b2-text">{{ trans("config.purchase_fields.actions") }}</h4></th>
                        </tr>
                    </thead>
                    <tbody></tbody>
                </table>
...
@push('scripts')
    ...
    <script>
        ...
        config.url = {
            purchase: "{{ url('purchases') }}",
            add: "{{ url('purchase/add') }}",
            edit: "{{ url('purchase/edit') }}",
            update: "{{ url('purchase/edit') }}",
            delete: "{{ url('purchase/delete') }}",
            list: "{{ url('jx-purchases') }}",
            download_url: "{{ url('purchase-download') }}",
            getLocationByAjax: "{{ url('getLocationByQuery') }}",
            getSupplierByAjax: "{{ url('getSupplierByQuery') }}",
            getCompanyByAjax: "{{ url('getCompanyByQuery') }}",
            custom_tax: "{{ url('get-customtax') }}",
            custom_element: "{{ url('get-element') }}",
            historyModalUrl: "{{ url('purchase-history') }}",
            purchase_import: "{{ url('purchase-import') }}",
            purchase_info: "{{ url('purchase/info') }}",
            purchase_info_tab: "{{ url('purchase/info_tab') }}",
        };
        ...
        config.translations = {
            ...
            history: '{{ trans("config.purchase_fields.history") }}',
