...
@section('title', trans('suppliers.view.header'))
...
<main class="main-content" id="mainContent">
  <div class="container-fluid px-0">
    <div class="header-actions-wrapper d-flex align-items-center justify-content-between px-4">
      <div class="mb-0">
        <h3 class="h3-text mb-0">{{ trans('suppliers.view.page_heading') }}</h3>
      </div>
      <div class="d-flex gap-8">
        <button class="header-icon-btn-only header-icon-btn-only-sm btn-download btn-suppliers-export"
          title="{{ trans('suppliers.view.download') }}">
        ...
        <button class="header-icon-btn-only header-icon-btn-only-sm btn-import btn-import-suppliers"
          title="{{ trans('suppliers.view.import') }}">
        ...
        <div class="column-toggle-wrapper" style="position: relative; display: inline-block;">
          <button class="header-icon-btn-only header-icon-btn-only-sm show-hide-columns"
            title="{{ trans('suppliers.view.show_columns') }}">
        ...
        <button class="amg-btn amg-btn-primary open-add-modal" type="button" data-bs-toggle="tooltip"
          title="{{ trans('suppliers.view.add_button') }}">
          ...
          <span>{{ trans('suppliers.view.add_button') }}</span>
        </button>
      </div>
    </div>
          ...
          <div class="d-flex align-items-center gap-2 mb-4" data-select2-id="select2-data-5-1om7">
            <div id="customLengthContainer" class="col-auto" data-select2-id="select2-data-4-sbib">
              <select id="showSelect"
                ...
                <option value="10" selected>{{ trans('suppliers.view.show') }} (10)</option>
                <option value="25">{{ trans('suppliers.view.show') }} (25)</option>
                <option value="50">{{ trans('suppliers.view.show') }} (50)</option>
                <option value="100">{{ trans('suppliers.view.show') }} (100)</option>
              </select>
            </div>
            ...
            <div>
              <div class="amg-list-searchbar">
                ...
                <input type="text" class="amg-list-searchbar__input" id="tableSearch"
                  placeholder="{{ trans('suppliers.view.search') }}...">
              </div>
            </div>
            <button class="amg-refresh-btn btn-reload-list">
              ...
              <span>{{ trans('suppliers.view.refresh') }}</span>
            </button>
          </div>
          <table id="mytable" class="table display supplier-table w-100">
            <thead>
              <tr>
                <th>
                  <h4>{{ trans('suppliers.table.id') }}</h4>
                </th>
                <th>
                  <h4>{{ trans('suppliers.table.supplier_name') }}</h4>
                </th>
                <th>
                  <h4>{{ trans('suppliers.table.contact_person') }}</h4>
                </th>
                <th>
                  <h4>{{ trans('suppliers.table.phone') }}</h4>
                </th>
                <th>
                  <h4>{{ trans('suppliers.table.email') }}</h4>
                </th>
                <th>
                  <h4>{{ trans('suppliers.table.address') }}</h4>
                </th>
                <th>
                  <h4>{{ trans('suppliers.table.city') }}</h4>
                </th>
                <th>
                  <h4>{{ trans('suppliers.table.state') }}</h4>
                </th>
                <th>
                  <h4>{{ trans('suppliers.table.country') }}</h4>
                </th>
                <th>
                  <h4>{{ trans('suppliers.table.actions') }}</h4>
                </th>
              </tr>
            </thead>
            <tbody></tbody>
...
<script type="text/javascript">
  ...
  config.url.list = "{{ url('ajax-suppliers-list') }}";
  config.url.add = "{{ url('supplier/add') }}";
  config.url.edit = "{{ url('supplier/edit') }}";
  config.url.getInfo = "{{ url('supplier/get-info') }}";
  config.url.info = "{{ url('supplier/info') }}";
  config.url.delete = "{{ url('suppliers/delete') }}";
  config.url.image_delete = "{{ url('supplier-image-delete') }}";
  config.url.download_url = "{{ url('export-suppliers') }}";
  config.url.country = "{{ url('getCountryByQuery') }}";
  config.url.state = "{{ url('fetchStateByAjax') }}";
  config.url.city = "{{ url('fetchCityByAjax') }}";
  config.url.getBusinessCategories = "{{ url('getBusinessCategories') }}";
  config.url.list_import = "{{ url('import-suppliers') }}";
  config.url.import_url = "{{ url('import-suppliers') }}"
  config.permissions = {!!json_encode($permissionArray) !!};
  config.token = "{{ csrf_token() }}";
  config.translations = {
    save: '{{ trans('suppliers.config.save ') }}',
    something_went_wrong: '{{ trans('suppliers.config.something_went_wrong ') }}',
    are_you_delete: '{{ trans('suppliers.config.are_you_delete ') }}',
    are_you_delete_attachment: '{{ trans('suppliers.config.are_you_delete_attachment ') }}',
    press_enter_with_Search: '{{ trans('suppliers.config.press_enter_with_Search ') }}',
    Search: '{{ trans('suppliers.config.Search ') }}',
    Download: '{{ trans('suppliers.config.Download ') }}',
    add_supplier: '{{ trans('suppliers.config.add_supplier ') }}',
    edit_supplier: '{{ trans('suppliers.config.edit_supplier ') }}',
    please_enter_valid_search: '{{ trans('suppliers.config.please_enter_valid_search ') }}',
    refresh_list: '{{ trans('suppliers.config.refresh_list ') }}',
    supplier_import: '{{ trans('suppliers.config.supplier_import ') }}',
    info: '{{ trans('suppliers.config.supplier_info ') }}',
    edit: '{{ trans('suppliers.config.edit ') }}',
    delete: '{{ trans('suppliers.config.delete ') }}',
    select_country: '{{ trans('suppliers.config.select_country ') }}',
    select_state: '{{ trans('suppliers.config.select_state ') }}',
    select_city: '{{ trans('suppliers.config.select_city ') }}',
  };
</script>

<script src="{!! CommonHelper::asset('js/common.js') !!}"></script>
<script src="{!! CommonHelper::asset('js/support_validate.js') !!}"></script>
<script src="{!! CommonHelper::asset('js/suppliers/index.js') !!}"></script>
@endpush