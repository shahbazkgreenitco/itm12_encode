...
@section('title', trans('internal_place_import.view.header'))
    ...
    <div
        ...
        <div class="py-3">
                <div class="container-fluid ps-0">
                    <button onclick="location.href='{{ url('internal-places') }}'"
                    ...
                    <h2 class="h2-text mb-0">{{ trans('internal_place_import.view.page_heading') }}</h2>
                </button>
            </div>
        </div>
    </div>
            ...
            <ul class="nav nav-underline" id="myTab" role="tablist">
                <li class="nav-item">
                    <a class="nav-link active" id="info-tab-link" data-bs-toggle="tab" href="#info-tab" role="tab">
                        <span>{{ trans('internal_place_import.view.bulk_place_import') }}</span>
                    </a>
                </li>
                <li class="nav-item">
                    <a class="nav-link" id="import-info-tab-link" data-bs-toggle="tab" href="#import-info-tab" role="tab">
                        <span>{{ trans('internal_place_import.view.bulk_place_import_info') }}</span>
                    </a>
                </li>
            </ul>
        </div>
                    ...
                    {{-- Success count --}}
                        @if (session('success', 0) > 0)
                            <div class="alert customize-alert alert-dismissible alert-light-success bg-success-subtle text-success fade show remove-close-icon"
                                ...
                                <div class="d-flex align-items-center me-3 gap-3 me-md-0">
                                        ...
                                        {{ session('success', 0) }}
                                        {{ trans('internal_place_import.view.internal_place_imported_successfully') }}
                                </div>
                            </div>
                        @endif
                        ...
                        @if (session('fail', 0) > 0)
                            <div class="alert customize-alert alert-dismissible alert-light-danger bg-danger-subtle text-danger fade show remove-close-icon"
                                ...
                                <div>
                                    {{ session('fail', 0) }} {{ trans('internal_place_import.view.internal_place_failed_import') }}
                    ...
                    @if(session('success', 0) == 0 && session('fail', 0) == 0)
                        <div class="alert customize-alert alert-dismissible alert-light-warning bg-warning-subtle text-warning fade show remove-close-icon" role="alert">
                            ...
                            <div class="d-flex align-items-center me-3 gap-3 me-md-0">
                                ...
                                {{ trans('internal_place_import.view.no_internal_place_imported') }}
                            </div>
                        </div>
                    @endif
                    ...
                    <div class="row g-3 align-items-stretch">
                        <div class="col-12 col-sm-5">
                            <div class="drop-zone" id="dropZone">
                                ...
                                <div id="defaultState">
                                    ...
                                    <p class="drag-text mt-3 mb-2 b1-text">{{ trans('internal_place_import.view.drag') }} &amp; {{ trans('internal_place_import.view.drop_here') }}</p>
                                    <button class="amg-btn amg-btn-secondary" style="min-width:203px;">
                                        ...
                                        <span>{{ trans('internal_place_import.view.upload_file') }}</span>
                                    </button>
                                    <span class="file-label b1-text">{{ trans('internal_place_import.view.files_selected') }}</span>
                                </div>
                                ...
                                <div id="selectedState" style="display:none;">
                                    ...
                                    <button class="amg-btn amg-btn-link amg-btn-link-plain mt-2" id="btnRemove">{{ trans('internal_place_import.view.remove_file') }}</button>
                                </div>
                            </div>
                        ...
                        <div class="col-12 col-sm-7">
                            <div class="right-panel h-100">
                                <a class="amg-btn amg-btn-white amg-btn-lg" href="{!! CommonHelper::asset('samples/PlaceImportFormat.xlsx') !!}" target="_blank">
                                    ...
                                    <span class="font-weight-normal">{{ trans('internal_place_import.view.download_template_button') }}</span>
                                </a>
                            </div>
                        </div>
                    </div>
                ...
                <div class="row justify-content-center">
                        ...
                        <div class="col-12 px-3">
                            ...
                            <div class="instructions-card mt-1">
                                ...
                                {{ trans('internal_place_import.view.read_notes') }}
                            </h4>
                            ...
                            <div class="ps-3 ps-sm-4">
                                ...
                                <div class="card-scroll-body" id="cardScrollBody">
                                    <ul class="ps-3">
                                        <li>
                                            <span class="b3-text me-3">{{ trans('internal_place_import.view.place') }}
                                                <span class="mandatory"></span></span>
                                            <span
                                                class="b5-text text-muted">{{ trans('internal_place_import.view.place_text') }}</span>
                                        </li>
    
                                        <li>
                                            <span
                                                class="b3-text me-3">{{ trans('internal_place_import.view.location') }}
                                                <span class="mandatory"></span></span>
                                            <span
                                                class="b5-text text-muted">{{ trans('internal_place_import.view.location_text') }}</span>
                                        </li>
    
                                        <li>
                                            <span
                                                class="b3-text me-3">{{ trans('internal_place_import.view.branch_code') }}
                                                <span class="mandatory"></span></span>
                                            <span
                                                class="b5-text text-muted">{{ trans('internal_place_import.view.branch_code_msg') }}</span>
                                        </li>

                                        <li>
                                            <span
                                                class="b3-text me-3">{{ trans('internal_place_import.view.company') }}
                                                <span class="mandatory"></span></span>
                                            <span
                                                class="b5-text text-muted">{{ trans('internal_place_import.view.company_text') }}</span>
                                        </li>
                                    </ul>
                                </div>
                            </div>
                                ...
                                <h4 class="b2-text" style="color:#7F7F7F">
                                {{ trans('internal_place_import.view.change_first_row') }}
                            </h4>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="d-flex align-items-center gap-2 mb-1" data-select2-id="select2-data-5-1om7">
                            ...
                            <div id="customLengthContainer" class="col-auto" data-select2-id="select2-data-4-sbib">
                                <select id="showSelect"
                                    ...
                                    <option value="10" selected>{{ trans('internal_place_import.view.show') }}
                                        (10)</option>
                                    <option value="25">{{ trans('internal_place_import.view.show') }} (25)
                                    </option>
                                    <option value="50">{{ trans('internal_place_import.view.show') }} (50)
                                    </option>
                                    <option value="100">{{ trans('internal_place_import.view.show') }} (100)
                                    </option>
                                </select>
                            </div>
                            ...
                            <div>
                                <div class="amg-list-searchbar">
                                    ...
                                    <input type="text" class="amg-list-searchbar__input" id="tableSearch" placeholder="{{ trans('internal_place_import.view.search') }}...">
                                </div>
                            </div>
                            ...
                            <button class="amg-refresh-btn btn-reload-list">
                                ...
                                <span>{{ trans('internal_place_import.view.refresh') }}</span>
                            </button>
                        </div>
                        <table id="tblHistory" class="table display">
                            <thead>
                                <tr>
                                    <th><h4>{{ trans("internal_place_import.table.id") }}</h4></th>
                                    <th><h4>{{ trans("internal_place_import.table.document_name") }}</h4></th>
                                    <th><h4>{{ trans("internal_place_import.table.total_upload") }}</h4></th>
                                    <th><h4>{{ trans("internal_place_import.table.total_success") }}</h4></th>
                                    <th><h4>{{ trans("internal_place_import.table.total_failure") }}</h4></th>
                                    <th><h4>{{ trans("internal_place_import.table.updated_at") }}</h4></th>
                                    <th><h4>{{ trans("internal_place_import.table.username") }}</h4></th>
                                    <th><h4>{{ trans("internal_place_import.table.actions") }}</h4></th>
                                </tr>
                            </thead>
                            <tbody></tbody>
                        </table>
...
@push('scripts')
    ...
    <script src="{!! CommonHelper::asset('js/place/place_import.js') !!}"></script>

   <script>
        var config = {};
        config.bulk_import_info = "{{ url('internal-places/bulk-import-info')}}";
        config.document_download = "{{ url('uploads/bulk_documents') }}";
        config.token = "{{ csrf_token() }}";
        config.datatable_translations=  @json(trans('datatable.datatable'));
        config.translations = {
            Download_Document: '{{ trans('internal_place_import.config.Download_Documents') }}',
            search: '{{ trans('internal_place_import.config.search') }}',
            reload: '{{ trans('internal_place_import.config.reload') }}',
            please_enter_valid_search: '{{ trans('internal_place_import.config.please_enter_valid_search') }}',
            import_file: "{{ trans('internal_place_import.config.import_file') }}",
            Please_upload_valid_xlsx: "{{ trans('internal_place_import.config.upload_valid') }}",
        };
...
<script type="text/javascript" src="{!! CommonHelper::asset('js/place/place_import.js') !!}"></script>
