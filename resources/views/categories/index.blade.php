...
@section('title', trans("config.categories_fields.categories"))
@section('content')
    <section class="content">
        <div id="main-user-list-wrapper">
            <div class="header-actions-wrapper d-flex align-items-center justify-content-between pe-4">
                <h3 class="h3-text mb-0">{{ trans("config.categories_fields.categories") }}</h3>
                <div class="d-flex gap-8">
                    <button class="header-icon-btn-only header-icon-btn-only-sm btn-download" type="button" data-bs-toggle="tooltip" data-bs-original-title="{{ trans('config.categories_fields.download') }}">
                        <svg viewBox="0 0 20 20">
                            <path d="M10 3V13M10 13L6 9M10 13L14 9M3 17H17" stroke="currentColor" />
                        </svg>
                    </button>
                    {{-- Add --}}
                    @if(Auth::user()->hasPermissionTo('CategoryAdd'))
                        <button class="amg-btn amg-btn-primary amg-btn-sm open-add-modal" type="button" data-bs-toggle="tooltip"
                            title="{{ trans('config.categories_fields.add_category') }}">
                            ...
                            <span>{{ trans('config.categories_fields.add_category') }}</span>
                        </button>
                    @endif
                </div>
            </div>
                            ...
                            <div class="d-flex align-items-center gap-2 mb-1">
                                <div class="col-auto">
                                    <select class="amg-table-pagination-dropdown userModulePageLenth user-list-page-length">
                                        <option value="10" selected>{{ trans('config.categories_fields.show') }} (10)</option>
                                        <option value="25">{{ trans('config.categories_fields.show') }} (25)</option>
                                        <option value="50">{{ trans('config.categories_fields.show') }} (50)</option>
                                        <option value="100">{{ trans('config.categories_fields.show') }} (100)</option>
                                    </select>
                                </div>
                                ...
                                <div>
                                    <div class="amg-list-searchbar">
                                        ...
                                        <input type="text" class="amg-list-searchbar__input user-list-search"
                                            id="user-list-search" placeholder="{{ trans('config.categories_fields.search_placeholder') }}">
                                    </div>
                                </div>
                                ...
                                <button class="amg-refresh-btn btn-reload-list">
                                    ...
                                    <span>{{ trans('config.categories_fields.refresh') }}</span>
                                </button>
                            </div>

                            <div class="js-user-list-view-panel">
                                <div class="table-responsive">
                                    <table id="mytable" class="table display app-data-table amg-datatable">
                                        <thead>
                                            <tr>

                                                <th>
                                                    <h4>{{ trans("config.categories_fields.id") }}</h4>
                                                </th>
                                                <th>
                                                    <h4>{{ trans("config.categories_fields.category_name") }}</h4>
                                                </th>
                                                <th>
                                                    <h4>{{ trans("config.categories_fields.asset_department") }}</h4>
                                                </th>
                                                <th>
                                                    <h4>{{ trans("config.categories_fields.type") }}</h4>
                                                </th>
                                                <th>
                                                    <h4>{{ trans("config.categories_fields.account_type") }}</h4>
                                                </th>
                                                <th>
                                                    <h4>{{ trans("config.categories_fields.threshold") }}</h4>
                                                </th>
                                                <th>
                                                    <h4>{{ trans("config.categories_fields.acceptance") }}</h4>
                                                </th>
                                                <th>
                                                    <h4>{{ trans("config.categories_fields.eula") }}</h4>
                                                </th>
                                                <th>
                                                    <h4>{{ trans("config.categories_fields.checkin_mail") }}</h4>
                                                </th>
                                                <th>
                                                    <h4>{{ trans("config.categories_fields.checkout_mail") }}</h4>
                                                </th>
                                                <th>
                                                    <h4>{{ trans("config.categories_fields.updated_at") }}</h4>
                                                </th>
                                                <th>
                                                    <h4>{{ trans("config.categories_fields.image") }}</h4>
                                                </th>
                                                <th>
                                                    <h4>{{ trans("config.categories_fields.actions_cat") }}</h4>
                                                </th>
                                            </tr>
                                        </thead>
                                        <tbody>
        ...
        @include("categories.modal_html")
    </section>
@endsection
@push('scripts')
    <script src="{!! CommonHelper::asset('js/categories/list.js') !!}"></script>
    <script src="{!! CommonHelper::asset('js/common.js') !!}"></script>
    <script>
        var config = {};
        config.url = {
            download_url: "{{ url('export-category') }}",
            list: "{{ url('category/ajax-list') }}",
            add: "{{ url('category/ajax-add') }}",
            edit: "{{ url('category/ajax-edit') }}",
            get: "{{ url('category/ajax-get') }}",
            delete: "{{ url('delete-category') }}",
            info: "{{ url('category/history') }}",
            getCustomFieldsetByModule: "{{ url('getCustomFieldsetByModule') }}",
            departments: "{{ route('getAssetDepartments') }}",
            history: "{{ url('category/history') }}"
        };
        config.imgviewpath = "{{ url('uploads/category') }}";
        config.account_types = {!! json_encode($vd['account_types']) !!};
        config.permissions = {!! json_encode($permissionArray) !!};
        config.token = "{{ csrf_token() }}";
        config.datatable_translations=  @json(trans('datatable.datatable'));
        config.translations = {
            are_you_delete: '{{ trans('config.categories_fields.are_you_delete') }}',
            something_went_wrong: '{{ trans('config.categories_fields.something_went_wrong') }}',
            add_category_details: '{{ trans('config.categories_fields.add_category_details') }}',
            edit_category_details: '{{ trans('config.categories_fields.edit_category_details') }}',
            save_category: '{{ trans('config.categories_fields.save_category') }}',
            press_enter_with_Search: '{{ trans('config.categories_fields.search') }}',
            Search: '{{ trans('config.categories_fields.search') }}',
            Download: '{{ trans('config.categories_fields.download') }}',
            add_category: '{{ trans('config.categories_fields.add_category') }}',
            history: '{{ trans('config.categories_fields.history') }}',
            select_the_custom_fieldset: '{{ trans('config.categories_fields.select_the_custom_fieldset') }}',
            edit: '{{ trans('config.categories_fields.edit') }}',
            delete: '{{ trans('config.categories_fields.delete') }}',
            Select_the_Department: '{{ trans('config.categories_fields.select_the_department') }}',
            Refresh_List: '{{ trans('config.categories_fields.refresh') }}',
            actions: '{{ trans('config.categories_fields.actions_cat') }}',
            eula: '{{ trans('config.categories_fields.eula') }}',
            select_account_type: '{{ trans('config.categories_fields.select_the_account_type') }}',
            select_type: '{{ trans('config.categories_fields.select_type') }}',
            select_the_custom_fieldset: '{{ trans('config.categories_fields.select_the_custom_fieldset') }}',
            asset_department_placeholder: '{{ trans('config.categories_fields.asset_department_placeholder') }}',
        };
    </script>
@endpush