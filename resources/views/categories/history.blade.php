...
@section('title', trans("config.categories_fields.categories_history"))
@section('content')

<main class="main-content" id="mainContent">
    <div class="header-actions-wrapper d-flex align-items-center justify-content-between px-4">
        <button onclick="location.href='{{ url('categories') }}'"
           ...
           <h3 class="h3-text mb-0">{{ trans('config.categories_fields.categories_history') }} - {{ $name }}</h3>
        </button>
        ...
        <div class="card">
            <div class="card-body table-responsive">
                    ...
                    <div>
                        <div class="amg-list-searchbar">
                            ...
                            <input type="text" class="amg-list-searchbar__input user-list-search" id="tableSearch"
                                placeholder="{{ trans('config.categories_fields.search_placeholder') }}">
                        </div>
                    </div>
                ...
                <table id="mytable" class="table display">
                    <thead>
                        <tr>
                            <th><h4>{{ trans("config.categories_fields.id") }}</h4></th>
                            <th><h4>{{ trans("config.categories_fields.category_name") }}</h4></th>
                            <th><h4>{{ trans("config.categories_fields.type") }}</h4></th>
                            <th><h4>{{ trans("config.categories_fields.account_type") }}</h4></th>
                            <th><h4>{{ trans("config.categories_fields.acceptance") }}</h4></th>
                            <th><h4>{{ trans("config.categories_fields.eula") }}</h4></th>
                            <th><h4>{{ trans("config.categories_fields.checkin_mail") }}</h4></th>
                            <th><h4>{{ trans("config.categories_fields.checkout_mail") }}</h4></th>
                            <th><h4>{{ trans("config.categories_fields.threshold") }}</h4></th>
                            <th><h4>{{ trans("config.categories_fields.updated_by") }}</h4></th>
                            <th><h4>{{ trans("config.categories_fields.updated_at") }}</h4></th>
                        </tr>
                    </thead>
                    <tbody></tbody>
                </table>
...
<script>
    var config = {};
    config.url = { info: "{{ url('jx_categories_info') }}"};
...
<script src="{!! CommonHelper::asset('js/categories/history.js') !!}"></script>
@endpush