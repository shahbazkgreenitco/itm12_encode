        ...
        <div class="header-actions-wrapper d-flex align-items-center justify-content-between px-4">
            <div class="ps-12">
                <h3 class="h3-text mb-0">
                    <a href="{{ url('custom-fields') }}" class="text-reset text-decoration-none"> 
                    ...
                    </a>Fieldset - {{ $fldSet->name }}
                </h3>
            </div>
            <div class="d-flex gap-8">                
                <!-- Add -->         
                @if(count($addableFields))      
                    <button class="amg-btn amg-btn-primary open-add-modal" type="button" data-bs-toggle="tooltip" title="Add Fields">
                        ...
                        <div class="app-list-toolbar d-flex flex-wrap align-items-center justify-content-between gap-3">
                            ...
                            <div>
                                <div class="amg-list-searchbar">
                                    ...
                                    <input type="text" class="amg-list-searchbar__input fieldset-details-list-search" id="fieldset-details-list-search" placeholder="Search...">
                                </div>
                            </div>
                            <!-- Refresh -->
                                ...
                                <table id="mytable" class="table display app-data-table">
                                    <thead>
                                        <tr>
                                            ...
                                            <th><h4>Format</h4></th>
                                            <th><h4>Form Element</h4></th>
                                            <th><h4>Required</h4></th>                                            
...
@include('config.custom-fields.field-add-modal')
@endsection
@push('css')
...
@push('scripts')
   <script src="https://cdn.datatables.net/rowreorder/1.4.1/js/dataTables.rowReorder.min.js"></script>
    <script src="{{ asset('js/config/custom-fields/custom-fieldset-dtl.js') }}"></script>
    <script>
        $(function() {
            var config = {};
            config.url = {};
            config.url.list = "{{ url('custom-fieldset/'.$fldSet->id) }}";
            config.url.add = "{{ url('custom-fieldset/'.$fldSet->id) }}";
            config.url.delete = "{{ url('deleteFieldsetFields') }}";
            config.url.fetchReference = "{{ url('fetch-custom-field-reference-field') }}";
            config.url.getFields = "{{ url('get-fields-by-type') }}";
            config.fieldset_id = "{{ $fldSet->id }}";
            config.url.reorder = "{{ url('custom-fields-record/reorder') }}";
            config.token = "{{ csrf_token() }}";
            config.translations = {
                are_you_delete: "Are you sure you want to delete this?",
                deleteSuccess: "{{ trans('config.custom_fields.something_went_wrong') }}",
                error: "Something went wrong"
            };

            new CustomFieldsetDetail(config);
        });
    </script>
@endpush