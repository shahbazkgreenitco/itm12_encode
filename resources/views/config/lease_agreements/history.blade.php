...
Module: Contract Agreement (History)
CAT/26/04
------------------------------------------------------------
Version: 1.1.0
Author: Safdar Ali
...
Description:
Contract Agreement history listing page with audit records.

Features:
- View contract change history
- Track updated data (old records)
...
[1.1.0] - UI improvements & history data fixes
[1.0.2] - localize heading and sorting icon for cols (manjeet) - 2026-07-26 
------------------------------------------------------------
*/
--}}
...
<section class="content">
    <div id="main-user-list-wrapper">
        <div class="header-actions-wrapper d-flex align-items-center justify-content-between pe-4">
            <button onclick="location.href='{{ url('lease-agreements') }}'"
                ...
                <h3 class="h3-text mb-0">{{ trans('contract_agreement.contract_agreement_fields.agreement_history') }}</h3>
            </button>
        </div>
                        ...
                        <div class="js-user-list-view-panel">
                            <div class="table-responsive">
                                <table id="mytable" class="table display app-data-table amg-datatable">
                                    <thead>
                                        <th><h4>{{ trans("contract_agreement.contract_agreement_fields.contractor_name") }}</h4>
                                        <th><h4>{{ trans("contract_agreement.contract_agreement_fields.contractor_number") }}</h4>
                                        <th><h4>{{ trans("contract_agreement.contract_agreement_fields.contract_type") }}</h4>
                                        <th><h4>{{ trans("contract_agreement.contract_agreement_fields.repair_maintenance") }}</h4>
                                        <th><h4>{{ trans("contract_agreement.contract_agreement_fields.description") }}</h4>
                                        <th><h4>{{ trans("contract_agreement.contract_agreement_fields.from_date") }}</h4>
                                        <th><h4>{{ trans("contract_agreement.contract_agreement_fields.to_date") }}</h4>
                                        <th><h4>{{ trans("contract_agreement.contract_agreement_fields.status") }}</h4>
                                        <th><h4>{{ trans("contract_agreement.contract_agreement_fields.updated_by") }}</h4>
                                        <th><h4>{{ trans("contract_agreement.contract_agreement_fields.updated_at") }}</h4>
                                        <th><h4>{{ trans("contract_agreement.contract_agreement_fields.actions") }}</h4>
                                    </thead>
                                    <tbody>
                                    </tbody>
                                </table>
...
<script type="text/javascript" src="{!! CommonHelper::asset('js/config/lease_agreements/history.js') !!}"></script>
<script type="text/javascript">
    var config = new Object;
	config.url = new Object;
	config.url.lease = "{{ url('leases-info/{id}') }}";
    config.url.info = "{{ url('jx_leases_info')}}";
	config.url.download= "{{ url('lease/download') }}";
	config.token = "{{ csrf_token() }}";
    config.info_id = "{{ $info_id }}";
