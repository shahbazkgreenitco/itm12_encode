...
@section('title', trans("content.ticket_procurement.PAB_Members_History"))
@section('content')
<div id="srat-members-history-wrapper">
    <div class="header-actions-wrapper d-flex align-items-center justify-content-between pe-4 mb-3">
        <button type="button" class="bg-transparent border-0 d-flex align-items-center gap-2"
            ...
            <h3 class="h3-text mb-0">{{ trans("content.ticket_procurement.PAB_Members_History") }} - {{ $pab_name }}</h3>
        </button>
    </div>
            ...
            <div class="card rounded-0">
                <div class="card-body">
                    ...
                    <div class="table-responsive">
                        <table id="mytable" class="table align-middle dark_thead">
                            <thead>
                                <tr>
                                    <th><h4>{{ trans("content.ticket_procurement.Member_Name") }}<h4></th>
                                    <th><h4>{{ trans("content.ticket_procurement.PAB_Name") }}<h4></th>
                                    <th><h4>{{ trans("content.ticket_procurement.Approval_Level") }}<h4></th>
                                    <th><h4>{{ trans("content.ticket_procurement.Approval_Mode") }}<h4></th>
                                    <th><h4>{{ trans("content.ticket_procurement.remark") }}<h4></th>
                                    <th><h4>{{ trans("content.ticket_procurement.created_at") }}<h4></th>
                                    <th><h4>{{ trans("content.ticket_procurement.updated_at") }}<h4></th>
                                    <th><h4>{{ trans("content.ticket_procurement.created_by") }}<h4></th>
                                    <th><h4>{{ trans("content.ticket_procurement.updated_by") }}<h4></th>
                                    <th><h4>{{ trans("content.ticket_procurement.deleted_by") }}<h4></th>
                                </tr>
                            </thead>
                            <tbody></tbody>
                        </table>
...
@endsection
@push('scripts')
<script src="{!! CommonHelper::asset('js/tickets/srat/memberHistory.js') !!}"></script>
<script>
    @if(Session::has('msg'))
        toastr.options = {
            closeButton: true,
...
$(function () {
    ...
    config.placeholder = {};
    config.url = {
        historyAjax: "{{ url('tickets/srat/srat-member-history-ajax') }}",
    };
