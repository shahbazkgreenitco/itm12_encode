...
@section('title', trans('content.ticket_procurement.ticket_procurement_history_breadcrumb'))
@section('content')

    <div id="srat-history-wrapper">
        <!-- Header -->
        ...
        <button type="button" class="bg-transparent border-0 d-flex align-items-center gap-2"
            ...
            <h3 class="h3-text mb-0"> {{ trans('content.ticket_procurement.ticket_procurement_history_breadcrumb') }}</h3>
        </button>
                ...
                <div class="card border-0 shadow-sm rounded-3">
                    <div class="card-body">
                        <div class="d-flex align-items-center gap-2 mb-1">
                            ...
                            <button class="amg-refresh-btn btn-reload-list" title="{{ trans('content.ticket_incident.Refresh_List') }}" data-toggle="tooltip" data-original-title="Add New Group">
                        ...
                        <div class="table-responsive">
                            <table id="mytable" class="table table-striped align-middle mb-0">
                                <thead class="table-dark">
                                    <tr>
                                        <th class="col1">
                                            <h4>{{ trans('content.ticket_procurement.PAB_Name') }}</h4>
                                        </th>
                                        <th class="col1">
                                            <h4>{{ trans('content.ticket_procurement.company') }}</h4>
                                        </th>
                                        <th class="col2">
                                            <h4>{{ trans('content.ticket_procurement.Approval_Mode') }}</h4>
                                        </th>
                                        <th>
                                            <h4>{{ trans('content.ticket_procurement.REQUIRED_APPROVAL_LEVEL') }}</h4>
                                        </th>
                                        <th>
                                            <h4>{{ trans('content.ticket_procurement.Description') }}</h4>
                                        </th>
                                        <th class="col3">
...
table.dataTable.table-striped>tbody>tr:nth-of-type(odd)>* {
    box-shadow: var(--dark-primary)!important;
}
</style>
@endpush
@push('scripts')
    <script src="{!! CommonHelper::asset('js/tickets/srat/history.js') !!}"></script>
    <script>
        @if(Session::has('msg'))
            toastr.options = {
                closeButton: true,
        ...
        $(document).ready(function() {
            ...
            config.placeholder = {};
            config.url = {
                historyList: "{{ url('tickets/srat/list') }}",
                historyAjax: "{{ url('tickets/srat/srat-history-ajax') }}"
            };
            config.pab_id = {!! json_encode($pab_id) !!};
            config.translations = {
