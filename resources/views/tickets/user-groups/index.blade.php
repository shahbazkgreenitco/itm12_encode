...
@section('title', trans('content.user_group.title'))
@section('content')
    <div id="cc-email-group-list-wrapper">
        <div class="header-actions-wrapper d-flex align-items-center justify-content-between pe-4">
            <h3 class="h3-text mb-0">{{trans('content.user_group.title')}}</h3>
            <div class="d-flex gap-8">
                ...
                <button class="amg-btn amg-btn-primary amg-btn-sm btn-add-group" type="button" data-bs-toggle="tooltip"
                    title="{{ trans('content.user_group.add_new_group') }}">
                    ...
                    <span>{{ trans('content.user_group.add_new_group') }}</span>
                </button>
            </div>
                ...
                <div class="card rounded-0">
                    <div class="card-body">
                        <div class="d-flex align-items-center gap-2 mb-1">
                            <div class="col-auto">
                                <select class="amg-table-pagination-dropdown amgTablePageLenth cc-email-group-list-page-length">
                                    <option value="10" selected>{{ __('content.user_group.show') }} (10)</option>
                                    <option value="25">{{ __('content.user_group.show') }} (25)</option>
                                    <option value="50">{{ __('content.user_group.show') }} (50)</option>
                                    <option value="100">{{ __('content.user_group.show') }} (100)</option>
                                </select>
                            </div>
                            ...
                            <div>
                                <div class="amg-list-searchbar">
                                    ...
                                    <input type="text" class="amg-list-searchbar__input user-list-search"
                                        placeholder="{{ __('content.user_group.press_enter_with_Search') }}">
                                </div>
                            </div>
                        ...
                        <div class="">
                            <div class="table-responsive">
                                <table id="cc-email-table" class="amg-datatable table display app-data-table">
                                    <thead>
                                        <tr>
                                            <th><h4 class="b2-text">{{ trans('content.user_group.name') }}</h4></th>
                                            <th><h4 class="b2-text">{{ trans('content.user_group.company') }}</h4></th>
                                            <th><h4 class="b2-text">{{ trans('content.user_group.department') }}</h4></th>
                                            <th><h4 class="b2-text">{{ trans('content.user_group.problem_category') }}</h4></th>
                                            <th><h4 class="b2-text">{{ trans('content.user_group.sub_category') }}</h4></th>
                                            <th><h4 class="b2-text">{{ trans('content.user_group.created_at') }}</h4></th>
                                            <th><h4 class="b2-text">{{ trans('content.user_group.member_count') }}</h4></th>
                                            <th><h4 class="b2-text">{{ trans('content.user_group.actions') }}</h4></th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                    </tbody>
        ...
        @include('tickets.user-groups.add-group')  
        @include('tickets.user-groups.cc-email-filter')        
    </div>
@endsection
...
@push('css')
    ...
    <style>
       #cc-email-mdl-frm .amg-form-field label.error {
...
@push('scripts')
    ...
    <script src="{!! CommonHelper::asset('js/tickets/user-groups/index.js') !!}"></script>
    <script type="text/javascript" src="{!! CommonHelper::asset('nfy/plugins/daterangepicker/daterangepicker.min.js') !!}"></script>
    <script type="text/javascript">
        $(document).ready(function() {
            var config = {};
            config.url = {};
            config.url.user_group_ajax = "{{ url('tickets/jx-user-groups-list') }}";
            config.url.delete = "{{ url('tickets/user-groups-delete') }}";
            config.url.add = "{{ url('tickets/user-groups-add') }}";
            config.url.edit = "{{ url('tickets/user-groups-edit') }}";
            config.url.update = "{{ url('tickets/user-groups-update') }}";
            config.url.attachmentUrl = "{{ url('uploads/ticket_status') }}";
            config.url.department = "{{ url('getDepartmentsWithCompanyByQuery') }}";
            config.url.group_member = "{{ url('tickets/user-groups-member') }}";
            ...
            config.translations = {
                add_new_group: '{{ trans('content.user_group.add_new_group') }}',
                add_cc_email_group: '{{ trans('content.user_group.add_cc_email_group') }}',
                manage_users: '{{ trans('content.user_group.manage_users') }}',
                edit_cc_email_group: '{{ trans('content.user_group.edit_cc_email_group') }}',
                edit_group: '{{ trans('content.user_group.edit_group') }}',
                Delete: '{{ trans('content.user_group.Delete') }}',
                delete_record: '{{ trans('content.user_group.delete_record') }}',
                record_deleted: '{{ trans('content.user_group.record_deleted') }}',
                Search: '{{ trans('content.user_group.Search') }}',
                Refresh_List: '{{ trans('content.user_group.Refresh_List') }}',
                press_enter_with_Search: '{{ trans('content.user_group.press_enter_with_Search') }}',
                select_department: '{{ trans('content.user_group.select_department') }}',
                No_Filter: '{{ trans('content.user_group.no_filter') }}',
                Filter: '{{ trans('content.user_group.filter') }}',
                filter_by_company: '{{ trans('content.filter_heading.filter_by_company') }}',
                filter_based_on: '{{ trans('content.filter_heading.filter_by_date') }}',
                ...
                select_company: '{{ trans('content.user_group.select_company') }}',
                add: '{{ trans('button.add') }}',
                update: '{{ trans('button.update') }}',
                Select_Problem_Category: '{{ trans('content.user_group.select_problem_category') }}',
                Select_Sub_Category: '{{ trans('content.user_group.select_sub_category') }}',

            };
            config.token = "{{ csrf_token() }}";
            new UserGroup(config);
        });
    </script>
@endpush
