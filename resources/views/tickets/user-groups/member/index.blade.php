...
@section('content')
    <div id="cc-email-group-member-list-wrapper">
        <div
            ...
            <div class="py-3 w-100">
                <div class="container-fluid d-flex align-items-center justify-content-between ps-0">
                    ...
                    <button onclick="location.href='{{ url('tickets/user-groups') }}'"
                        ...
                        <h3 class="h3-text mb-0">{{ trans('content.user_group.usergroup_member') }} - {{ $sg->name }}</h3>
                    </button>

                    <div class="d-flex gap-8">
                        <button class="amg-btn amg-btn-primary amg-btn-sm btn-add-member" type="button" data-bs-toggle="tooltip"
                            title="Add New Group">
                            ...
                            <span>{{ trans("content.user_group.add_new_member") }}</span>
                        </button>
                    </div>
                ...
                <div class="card rounded-0">
                    <div class="card-body">
                        <div class="d-flex align-items-center gap-2 mb-1">
                            <div class="col-auto">
                                <select
                                    class="amg-table-pagination-dropdown amgTablePageLenth cc-email-group-member-list-page-length">
                            ...
                            <div>
                                <div class="amg-list-searchbar">
                                    ...
                                    <input type="text" class="amg-list-searchbar__input user-list-search"
                                        placeholder="{{ trans('content.user_group.press_enter_with_Search') }}">
                                </div>
                            </div>

                            <button class="amg-refresh-btn btn-reload-list btn-reload-list"
                                title="{{ trans('content.ticket_incident.Refresh_List') }}">
                        ...
                        <div class="">
                            <div class="table-responsive">
                                <table id="cc-email-member-table" class="amg-datatable table display app-data-table">
                                    <thead>
                                        <tr>
                                            <th><h4>{{ trans('content.user_group.name') }}<h4></th></th>
                                            <th><h4>{{ trans('content.user_group.email') }}<h4></th></th>
                                            <th><h4>{{ trans('content.user_group.created_at') }}<h4></th></th>
                                            <th><h4>{{ trans('content.user_group.updated_at') }}<h4></th></th>
                                            <th><h4>{{ trans('content.user_group.actions') }}<h4></th></th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                    </tbody>
        ...
        @include('tickets.user-groups.member.add-group-member')  
...
@push('css')
    ...
    <style>
        #cc-email-group-member-list-wrapper .amg-form-field label.error {
...
@push('scripts')
    ...
    <script src="{!! CommonHelper::asset('js/tickets/user-groups/members.js') !!}"></script>
    ...
    <script type="text/javascript">
        $(document).ready(function() {          
            ...
            config.url.getMemberList = "{{ url('tickets/user-groups-member-list') }}";
            config.url.edit = "{{ url('tickets/user-groups-member-edit') }}";
            config.url.update = "{{ url('tickets/user-groups-member-update') }}";
            config.url.delete = "{{ url('tickets/user-groups-member-delete') }}";
            config.url.add = "{{  url('tickets/user-groups/members') }}"+"/"+{!! json_encode($sg->id) !!};
            config.group = {!! json_encode($sg) !!};
            config.datatable_translations =  @json(trans('datatable.datatable'));
            config.translations = {
                add_new_member: '{{ trans('content.user_group.add_new_member') }}',
                edit_member: '{{ trans('content.user_group.edit_member') }}',
                delete_member: '{{ trans('content.user_group.delete_member') }}',
                ...
                delete_record: '{{ trans('content.user_group.confirmation_delete') }}',
                user_placeholder: '{{ trans('content.user_group.user_placeholder') }}',
                something_went_wrong:'{{ trans('content.user_group.something_went_wrong') }}',
            };
            config.token = "{{ csrf_token() }}";
            new userGroupMembers(config);
        });
    </script>
@endpush
