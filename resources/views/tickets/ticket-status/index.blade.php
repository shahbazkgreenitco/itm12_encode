...
@section('title', trans('ticket-status.page.ticket_status'))
@section('content')
<div id="main-ticket-status-wrapper">
    <div class="header-actions-wrapper d-flex align-items-center justify-content-between pe-4">
    <h3 class="h3-text mb-0">{{trans('ticket-status.page.ticket_status')}}</h3>
    <div class="d-flex gap-8">
        {{-- Download Excel --}}
        <button class="header-icon-btn header-icon-btn-sm btn-open-filter" type="button" data-bs-toggle="tooltip" title="{{trans('ticket-status.page.filter')}}">
            ...
            <span class="b6-text opacity-50">{{trans('ticket-status.page.filter')}}</span>
            <span class="filter-count-badge d-none" aria-label="Active filters">0</span>
        </button>

        {{-- <button class="header-icon-btn-only header-icon-btn-only-sm btn-download" type="button" data-bs-toggle="tooltip" title="{{trans('ticket-status.page.download')}}">
        ...
        <button class="amg-btn amg-btn-primary add-ticket-type amg-btn-sm btn-add-status" type="button" data-bs-toggle="tooltip" title="{{trans('ticket-status.page.add_ticket_status')}}">
                ...
                <span>{{trans('ticket-status.page.add_ticket_status')}}</span>
        </button>
    </div>
        ...
        <div class="container-fluid px-0">
            <div class="card rounded-0">
                ...
                <div class="d-flex align-items-center gap-2 mb-1">
                    <div class="col-auto">
                        <select class="userModulePageLenth tkt-status-page-length amg-table-pagination-dropdown">
                            <option value="10" selected>{{trans('ticket-status.table.show_10')}}</option>
                            <option value="25">{{trans('ticket-status.table.show_25')}}</option>
                            <option value="50">{{trans('ticket-status.table.show_50')}}</option>
                            <option value="100">{{trans('ticket-status.table.show_100')}}</option>
                        </select>
                    </div>
                    ...
                    <div>
                        <div class="amg-list-searchbar">
                            <button type="button" class="amg-list-searchbar__icon-btn btn-searchbox" aria-label="Search Ticket Status" title="{{trans('ticket-status.table.search_ticket_status')}}">
                            ...
                            <input type="text" name="search" class="amg-list-searchbar__input searchbox plain-search" placeholder="{{trans('ticket-status.table.search_ticket_status')}}" autocomplete="off">
                        </div>
                    </div>
                    ...
                    <button class="amg-refresh-btn btn-reload-list btn-reload-list">
                        ...
                        <span>{{trans('ticket-status.common.refresh')}}</span>
                    </button>
                </div>
                    <div class="list-view-panel">
                        <div class="table-responsive">
                            <table id="mytable" class="table display app-data-table">
                                <thead>
                                    <tr>
                                        <th><h4>{{trans('ticket-status.table.status_id')}}</h4></th>
                                        <th><h4>{{trans('ticket-status.table.status_name')}}</h4></th>
                                        <th><h4>{{trans('ticket-status.table.company')}}</h4></th>
                                        <th><h4>{{trans('ticket-status.table.status')}}</h4></th>
                                        <th><h4>{{trans('ticket-status.table.tat_halt')}}</h4></th>
                                        <th><h4>{{trans('ticket-status.table.attachment')}}</h4></th>
                                        <th><h4>{{trans('ticket-status.table.color')}}</h4></th>
                                        <th><h4>{{trans('ticket-status.table.created_at')}}</h4></th>
                                        <th><h4>{{trans('ticket-status.table.updated_at')}}</h4></th>
                                        <th><h4>{{trans('ticket-status.table.action')}}</h4></th>
                                    </tr>
                                </thead>
                                <tbody>
                                </tbody>
    ...
    @include('tickets.ticket-status.add-status')
    @include('tickets.ticket-status.update-status')
    @include('tickets.ticket-status.filter')
</div>
@endsection
...
@push('scripts')
   ...
   {{-- <script src="{!! CommonHelper::asset('nfy/plugins/daterangepicker/flatpickr.js') !!}"></script> --}}
   <script src="{!! CommonHelper::asset('nfy/plugins/daterangepicker/daterangepicker.min.js') !!}"></script>
   <script src="{!! CommonHelper::asset('js/tickets/ticket-status/ticket-status.js') !!}"></script>
   <script src="{!! CommonHelper::asset('js/jquery.repeater.min.js') !!}"></script>
   <script>
        $(document).ready(function() {
       var config = {};
       config.url = {};
       config.url.status_list = "{{ url('jx-status-list') }}";
       config.url.delete = "{{ url('tickets/status/deletestatus/delete') }}";
       config.url.create_status = "{{ url('tickets/status/createStatus') }}";
       config.url.status = "{{ url('ticket-status') }}";
       config.url.edit = "{{ url('tickets/status/edit') }}";
       config.url.update = "{{ url('tickets/status/update') }}";
       config.url.attachmentUrl = "{{ url('uploads/ticket_status') }}";
       config.url.getForm = "{{ url('getforms') }}";
       config.imgviewpath = "{{ url('uploads/ticket_status') }}";
       config.url.getSubcategory = "{{ url('tickets/subcategories') }}";
       config.url.getCompany = "{{ url('getCompanyByUserAccess') }}";
       config.url.getTicketType ="{{ url('tickets/getTicketTypeByCompany') }}";
       config.translations = {
           something_went_wrong: "{{trans('ticket-status.messages.something_went_wrong')}}",
           add_new_status: "{{trans('ticket-status.messages.add_new_status')}}",
           edit_status: "{{trans('ticket-status.messages.edit_status')}}",
           delete_title: "{{trans('ticket-status.messages.delete_title')}}",
           delete_record: "{{trans('ticket-status.messages.delete_record')}}",
           search_invalid: "{{trans('ticket-status.messages.search_invalid')}}",
           no_filter: "{{trans('ticket-status.common.no_filter')}}",
           success_title: "{{trans('ticket-status.messages.success_title')}}",
           error_title: "{{trans('ticket-status.messages.error_title')}}",
           confirm_title: "{{trans('ticket-status.messages.confirm_title')}}",
           confirm_button: "{{trans('ticket-status.messages.confirm_button')}}",
           cancel_button: "{{trans('ticket-status.messages.cancel_button')}}",
           ok_button: "{{trans('ticket-status.messages.ok_button')}}",
           duplicate_problem_category: "{{trans('ticket-status.messages.duplicate_problem_category')}}",
           duplicate_sub_category: "{{trans('ticket-status.messages.duplicate_sub_category')}}",
           select_date_range: "{{trans('ticket-status.common.select_date_range')}}"
      };
      config.token = "{{ csrf_token() }}";
      config.company = {!! json_encode($defaultCompany) !!};
      config.other_filters = {};
      new TicketStatus(config);
   });
    </script>
@endpush
