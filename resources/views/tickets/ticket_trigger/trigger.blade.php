    ...
    <section class="content reports-page">
        <div id="main-schedule-wrapper">
            <div class="header-actions-wrapper d-flex align-items-center justify-content-between pe-4">
                <h3 class="h3-text mb-0">{{ __('content.ticket_trigger.ticket_trigger') }}</h3>
                <div class="d-flex gap-8">
        
                    @if (Auth::user()->hasPermissionTo('TicketTriggerAdd'))
                        <button class="amg-btn amg-btn-primary amg-btn-sm btn-add-trigger" type="button" data-bs-toggle="tooltip" title="{{ trans('content.ticket_trigger.add_new_trigger') }}">
                            ...
                            <span>{{ __('trigger.add_trigger') }}</span>
                        </button>
                    @endif
                </div>
            </div>
                    ...
                    <div class="card rounded-0">
                        <div class="card-body pt-3">
                            <div class="d-flex align-items-center gap-2 mb-1">
                                <div class="col-auto">
                                    <select id="user-list-page-length" class="amg-table-pagination-dropdown userModulePageLenth outgoing-mail-page-length">
                                        <option value="10" selected>{{ __('trigger.show') }} (10)</option>
                                        <option value="25">{{ __('trigger.show') }} (25)</option>
                                        <option value="50">{{ __('trigger.show') }} (50)</option>
                                        <option value="100">{{ __('trigger.show') }} (100)</option>
                                    </select>
                                </div>                                  
                                ...
                                <div class="amg-list-searchbar">
                                    ...
                                    <input type="text" class="amg-list-searchbar__input searchbox plain-search" placeholder="{{ trans('content.ticket_trigger.serach_option') }}" autocomplete="off">
                                </div>
                            ...
                            <div class="js-user-list-view-panel list-view-panel">
                                <div class="table-responsive">
                                    <table id="mytable" class="amg-datatable table display app-data-table">
                                        <thead>
                                            <tr>
                                                <th><h4 class="b2-text">{{ __('trigger.company_name') }}</h4></th>
                                                <th><h4 class="b2-text">{{ __('trigger.title') }}</h4></th>
                                                <th><h4 class="b2-text">{{ __('trigger.description') }}</h4></th>
                                                <th><h4 class="b2-text">{{ __('trigger.status') }}</h4></th>
                                                <th><h4 class="b2-text">{{ __('trigger.created_at') }}</h4></th>
                                                <th><h4 class="b2-text">{{ __('trigger.updated_at') }}</h4></th>
                                                <th><h4 class="b2-text">{{ __('trigger.action') }}</h4></th>
                                            </tr>
                                        </thead>
                                        <tbody></tbody>
                                    </table>
        ...
        @include("tickets.ticket_trigger.modal_trigger")
        @include('tickets.ticket_trigger.trigger_history')
...
@push('scripts')
    ...
    <script type="text/javascript" src="{!! CommonHelper::asset('js/tickets/tickets_trigger/ticketTrigger.js') !!}"></script>
    <script type="text/javascript">
        $(document).ready(function() {
            var config = {};
            config.url = {};
            config.url.base_url = "{{ url('') }}";
            config.url.getTicketTriggers = "{{ url('tickets/getTicketTriggers') }}";
            config.url.saveTicketTrigger = "{{ url('tickets/trigger/save') }}";
            config.url.editTicketTrigger = "{{ url('tickets/trigger/edit') }}";
            config.url.updateTicketTrigger = "{{ url('tickets/trigger/update') }}";
            config.url.removeTicketTrigger = "{{ url('tickets/trigger/remove') }}";
            config.url.getOperators = "{{ url('tickets/trigger/getOperators') }}";
            config.url.download_url = "{{ url('exportTicketTriggers?search=') }}";
            config.url.history_page = "{{ url('tickets/trigger/getHistory') }}";
            config.url.getCompanyByUserAccess = "{{ url('getCompanyByUserAccess') }}";
            config.url.getDepartmentsWithCompanyByQuery = "{{ url('getDepartmentsWithCompanyByQuery') }}";
            ...
            config.assignTo = {!! json_encode($assign_to) !!};
            ...
            config.translations = {
                ...
                add_new_trigger: '{{ trans('content.ticket_trigger.add_new_trigger') }}',
                edit_trigger: '{{ trans('content.ticket_trigger.edit_trigger') }}',
                save: '{{ trans('button.save') }}',
                Edit: '{{ trans('content.ticket_trigger.Edit') }}',
                Delete: '{{ trans('content.ticket_trigger.Delete') }}',
                Add_Trigger: '{{ trans('content.ticket_trigger.Add_Trigger') }}',
                Search: '{{ trans('content.ticket_trigger.Search') }}',
                serach_option: '{{ trans('content.ticket_trigger.serach_option') }}',
                Refresh_List: '{{ trans('content.ticket_trigger.Refresh_List') }}',
                Events: '{{ trans('content.ticket_trigger.Events') }}',
                Conditions: '{{ trans('content.ticket_trigger.Conditions') }}',
                Select_Status: '{{ trans('content.ticket_trigger.Select_Status') }}',
                select_actions: '{{ trans('trigger.select_actions') }}',
                Select_User: '{{ trans('content.ticket_trigger.Select_User') }}',
                actions: '{{ trans('content.ticket_trigger.actions') }}',
                Add_new: '{{ trans('content.ticket_trigger.Add_new') }}',
                delete_trigger: '{{ trans('content.ticket_trigger.delete_trigger') }}',
                history: '{{ trans('content.ticket_trigger.history') }}',
                read_more: '{{ trans('content.ticket_trigger.read_more') }}',
                company_placeholder: '{{ trans('content.ticket_trigger.company_placeholder') }}'
            };
