...
@section('title', 'Ticket Schedular')
@section('content')
<div id="main-schedule-wrapper">
    <div class="header-actions-wrapper d-flex align-items-center justify-content-between pe-4">
        <h3 class="h3-text mb-0">{{ trans('ticket_schedular.schedular_listing_fields.schedular_listing') }}</h3>
        <div class="d-flex gap-8">

            @if(Auth::user()->hasPermissionTo('SchedularDownload'))
                <button class="header-icon-btn-only header-icon-btn-only-sm btn-download" type="button" data-bs-toggle="tooltip" title='{{ trans('ticket_schedular.schedular_listing_fields.Download') }}'>
            ...
            @if (Auth::user()->hasPermissionTo('SchedularAdd'))
            <button class="amg-btn amg-btn-primary amg-btn-sm btn-add-mail" type="button" data-bs-toggle="tooltip" title="{{ trans('ticket_schedular.schedular_listing_fields.Add_Schedular') }}">
                ...
                <span>{{ trans('ticket_schedular.schedular_listing_fields.Add_Schedular') }}</span>
            </button>
            @endif
        </div>
    </div>
            ...
            <div class="card rounded-0">
                <div class="card-body pt-3">
                    <div class="d-flex align-items-center gap-2 mb-1">
                        <div class="col-auto">
                            <select class="amg-table-pagination-dropdown userModulePageLenth outgoing-mail-page-length">
                                <option value="10" selected>{{ trans('ticket_schedular.schedular_listing_fields.show') }} (10)</option>
                                <option value="25">{{ trans("ticket_schedular.schedular_listing_fields.show") }} (25)</option>
                                <option value="50">{{ trans("ticket_schedular.schedular_listing_fields.show") }} (50)</option>
                                <option value="100">{{ trans("ticket_schedular.schedular_listing_fields.show") }} (100)</option>
                            </select>
                        </div>
                        ...
                        <div>
                            <div class="amg-list-searchbar">
                                <svg class="amg-list-searchbartransicon btn-search-mail-list" width="18" height="18" viewBox="0 0 20 20" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                                ...
                                <input type="text" class="amg-list-searchbar__input searchbox plain-search" placeholder="{{ trans('outgoing-mail-setting.page.search_placeholder') }}" autocomplete="off">
                            </div>
                        </div>

                        <button class="amg-refresh-btn btn-reload-list">
                            <svg class="amg-refresh-btntransicon" width="20" height="18" viewBox="0 0 20 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                            ...
                            <span>{{ trans('ticket_schedular.schedular_listing_fields.Refresh_List') }}</span>
                        </button>
                    </div>

                    <div class="js-user-list-view-panel list-view-panel">
                        <div class="table-responsive">
                            <table id="mytable" class="table display app-data-table">
                                <thead>
                                    <tr>
                                        <th><h4>{{ trans('ticket_schedular.schedular_listing_fields.company_name') }}</h4></th>
                                        <th><h4>{{ trans('ticket_schedular.schedular_listing_fields.department') }}</h4></th>
                                        <th><h4>{{ trans('ticket_schedular.schedular_listing_fields.user') }}</h4></th>
                                        <th><h4>{{ trans('ticket_schedular.schedular_listing_fields.problem_category') }}</h4></th>
                                        <th><h4>{{ trans('ticket_schedular.schedular_listing_fields.problem_sub_category') }}</h4></th>
                                        <th><h4>{{ trans('ticket_schedular.schedular_listing_fields.priority') }}</h4></th>
                                        <th><h4>{{ trans('ticket_schedular.schedular_listing_fields.tat') }}</h4></th>
                                        <th><h4>{{ trans('ticket_schedular.schedular_listing_fields.device') }}</h4></th>
                                        <th><h4>{{ trans('ticket_schedular.schedular_listing_fields.cron_schedular') }}</h4></th>
                                        <th><h4>{{ trans('ticket_schedular.schedular_listing_fields.subject') }}</h4></th>
                                        <th><h4>{{ trans('ticket_schedular.schedular_listing_fields.updated_on') }}</h4></th>
                                        <th><h4>{{ trans('ticket_schedular.schedular_listing_fields.actions') }}</h4></th>
                                    </tr>
                                </thead>
                                <tbody></tbody>
                            </table>
    ...
    @include('tickets.schedule.schedule-modal')
</div>
@endsection
...
<style>
    .amg-uploader__preview {
        max-height: 180px;
        overflow-y: auto;
        overflow-x: hidden;
        scrollbar-width: thin;
        scrollbar-color: #b70000 #ffffff;
    }

    .amg-uploader__preview::-webkit-scrollbar {
        width: 4px; 
    }

    #main-schedule-wrapper .user-list-avatar {
        width: 30px;
        height: 30px;
        flex: 0 0 30px;
        border-radius: 999px;
    ...
    #main-schedule-wrapper .user-list-avatar-fallback {
        display: inline-flex;
        align-items: center;
...
<script src="{{ CommonHelper::asset('js/tickets/ticket-schedular/ticket-schedular.js') }}"></script>
<script src="{{ CommonHelper::asset('js/tickets/work-hour-count-down.js') }}"></script>

<script src="{{CommonHelper::asset('assets/js/plugins/moment.min.js')}}"></script>
<script src="{{CommonHelper::asset('assets/js/plugins/moment-timezone-with-data.min.js')}}"></script>
<script src="{{CommonHelper::asset('assets/js/plugins/index.min.js')}}"></script>
<script src="{{CommonHelper::asset('assets/js/plugins/jquery.countdown.min.js')}}"></script>
...
<script>
    $(document).ready(function() {
        ...
        var config = {};
        config.url = {};
        config.url.shedular_list = "{{ url('jx-schedular-list') }}";
        config.url.save = "{{ url('tickets/initSchedule') }}";
        config.url.delete = "{{ url('tickets/deleteschedular/delete') }}";
        config.url.edit = "{{ url('tickets/editschedular/edit') }}";
		config.url.download_url = "{{ url('export-schedular?search=') }}";
        ...
        config.user = {!! json_encode(Auth::user()->only("id", "first_name", "last_name", "username","displayName")) !!};
        ...
        config.translations = {!! json_encode([
            'New_Schedular' => trans('ticket_schedular.schedular_listing_fields.New_Schedular'),
            'Edit_Schedular' => trans('ticket_schedular.schedular_listing_fields.Edit_Schedular'),
            'create' => trans('ticket_schedular.schedular_listing_fields.create'),
            'save_changes' => trans('ticket_schedular.schedular_listing_fields.save_changes'),
            'select_reply_to_account' => trans('ticket_schedular.schedular_listing_fields.select_reply_to_account'),
            'select_priority' => trans('ticket_schedular.schedular_listing_fields.select_priority'),
            'Select_Problem_Category' => trans('ticket_schedular.schedular_listing_fields.Select_Problem_Category'),
            'enter_first_few_letter' => trans('ticket_schedular.schedular_listing_fields.enter_first_few_letter'),
            'connect' => trans('ticket_schedular.schedular_listing_fields.connect'),
            'select_device' => trans('ticket_schedular.schedular_listing_fields.select_device'),
            'something_went_wrong' => trans('ticket_schedular.user_fields.something_went_wrong'),
            'delete_record' => trans('ticket_schedular.schedular_listing_fields.delete_record'),
            'Edit' => trans('ticket_schedular.schedular_listing_fields.Edit'),
            'Delete' => trans('ticket_schedular.schedular_listing_fields.Delete'),
            'company_placeholder' => trans('ticket_schedular.schedular_listing_fields.company_placeholder'),
            'note' => trans('ticket_schedular.schedular_listing_fields.note_placeholder')
        ]) !!};
        new MyApp(config);
    });
</script>
