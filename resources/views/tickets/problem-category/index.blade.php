  ...
  "versions": [
    {
      ...
      "from": "2026-06",
      "reviewer": null,
      "description": "UI Change According to the User Module "
    },
    {
      "version": "1.0",
...
@section('title', trans("problem_category.service_ticket_fields.Problem_Category"))
@section('content')
<div id="main-user-list-wrapper" class="pbmg-list-wrapper">
    <div class="header-actions-wrapper d-flex align-items-center justify-content-between pe-4">
        <h3 class="h3-text mb-0">{{ trans("problem_category.service_ticket_fields.problem_categories") }}</h3>
    <div class="d-flex gap-8">
        <button class="header-icon-btn header-icon-btn-sm btn-open-filter" type="button" data-bs-toggle="tooltip" title="{{ trans("problem_category.service_ticket_fields.Filter") }}">
            ...
            <span class="b6-text opacity-50">{{ trans("problem_category.service_ticket_fields.Filter") }}</span>
            <span class="filter-count-badge d-none" aria-label="Active filters">0</span>
        </button>

        @if(Auth::user()->hasPermissionTo('ProblemCategoriesDownload'))
            <button class="header-icon-btn-only header-icon-btn-only-sm btn-export-problem-category" type="button" data-bs-toggle="tooltip" title="{{ trans("problem_category.service_ticket_fields.download_excel") }}">
        ...
        @if(Auth::user()->hasPermissionTo('ProblemCategoriesImport'))
            <button class="header-icon-btn-only header-icon-btn-only-sm btn-import-problem-category" type="button" data-bs-toggle="tooltip" title="{{ trans("problem_category.service_ticket_fields.import") }}">
        ...
        @if(Auth::user()->hasPermissionTo('ProblemCategoriesAdd'))
            <button class="amg-btn amg-btn-primary amg-btn-sm btn-add-type" type="button" data-bs-toggle="tooltip" title="{{ trans("problem_category.service_ticket_fields.add_category") }}">
                ...
                <span>{{ trans("problem_category.service_ticket_fields.add_category") }}</span>
            </button>
        @endif
    </div>
            ...
            <div class="card rounded-0">
                ...
                <div class="d-flex align-items-center gap-2 mb-1">
                    ...
                    <div class="amg-list-searchbar">
                        ...
                        <input type="text" class="amg-list-searchbar__input user-list-search searchbox plain-search" placeholder="Search...">
                    </div>
                    ...
                    <div class="js-user-list-view-panel">
                        <div class="table-responsive">
                            <table id="mytable" class="mytable amg-datatable table display app-data-table">
                                <thead>
                                    <tr>
                                        <th width="30%"><h4>{{ trans("problem_category.service_ticket_fields.problem_category") }}</h4></th>
                                        <th><h4 class="b2-text">{{ trans("problem_category.service_ticket_fields.company") }}</h4></th>
                                        <th><h4 class="b2-text">{{ trans("problem_category.service_ticket_fields.department") }}</h4></th>
                                        <th><h4 class="b2-text">{{ trans("problem_category.service_ticket_fields.Parent_Category") }}</h4></th>
                                        <th><h4 class="b2-text">{{ trans("problem_category.service_ticket_fields.initial") }}</h4></th>
                                        <th><h4 class="b2-text">{{ trans("problem_category.service_ticket_fields.Attender") }}</h4></th>
                                        <th><h4 class="b2-text">{{ trans("problem_category.service_ticket_fields.Priority") }}</h4></th>
                                        <th><h4 class="b2-text">{{ trans("problem_category.service_ticket_fields.TAT_h") }}</h4></th>
                                        @if(config("services.task_module.enabled"))
                                            @can("TaskRead")
                                                <th><h4 class="b2-text">{{ trans("problem_category.service_ticket_fields.task_count") }}</h4></th>
                                            @endcan
                                        @endif
                                        <th><h4 class="b2-text">{{ trans("problem_category.service_ticket_fields.th_status") }}</h4></th>
                                        @if(in_array(config('app.client'), ["grdemo", "ril","rolepermission"]))
                                            <th><h4 class="b2-text">{{ trans("problem_category.service_ticket_fields.Response_sla") }} (%)</h4></th>
                                        @else
                                            <th><h4 class="b2-text">{{ trans("problem_category.service_ticket_fields.Response_sla") }} (Hrs)</h4></th>
                                        @endif
                                        <th><h4 class="b2-text">{{ trans("problem_category.service_ticket_fields.Workaround_sla") }} (Hrs)</h4></th>
                                        <th><h4 class="b2-text">{{ trans('problem_category.service_ticket_fields.close_ticket_after_days') }}</h4></th>
                                        <th><h4 class="b2-text">{{ trans('problem_category.service_ticket_fields.reopen_ticket_until_days') }}</h4></th>
                                        <th><h4 class="b2-text">{{ trans("problem_category.service_ticket_fields.notes") }}</h4></th>
                                        <th><h4 class="b2-text">{{ trans("problem_category.service_ticket_fields.updated_on") }}</h4></th>
                                        <th><h4 class="b2-text">{{ trans("problem_category.service_ticket_fields.Actions") }}</h4></th>
                                    </tr>
                                </thead>
                                <tbody>
                                </tbody>
    ...
    @include('tickets.problem-category.add-modal')
    @include('tickets.problem-category.escalation')
    @include('tickets.problem-category.filter')
</div>
@endsection
...
<style>
    ...
    /* #main-user-list-wrapper #mytable .amg-col-actions.is-action-menu-open {
        z-index: 20 !important;
    }

    #main-user-list-wrapper #mytable tbody tr.is-action-menu-open {
        position: relative;
        z-index: 10;
    } */

    #frm_problem_type .note-toolbar i.Tt {
        font-style: normal;
        font-weight: 600;
        font-size: 0.8rem;
    }
    #frm_problem_type .note-toolbar i.Tt::before {
        content: "Tt";
    }
    [data-bs-theme="dark"] .note-editor .note-editable,
    [data-bs-theme="dark"] .note-editor .note-editable p,
    [data-bs-theme="dark"] .note-editor .note-editable span,
    [data-bs-theme="dark"] .note-editor .note-editable div {
...
@push('scripts')
    ...
    <script src="{!! CommonHelper::asset('js/tickets/problem-category/index.js') !!}"></script>
    <script>
        $(document).ready(function() {
            ...
            config.placeholder = {};
            config.url = {
                types: "{{ url('tickets/ajax-problem-categories') }}",
                add: "{{ url('tickets/create-problem-category') }}",
                edit: "{{ url('tickets/edit-problem-category') }}",
                delete: "{{ url('tickets/delete-problem-category') }}",
                get: "{{ url('tickets/get-problem-category-for-edit') }}",
                get_esclations_info: "{{ url('tickets/problem-categories/get-esclations-info') }}",
                add_esclation: "{{ url('tickets/problem-categories/add-esclation') }}",
                update_esclation: "{{ url('tickets/problem-categories/update-esclation') }}",
                del_esclation: "{{ url('tickets/problem-categories/del-esclation') }}",
                get_esclatable_users: "{{ url('tickets/problem-categories/get-esclatable-users') }}",
                download_url:  "{{ url('tickets/problem-categories/export-problemcategory') }}",
                download_pdf:  "{{ url('tickets/problem-categories/exportProblemCategoryPDF') }}",
                import_url:  "{{ route('problemCat-import') }}",
                history: "{{ url('tickets/problem-categories-history') }}",
                esclation_history: "{{ url('tickets/problem-categories/esclation-history') }}",
                base_url: "{{ url('') }}",
            };
            // config.is_super_user = {{ Auth::user()->isSuperUser() }};
            config.url.getformname = "{{ url('tickets/formList') }}";
            config.url.getPabApproved = "{{ url('tickets/getAuthorityBoard') }}";
            config.url.departments_based_on_privilage = "{{ url('tickets/departments') }}";
            config.url.problem_categories_by_dept = "{{ url('tickets/problem-categories/by-dept') }}";
            config.url.departments_with_company = "{{ url('getDepartmentsWithCompanyByQuery') }}";
            config.url.getTicketAttendersByDepartment = "{{ url('getTicketAttendersByDepartment') }}";
            config.url.getAllocationGroups = "{{ route('getUserGroupSelect2') }}";
            config.url.fetchFieldSet = "{{ url('custom-fields/selectFieldSet') }}";
            config.url.get_esclatable_group = "{{ url('tickets/get-esclatable-groups') }}";
            config.url.getescalate_to = "{{ url('getEscalateUsers') }}";
            config.url.getCCEmailUserGroup = "{{ url('tickets/get-cc-user-groups') }}";
            config.url.getTaskList = "{{ url('tickets/problem-categories/tasklist') }}";
            config.url.getRoles = "{{ url('ajax-roles') }}";
            config.priorities = {!! json_encode($priorities) !!};
            config.form = {!! json_encode($form) !!};
            config.custom_form = {!! json_encode($custom_form) !!};
            config.permissions = {!! json_encode($permissionArray) !!};
            config.client = "{{ config('app.client') }}";
            config.title.New_Problem_Category= "{!! trans('problem_category.service_ticket_fields.New_Problem_Category') !!}";
            config.title.Edit_Problem_Category= "{!! trans('problem_category.service_ticket_fields.Edit_Problem_Category') !!}";
            ...
            config.translations = {
                ...
                New_Problem_Category: '{{ trans('problem_category.service_ticket_fields.New_Problem_Category') }}',
                Edit_Problem_Category: '{{ trans('problem_category.service_ticket_fields.Edit_Problem_Category') }}',
                Save_Changes: '{{ trans('problem_category.service_ticket_fields.Save_Changes') }}',
                No_ParentCategory: '{{ trans('problem_category.service_ticket_fields.No_ParentCategory') }}',
                are_you_delete_problemcategory: '{{ trans('problem_category.service_ticket_fields.are_you_delete_problemcategory') }}',
                Select_Priority_Type: '{{ trans('problem_category.service_ticket_fields.Select_Priority_Type') }}',
                select_the_department: '{{ trans('problem_category.service_ticket_fields.select_the_department') }}',
                Select_Parent_Category: '{{ trans('problem_category.service_ticket_fields.Select_Parent_Category') }}',
                Select_the_User: '{{ trans('problem_category.service_ticket_fields.Select_the_User') }}',
                Edit_Category: '{{ trans('problem_category.service_ticket_fields.Edit_Category') }}',
                Delete_Category: '{{ trans('problem_category.service_ticket_fields.Delete_Category') }}',
                ...
                Create: '{{ trans('problem_category.service_ticket_fields.Create') }}',
                refresh_list: '{{ trans('problem_category.service_ticket_fields.refresh_list') }}',
                download_excel: '{{ trans('problem_category.service_ticket_fields.download_excel') }}',
                download_pdf: '{{ trans('problem_category.service_ticket_fields.download_pdf') }}',
                add_category: '{{ trans('problem_category.service_ticket_fields.add_category') }}',
                sla_count: '{{ trans('problem_category.service_ticket_fields.sla_count') }}',
                comment_summer:'{{ trans('problem_category.service_ticket_fields.share_comment') }}',
                Filter_By_Priority: '{{ trans('problem_category.service_ticket_fields.Filter_By_Priority') }}',
                filter_by_department: '{{ trans('problem_category.service_ticket_fields.filter_by_department') }}',
                filter_by_allocation_group: '{{ trans('problem_category.service_ticket_fields.filter_by_allocation_Group') }}',
                filter_by_authority_approval: '{{ trans('problem_category.service_ticket_fields.filter_by_authority_approval') }}',
                filter_by_authority_board: '{{ trans('problem_category.service_ticket_fields.filter_by_authority_board') }}',
                filter_by_formname: '{{ trans('problem_category.service_ticket_fields.filter_by_formname') }}',
                select_fieldset: '{{ trans('problem_category.service_ticket_fields.custom_fieldset') }}',
                filter_by_escalation_for: '{{ trans('problem_category.service_ticket_fields.filter_by_escalation_for') }}',
                filter_by_escalation_to: '{{ trans('problem_category.service_ticket_fields.filter_by_escalation_to') }}',
                select_cc_groups: '{{ trans('problem_category.service_ticket_fields.select_cc_groups') }}',
                something_went_wrong:'{{ trans('problem_category.service_ticket_fields.something_went_wrong') }}',
                select_role:'{{ trans('problem_category.service_ticket_fields.select_role') }}',
                filter_by_roles:'{{ trans('problem_category.service_ticket_fields.filter_by_roles') }}',
                Filter:'{{ trans('problem_category.service_ticket_fields.Filter') }}',
                history: '{{ trans('problem_category.service_ticket_fields.history') }}',
                define_tasks: '{{ trans('problem_category.service_ticket_fields.define_tasks')  }}',
                filter_by_status: '{{ trans("problem_category.service_ticket_fields.filter_by_status") }}',
                filter_by_category: '{{ trans("problem_category.service_ticket_fields.filter_by_category") }}',
            };
