  ...
  "versions": [
    {
      ...
      "from": "2026-06",
      "reviewer": null,
      "description": "UI Change according to user module"
    }
    {
      "version": "1.0",
...
@section('title', trans("problem_category.service_ticket_fields.ticket_problem_history"))
@section('content')
<div id="main-user-list-wrapper" class="pbmg-list-wrapper">
    <div class="header-actions-wrapper d-flex align-items-center justify-content-between pe-4">
        <h3 class="h3-text mb-0">{{ trans("problem_category.service_ticket_fields.ticket_problem_history") }}</h3>
    </div>
...
@push('scripts')
<script src="{!! CommonHelper::asset('js/tickets/problem-category/history.js') !!}"></script>
<script>
    $(document).ready(function() {
        var config = {};
        config.url = {
            historyAjax: "{{ url('tickets/problem-categories-history-ajax') }}",
        };
        ...
        config.columnLabels = {
            company: '{{ trans("problem_category.service_ticket_fields.company") }}',
            problem_category: '{{ trans("problem_category.service_ticket_fields.problem_category") }}',
            ticket_attender: '{{ trans("problem_category.service_ticket_fields.Attender") }}',
            department: '{{ trans("problem_category.service_ticket_fields.department") }}',
            parent_category: '{{ trans("problem_category.service_ticket_fields.Parent_Category") }}',
            priority: '{{ trans("problem_category.service_ticket_fields.Priority") }}',
            tat: '{{ trans("problem_category.service_ticket_fields.TAT_h") }}',
            response_sla: '@if(in_array(config("app.client"), ["grdemo", "ril", "rolepermission"])){{ trans("problem_category.service_ticket_fields.Response_sla") }} (%)@else{{ trans("problem_category.service_ticket_fields.Response_sla") }} (Hrs)@endif',
            workaround_sla: '{{ trans("problem_category.service_ticket_fields.Workaround_sla") }} (Hrs)',
            close_ticket_after_days: '{{ trans("problem_category.service_ticket_fields.close_ticket_after_days") }}',
            reopen_ticket_until_days: '{{ trans("problem_category.service_ticket_fields.reopen_ticket_until_days") }}',
            remarks: '{{ trans("problem_category.service_ticket_fields.notes") }}',
            last_updated_at: '{{ trans("problem_category.service_ticket_fields.updated_on") }}',
            updated_by_name: '{{ trans("problem_category.service_ticket_fields.updated_by") }}',
        };
