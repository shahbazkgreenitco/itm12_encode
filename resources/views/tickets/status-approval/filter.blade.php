        ...
        <div class="modal-content rounded-5">
            <div class="modal-header d-flex align-items-center py-3 pt-4">
                <h3 class="modal-title s1-text fw-semibold px-4">
                    {{ trans('ticket-types.ticket_approval.status_approval_filter') }}
                </h3>
                ...
                <div class="row g-3 px-4">
                    <div class="col-md-3">
                        <label class="form-label b5-text">
                            {{ trans('ticket-types.ticket_approval.filter_by_department') }}
                        </label>
                        <select class="form-select filter-input" id="filter_by_department" name="filter_by_department"></select>
                    </div>
                    <div class="col-md-3">
                        <label class="form-label b5-text">
                            {{ trans('ticket-types.ticket_approval.filter_by_problem_category') }}
                        </label>
                        <select class="form-select filter-input" id="filter_by_problem_category" name="filter_by_problem_category"></select>
                    </div>
                    <div class="col-md-3">
                        <label class="form-label b5-text">
                            {{ trans('ticket-types.ticket_approval.filter_by_subcategory') }}
                        </label>
                        <select class="form-select filter-input" id="filter_by_sub_category" name="filter_by_sub_category"></select>
                    </div>
                    <div class="col-md-3">
                        <label class="form-label b5-text">
                            {{ trans('ticket-types.ticket_approval.filter_by_status') }}
                        </label>
                        <select class="form-select filter-input" id="filter_by_status" name="filter_by_status"></select>
                    </div>
                </div>
                <div class="row g-3 mt-8 px-4">
                    <div class="col-md-3">
                        <label class="form-label b5-text">
                            {{ trans('ticket-types.ticket_approval.filter_by_date') }}
                        </label>
                        <select name="filter_by_date" id="filter_by_date" autocomplete="off" class="form-select filter-input">
                            <option value="">
                                {{ trans('ticket-types.ticket_approval.no_filter') }}
                            </option>
                            <option value="1">
                                {{ trans('ticket-types.ticket_approval.created_date') }}
                            </option>
                            <option value="2">
                                {{ trans('ticket-types.ticket_approval.updated_date') }}
                            </option>
                        </select>
                    </div>
                    <div class="col-md-7">
                        <label class="form-label b5-text">
                            {{ trans('ticket-types.ticket_approval.filter_by_date_range') }}
                        </label>
                        <div id="reportrange" class="form-control d-flex align-items-center justify-content-between">
                            <span>
                                {{ trans('ticket-types.ticket_approval.select_date_range') }}
                            </span>
            ...
            <div class="modal-footer justify-content-end mb-4 py-0">
                <div class="amg-btn-group mt-3 gap-3 px-4">
                    <button type="button" class="amg-btn amg-btn-primary s2-text btn-filter" style="min-width: 300px;">
                        {{ trans('ticket-types.ticket_approval.apply') }}
                    </button>
                    <button type="button" id="btnClrFilter" class="amg-btn amg-btn-secondary s2-text hover-opacity-80 btn-clear-filter" style="min-width: 120px;">
                        {{ trans('ticket-types.ticket_approval.clear') }}
                    </button>
                </div>
            </div>
        </div>
