...
@section('title', trans('ticket-types.ticket_approval.title'))
@section('content')
<div id="main-status-approval-wrapper">
    <div class="header-actions-wrapper d-flex align-items-center justify-content-between pe-4">
        <h3 class="h3-text mb-0">{{ trans('ticket-types.ticket_approval.title') }}</h3>
        <div class="d-flex gap-8">
            <div class="column-visibility-dropdown">
                <button class="header-icon-btn-only header-icon-btn-only-sm btn-visible-content" id="columnVisibilityButton" type="button" data-bs-toggle="tooltip" title="{{ trans('ticket-types.ticket_approval.show_hide_columns') }}">
        ...
        {{-- Filter --}}
            <button class="header-icon-btn header-icon-btn-sm btn-open-filter" type="button" data-bs-toggle="tooltip" title="{{ trans('user.user_toolbar.filter') }}">
                ...
                <span class="b5-text opacity-50">{{ trans('user.user_toolbar.filter') }}</span>
                <span class="filter-count-badge d-none" aria-label="Active filters">0</span>
            </button>

            {{-- Download Excel --}}
            @if(Auth::user()->hasPermissionTo('UserDownload'))
                <button class="header-icon-btn-only header-icon-btn-only-sm btn-download" type="button" data-bs-toggle="tooltip" title="{{ trans('ticket-types.ticket_approval.download') }} Excel">
            ...
            {{-- Add Status Approval --}}
            @if(Auth::check())
                    <button class="amg-btn amg-btn-primary amg-btn-sm btn-add-status-approval" type="button" data-bs-toggle="tooltip" title="{{ trans('ticket-types.ticket_approval.add_status_approval') }}">
                        ...
                        <span>{{ trans('ticket-types.ticket_approval.add_status_approval') }}</span>
                </button>
            @endif
        </div>
    </div>
                ...
                <div class="d-flex align-items-center gap-2 mb-1">
                    ...
                    <div>
                        <div class="amg-list-searchbar">
                            ...
                            <input type="text" class="amg-list-searchbar__input user-list-search searchbox plain-search" placeholder="{{trans('ticket-status.table.search_ticket_status')}}">
                        </div>
                    </div>
                    ...
                    <div class="list-view-panel">
                        <div class="table-responsive">
                            <table id="statusApproval" class="table display app-data-table">
                                <thead>
                                    <tr>
                                        <th><h4>{{ trans('ticket-types.ticket_approval.table.company') }}</h4></th>
                                        <th><h4>{{ trans('ticket-types.ticket_approval.table.department') }}</h4></th>
                                        <th><h4>{{ trans('ticket-types.ticket_approval.table.category') }}</h4></th>
                                        <th><h4>{{ trans('ticket-types.ticket_approval.table.subcategory') }}</h4></th>
                                        <th><h4>{{ trans('ticket-types.ticket_approval.table.status') }}</h4></th>
                                        <th><h4>{{ trans('ticket-types.ticket_approval.table.fallback_status') }}</h4></th>
                                        <th><h4>{{ trans('ticket-types.ticket_approval.table.created_at') }}</h4></th>
                                        <th><h4>{{ trans('ticket-types.ticket_approval.table.updated_at') }}</h4></th>
                                        <th><h4>{{ trans('ticket-types.ticket_approval.table.action') }}</h4></th>
                                    </tr>
                                </thead>
                                <tbody>
                                </tbody>
    ...
    @include('tickets.status-approval.add-modal')
    @include('tickets.status-approval.filter')
</div>
@endsection
...
@push('scripts')
    ...
    <script type="text/javascript" src="{!! CommonHelper::asset('js/tickets/ticket-status/ticketStatusApproval.js') !!}"></script>
    <script type="text/javascript">
        $(document).ready(function () {
            var config = {};
            config.url = {};
            config.url.status_approval_list = "{{ url('status-approval-list') }}";
            ...
            config.url.getCcUserGroups = "{{ url('tickets/get-cc-user-groups') }}";
            config.url.getUserByQuery = "{{ url('getUserByQuery') }}";
            config.url.ajaxCreateStatusApproval = "{{ url('create-status-approval') }}";
            config.url.ajaxDeleteStatusApproval = "{{ url('delete-status-approval') }}";
            config.url.ajaxEditStatusApproval = "{{ url('edit-status-approval') }}";
            config.url.ajaxUpdateStatusApproval = "{{ url('update-status-approval') }}";
            config.url.statusApprovalExport = "{{ url('status-approval/export') }}";
            config.translations = @json(trans('ticket-types.ticket_approval'));
            config.token = "{{ csrf_token() }}";
            new TicketStatusApproval(config);
        });
    </script>
@endpush
