...
@section('title', trans('outgoing-mail-setting.page.title'))

@section('content')
<div id="main-outgoing-mail-wrapper">
    <div class="header-actions-wrapper d-flex align-items-center justify-content-between pe-4">
        <h3 class="h3-text mb-0">{{ trans('outgoing-mail-setting.page.title') }}</h3>
        <div class="d-flex gap-8">
            <button class="amg-btn amg-btn-primary amg-btn-sm btn-add-mail" type="button" data-bs-toggle="tooltip" title="{{ trans('outgoing-mail-setting.page.add_new_mail') }}">
                ...
                <span>{{ trans('outgoing-mail-setting.page.add_new_mail') }}</span>
            </button>
        </div>
    </div>
            ...
            <div class="card rounded-0">
                <div class="card-body pt-3">
                    <div class="d-flex align-items-center gap-2 mb-1">
                        <div class="col-auto">
                            <select class="userModulePageLenth outgoing-mail-page-length amg-table-pagination-dropdown">
                                <option value="10" selected>{{ trans('outgoing-mail-setting.page.show_10') }}</option>
                                <option value="25">{{ trans('outgoing-mail-setting.page.show_25') }}</option>
                                <option value="50">{{ trans('outgoing-mail-setting.page.show_50') }}</option>
                                <option value="100">{{ trans('outgoing-mail-setting.page.show_100') }}</option>
                            </select>
                        </div>
                        ...
                        <button class="amg-refresh-btn btn-reload-list">
                            ...
                            <span>{{ trans('outgoing-mail-setting.page.refresh') }}</span>
                        </button>
                    </div>
                    ...
                    <div class="js-user-list-view-panel list-view-panel">
                        <div class="table-responsive">
                            <table id="mytable" class="table display app-data-table">
                                <thead>
                                    <tr>
                                        <th><h4>{{ trans('outgoing-mail-setting.table.mail_driver') }}</h4></th>
                                        <th><h4>{{ trans('outgoing-mail-setting.table.company') }}</h4></th>
                                        <th><h4>{{ trans('outgoing-mail-setting.table.mail_status') }}</h4></th>
                                        <th><h4>{{ trans('outgoing-mail-setting.table.mail_host') }}</h4></th>
                                        <th><h4>{{ trans('outgoing-mail-setting.table.mail_port') }}</h4></th>
                                        <th><h4>{{ trans('outgoing-mail-setting.table.mail_username') }}</h4></th>
                                        <th><h4>{{ trans('outgoing-mail-setting.table.mail_from_address') }}</h4></th>
                                        <th><h4>{{ trans('outgoing-mail-setting.table.mail_from_name') }}</h4></th>
                                        <th><h4>{{ trans('outgoing-mail-setting.table.updated_at') }}</h4></th>
                                        <th><h4>{{ trans('outgoing-mail-setting.table.actions') }}</h4></th>
                                    </tr>
                                </thead>
                                <tbody></tbody>
                            </table>
    ...
    @include('tickets.outgoing-mail.add-modal')
    @include('tickets.outgoing-mail.update-modal')
</div>
@endsection

@push('scripts')
<script src="{{ CommonHelper::asset('js/tickets/outgoing-mail-setting/outgoing-mail.js') }}"></script>
<script>
    $(document).ready(function() {
        var config = {};
        config.url = {};
        config.url.mail_list = "{{ url('jx-mail-list') }}";
        config.url.delete = "{{ url('tickets/out-going-mail/deletemail/delete') }}";
        config.url.create_mail = "{{ url('tickets/out-going-mail/createMail') }}";
        config.url.edit = "{{ url('tickets/out-going-mail/edit') }}";
        config.url.update = "{{ url('tickets/out-going-mail/update') }}";
        config.url.getCompany = "{{ url('getCompanyByUserAccess') }}";
        config.token = "{{ csrf_token() }}";
        config.company = {!! json_encode($defaultCompany) !!};
        config.translations = @json(trans('outgoing-mail-setting.js'));
        config.validation = @json(trans('outgoing-mail-setting.validation'));

        new TicketMail(config);
    });
</script>
@endpush
