...
/**
* ------------------------------------------------------------
* File: manage-group-users.blade.php
...
@section('title', trans('escalation_group.manage_users'))
@section('content')
<div id="main-escalation-group-users-wrapper">
    <!-- Header -->
    <div class="header-actions-wrapper d-flex align-items-center justify-content-between pe-4">
        <div class="d-flex align-items-center gap-1">
             @php
                $groupName = $esc->name;
                $isLong = strlen($groupName) > 40;
                $shortName = $isLong ? substr($groupName, 0, 40) . '...' : $groupName;
            @endphp            
            <button onclick="location.href='{{ url('tickets/escalation-groups') }}'" class="d-flex gap-3 align-items-center bg-transparent outline-none border-0" data-bs-toggle="tooltip" title="Back">
            ...
            <h3 class="h3-text mb-0"@if($isLong) data-bs-toggle="tooltip" title="{{ $groupName }}" @endif> {{ trans("escalation_group.manage_users") }} - {{ $shortName }}</h3>
        </div>

        <div class="d-flex gap-8 align-items-center">
            <button class="amg-btn amg-btn-primary amg-btn-sm open-addGroupUserForm-modal"
                type="button">
                <svg width="19"
                    height="19"
                    viewBox="0 0 19 19"
                ...
                <span>
                    {{ trans('escalation_group.add_group_user') }}
                </span>
            </button>
        </div>
    </div>
                ...
                <div class="card-body pt-3">
                    ...
                    <div class="d-flex align-items-center gap-2 mb-1">
                        ...
                        <div>
                            <div class="amg-list-searchbar">
                                ...
                                <input type="text" name="search" class="amg-list-searchbar__input searchbox plain-search" placeholder="{{ trans('escalation_group.press_enter_with_Search') }}">
                            </div>
                        </div>
                        <!-- Refresh -->
                        <button class="amg-refresh-btn btn-reload-list">
                            ...
                            <span>
                                {{ trans('escalation_group.refresh') }}
                            </span>
                        </button>
                    </div>
                    <!-- Table -->
                    <div class="list-view-panel">
                        <div class="table-responsive">
                            <table id="groupoUsers"
                                class="table display app-data-table">
                                <thead>
                                    <tr>
                                        <th><h4>{{ trans('escalation_group.user_name') }}</h4></th>
                                        <th><h4>{{ trans('escalation_group.location') }}</h4></th>
                                        <th><h4>{{ trans('escalation_group.access_location') }}</h4></th>
                                        <th><h4>{{ trans('escalation_group.internal_location_access') }}</h4></th>
    ...
    @include('tickets.escalation-group.add-user-modal')
</div>
@endsection
@push('css')
<style>
    #main-escalation-group-users-wrapper {
        --user-list-avatar-bg: #eef2f7;
        --user-list-avatar-text: #41516b;
    }

    #main-escalation-group-users-wrapper .user-list-avatar {
        width: 30px;
        height: 30px;
        flex: 0 0 30px;
        border-radius: 999px;
        object-fit: cover;
        background: var(--user-list-avatar-bg);
        border: 1px solid rgba(148, 163, 184, .16);
    }

    #main-escalation-group-users-wrapper .user-list-avatar-fallback {
        display: inline-flex;
        align-items: center;
        justify-content: center;
        font-size: 10px;
        font-weight: 600;
        color: var(--user-list-avatar-text);
        background: var(--user-list-avatar-bg);
    }

    [data-bs-theme="dark"] #main-escalation-group-users-wrapper {
        --user-list-avatar-bg: #1d293d;
        --user-list-avatar-text: #dbe5f2;
    }
</style>
@endpush
@push('scripts')
<script type="text/javascript" src="{!! CommonHelper::asset('js/tickets/escalation-group/manageUsers.js') !!}"></script>
<script type="text/javascript">
    ...
    config.url.manage_users = "{{ url('tickets/escalation-groups/manage-users') }}";
    config.url.getUserAllocationGroups = "{{ url('jx-get-user-allocation-groups') }}";
    config.url.addGroupUser = "{{ url('jx-add-escalation-group-user') }}";
    config.url.getUserEscalationGroupMembers = "{{url('tickets/escalation-groups/manage-users')}}";
    config.url.getGroupMember = "{{ url('jx-get-user-escalation-member') }}";
    config.url.editGroupMember = "{{ url('jx-escalation-edit-group-member-details') }}";
    config.url.getUserByAjax = "{{ url('getUserByQuery') }}";
    config.permissions = {!! json_encode($permissionArray, true) !!};

    config.url.delete = "{{ url('jx-escalation-user-delete') }}";
    config.url.getEscCompanyUsersByQuery = "{{ url('getEscCompanyUsersByQuery') }}";
    config.company_id = {!! json_encode($company_id) !!};
    config.group_Id = {!! json_encode($esc->id) !!};
    config.ticketHandlers = {!! json_encode($users) !!};
    config.locationAccess = {!! json_encode($locations) !!};
    ...
    config.location_based = {!! json_encode($esc->location_based) !!};
    config.token = "{{ csrf_token() }}";

    new EscalationGroupsMembers(config);
