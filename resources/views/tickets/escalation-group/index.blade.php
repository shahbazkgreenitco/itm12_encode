...
@section('title', trans('escalation_group.title'))
@section('content')
<div id="main-escalation-group-wrapper">
    <div class="header-actions-wrapper d-flex align-items-center justify-content-between pe-4">
        <h3 class="h3-text mb-0">{{ trans('escalation_group.title') }}</h3>
        <div class="d-flex gap-8">
        
            <button class="header-icon-btn header-icon-btn-sm btn-open-filter" type="button" data-bs-toggle="tooltip" title="{{ trans('escalation_group.filter') }}">
                ...
                <span class="b5-text opacity-50">{{ trans('escalation_group.filter') }}</span>
                <span class="filter-count-badge d-none" aria-label="{{ trans('escalation_group.active_filters') }}">0</span>
            </button>

            <button class="header-icon-btn-only header-icon-btn-only-sm btn-download" type="button" data-bs-toggle="tooltip" title="{{ trans('escalation_group.download_excel') }}">
                <svg viewBox="0 0 20 20" fill="none">
                    <path d="M10 3V13M10 13L6 9M10 13L14 9M3 17H17" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"></path>
                </svg>
            </button>

            <button class="header-icon-btn-only header-icon-btn-only-sm btn-users-import escalation-bulk-import" type="button" data-bs-toggle="tooltip" title="{{ trans('escalation_group.bulk_import') }}">
            ...
            @if(Auth::check())
                    <button class="amg-btn amg-btn-primary amg-btn-sm open-add-modal" type="button" data-bs-toggle="tooltip" title="{{ trans('escalation_group.add_group') }}">
                        ...
                        <span>{{ trans('escalation_group.add_group') }}</span>
                </button>
            @endif
        </div>
    </div>
                ...
                <div class="d-flex align-items-center gap-2 mb-1">
                    <div class="col-auto">
                        <select class="amg-table-pagination-dropdown userModulePageLenth user-list-page-length">
                            <option value="10" selected>{{ trans('escalation_group.table.show_10') }}</option>
                            <option value="25">{{ trans('escalation_group.table.show_25') }}</option>
                            <option value="50">{{ trans('escalation_group.table.show_50') }}</option>
                            <option value="100">{{ trans('escalation_group.table.show_100') }}</option>
                        </select>
                    </div>
                    ...
                    <div>
                        <div class="amg-list-searchbar">
                            ...
                            <input type="text" name="search" class="amg-list-searchbar__input user-list-search searchbox plain-search" placeholder="{{ trans('escalation_group.press_enter_with_Search') }}">
                        </div>
                    </div>
                    ...
                    <button class="amg-refresh-btn btn-reload-list btn-reload-list">
                        ...
                        <span>{{ trans('escalation_group.refresh') }}</span>
                    </button>
                </div>
                    ...
                    <div class="list-view-panel">
                        <div class="table-responsive">
                            <table id="mytable" class="table display app-data-table">
                                <thead>
                                    <tr>
                                        <th><h4>{{ trans('escalation_group.table.group_name') }}</h4></th>
                                        <th><h4>{{ trans('escalation_group.table.company') }}</h4></th>
                                        <th><h4>{{ trans('escalation_group.table.department') }}</h4></th>
                                        <th><h4>{{ trans('escalation_group.table.problem_category') }}</h4></th>
                                        <th><h4>{{ trans('escalation_group.table.sub_category') }}</h4></th>
                                        <th><h4>{{ trans('escalation_group.table.member_count') }}</h4></th>
                                        <th><h4>{{ trans('escalation_group.table.created_at') }}</h4></th>
                                        <th><h4>{{ trans('escalation_group.table.updated_at') }}</h4></th>
                                        <th><h4>{{ trans('escalation_group.table.action') }}</h4></th>
                                    </tr>
                                </thead>
                                <tbody>
                                </tbody>
    ...
    @include('tickets.escalation-group.add-modal')
    @include('tickets.escalation-group.filter')
</div>
@endsection
...
<style>
    ...
    .amg-modal.amg-filter-modal .modal-body{
...
@push('scripts')
    ...
    <script type="text/javascript" src="{!! CommonHelper::asset('js/tickets/escalation-group/index.js') !!}"></script>
    <script type="text/javascript">
        $(document).ready(function () {
        var config = new Object;
        config.url = new Object;

        config.url.getUserAllocationGroups = "{{ url('jx-get-escalation-groups') }}";
        config.url.history = "{{ url('auto-allocation-groups_history') }}";
        config.url.add = "{{ url('tickets/escalation-groups/store') }}";
        config.url.getDepartments = "{{url('departments/by-company/privilage')}}";
        config.url.manage_users = "{{ url('user-escalation-groups/manage-users') }}";
        config.url.getGroupDetails = "{{ url('jx-escalation-group-edit') }}";
        // config.url.getGroupDetails = "{{ url('jx-get-escalation-group') }}";
        config.url.editGroupDetails = "{{ url('jx-escalation-group-edit') }}";
        config.url.delete = "{{ url('jx-escalation-group-delete') }}";
        config.url.download_url = "{{ url('escalation-groups-export') }}";
        ...
        config.url.bulkImport = "{{ url('tickets/bulk-escalation-import') }}";
        ...
        config.translations = @json(trans('escalation_group'));

        new EscalationGroup(config);
            });
    </script>
@endpush
