...
@section('title', trans('projects.view.header'))
    ...
    <main class="main-content" id="mainContent">
        <div class="container-fluid px-0">
            <div class="header-actions-wrapper d-flex align-items-center justify-content-between px-4">
                <div class="mb-0">
                    <h3 class="h3-text mb-0">{{ trans('projects.view.page_heading') }}</h3>
                </div>
                <div class="d-flex gap-8">
                    <button class="header-icon-btn-only header-icon-btn-only-sm btn-download btn-projects-export"
                        title="{{ trans('projects.view.download') }}">
                    ...
                    <button class="header-icon-btn-only header-icon-btn-only-sm btn-import btn-projects-import"
                        title="{{ trans('projects.view.import_project') }}">
                    ...
                    <div class="column-toggle-wrapper" style="position: relative; display: inline-block;">
                        ...
                        <button class="header-icon-btn-only header-icon-btn-only-sm show-hide-columns"
                            title="{{ trans('projects.view.show_columns') }}">
                    ...
                    <button class="amg-btn amg-btn-primary open-add-modal" type="button" data-bs-toggle="tooltip"
                        ...
                        <span>{{ trans('projects.view.add') }}</span>
                    </button>
                </div>
        </div>
                    ...
                    <div class="card-body table-responsive">
                        <div class="d-flex align-items-center gap-2 mb-1" data-select2-id="select2-data-5-1om7">
                            <div id="customLengthContainer" class="col-auto" data-select2-id="select2-data-4-sbib">
                                <select id="showSelect"
                                    ...
                                    <option value="10" selected>{{ trans('projects.view.show') }} (10)</option>
                                    <option value="25">{{ trans('projects.view.show') }} (25)</option>
                                    <option value="50">{{ trans('projects.view.show') }} (50)</option>
                                    <option value="100">{{ trans('projects.view.show') }} (100)</option>
                                </select>
                            </div>
                            ...
                            <div style="width: 280px;">
                                <div class="amg-list-searchbar">
                                    ...
                                    <input type="text" class="amg-list-searchbar__input" id="tableSearch"
                                        placeholder="{{ trans('projects.view.search') }}...">
                                </div>
                            </div>
                            ...
                            <button class="amg-refresh-btn btn-reload-list">
                                ...
                                <span>{{ trans('projects.view.refresh') }}</span>
                            </button>
                        </div>
                        <table id="mytable" class="table display">
                            <thead>
                                <tr>
                                    <th>
                                        <h4>{{ trans('projects.table.id') }}</h4>
                                    </th>
                                    <th>
                                        <h4>{{ trans('projects.table.project_info') }}</h4>
                                    </th>
                                    <th>
                                        <h4>{{ trans('projects.table.project_head') }}</h4>
                                    </th>
                                    <th>
                                        <h4>{{ trans('projects.table.company') }}</h4>
                                    </th>
                                    <th>
                                        <h4>{{ trans('projects.table.start_date') }}</h4>
                                    </th>
                                    <th>
                                        <h4>{{ trans('projects.table.updated_at') }}</h4>
                                    </th>
                                    <th>
                                        <h4>{{ trans('projects.table.actions') }}</h4>
                                    </th>
                                </tr>
                            </thead>
                            <tbody></tbody>
        ...
        @include('projects.modal_html')
        @include('projects.description')
    </main>
@endsection

@push('css')
{{-- using cdn instead of local summernote since we are using bootstrap 5 and local summernote is using bootstrap 4 --}}
...
<script type="text/javascript">
		...
		config.url.list = "{{ url('projects/ajaxList') }}";
		config.url.add = "{{ url('projects/ajaxAdd') }}";
		config.url.edit = "{{ url('projects/ajaxEdit') }}";
		config.url.get = "{{ url('projects/ajaxGet') }}";
		config.url.delete = "{{ url('projects/ajaxDelete') }}";
		config.url.delete = "{{ url('projects/ajaxDelete') }}";
		config.url.project_import = "{{ url('projects/import') }}";
		config.url.download_url = "{{ url('projects/export-project-info') }}";
		config.getUsers = "{{ url('getUsers') }}";
		config.getCompanyUsers = "{{ url('getCompanyByUserAccess') }}";
		config.token = "{{ csrf_token() }}";
		config.permissions = {!! json_encode($permissionArray) !!};
    config.company_default = {!! json_encode($company) !!};

		config.translations = {
			add_new_project: '{{ trans('projects.config.add_new_project') }}',
			edit_project: '{{ trans('projects.config.edit_project') }}',
			save: '{{ trans('projects.config.save') }}',
			edit: '{{ trans('projects.config.edit') }}',
			delete: '{{ trans('projects.config.delete') }}',
			valid_value_search: '{{ trans('projects.config.valid_value_search') }}',
			please_enter_valid_search: '{{ trans('projects.config.please_enter_valid_search') }}',
      something_went_wrong_details: '{{ trans('projects.config.something_went_wrong_details') }}',
      something_went_wrong: '{{ trans('projects.config.something_went_wrong') }}',
      are_you_sure_delete: '{{ trans('projects.config.are_you_sure_delete') }}',
      project_added_successfully: '{{ trans('projects.config.project_added_successfully') }}',
      modal_yes_button: '{{ trans('projects.config.modal_yes_button') }}',
      modal_cancel_button: '{{ trans('projects.config.modal_cancel_button') }}',
      deleted_successfully: '{{ trans('projects.config.deleted_successfully') }}',
      deleted_fail: '{{ trans('projects.config.deleted_fail') }}',
		};
	</script>

	<script src="{!! CommonHelper::asset('js/common.js') !!}"></script>
	<script src="{!! CommonHelper::asset('js/support_validate.js') !!}"></script>
	<script src="{!! CommonHelper::asset('js/projects/index.js') !!}"></script>
@endpush
