...
* ------------------------------------------------------------
* Change Log:
* [1.0.3] - Changing the pdf download icon - prithvi - 7/28/26
* [1.0.2] - changes for the tooltip issues and spacing issue between search and refresh button - prithvi - 7/23/26
...
@section('title', trans('device_type.view.header'))
...
<main class="main-content" id="mainContent">
  <div class="container-fluid px-0">
    <div class="header-actions-wrapper d-flex align-items-center justify-content-between px-4">
      <div class="mb-0">
        <h3 class="h3-text mb-0">{{ trans('device_type.view.page_heading') }}</h3>
      </div>
      <div class="d-flex gap-8">
        <button class="header-icon-btn-only header-icon-btn-only-sm btn-download btn-devicetype-export"
          title="{{ trans('device_type.view.download') }}" data-bs-toggle="tooltip" data-bs-title="{{ trans('device_type.view.download') }}">
        ...
        <div class="column-toggle-wrapper" style="position: relative; display: inline-block;">
          ...
          <button class="header-icon-btn-only header-icon-btn-only-sm show-hide-columns" data-bs-toggle="tooltip" data-bs-title="{{ trans('device_type.view.show_columns') }}"
            title="{{ trans('device_type.view.show_columns') }}">
        ...
        <button class="amg-btn amg-btn-primary amg-btn-sm open-add-modal" type="button" data-bs-toggle="tooltip"
          title="{{ trans('device_type.view.add_button') }}">
          ...
          <span>{{ trans('device_type.view.add_button') }}</span>
        </button>
      </div>
    </div>
        ...
        <div class="card-body table-responsive">
          <div class="d-flex align-items-center gap-2 mb-1" data-select2-id="select2-data-5-1om7">
            <div id="customLengthContainer" class="col-auto" data-select2-id="select2-data-4-sbib">
              <select id="showSelect"
                ...
                <option value="10" selected>{{ trans('device_type.view.show') }} (10)</option>
                <option value="25">{{ trans('device_type.view.show') }} (25)</option>
                <option value="50">{{ trans('device_type.view.show') }} (50)</option>
                <option value="100">{{ trans('device_type.view.show') }} (100)</option>
              </select>
            </div>
            ...
            <div>
              <div class="amg-list-searchbar">
                ...
                <input type="text" class="amg-list-searchbar__input" id="tableSearch"
                  placeholder="{{ trans('device_type.view.search') }}...">
              </div>
            </div>
            ...
            <button class="amg-refresh-btn btn-reload-list">
              ...
              <span>{{ trans('device_type.view.refresh') }}</span>
            </button>
          </div>
          <table id="mytable" class="table display">
            <thead>
              <tr>
                <th>
                  <h4>{{ trans('device_type.table.id') }}</h4>
                </th>
                <th>
                  <h4>{{ trans('device_type.table.device_type') }}</h4>
                </th>
                <th>
                  <h4>{{ trans('device_type.table.actions') }}</h4>
                </th>
              </tr>
            </thead>
            <tbody></tbody>
  ...
  @include('device_type.modal_html')
</main>
@endsection
...
<script src="{!! CommonHelper::asset('js/device_type/index.js') !!}"></script>

<script type="text/javascript">
  var config = new Object;
  config.url = new Object;
  config.translations = {
    Refresh_List: '{{ trans('device_type.config.refresh_list') }}',
    Search: '{{ trans('device_type.config.search') }}',
    edit: '{{ trans('device_type.config.edit') }}',
    save: '{{ trans('device_type.config.save') }}',
    delete: '{{ trans('device_type.config.delete') }}',
    edit_device: '{{ trans('device_type.config.edit_device') }}',
    add_device: '{{ trans('device_type.config.add_device') }}',
    please_enter_valid_search: '{{ trans('device_type.config.please_enter_valid_search') }}',
    something_went_wrong_details: '{{ trans('device_type.config.something_went_wrong_details') }}',
    save_changes: '{{ trans('device_type.config.save_changes') }}',
    something_went_wrong_some_time: '{{ trans('device_type.config.something_went_wrong_some_time') }}',
    confirm_delete: '{{ trans('device_type.config.confirm_delete') }}'
  };
  config.url.devicetype = "{{ url('jx-deviceTypes') }}";
  config.url.add = "{{ url('jx-devicetype-add') }}";
  config.url.edit = "{{ url('jx-devicetype-edit') }}";
  config.url.get = "{{ url('jx-get-devicetype') }}";
  config.url.delete = "{{ url('jx-devicetype-delete') }}";
  config.url.download_url = "{{ url('export-devicetype') }}";
  config.permissions = {!!json_encode($permissionArray) !!};
  config.token = "{{ csrf_token() }}";
  new DeviceType(config);
</script>
@endpush