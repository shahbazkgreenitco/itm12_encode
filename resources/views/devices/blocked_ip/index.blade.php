...
<section class="content">
        ...
        <div class="header-actions-wrapper d-flex align-items-center justify-content-between pe-4">
            ...
            <div class="d-flex gap-8">
                ...
                <button class="amg-btn amg-btn-primary amg-btn-sm open-add-modal" type="button" data-bs-toggle="tooltip"
                    title="{{ trans('block_ip.block_ip_fields.add_new_ip') }}">
                    ...
                    <span>{{ trans('block_ip.block_ip_fields.add_new_ip') }}</span>
                </button>
            </div>
        </div>
                        ...
                        <div class="d-flex align-items-center gap-2 mb-1">
                            <div class="col-auto">
                                <select class="amg-table-pagination-dropdown userModulePageLenth user-list-page-length">
                                    <option value="10" selected>{{ trans('block_ip.block_ip_fields.show') }} (10)
                                    </option>
                                    <option value="25">{{ trans('block_ip.block_ip_fields.show') }} (25)</option>
                                    <option value="50">{{ trans('block_ip.block_ip_fields.show') }} (50)</option>
                                    <option value="100">{{ trans('block_ip.block_ip_fields.show') }} (100)</option>
                                </select>
                            </div>
                            ...
                            <div>
                                <div class="amg-list-searchbar">
                                    ...
                                    <input type="text" class="amg-list-searchbar__input user-list-search"
                                        placeholder="{{ trans('block_ip.block_ip_fields.search_placeholder') }}">
                                </div>
                            </div>
                            ...
                            <button class="amg-refresh-btn btn-reload-list">
                                ...
                                <span>{{ trans('block_ip.block_ip_fields.refresh_list') }}</span>
                            </button>
                        </div>
                            ...
                            <div class="table-responsive">
                                <table id="mytable" class="table display app-data-table">
                                    <thead>
                                        <tr>
                                            <th>#</th>
                                            <th>{{ trans("block_ip.block_ip_fields.type") }}</th>
                                            <th>{{ trans("block_ip.block_ip_fields.device_ip") }}</th>
                                            <th>{{ trans("block_ip.block_ip_fields.created_at") }}</th>
                                            <th>{{ trans("block_ip.block_ip_fields.updated_at") }}</th>
                                            <th>{{ trans("block_ip.block_ip_fields.action") }}</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                    </tbody>
    ...
    @include('devices.blocked_ip.modal_html')

</section>
@endsection

@push('scripts')
<script type="text/javascript" src="{!! CommonHelper::asset('js/devices/blocked_ip/index.js') !!}"></script>
<script type="text/javascript">
    $(document).ready(function() {
		var config = {};
		config.url = {};
		config.url.block = "{{ url('jx-blockIp') }}";
		config.url.add = "{{ url('blockIp/add') }}";
		config.url.get = "{{ url('blockIp/get') }}";
		config.url.edit = "{{ url('blockIp/edit') }}";
		config.getIpByAjax = "{{ url('getIpByAjax') }}";
		config.url.delete = "{{ url('blockIp/delete') }}";
		config.DownloadUrl = "{{ url('export-blockIp') }}";
		config.token = "{{ csrf_token() }}";

		config.translations = {
            press_enter_with_search: '{{ trans('block_ip.block_ip_fields.press_enter_with_search') }}',
			please_enter_valid_search: '{{ trans('block_ip.block_ip_fields.please_enter_valid_search') }}',
			something_went_wrong: '{{ trans('block_ip.block_ip_fields.something_went_wrong') }}',
			Search: '{{ trans('block_ip.block_ip_fields.Search') }}',
			refresh_list: '{{ trans('block_ip.block_ip_fields.refresh_list') }}',
			select_the_ip: '{{ trans('block_ip.block_ip_fields.select_the_ip') }}',
			save: '{{ trans('block_ip.block_ip_fields.save') }}',
			add_new_ip: '{{ trans('block_ip.block_ip_fields.add_new_ip') }}',
			Download :'{{ trans('block_ip.block_ip_fields.download') }}',
            Edit: '{{ trans('block_ip.block_ip_fields.edit_ip') }}',
            Delete:'{{ trans('block_ip.block_ip_fields.delete') }}',
            success: '{{ trans('block_ip.block_ip_fields.success') }}',
            error: '{{ trans('block_ip.block_ip_fields.error') }}',
            delete_ip_msg: '{{ trans('block_ip.block_ip_fields.delete_ip_msg') }}',
            yes_delete: '{{ trans('block_ip.block_ip_fields.yes_delete') }}',
            cancel: '{{ trans('block_ip.block_ip_fields.cancel') }}',
            delete_success: '{{ trans('block_ip.block_ip_fields.delete_success') }}',
            delete_failed: '{{ trans('block_ip.block_ip_fields.delete_failed') }}',
		};
	
		config.permissions = {!! json_encode($permissionArray) !!};
        new BlockedIp(config);
    });
</script>
@endpush