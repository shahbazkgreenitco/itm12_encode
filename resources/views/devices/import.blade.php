...
------------------------------------------------------------
Change Log:
[1.0.0] - Initial version - Modern UI implementation
------------------------------------------------------------
*/
--}}
@extends('layouts.layout1')
@section('title', trans('devices.device_import.import_devices'))
@section('content')
<style>
    :root {
        --primary: #1a2e5a;
        ...
        <div class="py-3">
            <div class="container-fluid">
                <button onclick="location.href='{{ url('devices') }}'"
                    ...
                    <h2 class="h2-text mb-0">{{ trans('devices.device_import.bulk_import') }}</h2>
                </button>
            </div>
        </div>
    </div>
            ...
            <ul class="nav nav-underline" id="myTab" role="tablist">
                <li class="nav-item">
                    <a class="nav-link active" id="info-tab-link" data-bs-toggle="tab" href="#info-tab" role="tab">
                        <span>{{ trans('devices.device_import.bulk_import') }}</span>
                    </a>
                </li>
                <li class="nav-item">
                    <a class="nav-link" id="import-info-tab-link" data-bs-toggle="tab" href="#import-info-tab"
                        role="tab">
                        <span>{{ trans('devices.device_import.device_import_info') }}</span>
                    </a>
                </li>
            </ul>
        </div>
            ...
            <div class="tab-pane fade show active container-fluid pt-3 pb-5" id="info-tab">
                ...
                <div class="alert customize-alert alert-dismissible alert-light-success bg-success-subtle text-success fade show remove-close-icon"
                    ...
                    <div class="d-flex align-items-center me-3 gap-3 me-md-0">
                        ...
                        {{ session('success', 0) }}
                        {{ trans('devices.device_import.import_success') }}
                    </div>
                </div>
                @endif
                ...
                <div class="alert customize-alert alert-dismissible alert-light-danger bg-danger-subtle text-danger fade show remove-close-icon"
                    ...
                    <div class="d-flex align-items-start me-3 gap-2 me-md-0">
                        ...
                        <div>
                            {{ session('fail', 0) }}
                            {{ trans('devices.device_import.import_failed') }}
                ...
                <div class="alert customize-alert alert-dismissible alert-light-warning bg-warning-subtle text-warning fade show remove-close-icon"
                    ...
                    <div class="d-flex align-items-center me-3 gap-3 me-md-0">
                        ...
                        {{ trans('devices.device_import.no_device_imported') }}
                    </div>
                </div>
                @endif
                @endif
                            ...
                            <div class="drop-zone" id="dropZone">
                                ...
                                <div id="defaultState">
                                    ...
                                    <p class="drag-text mt-3 mb-2 b1-text">{{
                                        trans("devices.device_import.drag_drop_file") }}</p>
                                    <button class="amg-btn amg-btn-secondary" style="min-width:203px;">
                                        ...
                                        <span>{{ trans("devices.device_import.upload_file_btn") }}</span>
                                    </button>
                                    <span class="file-label b1-text">{{ trans("devices.device_import.no_file_selected")
                                        }}</span>
                                </div>
                                ...
                                <div id="selectedState" style="display:none;">
                                    ...
                                    <p class="file-selected-name mb-2 mt-3" id="selectedFileName">{{
                                        trans('devices.device_import.format_excel_sheet') }}</p>
                                    <button class="amg-btn amg-btn-primary" style="min-width:203px" type="button"
                                        ...
                                        <span>{{ trans('devices.device_import.save_import') }}</span>
                                    </button>
                                    <button class="amg-btn amg-btn-link amg-btn-link-plain mt-2" id="btnRemove">
                                        {{ trans('devices.device_import.remove_file') }}
                                    </button>
                                </div>
                            </div>
                        ...
                        <div class="col-12 col-sm-7">
                            <div class="right-panel h-100">
                                <a class="amg-btn amg-btn-white amg-btn-lg"
                                    href="{!! CommonHelper::asset('samples/DeviceImportFormat.xlsx') !!}"
                                    target="_blank">
                                    ...
                                    <span class="font-weight-normal">{{ trans('devices.device_import.download_template')
                                        }}</span>
                                </a>
                                {{-- @if(!empty($customFieldset))
                                <a class="amg-btn amg-btn-white amg-btn-lg ms-2"
                                    href="{{ url('download-custom-fields-code') }}" target="_blank">
                                    <svg width="14" height="17" viewBox="0 0 14 17" fill="currentColor"
                                        xmlns="http://www.w3.org/2000/svg">
                                        <path
                                            d="M9 3.99935L5 3.99935C4.45 3.99935 4 4.44935 4 4.99935L4 9.99935L2.41 9.99935C1.52 9.99935 1.07 11.0794 1.7 11.7094L6.29 16.2994C6.38251 16.3921 6.4924 16.4656 6.61338 16.5158C6.73435 16.566 6.86403 16.5918 6.995 16.5918C7.12597 16.5918 7.25565 16.566 7.37662 16.5158C7.4976 16.4656 7.60749 16.3921 7.7 16.2994L12.29 11.7094C12.92 11.0794 12.48 9.99935 11.59 9.99935L10 9.99935L10 4.99935C10 4.44935 9.55 3.99935 9 3.99935ZM13 1.99935L1 1.99935C0.450001 1.99935 1.31505e-06 1.54935 1.36313e-06 0.999352C1.41122e-06 0.449353 0.450001 -0.000647776 1 -0.000647728L13 -0.000646679C13.55 -0.000646631 14 0.449354 14 0.999353C14 1.54935 13.55 1.99935 13 1.99935Z"
                                    ...
                                    <span class="font-weight-normal">{{
                                        trans('devices.device_import.download_custom_field_codes') }}</span>
                                </a>
                                @endif --}}
                            </div>
                        </div>
                    ...
                    <div class="col-12 px-5">
                        ...
                        <div class="instructions-card mt-3">
                            ...
                            <h4 class="h4-text" style="color:#7F7F7F">
                                {{ trans("devices.device_import.upload_notes_title") }}
                            </h4>
                            ...
                            <div class="ps-3 ps-sm-4">
                                ...
                                <div class="card-scroll-body" id="cardScrollBody">
                                    <ul class="ps-3">
                                        <li>
                                            <span class="s1-text">{{ trans("devices.device_import.field_device_tag")
                                                }}</span>
                                            <span class="b1-text text-muted">{{ trans("devices.device_import.tag_field")
                                                }}</span>
                                        </li>
                                        <li>
                                            <span class="s1-text me-3">{{ trans("devices.device_import.field_company")
                                                }} <span class="mandatory"></span></span>
                                            <span class="b1-text text-muted">{{
                                                trans("devices.device_import.company_field") }}</span>
                                        </li>
                                        <li>
                                            <span class="s1-text">{{ trans("devices.device_import.field_device_name")
                                                }}</span>
                                            <span class="b1-text text-muted">{{
                                                trans("devices.device_import.device_field") }}</span>
                                        </li>
                                        <li>
                                            <span class="s1-text me-3">{{ trans("devices.device_import.field_serial") }}
                                                <span class="mandatory"></span></span>
                                            <span class="b1-text text-muted">{{
                                                trans("devices.device_import.serial_field") }}</span>
                                        </li>
                                        <li>
                                            <span class="s1-text">{{ trans("devices.device_import.field_product_number")
                                                }}</span>
                                            <span class="b1-text text-muted">{{
                                                trans("devices.device_import.product_number_field") }}</span>
                                        </li>
                                        <li>
                                            <span class="s1-text me-3">{{
                                                trans("devices.device_import.field_manufacturer") }} <span
                                                    class="mandatory"></span></span>
                                            <span class="b1-text text-muted">{{
                                                trans("devices.device_import.manufacture_field") }}</span>
                                        </li>
                                        <li>
                                            <span class="s1-text me-3">{{ trans("devices.device_import.field_model") }}
                                                <span class="mandatory"></span></span>
                                            <span class="b1-text text-muted">{{
                                                trans("devices.device_import.model_field") }}</span>
                                        </li>
                                        <li>
                                            <span class="s1-text me-3">{{ trans("devices.device_import.field_category")
                                                }} <span class="mandatory"></span></span>
                                            <span class="b1-text text-muted">{{
                                                trans("devices.device_import.category_field") }}</span>
                                        </li>
                                        <li>
                                            <span class="s1-text me-3">{{
                                                trans("devices.device_import.field_device_status") }} <span
                                                    class="mandatory"></span></span>
                                            <span class="b1-text text-muted">{{
                                                trans("devices.device_import.status_field_device_import") }}</span>
                                        </li>
                                        <li>
                                            <span class="s1-text">{{ trans("devices.device_import.field_checkout_for")
                                                }}</span>
                                            <span class="b1-text text-muted">{{
                                                trans("devices.device_import.assigned_for_field_device_import")
                                                }}</span>
                                        </li>
                                        <li>
                                            <span class="s1-text">{{ trans("devices.device_import.field_checkout_to")
                                                }}</span>
                                            <span class="b1-text text-muted">{{
                                                trans("devices.device_import.assigned_to_field_device_import") }}</span>
                                        </li>
                                        <li>
                                            <span class="s1-text me-3">{{
                                                trans("devices.device_import.field_checkout_reason") }}
                                                @if(in_array(strtolower(trim(config('app.client'))), ['knightfrank',
                                                'rolepermission']))
                                                <span class="mandatory"></span>
                                                @endif
                                            </span>
                                            <span class="b1-text text-muted">{{
                                                trans("devices.device_import.checkout_reason_field") }}</span>
                                        </li>
                                        <li>
                                            <span class="s1-text">{{
                                                trans("devices.device_import.field_expected_checkin") }}</span>
                                            <span class="b1-text text-muted">{{
                                                trans("devices.device_import.expected_checkin_field") }}</span>
                                        </li>
                                        <li>
                                            <span class="s1-text">{{
                                                trans("devices.device_import.field_allocation_type") }}</span>
                                            <span class="b1-text text-muted">{{
                                                trans("devices.device_import.allocation_type_field_device_import")
                                                }}</span>
                                        </li>
                                        <li>
                                            <span class="s1-text">{{ trans("devices.device_import.field_department")
                                                }}</span>
                                            <span class="b1-text text-muted">{{
                                                trans("devices.device_import.department_field") }}</span>
                                        </li>
                                        <li>
                                            <span class="s1-text me-3">{{ trans("devices.device_import.field_location")
                                                }} <span class="mandatory"></span></span>
                                            <span class="b1-text text-muted">{{
                                                trans("devices.device_import.location_field") }}</span>
                                        </li>
                                        <li>
                                            <span class="s1-text">{{ trans("devices.device_import.field_place")
                                                }}</span>
                                            <span class="b1-text text-muted">{{
                                                trans("devices.device_import.place_field") }}</span>
                                        </li>
                                        <li>
                                            <span class="s1-text">{{ trans("devices.device_import.field_purchase_date")
                                                }}</span>
                                            <span class="b1-text text-muted">{{
                                                trans("devices.device_import.purchase_date_field") }}</span>
                                        </li>
                                        <li>
                                            <span class="s1-text">{{
                                                trans("devices.device_import.field_currency_format") }}</span>
                                            <span class="b1-text text-muted">{{
                                                trans("devices.device_import.purchase_currency_format_field") }}</span>
                                        </li>
                                        <li>
                                            <span class="s1-text">{{ trans("devices.device_import.field_purchase_cost")
                                                }}</span>
                                            <span class="b1-text text-muted">{{
                                                trans("devices.device_import.purchase_field") }}</span>
                                        </li>
                                        <li>
                                            <span class="s1-text">{{
                                                trans("devices.device_import.field_purchase_reference") }}</span>
                                            <span class="b1-text text-muted">{{
                                                trans("devices.device_import.purchase_reference_field") }}</span>
                                        </li>
                                        <li>
                                            <span class="s1-text">{{ trans("devices.device_import.field_uuid") }}</span>
                                            <span class="b1-text text-muted">{{
                                                trans("devices.device_import.uuid_field") }}</span>
                                        </li>
                                        <li>
                                            <span class="s1-text">{{
                                                trans("devices.device_import.field_warranty_start_date") }}</span>
                                            <span class="b1-text text-muted">{{
                                                trans("devices.device_import.warranty_start_date_field") }}</span>
                                        </li>
                                        <li>
                                            <span class="s1-text">{{
                                                trans("devices.device_import.field_warranty_end_date") }}</span>
                                            <span class="b1-text text-muted">{{
                                                trans("devices.device_import.warranty_end_date_field") }}</span>
                                        </li>
                                        <li>
                                            <span class="s1-text">{{ trans("devices.device_import.field_warranty")
                                                }}</span>
                                            <span class="b1-text text-muted">{{
                                                trans("devices.device_import.warranty_field") }}</span>
                                        </li>
                                        <li>
                                            <span class="s1-text">{{
                                                trans("devices.device_import.field_amc_expire_date") }}</span>
                                            <span class="b1-text text-muted">{{
                                                trans("devices.device_import.amc_expire_date_field") }}</span>
                                        </li>
                                        <li>
                                            <span class="s1-text">{{ trans("devices.device_import.field_amc_supplier")
                                                }}</span>
                                            <span class="b1-text text-muted">{{
                                                trans("devices.device_import.amc_supplier_field") }}</span>
                                        </li>
                                        <li>
                                            <span class="s1-text">{{ trans("devices.device_import.field_notes")
                                                }}</span>
                                            <span class="b1-text text-muted">{{
                                                trans("devices.device_import.note_field") }}</span>
                                        </li>
                                        <li>
                                            <span class="s1-text">{{ trans("devices.device_import.field_order_number")
                                                }}</span>
                                            <span class="b1-text text-muted">{{
                                                trans("devices.device_import.order_number_field") }}</span>
                                        </li>
                                        <li>
                                            <span class="s1-text">{{ trans("devices.device_import.field_asset_owner")
                                                }}</span>
                                            <span class="b1-text text-muted">{{
                                                trans("devices.device_import.asset_owner_field") }}</span>
                                        </li>
                                        <li>
                                            <span class="s1-text">{{ trans("devices.device_import.field_supplier")
                                                }}</span>
                                            <span class="b1-text text-muted">{{
                                                trans("devices.device_import.supplier_field") }}</span>
                                        </li>
                                        <li>
                                            <span class="s1-text">{{ trans("devices.device_import.field_ip") }}</span>
                                            <span class="b1-text text-muted">{{ trans("devices.device_import.ip_field")
                                                }}</span>
                                        </li>
                                        <li>
                                            <span class="s1-text">{{ trans("devices.device_import.field_mac") }}</span>
                                            <span class="b1-text text-muted">{{ trans("devices.device_import.mac_field")
                                                }}</span>
                                        </li>
                                        <li>
                                            <span class="s1-text">{{ trans("devices.device_import.field_device_type")
                                                }}</span>
                                            <span class="b1-text text-muted">{{
                                                trans("devices.device_import.device_type_field") }}</span>
                                        </li>
                                        <li>
                                            <span class="s1-text">{{ trans("devices.device_import.field_device_from")
                                                }}</span>
                                            <span class="b1-text text-muted">{{
                                                trans("devices.device_import.device_from_field") }}</span>
                                        </li>
                                        <li>
                                            <span class="s1-text">{{
                                                trans("devices.device_import.field_contract_reference") }}</span>
                                            <span class="b1-text text-muted">{{
                                                trans("devices.device_import.contract_agreement_field") }}</span>
                                        </li>
                                        <li>
                                            <span class="s1-text">{{ trans("devices.device_import.field_stock_place")
                                                }}</span>
                                            <span class="b1-text text-muted">{{
                                                trans("devices.device_import.stock_place_field") }}</span>
                                        </li>
                                        <li>
                                            <span class="s1-text">{{ trans("devices.device_import.field_requestable")
                                                }}</span>
                                            <span class="b1-text text-muted">{{
                                                trans("devices.device_import.requestable_field") }}</span>
                                        </li>
                                        <li>
                                            <span class="s1-text">{{ trans("devices.device_import.field_high_priority")
                                                }}</span>
                                            <span class="b1-text text-muted">{{
                                                trans("devices.device_import.high_priority_field") }}</span>
                                        </li>
                                        <li>
                                            <span class="s1-text me-3">{{
                                                trans("devices.device_import.field_sez_device") }}</span>
                                            <span class="b1-text text-muted">{{
                                                trans("devices.device_import.sez_device_field") }}</span>
                                        </li>
                                        @if(isset($settings->accessories_custom_fieldset_id) &&
                                        $settings->accessories_custom_fieldset_id != null)
                                        <li>
                                            <strong>{{ trans("devices.device_import.field_custom_fields") }}:</strong>
                                            <p class="b1-text text-muted">{{
                                                trans("devices.device_import.custom_field_val") }}</p>
                                        </li>
                                        @endif
                                    </ul>
                                    <p><i>{{ trans("devices.device_import.import_footer") }}</i></p>
                                </div>

                                <a class="amg-btn amg-btn-link amg-btn-link-plain s1-text mt-2" id="readMoreBtn">
                                    {{ trans("devices.device_import.read_more") }}
                                </a>
                            </div>

                            <h4 class="h4-text" style="color:#7F7F7F">
                                {{ trans('devices.device_import.do_not_change_header') }}
                            </h4>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="card-body">
                        <div class="d-flex align-items-center gap-2 mb-4" data-select2-id="select2-data-5-1om7">
                            ...
                            <div id="customLengthContainer" class="col-auto" data-select2-id="select2-data-4-sbib">
                                <select id="showSelect" class="showSelect">
                                    <option value="10" selected>{{ trans('devices.device_import.show') }}(10)</option>
                                    <option value="25">{{ trans('devices.device_import.show') }}(25)</option>
                                    <option value="50">{{ trans('devices.device_import.show') }}(50)</option>
                                    <option value="100">{{ trans('devices.device_import.show') }}(100)</option>
                                </select>
                            </div>
                            ...
                            <div>
                                <div class="amg-list-searchbar">
                                    ...
                                    <input type="text" class="amg-list-searchbar__input" id="tableSearch"
                                        placeholder="{{ trans('devices.device_import.search_placeholder') }}">
                                </div>
                            </div>
                            ...
                            <button class="amg-refresh-btn btn-reload-list">
                                ...
                                <span>{{ trans('devices.device_import.refresh') }}</span>
                            </button>
                        </div>
                        <table id="tblHistory" class="table display">
                            <thead>
                                <tr>
                                    <th>
                                        <h4>{{ trans('devices.device_import.id') }}</h4>
                                    </th>
                                    <th>
                                        <h4>{{ trans('devices.device_import.document_name') }}</h4>
                                    </th>
                                    <th>
                                        <h4>{{ trans('devices.device_import.total_upload') }}</h4>
                                    </th>
                                    <th>
                                        <h4>{{ trans('devices.device_import.total_success') }}</h4>
                                    </th>
                                    <th>
                                        <h4>{{ trans('devices.device_import.total_failure') }}</h4>
                                    </th>
                                    <th>
                                        <h4>{{ trans('devices.device_import.updated_at') }}</h4>
                                    </th>
                                    <th>
                                        <h4>{{ trans('devices.device_import.username') }}</h4>
                                    </th>
                                    <th>
                                        <h4>{{ trans('devices.device_import.actions') }}</h4>
                                    </th>
                                </tr>
                            </thead>
                            <tbody></tbody>
...
<script type="text/javascript" src="{!! CommonHelper::asset('js/devices/device_import.js') !!}"></script>

<script>
    var config = {};
    config.device_import_info = "{{ url('devices/bulk/delete-info') }}";
    config.document_download = "{{ url('uploads/bulk_documents') }}";
    config.token = "{{ csrf_token() }}";
    config.translations = {
        Download_Document: '{{ trans('devices.device_import.download_document') }}',
        Please_upload_valid_xlsx: '{{ trans('devices.device_import.please_upload_valid_xlsx') }}',
        press_enter_with_Search: '{{ trans('devices.device_import.press_enter_with_Search') }}',
        search: '{{ trans('devices.device_import.search') }}',
        Refresh_List: '{{ trans('devices.device_import.Refresh_List') }}',
        import_file: '{{ trans('devices.device_import.import_file') }}',
        please_enter_valid_search: '{{ trans('devices.device_import.please_enter_valid_search') }}',
        download: '{{ trans('devices.device_import.download_document') }}',
    };
    ...
    new MyApp(config);
        ...
        $('#btnSave').on('click', function (e) {
            ...
            if (!['xlsx'].includes(ext)) {
                swal.fire('Invalid file type. Please upload an .xlsx file.');
                return;
            }
        ...
        $('#dropZone').on('drop', function (e) {
            ...
            if (files.length > 0) {
                ...
                if (!['xlsx'].includes(ext)) {
                    swal.fire('Invalid file type. Please drop an .xlsx file.');
                    return;
                }
                droppedFile = files[0];
