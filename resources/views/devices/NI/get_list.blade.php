...
@section('title', "Network inventory")
@section('content')
<section class="content">
    <div id="main-user-list-wrapper">
        <div class="header-actions-wrapper d-flex align-items-center justify-content-between pe-4">
            <h3 class="h3-text mb-0">{{ trans('devices/ni/ni_listing.network_inventory_device') }}</h3>
            <div class="d-flex gap-8">
                {{-- Filter --}}
                <button class="header-icon-btn header-icon-btn-sm btn-open-filter" type="button" data-bs-toggle="tooltip" title="{{ trans('devices/ni/ni_listing.filter') }}">
                    ...
                    <span class="b6-text opacity-50">{{ trans('devices/ni/ni_listing.filter') }}</span>
                    <span class="filter-count-badge d-none" aria-label="Active filters">0</span>
                </button>

                {{-- Download Excel --}}
                @if(Auth::user()->hasPermissionTo('NIDownload'))
                    <button class="header-icon-btn-only header-icon-btn-only-sm btn-ni-export"  type="button" data-bs-toggle="tooltip"
                        title="{{ trans('devices/ni/ni_listing.download') }}">
                ...
                {{-- Download pdf --}}
                @if(Auth::user()->hasPermissionTo('NIDownload'))
                    <button class="header-icon-btn-only header-icon-btn-only-sm btn-ni-export-pdf" type="button" data-bs-toggle="tooltip" title="{{ trans('devices/ni/ni_listing.download_pdf') }}">
                        <svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M4 21.4V2.6C4 2.26863 4.26863 2 4.6 2H16.2515C16.4106 2 16.5632 2.06321 16.6757 2.17574L19.8243 5.32426C19.9368 5.43679 20 5.5894 20 5.74853V21.4C20 21.7314 19.7314 22 19.4 22H4.6C4.26863 22 4 21.7314 4 21.4Z" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                            <path d="M16 2V5.4C16 5.73137 16.2686 6 16.6 6H20" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                        </svg>
                ...
                <div class="card rounded-0">
                    <div class="card-body pt-3">
                        <div class="d-flex align-items-center gap-2 mb-1">
                            <div class="col-auto">
                                <select class="amg-table-pagination-dropdown userModulePageLenth ni-list-page-length">
                                    <option value="10" selected>{{ trans("devices/ni/ni_listing.show") }} (10)</option>
                                    <option value="25">{{  trans("devices/ni/ni_listing.show") }} (25)</option>
                                    <option value="50">{{  trans("devices/ni/ni_listing.show") }} (50)</option>
                                    <option value="100">{{  trans("devices/ni/ni_listing.show") }} (100)</option>
                                </select>
                            </div>
                            ...
                            <div>
                                <div class="amg-list-searchbar">
                                    ...
                                    <input type="text" class="amg-list-searchbar__input ni-list-search"
                                        placeholder="{{ trans("devices/ni/ni_listing.search") }}">
                                </div>
                            </div>
                            ...
                            <button class="amg-refresh-btn btn-reload-list">
                                ...
                                <span>{{ trans("devices/ni/ni_listing.refresh") }}</span>
                            </button>
                        </div>

                        <div class="js-ni-list-view-panel">
                            <div class="table-responsive">
                                <table id="mytable" class="amg-datatable table display app-data-table">
                                    <thead>
                                        <tr>
                                            <th><h4>{{ trans('devices/ni/ni_listing.device_tag') }}</h4></th>
                                            <th><h4>{{ trans('devices/ni/ni_listing.device_info') }}</h4></th>
                                            <th><h4>{{ trans('devices/ni/ni_listing.domain') }}</h4></th>
                                            <th><h4>{{ trans('devices/ni/ni_listing.os') }}</h4></th>
                                            <th><h4>{{ trans('devices/ni/ni_listing.disk_storage_and_ram_size') }}</h4></th>
                                            <th><h4>{{ trans('devices/ni/ni_listing.network_info') }}</h4></th>
                                            <th><h4>{{ trans('devices/ni/ni_listing.updated_on') }}</h4></th>
                                            <th><h4>{{ trans('devices/ni/ni_listing.action') }}</h4></th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                    </tbody>
    ...
    @include('devices.NI.filter_modal');
...
<script>
    var config = {};
    	...
    	config.manufacturers = {!! json_encode($vd->manufacturers) !!};
		config.models = {!! json_encode($vd->models) !!};
		config.oss = {!! json_encode($vd->oss) !!};
		config.computerSystemTypes = {!! json_encode($vd->computerSystemTypes) !!};
		config.processorNames = {!! json_encode($vd->processorNames) !!};
        config.version = {!! json_encode($vd->version) !!};
        config.internal_place = {!! json_encode($vd->internal_places) !!};
        
        config.url = {
            list: "{{ url('devices/network_inventory/jx-list') }}",
            download_url: "{{ url('devices/network_inventory/export-inventory-details') }}",
            download_pdf_url : "{{ url('devices/network_inventory/jx-export-pdf-format') }}",
            device_info: "{{ url('device/info') }}"
        };
        config.token = "{{ csrf_token() }}";
        config.datatable_translations=  @json(trans('datatable.datatable'));
        config.translations = {
            view: '{{ trans("devices/ni/ni_listing.view") }}',
            virtual_device: '{{ trans("devices/ni/ni_listing.virtual_device") }}',
            name: '{{ trans("devices/ni/ni_listing.name") }}',
            manufacturer: '{{ trans("devices/ni/ni_listing.manufacturer") }}',
            model: '{{ trans("devices/ni/ni_listing.model") }}',
            serial_no: '{{ trans("devices/ni/ni_listing.serial_no") }}',
            processor_name: '{{ trans("devices/ni/ni_listing.processor_name") }}',
            system_type: '{{ trans("devices/ni/ni_listing.system_type") }}',
            ip_address: '{{ trans("devices/ni/ni_listing.ip_address") }}',
            mac_address: '{{ trans("devices/ni/ni_listing.mac_address") }}',
            ram_size: '{{ trans("devices/ni/ni_listing.ram_size") }}',
            hard_disk_size: '{{ trans("devices/ni/ni_listing.hard_disk_size") }}',
            ni_export_devices: '{{ trans("devices/ni/ni_listing.ni_export_devices") }}',
            network_inventory_devices_inventory_export_report: '{{ trans("devices/ni/ni_listing.network_inventory_devices_inventory_export_report") }}'
        };
</script>
<script src="{!! CommonHelper::asset('js/devices/network_inventories/get_list.js') !!}"></script>
@endpush