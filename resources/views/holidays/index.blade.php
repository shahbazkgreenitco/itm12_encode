...
@section('title', trans("config.holiday_fields.holidays"))
@section('content')
<div id="main-user-list-wrapper">
    <section class="content">    
        <div class="header-actions-wrapper d-flex align-items-center justify-content-between pe-4">
                <h3 class="h3-text mb-0">{{ trans("config.holiday_fields.holidays") }}</h3>
                <div class="d-flex gap-8">         
                    {{-- Download Excel --}}
                    <button class="header-icon-btn-only header-icon-btn-only-sm btn-holiday-export" type="button" data-bs-toggle="tooltip" title="{{ trans('config.allocation_type.download') }}">
                    ...
                    @if(Auth::user()->hasPermissionTo('HolidaysAdd'))
                        <button class="amg-btn amg-btn-primary amg-btn-sm open-add-modal" type="button" data-bs-toggle="tooltip" title="{{ trans('config.holiday_fields.add_holiday') }}">
                                ...
                                <span>{{ trans('config.holiday_fields.add_holiday') }}</span>
                        </button>
                    @endif
                </div>
        </div>
                        ...
                        <div class="d-flex align-items-center gap-2 mb-1">                       
                            ...
                            <div>
                                <div class="amg-list-searchbar">
                                    ...
                                    <input type="text" class="amg-list-searchbar__input holiday-list-search searchbox plain-search" id="holiday-list-search" placeholder="Search...">
                                </div>
                            </div>
                                ...
                                <table id="mytable" class="table display app-data-table">
                                    <thead>
                                        <tr>
                                            ...
                                            <th><h4>{{ trans("config.holiday_fields.name") }}</h4></th>
                                            <th><h4>{{ trans("config.holiday_fields.holiday_date") }}</h4></th>
                                            <th><h4>{{ trans("config.holiday_fields.location") }}</h4></th>
                                            <th><h4>{{ trans("config.holiday_fields.countries") }}</h4></th>
                                            <th><h4>{{ trans("config.holiday_fields.created_by") }}</h4></th>
                                            <th><h4>{{ trans("config.holiday_fields.created_at") }}</h4></th>
                                            <th><h4>{{ trans("config.holiday_fields.updated_at") }}</h4></th>                                        
        ...
        @include('holidays.modal_html')
        @include('holidays.modal_country_location')
    </section>
</div>
@endsection
@push('css')
...
@push('scripts')
<script type="text/javascript" src="{!! CommonHelper::asset('js/holidays/index.js') !!}"></script>
<script type="text/javascript" src="{!! CommonHelper::asset('nfy/plugins/flatpickr/flatpickr.js') !!}"></script>
<script type="text/javascript">
	var config = new Object;
	config.url = new Object;
	config.translations = {
		press_enter_with_Search: '{{ trans('content.my_items_fields.press_enter_with_Search') }}',
		are_you_delete: '{{ trans('config.holiday_fields.are_you_delete') }}',
		something_went_wrong: '{{ trans('config.holiday_fields.something_went_wrong') }}',
        add_holiday: '{{ trans('config.holiday_fields.add_holiday') }}',
		...
		download: '{{trans('config.holiday_fields.download')}}',
		search: '{{trans('config.holiday_fields.search')}}',
    };
	config.url.holidays = "{{ url('jx-holidays') }}";
	config.url.add = "{{ url('jx-holidays-add') }}";
	config.url.edit = "{{ url('jx-holidays-edit') }}";
	config.url.get = "{{ url('jx-holidays-get') }}";
	config.url.delete = "{{ url('jx-holidays-delete') }}";
	config.url.locations = "{{ url('jx-holidays-locations') }}";
	config.url.countries = "{{ url('jx-holidays-countries') }}";
	config.url.download_url = "{{ url('export-holidays') }}";
