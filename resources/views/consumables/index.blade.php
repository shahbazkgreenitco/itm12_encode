...
* [1.0.1] - Changes for the translation changes
...
@section('title', trans('consumables.view.header'))
    ...
    <main class="main-content" id="mainContent">
        <div class="container-fluid px-0">
            <div class="header-actions-wrapper d-flex align-items-center justify-content-between px-4">
                <div class="mb-0">
                    <h2>{{ trans('consumables.view.page_heading') }}</h2>
                </div>
                <div class="d-flex gap-8">
                    ...
                    @if (Auth::user()->hasPermissionTo('ConsumableDownload'))
                        <button class="header-icon-btn-only header-icon-btn-only-sm btn-download btn-export-consumables"
                            data-bs-toggle="tooltip" title="{{ trans('consumables.view.download') }}">
                            <svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                ...
                                <path d="M8 14L12 14" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"
                                    stroke-linejoin="round"></path>
                                <path d="M16 2V5.4C16 5.73137 16.2686 6 16.6 6H20" stroke="currentColor" stroke-width="1.5"
                                    stroke-linecap="round" stroke-linejoin="round"></path>
                            </svg>
                        </button>
                        <button class="header-icon-btn-only header-icon-btn-only-sm btn-download btn-export-consumables-pdf"
                            data-bs-toggle="tooltip" title="{{ trans('consumables.view.download_pdf') }}">
                            <svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path
                                    d="M4 21.4V2.6C4 2.26863 4.26863 2 4.6 2H16.2515C16.4106 2 16.5632 2.06321 16.6757 2.17574L19.8243 5.32426C19.9368 5.43679 20 5.5894 20 5.74853V21.4C20 21.7314 19.7314 22 19.4 22H4.6C4.26863 22 4 21.7314 4 21.4Z"
                                    stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
                    ...
                    @endif
                    @if (Auth::user()->hasPermissionTo('ConsumableAdd'))
                        <button class="header-icon-btn-only header-icon-btn-only-sm btn-import btn-import-consumables"
                            data-bs-toggle="tooltip" title="{{ trans('consumables.view.import_consumables') }}">
                    ...
                    @endif
                    <button class="header-icon-btn-only header-icon-btn-only-sm non-deleted-icon btn-deleted-consumable" type="button"
                        data-bs-toggle="tooltip" title="{{ trans('consumables.view.show_deleted_consumables') }}">
                    ...
                    <button class="header-icon-btn-only header-icon-btn-only-sm btn-deleted-consumable deleted-icon d-none"
                        type="button" data-bs-toggle="tooltip"
                        title="{{ trans('consumables.view.show_non_deleted_consumables') }}">
                    ...
                    @if (Auth::user()->hasPermissionTo('ConsumableBulkCheckout'))
                        <button class="header-icon-btn-only header-icon-btn-only-sm btn-bulk-checkout" type="button"
                            data-bs-toggle="tooltip" title="{{ trans('consumables.view.bulk_checkout') }}">
                    ...
                    @endif
                    @if (Auth::user()->hasPermissionTo('ConsumableCheckin'))
                        <button class="header-icon-btn-only header-icon-btn-only-sm btn-bulk-checkin" type="button" data-bs-toggle="tooltip" title="{{ trans('consumables.view.bulk_checkin') }}">
                            <svg width="18" height="18" viewBox="0 0 24 24" fill="none">
                    ...
                    <div class="column-toggle-wrapper" style="position: relative; display: inline-block;">
                        <button class="header-icon-btn-only header-icon-btn-only-sm show-hide-columns"
                            data-bs-toggle="tooltip" title="{{ trans('consumables.view.show_columns') }}">
                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
                    ...
                    <button class="amg-btn amg-btn-primary amg-btn-sm btn-add-consumable" type="button" data-bs-toggle="modal"
                        data-bs-target="#consumablemodal" action="add" title="{{ trans('consumables.view.add') }}">
                        ...
                        <span>{{ trans('consumables.view.add') }}</span>
                    </button>
                </div>
            </div>
                    ...
                    <div class="card-body table-responsive">
                        <div class="d-flex align-items-center gap-2 mb-1" data-select2-id="select2-data-5-1om7">
                            ...
                            <div>
                                <div class="amg-list-searchbar">
                                    ...
                                    <input type="text" class="amg-list-searchbar__input" id="tableSearch"
                                        placeholder="{{ trans('consumables.view.search') }}...">
                                </div>
                            </div>
                            <button class="amg-refresh-btn btn-reload-list">
                                ...
                                <span>{{ trans('consumables.view.refresh') }}</span>
                            </button>
                        </div>
                        <table id="mytable" class="mytable table dark_thead">
                            <thead>
                                <tr>
                                    <th><h4>{{ trans('consumables.view.id') }}</h4></th>
                                    <th><h4 class="text-start">{{ trans('consumables.view.Consumable') }}</h4></th>
                                    <th><h4>{{ trans('consumables.view.Location') }}</h4></th>
                                    <th><h4>{{ trans('consumables.view.Total') }}</h4></th>
                                    <th><h4>{{ trans('consumables.view.Avail') }}</h4></th>
                                    <th><h4>{{ trans('consumables.view.scrap') }}</h4></th>
                                    <th><h4>{{ trans('consumables.view.threshold') }}</h4></th>
                                    <th><h4>{{ trans('consumables.view.purchase_info') }}</h4></th>
                                    <th><h4>{{ trans('consumables.view.Updated_On') }}</h4></th>
                                    <th><h4>{{ trans('consumables.view.actions') }}</h4></th>
                                </tr>
                            </thead>
                            <tbody></tbody>
                        </table>
...
@push('scripts')
    ...
    <script type="text/javascript" src="{!! CommonHelper::asset('js/consumables/index.js') !!}"></script>
    ...
    <script type="text/javascript">
        ...
        var getSupplierByAjax = "{{ url('getSupplierByQuery') }}";
        var getInvoiceByAjax = "{{ url('getInvoiceByQuery') }}";
        config.url.consumables = "{{ url('jx-consumables') }}";
        config.url.infourl = "{{ url('consumable-info') }}";
        config.url.add = "{{ url('consumables') }}";
        config.url.edit = "{{ url('consumables') }}";
        config.url.restore = "{{ url('consumable-restore') }}";
        ...
        config.url.history = "{{ url('consumable-history') }}";
        config.url.download_url = "{{ url('export-consumables') }}";
        config.url.download_url_pdf = "{{ url('export-consumables-pdf') }}";
        config.getUserByAjax = "{{ url('getUserForDropDown') }}";
        config.ajaxGetInternalPlace = "{{ url('ajaxGetInternalPlace') }}";
        config.url.bulk_checkout = "{{ url('consumables/bulk/checkout') }}";
        config.url.bulk_checkin = "{{ url('consumables/bulk/checkin') }}";
        config.url.import = "{{ url('import-consumables') }}";
        ...
        config.units = {!! json_encode($units) !!};
        ...
        config.translations = {
            Restore_Consumable: '{{ trans('consumables.config.Restore_Consumable') }}',
            Edit_Consumable: '{{ trans('consumables.config.Edit_Consumable') }}',
            add: '{{ trans('consumables.config.add') }}',
            edit: '{{ trans('consumables.config.edit') }}',
            Add_Consumable: '{{ trans('consumables.config.Add_Consumable') }}',
            Clone_Consumable: '{{ trans('consumables.config.Clone_Consumable') }}',
            Delete_Consumable: '{{ trans('consumables.config.Delete_Consumable') }}',
            Consumable_Detail: '{{ trans('consumables.config.Consumable_Detail') }}',
            Batch_No: '{{ trans('consumables.config.Batch_No') }}',
            Category: '{{ trans('consumables.config.Category') }}',
            select_internal_place: '{{ trans('consumables.config.select_internal_place') }}',
            Company_Name: '{{ trans('consumables.config.Company_Name') }}',
            press_enter_with_Search: '{{ trans('consumables.config.press_enter_with_Search') }}',
            Search: '{{ trans('consumables.config.Search') }}',
            Refresh_List: '{{ trans('consumables.config.Refresh_List') }}',
            Bulk_Checkout: '{{ trans('consumables.config.Bulk_Checkout') }}',
            Bulk_Checkin: '{{ trans('consumables.config.Bulk_Checkin') }}',
            Download: '{{ trans('consumables.config.Download') }}',
            Download_PDF: '{{ trans('consumables.config.Download_PDF') }}',
            please_enter_valid_search: '{{ trans('consumables.config.please_enter_valid_search') }}',
            Select_the_Supplier: '{{ trans('consumables.config.Select_the_Supplier') }}',
            Select_the_Purchase_Invoice: '{{ trans('consumables.config.Select_the_Purchase_Invoice') }}',
            Select_the_User: '{{ trans('consumables.config.Select_the_User') }}',
            are_you_want_delete: '{{ trans('consumables.config.are_you_want_delete') }}',
            Download_Document: '{{ trans('consumables.config.Download_Document') }}',
            Export_PDF: '{{ trans('consumables.config.Export_PDF') }}',
            Reload: '{{ trans('consumables.config.Reload') }}',
            Export: '{{ trans('consumables.config.Export') }}',
            Upload_Document: '{{ trans('consumables.config.Upload_Document') }}',
            Consumable_Name: '{{ trans('consumables.config.Consumable_Name') }}',
            select_the_location: '{{ trans('consumables.config.Select_Location') }}',
            select_the_unit: '{{ trans('consumables.config.select_the_unit') }}',
            select_the_department: '{{ trans('consumables.config.select_the_department') }}',
            select_the_category: '{{ trans('consumables.config.Select_Category') }}',
            select_the_company: '{{ trans('consumables.config.Select_Company') }}',
            Select_the_Device: '{{ trans('consumables.config.Select_the_Device') }}',
            select_the_model: '{{ trans('consumables.config.select_the_model') }}',
            select_the_internal_place: '{{ trans('consumables.config.select_the_internal_place') }}',
            select_the_project: '{{ trans('consumables.config.select_the_project') }}',
            select_the_contract: '{{ trans('consumables.config.select_the_contract') }}',
            select_the_component: '{{ trans('consumables.config.select_the_component') }}',
            select_the_license: '{{ trans('consumables.config.select_the_license') }}',
            select_the_task: '{{ trans('consumables.config.select_the_task') }}',
            select_the_change_management: '{{ trans('consumables.config.select_the_change_management') }}',
            select_the_manufacturer: '{{ trans('consumables.config.Select_Manufacturer') }}',
            select_the_ticket: '{{ trans('consumables.config.select_the_ticket') }}',
            select_the_ticket_procure_request: '{{ trans('consumables.config.select_the_ticket_procure_request') }}',
            location: '{{ trans('consumables.config.location') }}',
            internal_place: '{{ trans('consumables.config.internal_place') }}',
            Consumable_Tag: '{{ trans('consumables.config.consumable_tag') }}',
            unique_tag: '{{ trans('consumables.config.unique_tag') }}',
            show_deleted_consumables: '{{ trans('consumables.config.show_deleted_consumables') }}',
            show_non_deleted_consumables: '{{ trans('consumables.config.show_non_deleted_consumables') }}',
            are_you_restore: '{{ trans('consumables.config.are_you_restore') }}',
        ...
        var currencies = {!! json_encode(App\Models\Currency::getCurrencies()) !!};
        config.custom_fields = {!! !empty($companyFieldset->fields)
            ? json_encode(CommonHelper::formCustomFields($companyFieldset->fields, "", 'consumable'))
            : json_encode('') !!};
        var dtblRef;
        $(document).ready(function() {
            dtblRef = new MyApp(config);
        });
    </script>
@endpush
