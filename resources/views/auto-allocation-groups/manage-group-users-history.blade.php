...
/**
* ------------------------------------------------------------
* File: manage-users-history.blade.php
...
* ------------------------------------------------------------
* Change Log:
* [2.0.0] - Migrated history page to Layout 1 UI design
* [1.0.0] - Initial setup
* ------------------------------------------------------------
*/
--}}
...
@section('title', trans("auto-allocation-group.manage_group_users_history"))
...
<div id="main-user-list-wrapper" class="auto-allocation-list-wrapper">
    ...
    <div class="header-actions-wrapper d-flex align-items-center justify-content-between pe-4">
        <div class="d-flex align-items-center gap-1">
            @php
                $allocationName = $groupName->name ?? '';
                $isLong = strlen($allocationName) > 50;
                $shortName = $isLong ? substr($allocationName, 0, 50) . '...' : $allocationName;
            @endphp         
            <button onclick="window.location.href='{{ url('user-allocation-groups/manage-users/' . $groupId) }}'" class="d-flex gap-3 align-items-center bg-transparent outline-none border-0" data-bs-toggle="tooltip" title="Back">
            ...
            <h3 class="h3-text mb-0"@if($isLong) data-bs-toggle="tooltip" title="{{ $allocationName }}"@endif> {{ trans("auto-allocation-group.manage_group_users_history") }} - {{ $shortName }}</h3>
        </div>       
            ...
            <div class="card rounded-0">
                ...
                <div class="card-body pt-3">
                    ...
                    <div class="d-flex align-items-center gap-2 mb-1">
                        ...
                        {{-- Page Length --}}
                        <div class="col-auto">
                            <select class="amg-table-pagination-dropdown userModulePageLenth historyPageLength">
                        ...
                        <div>
                            <div class="amg-list-searchbar">
                               <svg class="amg-list-searchbar__icon" width="18" height="18" viewBox="0 0 20 20" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                                ...
                                <input type="text" class="amg-list-searchbar__input searchbox plain-search" placeholder="{{ trans('auto-allocation-group.manage_user.search_placeholder') }}">
                            </div>
                        </div>
                            ...
                            <table id="mytable" class="table display app-data-table">
                                ...
                                <thead>
                                    ...
                                    <tr>
                                        ...
                                        <th>
                                            <h4>{{ trans("auto-allocation-group.user_name") }}</h4>
                                        </th>

                                        <th>
                                            <h4>{{ trans("auto-allocation-group.location") }}</h4>
                                        </th>

                                        <th>
                                            <h4>{{ trans("auto-allocation-group.location_access") }}</h4>
                                        </th>

                                        <th>
                                            <h4>{{ trans("auto-allocation-group.max_ticket_per_hr") }}</h4>
                                        </th>

                                        <th>
                                            <h4>{{ trans("auto-allocation-group.max_ticket_per_day") }}</h4>
                                        </th>

                                        <th>
                                            <h4>{{ trans("content.ticket_procurement.remark") }}</h4>
                                        </th>

                                        <th>
                                            <h4>{{ trans("content.ticket_procurement.created_at") }}</h4>
                                        </th>

                                        <th>
                                            <h4>{{ trans("content.ticket_procurement.updated_at") }}</h4>
                                        </th>

                                        <th>
                                            <h4>{{ trans("content.ticket_procurement.created_by") }}</h4>
                                        </th>

                                        <th>
                                            <h4>{{ trans("content.ticket_procurement.updated_by") }}</h4>
                                        </th>

                                        <th>
                                            <h4>{{ trans("content.ticket_procurement.deleted_by") }}</h4>
                                        </th>
...
<script type="text/javascript"
        src="{!! CommonHelper::asset('newjs/select2.min.js') !!}">
</script>

<script type="text/javascript"
        src="{!! CommonHelper::asset('js/auto-allocation-groups/member-history.js') !!}">
</script>
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        config.url = {
            historyAjax: "{{ url('jx-get-user-allocation-members-history') }}",
        };

        config.groupId = {!! json_encode($groupId) !!};

        config.back = document.referrer;

        config.translations = @json(trans('auto-allocation-group'));

        config.token = "{{ csrf_token() }}";

        new AutoAllocGroupMembersHistory(config);
