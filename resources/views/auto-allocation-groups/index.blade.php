...
@section('title', trans('auto-allocation-group.title'))
@section('content')
<div id="main-user-list-wrapper" class="auto-allocation-list-wrapper">
    <div class="header-actions-wrapper d-flex align-items-center justify-content-between pe-4">
        <h3 class="h3-text mb-0">{{ trans('auto-allocation-group.title') }}</h3>
        <div class="d-flex gap-8">
            <button class="header-icon-btn header-icon-btn-sm btn-open-filter" type="button" data-bs-toggle="tooltip" title="{{ trans('auto-allocation-group.filter') }}">
               <svg viewBox="0 0 20 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <span class="b5-text opacity-50">
                    {{ trans('auto-allocation-group.filter') }}
                </span>
                <span class="filter-count-badge d-none" aria-label="{{ trans('auto-allocation-group.active_filters') }}">
                    0
                </span>
            </button>
            @if(Auth::user()->hasPermissionTo('TicketAllocationGroupAdd'))
            <button class="header-icon-btn-only header-icon-btn-only-sm btn-download" type="button" data-bs-toggle="tooltip" title="{{ trans('auto-allocation-group.download_excel') }}">
            ...
            @if(Auth::user()->hasPermissionTo('TicketAllocationGroupImport'))
            <button class="header-icon-btn-only header-icon-btn-only-sm escalation-bulk-import" type="button" data-bs-toggle="tooltip" title="{{ trans('auto-allocation-group.bulk_import') }}">
            ...
            @if(Auth::user()->hasPermissionTo('TicketAllocationGroupAdd'))
                <button class="amg-btn amg-btn-primary amg-btn-sm open-add-modal" type="button" data-bs-toggle="tooltip" title="{{ trans('auto-allocation-group.add_group') }}">
                    ...
                    <span>{{ trans('auto-allocation-group.add_group') }}</span>
                </button>
            @endif
        </div>
    </div>
                    ...
                    <div class="d-flex align-items-center gap-2 mb-1">
                        ...
                        <div>
                            <div class="amg-list-searchbar">
                                ...
                                <input type="text" name="search" class="amg-list-searchbar__input user-list-search searchbox plain-search" placeholder="{{ trans('auto-allocation-group.search_placeholder') }}">
                            </div>
                        </div>

                        <button class="amg-refresh-btn btn-reload-list btn-reload-list" data-bs-toggle="tooltip" title="{{ trans('auto-allocation-group.refresh') }}">
                            ...
                            <span>{{ trans('auto-allocation-group.refresh') }}</span>
                        </button>
                    </div>
                    ...
                    <div class="list-view-panel">
                        <div class="table-responsive">
                            <table id="mytable" class="table display app-data-table">
                                <thead>
                                    <tr>
                                        <th><h4>{{ trans('auto-allocation-group.table.group_name') }}</h4></th>
                                        <th><h4>{{ trans('auto-allocation-group.table.company') }}</h4></th>
                                        <th><h4>{{ trans('auto-allocation-group.table.department') }}</h4></th>
                                        <th><h4>{{ trans('auto-allocation-group.table.member_count') }}</h4></th>
                                        <th><h4>{{ trans('auto-allocation-group.table.location_type') }}</h4></th>
                                        <th><h4>{{ trans('auto-allocation-group.table.location_approval_required') }}</h4></th>
                                        <th><h4>{{ trans('auto-allocation-group.table.updated_at') }}</h4></th>
    ...
    @include('auto-allocation-groups.add-modal')
    @include('auto-allocation-groups.clone-modal')
    @include('auto-allocation-groups.filter')
</div>
@endsection

@push('scripts')

    <script type="text/javascript" src="{!! CommonHelper::asset('js/auto-allocation-groups/index.js') !!}"></script>
    <script type="text/javascript">
        $(document).ready(function () {
            var config = new Object;
            config.url = new Object;
            config.translations = @json(trans('auto-allocation-group'));
            config.url.getUserAllocationGroups = "{{ url('jx-get-user-allocation-groups') }}";
            config.url.history = "{{ url('auto-allocation-groups_history') }}";
            config.url.add = "{{ url('jx-add-user-group') }}";
            config.url.getDepartments = "{{url('departments/by-company/privilage')}}";
            config.url.manage_users = "{{ url('user-allocation-groups/manage-users') }}";
            config.url.getGroupDetails = "{{ url('jx-get-group-details') }}";
            config.url.editGroupDetails = "{{ url('jx-edit-group-details') }}";
            config.url.delete = "{{ url('jx-group-delete') }}";
            config.url.download_url = "{{ url('auto-allocation-export') }}";
            config.url.cloneAllocationGroup = "{{ route('cloneAllocationGroup') }}";
            config.url.department = "{{ url('getDepartmentsWithCompanyByQuery') }}";
            config.url.autoAllocationDepartment = "{{ url('getAutoAllocationDepartment') }}";
            config.url.getCompanyByUserAccess = "{{ url('getCompanyByUserAccess') }}";
            config.url.import_url = "{{ url('auto-allocation-import') }}";
            config.company_id = "{{$companyId}}";
            config.token = "{{ csrf_token() }}";
            config.permissions = {!! json_encode($permissionArray) !!};
            new AutoAllocGroups(config);
                    });
    </script>
@endpush
