...
@section('title', trans('auto-allocation-group.history'))
@section('content')
<div id="main-user-list-wrapper" class="auto-allocation-list-wrapper">
    <div class="header-actions-wrapper d-flex align-items-center justify-content-between pe-4">
        <div class="d-flex align-items-center gap-1">
            @php
                $allocationName = $allocation_name->name ?? '';
            ...
            <h3 class="h3-text mb-0"@if($isLong) data-bs-toggle="tooltip" title="{{ $allocationName }}"@endif> {{ trans('auto-allocation-group.history') }} - {{ $shortName }}</h3>
        </div>
    </div>
                    ...
                    <div class="d-flex align-items-center gap-2 mb-3">
                        ...
                        <div class="col-auto">
                            <select class="amg-table-pagination-dropdown userModulePageLenth history-page-length">
                                <option value="10" selected>
                                    {{ trans('auto-allocation-group.table.show_10') }}
                                </option>
                                <option value="25">
                                    {{ trans('auto-allocation-group.table.show_25') }}
                                </option>
                                <option value="50">
                                    {{ trans('auto-allocation-group.table.show_50') }}
                                </option>
                                <option value="100">
                                    {{ trans('auto-allocation-group.table.show_100') }}
                                </option>
                            </select>
                        </div>
                        ...
                        <div>
                            <div class="amg-list-searchbar">
                                ...
                                <input type="text"
                                       name="search"
                                       class="amg-list-searchbar__input history-search searchbox plain-search"
                        ...
                        <div class="table-responsive">
                            <table id="mytable"
                                ...
                                <thead>
                                    <tr>
                                        <th>
                                            <h4>{{ trans("auto-allocation-group.table.group_name") }}</h4>
                                        </th>
                                        <th>
                                            <h4>{{ trans("auto-allocation-group.table.company") }}</h4>
                                        </th>
                                        <th>
                                            <h4>{{ trans("auto-allocation-group.table.department") }}</h4>
                                        </th>
                                        <th>
                                            <h4>{{ trans("auto-allocation-group.form.group_description") }}</h4>
                                        </th>
                                        <th>
                                            <h4>{{ trans("auto-allocation-group.location_approval_required") }}</h4>
                                        </th>
                                        <th>
                                            <h4>{{ trans("auto-allocation-group.location_approval_required_for") }}</h4>
                                        </th>
                                        <th>
                                            <h4>{{ trans("auto-allocation-group.remark") }}</h4>
                                        </th>
                                        <th>
                                            <h4>{{ trans("auto-allocation-group.created_at") }}</h4>
                                        </th>
                                        <th>
                                            <h4>{{ trans("auto-allocation-group.table.updated_at") }}</h4>
                                        </th>
                                        <th>
                                            <h4>{{ trans("auto-allocation-group.created_by") }}</h4>
                                        </th>
                                        <th>
                                            <h4>{{ trans("auto-allocation-group.updated_by") }}</h4>
                                        </th>
                                        <th>
                                            <h4>{{ trans("auto-allocation-group.deleted_by") }}</h4>
                                        </th>
                                    </tr>
                                </thead>
                                <tbody></tbody>
...
<script type="text/javascript" src="{!! CommonHelper::asset('js/auto-allocation-groups/history.js') !!}"></script>
<script type="text/javascript">
    var config = {};
    config.title = {};
    config.placeholder = {};
    config.url = {
        historyList: "{{ url('auto-allocation-groups') }}",
        historyAjax: "{{ url('jx-get-user-allocation-groups-history') }}",
    };

    config.allocation_id = {!! json_encode($allocation_id) !!};
    config.translations = @json(trans('auto-allocation-group'));
    config.token = "{{ csrf_token() }}";
    new AutoAllocGroupsHistory(config);
