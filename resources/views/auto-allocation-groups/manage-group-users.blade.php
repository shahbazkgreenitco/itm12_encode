...
/**
* ------------------------------------------------------------
* File: manage-users.blade.php
* Module: Auto Allocation Groups
* AAG/26/01
* ------------------------------------------------------------
* Version: 2.0.0
...
* ------------------------------------------------------------
* Change Log:
* [2.0.0] - Migrated manage users page to new Layout 1 UI design
* [1.0.0] - Initial setup
* ------------------------------------------------------------
*/
--}}
...
@section('title', trans("auto-allocation-group.manage_group_users"))
...
<div id="main-user-list-wrapper" class="auto-allocation-list-wrapper">
    ...
    <div class="header-actions-wrapper d-flex align-items-center justify-content-between pe-4">
        <div class="d-flex align-items-center gap-1">
            ...
            <h3 class="h3-text mb-0"@if($isLong) data-bs-toggle="tooltip" title="{{ $groupName }}" @endif> {{ trans("auto-allocation-group.manage_group_users") }} - {{ $shortName }}</h3>
        </div>

        <div class="d-flex gap-8 align-items-center">
            
            @can("TicketAllocationGroupUserAdd")
            <button class="amg-btn amg-btn-primary amg-btn-sm open-addGroupUserForm-modal"
                    type="button"
                    data-bs-toggle="tooltip"
                    title="{{ trans('auto-allocation-group.add_group_user') }}">
                ...
                <span>
                    {{ trans("auto-allocation-group.add_group_user") }}
                </span>
            </button>
            @endcan

            @can("TicketAllocationGroupUserRead")
            <a href="{{ url('user-allocation-groups/manage-users/history',$groupDetails->id) }}"
               target="_blank"
               class="header-icon-btn-only header-icon-btn-only-sm"
               data-bs-toggle="tooltip"
               title="{{ trans('auto-allocation-group.history') }}">
                        ...
                        <div>
                            <div class="amg-list-searchbar">
                               ...
                               <svg class="amg-list-searchbar__icon" width="18" height="18" viewBox="0 0 20 20" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                                ...
                                <input type="text"
                                       name="search"
                                       class="amg-list-searchbar__input user-list-search searchbox plain-search"
                                       placeholder="{{ trans('auto-allocation-group.manage_user.search_placeholder') }}">
                            </div>
                        </div>

                        {{-- Refresh --}}
                        <button class="amg-refresh-btn btn-reload-list" data-bs-toggle="tooltip" title="{{ trans('auto-allocation-group.refresh') }}">
                            ...
                            <span>
                                {{ trans('auto-allocation-group.refresh') }}
                            </span>
                        </button>
                    ...
                    <div class="list-view-panel">
                        <div class="table-responsive">
                            ...
                            <table id="mytable" class="table display app-data-table">
                                <thead>
                                    <tr>
                                        <th><h4>{{ trans("auto-allocation-group.user_name") }}</h4></th>
                                        <th><h4>{{ trans("auto-allocation-group.location") }}</h4></th>
                                        <th><h4>{{ trans("auto-allocation-group.location_access") }}</h4></th>
                                        @if($groupDetails->location_approval_required_for == 3)
                                            <th><h4>{{ trans("auto-allocation-group.manage_user.internal_place_location") }}</h4></th>
                                        @endif
                                        <th><h4>{{ trans("auto-allocation-group.max_ticket_per_hr") }}</h4></th>
                                        <th><h4>{{ trans("auto-allocation-group.max_ticket_per_day") }}</h4></th>
                                        <th><h4>{{ trans("auto-allocation-group.status") }}</h4></th>
    ...
    {{-- Modals --}}
    @include('auto-allocation-groups.add-user-modal-html')
    @include('auto-allocation-groups.locations-modal')
</div>
@endsection
@push('scripts')
<script type="text/javascript" src="{!! CommonHelper::asset('js/auto-allocation-groups/manageUsers.js') !!}"></script>
<script type="text/javascript">
    $(document).ready(function () {
        ...
        config.url.addGroupUser = "{{ url('jx-add-group-user') }}";
        config.url.getUserAllocationGroupMembers = "{{url('jx-get-user-allocation-members')}}";
        config.url.getGroupMember = "{{ url('jx-get-get-group-member-details') }}";
        config.url.editGroupMember = "{{ url('jx-edit-group-member-details') }}";
        config.url.getCompanyWiseLocation = "{{ url('get-company-wise-location') }}";
        config.url.getUsers = "{{ url('getTicketAttendersByDepartment') }}/{{ $groupDetails->id }}";
        config.url.getInternalPlaceByAjax = "{{ url('getInternalPlaceByAjax') }}";
        config.permissions = {!! json_encode($permissionArray) !!};
        config.url.delete = "{{ url('jx-group-user-delete') }}";
        config.groupId = "{{$groupDetails->id}}";
        config.company_id = "{{$companyId}}";
        config.locationAccess = {!! json_encode($location_access) !!};
        config.autoAllocationFor = {!! json_encode($groupDetails->location_approval_required_for) !!};
        config.token = "{{ csrf_token() }}";
        new AutoAllocGroupsMembers(config);
    });
</script>
@endpush
