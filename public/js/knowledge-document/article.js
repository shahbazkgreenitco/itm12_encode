var Article = function (config) {
    ...
    t.staring = function(e) {
        ...
        var message = t.config.star_exsist ? config.translations.unstar_article : config.translations.are_you_star;
    ...
    t.frmupdValidator = t.frmUpdate.validate({
        ...
        errorPlacement: function (error, element) {
            error.appendTo(element.closest('.amg-form-field-row').find('.amg-form-error-wrap'));
        }
	});

    t.frmUpdate.find("select[name='company'],select[name='department_id'],select[name='parent_category_id'],select[name='status'],select[name='tags[]']").on("change", function () {
        var element = $(this);
        element.closest(".amg-form-field-row").find(".shows_error").empty();
    ...
    t.updateDocument = function(e) {
		e.preventDefault();
        const totalFiles = (t.updateUploader.items?.length || 0) + (t.updateUploader.files?.length || 0);
        if (totalFiles > 10) {
        ...
        if ($.trim(contentHolder.text()) == '' && contentHolder.find('img, iframe, video').length === 0) {
            ...
            // return false;//earlier return with only content message visible
        }else{
            $('#shows_error').empty();
        }
		if (t.frmupdValidator.form() == false || !isContentValid) {
			return false;
		}
	...
	t.loadForm = function(data, forAction) {
        ...
        if (data.card_img) {
            $(".imgviewcover").removeClass("d-none");
            $("#imgview").attr("src", data.card_img);
        } else {
            $(".imgviewcover").addClass("d-none");
            $("#imgview").attr("src", "");
        }
		t.data.id = data.id;
	};
