var CustomForm = function(config) {
    var t = this;
    t.config = config;
    var count = 0;
    var cls= '';
    ...
    $('#mdl-servicerequest').on('shown.bs.modal', function() {
            ...
            ajax: {
                ...
                data: function(p) {
                    return {
                        ...
                        page: p.page || 1,
                        company_id:$('#ticket-mdl-frm').find('#company_id').val(),
                    };
                },
                delay: 300
            },
        ...
        $(document).on('click', '.next', function () {
            ...
            if ($('input[name="employee_type"][value="Self"]').is(':checked')) {
                ...
                $('#wbs_center').prop('readonly', false);
            ...
            // if ($('input[name="employee_type"][value="Non LTTS Employee"]').is(':checked')) {
            //     $('#cost_center').prop('readonly', false).val('');
            // }
        });
        ...
        t.loadFormUser = function(user) {
            ...
            if (user) {
                ...
                if($('input[name="employee_type"][value="LTTS Employee"]').is(':checked')) {
                    ...
                    const $creatByNp = $('#createTicket #ticket-mdl-frm');
                ...
                buildWrap.find('#wbs_center').prop('readonly', false);
        ...
        $(this).find('#build-wrap').find(".category_id").select2($.extend({}, select2Opts, {
            ...
            ajax: {
                url: function() {
                    var type_val = $("#createTicket").find("#ticket-mdl-frm").find("#department_id").val();
                    const location_id = $('#build-wrap').find("#b_location").val();
                    if (type_val > 0 && !isNaN(type_val)) {
                        return t.config.url.getCategoryByAjax + "?department_id=" + type_val + "&location_id=" + location_id;
                    }
                },
                dataType: "json",
                data: function(p) {
        ...
        $(this).find('#build-wrap').find(".item").select2($.extend({}, select2Opts, {
            ...
            ajax: {
                url: function() {
                    ...
                    const location_id = $('#build-wrap').find("#b_location").val();
                    if (type_val > 0 && !isNaN(type_val)) {
                        return t.config.url.getDataForCustomFormBaseOnType + "/?cat_id=" + type_val + "&location_id=" + location_id;
                    }
                },
                dataType: "json",
                data: function(p) {
        ...
        window.loadFormData = function (data) {
            ...
            if (data.field_values.employee_type === 'Non LTTS Employee') {
                $('.psno-container').hide();
            ...
            buildWrap.find('#wbs_center').prop('readonly', false).val(data.field_values.wbs_center || '');
        ...
        row.find(".item").select2({
            ...
            ajax: {
                url: function () {
                    ...
                    const location_id = $('#build-wrap').find("#b_location").val();
                    if (categoryVal) {
                        return `${t.config.url.getDataForCustomFormBaseOnType}/?cat_id=${categoryVal}&location_id=${location_id}`;
                    }
                    return null;
                },
                dataType: "json",
    ...
    function validateStep(step) {
        ...
        inputs.each(function() {
            ...
            if(fieldName === "emp_no" && employeeType == "LTTS Employee") {
                if (value === "" || value.trim() === "")
                {
                    errorMsg = "EMP/PS Number is required.";
                }
            } else if(fieldName === "b_location" && employeeType == "LTTS Employee") {
                if (value === "" || value.trim() === "") {
                    errorMsg = "Location is required.";
                }
            } else if((fieldName === "email_id" || fieldName === "contact") && employeeType == "Non LTTS Employee") {
            ...
            } else if(fieldName === "wbs_center") {
                if (value === "") { errorMsg = "WBS Center is required."; }
            } else if(fieldName === "emp_name" && employeeType == "Non LTTS Employee") {
    ...
    $(document).on('change', 'input[name="employee_type"]', function () {
        ...
        if (selectedValue === 'Non LTTS Employee') {
            $('.psno-container').hide();
            $(document).find('#emp_no, #emp_name, #contact, #email_id, #wbs_center, #cost_center, #wbs_note').prop('readonly', false).val('');
            ...
            if(selectedValue === 'Self') {
                ...
                $(document).find('#wbs_center').prop('readonly', false).val('');
            ...
            } else if (selectedValue === 'LTTS Employee') {
                ...
                $(document).find('#cost_center, #wbs_center, #wbs_note').prop('readonly', false).val('');
                $(document).find('#wbs_note').prop('readonly', false).val('');
                $(document).find('#emp_number').prop('readonly', true).val('');
                $('input[name="department"]').val('');
                $('input[name="department_id"]').val('');
