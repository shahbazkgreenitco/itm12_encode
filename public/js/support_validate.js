$(document).ready(function() {
    ...
    jQuery.validator.addMethod('username', function(v, e) {
        return this.optional(e) || /^(?! )[a-zA-Z0-9\u00C0-\u00FF\u0100-\u017F\u0180-\u024F\-\/_:,@'.)(: ]{0,100}$/.test(v);//this regex accepts space in between text
    ...
    $.validator.addMethod("validLogo", function(value, element) {
        if (element.files.length === 0) return true;
        var file = element.files[0];
        var maxSize = 10 * 1024 * 1024; // 10 MB
        var allowedTypes = /(\.jpg|\.jpeg|\.png)$/i;
        if (file.size > maxSize) return false;
        if (!allowedTypes.test(file.name)) return false;
        return true;
    }, "File must be JPG, JPEG or PNG and less than 10 MB");
    ...
    $.validator.addMethod(
        ...
        function(value, element) {
            if (this.optional(element))
                ...
                $('#createTicket').find('#shows_error_cc').html('');
    ...
    $.fn.tooltip = function(options) {
        if (typeof options === 'string') {
            this.each(function() {
                ...
                if (tooltip && typeof tooltip[options] === 'function') tooltip[options]();
                else if (options === 'destroy' || options === 'dispose') tooltip && tooltip.dispose();
            });
            return this;
        }
        this.each(function() {
            if (!bootstrap.Tooltip.getInstance(this)) new bootstrap.Tooltip(this, options || {});
        });
        return this;
    };
}

function getErrorWrap(element) {
    var $element = $(element);
    var row = $element.closest(".amg-form-field-row");

    if (!row.length) return $();

    var wrap = row.children(".amg-form-error-wrap");
...
function updateValidationState(element, hasError) {
    var $element = $(element);
