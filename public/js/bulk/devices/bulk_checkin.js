var DeviceCheckinFilePhase = function (config) {
    var t = this;
    t.config = config;
    t.form = $("#confirmForm");
    ...
    t.validate = function (e) {
        ...
        if (file_name === "" || t.file[0].files.length === 0) {
            sweetAlert("center", "error", {
                msg: t.config.translations.please_choose_file,
            });
            e.preventDefault();
            return false;
        }
...
var DeviceCheckinInfoPhase = function (config) {
    var t = this;
    t.config = config;
    t.tab = $("section.content").find("#checkin-info-tab");
    ...
    t.initDataTable = function () {
        t.dTbl = t.table.DataTable({
            ...
            order: [[5, "desc"]],
            ajax: {
                url: t.config.bulk_checkin_info,
                type: "POST",
                data: function (d) {
                    d._token = t.config.token;
                    d.device_id = 1;
                    d.action_type = 2; // Checkin action type
                }
            },
                    ...
                    render: function (d) {
                        return `
                            <a 
                                ...
                                class="btn dtActbtn"
                                data-toggle="tooltip"
                                title="${t.config.translations.download || 'Download'}">
...
var MyApp = function (config) {
    ...
    t.DeviceCheckinFilePhase = new DeviceCheckinFilePhase(config);
    t.DeviceCheckinInfoPhase = new DeviceCheckinInfoPhase(config);
